    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            with st.expander("🏷️ إدارة الباركود الرئيسي (الربط الدائم بين الباركود والصنف)"):
                ...
                # ---------- إضافة باركود يدوي لأي صنف من البرنامج (مش لازم يكون جه من ملف إكسيل) ----------
                ...
                st.markdown("---")
                st.markdown("##### ✍️ إضافة باركود يدوي لصنف من البرنامج")
                st.caption(
                    "اختار أي صنف من كل أصناف البرنامج (تلات المصانع) واكتب باركوده يدوي — "
                    "مفيد للأصناف اللي لسه ملهاش باركود محفوظ، عشان المرة الجاية اللي ترفع فيها "
                    "ملف الإكسيل يتطابق بالباركود على طول من غير ما يحتاج مطابقة يدوية"
                )
                all_app_products_bc = sorted(pm_df_c["الصنف"].dropna().astype(str).unique().tolist())
                active_master_names_bc = set()
                if not barcode_master_view.empty and "الحالة" in barcode_master_view.columns:
                    active_master_names_bc = set(
                        barcode_master_view[barcode_master_view["الحالة"] == "نشط"]["اسم الصنف في البرنامج"]
                    )
                products_without_barcode_bc = [p for p in all_app_products_bc if p not in active_master_names_bc]
                if products_without_barcode_bc:
                    st.caption(f"📋 {len(products_without_barcode_bc)} صنف من البرنامج لسه ملوش باركود محفوظ")

                manual_bc_c1, manual_bc_c2, manual_bc_c3 = st.columns([2, 2, 1])
                with manual_bc_c1:
                    manual_bc_show_all = st.checkbox(
                        "اعرض كل الأصناف (مش بس اللي بدون باركود)", value=False, key="manual_bc_show_all_c"
                    )
                    manual_bc_options = all_app_products_bc if manual_bc_show_all else products_without_barcode_bc
                    manual_bc_product = st.selectbox(
                        "الصنف", [""] + manual_bc_options, key="manual_bc_product_pick_c"
                    )
                with manual_bc_c2:
                    manual_bc_value = st.text_input("الباركود", key="manual_bc_value_c")
                with manual_bc_c3:
                    st.write("")
                    st.write("")
                    manual_bc_save = st.button("💾 حفظ الباركود", key="manual_bc_save_btn_c")

                if manual_bc_save:
                    if not manual_bc_product:
                        st.warning("اختار الصنف الأول ⚠")
                    elif not manual_bc_value.strip():
                        st.warning("اكتب الباركود الأول ⚠")
                    else:
                        ok_bc, msg_bc = save_barcode_mapping_manual(manual_bc_value, manual_bc_product)
                        (st.success if ok_bc else st.error)(msg_bc)
                        if ok_bc:
                            st.rerun()
