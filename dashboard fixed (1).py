...
if page == "الرئيسية":
    ...
    if maint_pass and maint_pass != maint_conf["maint_password"]:
        ...
        if pending_df.empty:
            st.info(f"مفيش أي أعطال محتاجة اعتماد في {factory_pick} دلوقتي ✅")
        else:
            st.caption(f"فيه {len(pending_df)} عطل مستني اعتماد في {factory_pick}")
    ...
    st.markdown(f"#### 🏆 ترتيب الخطوط حسب عدد الأعطال (من أول شهر {report_month_str} حتى اليوم) — {factory_pick}")
    st.bar_chart(line_fault_counts_sorted)

    # ---- إجمالي أعطال خطوط الإنتاج (كل المصانع مع بعض) — عدد + وقت التوقف بالدقيقة والساعة ----
    all_log_upto = month_log_upto[month_log_upto["الخط"].isin(ALL_FAULT_LINES)].copy()
    all_log_upto["مدة العطل (دقيقة)"] = pd.to_numeric(all_log_upto["مدة العطل (دقيقة)"], errors="coerce").fillna(0)
    line_totals_all = all_log_upto.groupby("الخط").agg(
        **{"عدد الأعطال": ("مدة العطل (دقيقة)", "count"), "إجمالي التوقف (دقيقة)": ("مدة العطل (دقيقة)", "sum")}
    ).reindex(ALL_FAULT_LINES).fillna(0)
    line_totals_all["عدد الأعطال"] = line_totals_all["عدد الأعطال"].astype(int)
    line_totals_all["إجمالي التوقف (ساعة)"] = (line_totals_all["إجمالي التوقف (دقيقة)"] / 60).round(2)
    line_totals_all = line_totals_all.reset_index().sort_values("إجمالي التوقف (دقيقة)", ascending=False).reset_index(drop=True)

    total_faults_all_count = int(line_totals_all["عدد الأعطال"].sum())
    total_faults_all_minutes = line_totals_all["إجمالي التوقف (دقيقة)"].sum()
    total_faults_all_hours = round(total_faults_all_minutes / 60, 2)

    st.markdown(f"#### 🏭 إجمالي أعطال خطوط الإنتاج — كل المصانع (من أول شهر {report_month_str} حتى اليوم)")
    st.caption(f"إجمالي عدد الأعطال: **{total_faults_all_count}** | إجمالي وقت التوقف: **{total_faults_all_minutes:,.0f} دقيقة** ({total_faults_all_hours:,.2f} ساعة)")
    st.dataframe(line_totals_all, use_container_width=True, hide_index=True)
    ...
    line_totals_all_html = line_totals_all.to_html(index=False, border=0, justify="center")
    reasons_html_table = (
    ...
    signature_html = f"""
    <div style="margin-top:16px; display:flex; gap:24px; page-break-inside:avoid; border-top:1px solid #d1d5db; padding-top:10px;">
        <div style="flex:1; font-size:10px;">
            <div style="margin-bottom:18px;">✅ اعتماد مدير صيانة مصنع 1: <b>{FACTORY_CONFIG['مصنع 1']['maint_name']}</b></div>
            <div>التوقيع: ......................................</div>
        </div>
        <div style="flex:1; font-size:10px;">
            <div style="margin-bottom:18px;">✅ اعتماد مدير صيانة مصنع 2: <b>{FACTORY_CONFIG['مصنع 2']['maint_name']}</b></div>
            <div>التوقيع: ......................................</div>
        </div>
    </div>
    """
    ...
    <div style="margin-top:8px; page-break-inside:avoid;">
        <h2 style="margin-top:0; font-size:11px;">🏭 إجمالي أعطال خطوط الإنتاج — كل المصانع (عدد + دقيقة + ساعة)</h2>
        <div class="side-small">{line_totals_all_html}</div>
    </div>
    <div style="margin-top:6px; display:flex; gap:12px; page-break-inside:avoid;">
        <div class="side-small" style="flex:1;">
            <h2 style="margin-top:0; font-size:11px;">🏆 ترتيب خطوط {factory_pick} حسب عدد الأعطال</h2>
            <div class="rank-list">
                {rank_rows_html}
            </div>
        </div>
        <div class="side-small" style="flex:1;">
            <h2 style="margin-top:0; font-size:11px;">📊 إجمالي التوقف الشهري لكل خط — {factory_pick} (دقيقة/ساعة)</h2>
            {line_summary_html}
        </div>
    </div>
    <div style="margin-top:8px; display:flex; gap:12px; page-break-inside:avoid;">
        <div style="flex:1;">
            <h2 style="margin-top:0; font-size:11px;">📈 تطور الأعطال اليومي — {factory_pick} (آخر 7 أيام)</h2>
    ...
    daily_single_page_height_mm = (
        ...
        + (len(line_totals_all) * 6)
        + (max(len(fault_lines), 1) * 6)
        + (len(line_summary_day) * 6)
        + 140  # صور الرسوم البيانية
        + (len(reasons_df) * 6)
        + (len(notes_list) * 6)
        + 60  # الإمضاءات
    )
    ...
    st.markdown("#### ✅ اعتماد التقرير")
    sig_col1, sig_col2 = st.columns(2)
    sig_col1.markdown(f"**اعتماد مدير صيانة مصنع 1:** {FACTORY_CONFIG['مصنع 1']['maint_name']}")
    sig_col2.markdown(f"**اعتماد مدير صيانة مصنع 2:** {FACTORY_CONFIG['مصنع 2']['maint_name']}")
