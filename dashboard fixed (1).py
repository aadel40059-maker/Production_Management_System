...
def parse_stock_balance_file(uploaded_file):
    """بتفتح ملف إكسيل رصيد الفروع/التوكيلات. لو لقت شيت فيه عمود 'الاجمالي' جاهز
    جنب 'اسم الصنف' (زي شيت التفاصيل اللي فيه كل الفروع+التوكيلات+الإجمالي في مكان واحد)،
    بتاخد رقم الإجمالي ده زي ما هو من غير ما تجمع حاجة بنفسها (عشان ده الرقم الصح المحسوب
    أصلاً في الملف)، وبتوقف عند أول شيت زي ده عشان متكررش الجمع مرتين من شيتات تانية.
    لو مفيش شيت فيه عمود إجمالي جاهز، بترجع تجمع أعمدة الفروع بنفسها كحل بديل.
    بترجع (dict التوقيع -> الإجمالي، أسماء الشيتات اللي اتقرت، أسماء الشيتات اللي اتجاهلت،
    dict التوقيع -> أول اسم أصلي كان في الإكسيل — مفيد للتشخيص لو صنف مش متطابق)"""
    try:
    ...
    def _find_header(raw):
        """بتدور على صف فيه عمود اسم الصنف (في أي عمود، مش بس أول عمود)، وترجع
        (رقم الصف، عمود اسم الصنف، عمود الإجمالي الجاهز لو موجود)"""
        max_scan_rows = min(10, len(raw))
        for r in range(max_scan_rows):
            item_col, total_col = None, None
    ...
    for sheet_name, raw in sheets_raw.items():
        ...
        for _, row in data.iterrows():
            ...
            if item_name == "" or item_name.lower() == "nan":
                continue
            if _is_total_row(item_name):
                continue
            total_val = _normalize_number(row.iloc[total_col_idx])
            key = _normalize_item_name(item_name)
            item_totals[key] = item_totals.get(key, 0.0) + total_val
            item_names.setdefault(key, item_name)

        return item_totals, [sheet_name], [s for s in sheets_raw if s != sheet_name], item_names

    # ---- الخطوة 2: مفيش أي شيت فيه عمود إجمالي جاهز، نرجع نجمع أعمدة الفروع بنفسنا ----
    item_totals, item_names = {}, {}
    matched_sheets, skipped_sheets = [], []
    for sheet_name, raw in sheets_raw.items():
        header_row_idx, item_col_idx, _ = _find_header(raw)
        # هنا بس بنشترط إن اسم الصنف يكون في أول عمود، عشان نضمن إن الشيت جدول بسيط
        # (صنف + أعمدة كميات فروع)، مش شيت فيه أعمدة زي الباركود/السعر ممكن تتلخبط لو جمعناها
        ...
        data = raw.iloc[header_row_idx + 1:].reset_index(drop=True)
        qty_col_idxs = [c for c in range(raw.shape[1]) if c != item_col_idx]
        ...
        for _, row in data.iterrows():
            ...
            key = _normalize_item_name(item_name)
            item_totals[key] = item_totals.get(key, 0.0) + row_total
            item_names.setdefault(key, item_name)
    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            stock_file_c = st.file_uploader(
                ...
                help="بيتقرا تلقائي أي شيت فيه عمود 'اسم الصنف' في أول عمود، وبيتجمع كل أعمدة الفروع/التوكيلات "
                     "لكل صنف كإجمالي واحد، مع تجاهل أي فواصل آلاف أو نقط في الأرقام"
            )
            stock_lookup_c = {}
            stock_names_c = {}
            if stock_file_c is not None:
                stock_lookup_c, matched_sheets_c, skipped_sheets_c, stock_names_c = parse_stock_balance_file(stock_file_c)
                if stock_lookup_c:
                    st.success(f"اتقرا رصيد {len(stock_lookup_c)} صنف من: {', '.join(matched_sheets_c)}")
                else:
                    st.warning("متلقاش في الملف ده أي شيت فيه عمود 'اسم الصنف' في أول عمود يتقرا منه")

            pm_df_c["حالة الأداء"] = [
                (f"{stock_lookup_c[_normalize_item_name(name)]:,.0f}"
                 if _normalize_item_name(name) in stock_lookup_c else "-")
                for name in pm_df_c["الصنف"]
            ]

            # ---------- تشخيص المطابقة: أي أصناف من التقرير متلاقتش في ملف المخزون وليه ----------
            if stock_file_c is not None and stock_lookup_c:
                unmatched_items_c = [n for n in pm_df_c["الصنف"] if _normalize_item_name(n) not in stock_lookup_c]
                if unmatched_items_c:
                    with st.expander(f"🔍 تشخيص المطابقة — {len(unmatched_items_c)} صنف من التقرير متلاقاش ليهم رصيد في الملف"):
                        for name in unmatched_items_c:
                            info = debug_product_signature(name)
                            same_core = [
                                stock_names_c[sig] for sig in stock_lookup_c
                                if sig.split("::")[0] == info["التوقيع النهائي"].split("::")[0] and sig.split("::")[0] != ""
                            ]
                            st.markdown(f"**{name}**")
                            st.write(
                                f"- بعد التطبيع: `{info['بعد التطبيع']}`\n"
                                f"- التعبئة المكتشفة: {info['التعبئة المكتشفة']}\n"
                                f"- الاسم الحقيقي (بدون تعبئة/براند): `{info['الاسم الحقيقي (بدون تعبئة/براند)']}`\n"
                                f"- التوقيع: `{info['التوقيع النهائي']}`"
                            )
                            if same_core:
                                st.write(f"- 💡 لقينا في ملف المخزون نفس الاسم بتعبئة مختلفة: {', '.join(set(same_core))} — راجع العدد/الوحدة")
                            else:
                                st.write("- ⚠ الاسم ده مش موجود خالص في ملف المخزون (ولا بتعبئة مختلفة)")
                            st.markdown("---")
