...
if page == "Dashboard":
    ...
    if summary.empty:
        ...
        pm_df = summary[["الصنف", day_col_pick, "المخطط بالكراتين (شهري)", "إجمالي الكراتين"]].copy()
        ...
        # ---------- بطاقات KPI ----------
        total_target_pm = summary[summary["المخطط بالكراتين (شهري)"] > 0]["المخطط بالكراتين (شهري)"].sum()
        total_actual_pm = summary[summary["المخطط بالكراتين (شهري)"] > 0]["إجمالي الكراتين"].sum()
        total_today_pm = summary[summary["المخطط بالكراتين (شهري)"] > 0][day_col_pick].sum()
