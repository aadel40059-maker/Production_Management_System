...
def build_dashboard_document(overall_title, overall_subtitle, sections):
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .dash-main {{ direction: rtl; flex: 1; min-width: 0; padding: 14px; }}
        ...
        .kpi-delta {{ font-size: 7.5px; font-weight: 800; margin-top: 3px; }}
...
if page == "الرئيسية":
        ...
        if not gsheet_exists(packing_sheet_r):
            ...
            if cat_summary_r.empty:
                ...
                pack_kpi_defs = [
                    ...
                    ("📈", f"{achievement_w:.1f}%", "نسبة التحقيق الإجمالية", "#7c3aed", delta_achv_w, delta_achv_w_pos),
                    ("🗂️", f"{categories_count_w}", "عدد الفئات", "#2563eb", None, None),
    ...
    with st.expander("🏭 تقرير البرودكشن الشهري", expanded=False):
        if not gsheet_exists(PRODUCTION_SHEET):
            ...
            if prod_month_r.empty:
                ...
                prod_kpi_defs = [
                    ("🏭", f"{lines_count_r}", "عدد الخطوط المنتجة", "#2563eb", None, None),
            ...
            if break_month_r.empty:
                ...
                if gsheet_exists("breakage_history"):
                    ...
                    if not break_prev_m.empty:
                        ...
                        prev_net_break = break_prev_m["كسر خارج (KG)"].sum() - break_prev_m["كسر داخل (KG)"].sum()

                delta_net_break, delta_net_break_pos = _delta_pack(total_net_r, prev_net_break)
                # الكسر: انخفاض القيمة يعتبر تحسّن، فالسهم بيتعكس (نزول = أخضر، زيادة = أحمر)
                if delta_net_break_pos is not None:
                    delta_net_break_pos = not delta_net_break_pos

                break_kpi_defs = [
                    ("📤", f"{total_out_r:,.0f} كجم", "إجمالي الكسر خارج", "#dc2626", None, None),
                    ("📥", f"{total_in_r:,.0f} كجم", "إجمالي الكسر داخل", "#16a34a", None, None),
                    ("⚖️", f"{total_net_r:,.0f} كجم", "صافي الكسر", "#e0a92e", delta_net_break, delta_net_break_pos),
                    ("📉", break_ratio_txt, "نسبة صافي الكسر من الإنتاج", "#7c3aed", None, None),
                ...
                if has_pie_break:
                    ...
                    st.pyplot(_donut_fig([ar_text(n) for n in break_line_summary_r["الخط"]], pie_vals_b.values,
                                          "نسبة كل خط من إجمالي الكسر", center_text=f"{total_net_r:,.0f}\nكجم"))
    ...
    if report_totals:
        ...
        if "breakage" in report_totals and report_totals["breakage"].get("line_df") is not None \
            ...
            ov_highlights.append({"البند": "🔻 أعلى خط كسر", "التفاصيل": f"{top_break_line['الخط']} — {top_break_line['صافي الكسر (KG)']:,.0f} كجم"})
        ...
        if "breakage" in report_totals and report_totals["breakage"].get("line_df") is not None \
            ...
            ov_chart_parts.append(dash_chart_card("📊 صافي الكسر لكل خط (كجم)", fig_to_base64_img(
                _bar_fig([ar_text(n) for n in break_ldf["الخط"]], break_ldf["صافي الكسر (KG)"],
                         "صافي الكسر لكل خط (كجم)", "#dc2626")
            )))
