...
if page == "Dashboard":
    ...
    if summary.empty:
        ...
        # ---------- تمييز الأصناف اللي نسبة تحقيقها منخفضة بسهم واضح + تلوين الصف ----------
        LOW_ACHIEVEMENT_THRESHOLD = 70  # %
        low_mask_bool = (print_df["نسبة التحقيق %"] < LOW_ACHIEVEMENT_THRESHOLD) & (print_df["الصنف"] != "الإجمالي")
        print_df.loc[low_mask_bool, "الصنف"] = "🔻 " + print_df.loc[low_mask_bool, "الصنف"].astype(str)
        low_highlight_mask = low_mask_bool.tolist()

        low_achievers_df = print_df[low_mask_bool][
            ["الصنف", "إجمالي الكراتين", "المخطط بالكراتين (شهري)", "نسبة التحقيق %"]
        ].sort_values("نسبة التحقيق %").reset_index(drop=True)
        ...
        if not low_achievers_df.empty:
            extra_tables_admin.insert(0, (f"🔻 أصناف بتحقيق أقل من {LOW_ACHIEVEMENT_THRESHOLD}%", low_achievers_df))
        ...
        printable_html = build_printable_html(
            ...
            bold=True,
            row_padding="8px 6px",
            highlight_mask=low_highlight_mask,
            highlight_color="#fde2e1",
            extra_tables=extra_tables_admin
        )
        st.download_button(
        ...
        # ==========================
        # تقرير تاني مخصوص لمدير الإنتاج — خط كبير، المخطط/الفعلي/المتبقي بس
        # ==========================
        pm_df = print_df[print_df["الصنف"] != "الإجمالي"][
            ["الصنف", "المخطط بالكراتين (شهري)", "إجمالي الكراتين"]
        ].copy()
        pm_df = pm_df.rename(columns={
            "المخطط بالكراتين (شهري)": "المخطط (كرتونة)",
            "إجمالي الكراتين": "الفعلي حتى الآن (كرتونة)"
        })
        pm_df["المتبقي (كرتونة)"] = pm_df["المخطط (كرتونة)"] - pm_df["الفعلي حتى الآن (كرتونة)"]
        pm_df["المتبقي (كرتونة)"] = pm_df["المتبقي (كرتونة)"].apply(lambda v: max(v, 0))

        pm_total_row = pd.DataFrame([{
            "الصنف": "الإجمالي",
            "المخطط (كرتونة)": pm_df["المخطط (كرتونة)"].sum(),
        ...
        st.download_button(
            ...
            mime="text/html",
            help="تقرير مبسّط بخط كبير: المخطط والفعلي والمتبقي لكل صنف — افتحه ودوس Ctrl+P واختار 'Save as PDF'"
        )
