...
if page == "الرئيسية":
    ...
    with st.expander("📊 تقرير تعبئة الشيفتات + إنتاج الخطوط لنفس اليوم"):
        ...
        if day_shift_log.empty:
            ...
            shift_printable_html = build_printable_html(
                f"تقرير الشيفتات — تعبئة وإنتاج الخطوط — {pack_factory_pick}",
            ...
            if shift_plan_day_col not in shift_plan_source.columns:
                ...
                if shift_plan_df.empty:
                    ...
                    shift_plan_html = build_printable_html(
                        f"تقرير التخطيط — تعبئة يوم {shift_plan_day} — {pack_factory_pick}",
    ...
    if summary.empty:
        ...
        if pack_factory_pick == "مصنع 2":
            ...
            # الترتيب المطلوب لمصنع 2: تعبئة اليوم، الفعلي/المخطط كرتونة، الفعلي/المخطط بالطن، نسبة الأداء، حالة الأداء، نسبة التحقيق
            pm_df = pm_df[[
                "الصنف", f"تعبئة يوم {day_pick}", "إجمالي الفعلي (كرتونة)", "إجمالي المخطط (كرتونة)",
        ...
        # ---------- جدول ملوّن: تعبئة اليوم بالفئات (حسب فئات المصنع المختار) ----------
        ...
        if combined_summary.empty:
            ...
            # كرت "تعبئة اليوم": كراتين ووزن تحت بعض في نفس الكرت
            today_value_html = f"""
            ...
            kpi_cards_c = [
                ...
                {
                    "icon": "📦", "label": f"تعبئة يوم {day_pick} — كل المصانع", "value": today_value_html,
                    "color": "#0891b2",
                    "delta": today_delta,
                    "delta_color": "#6b7280",
                },
                ...
                {
                    "icon": "🏁", "label": "نسبة تحقيق الخطة الشهرية بالوزن — كل المصانع", "value": f"{full_plan_pct_kg_c:.1f}%",
                    "color": "#16a34a" if full_plan_pct_kg_c >= 90 else ("#d97706" if full_plan_pct_kg_c >= 70 else "#dc2626"),
            ...
            # ---------- نسب تحقيق طلبيات التصدير — كرت لكل طلبية (الصنف — البلد) ----------
            ...
            if not export_orders_df_c.empty:
                ...
                st.markdown("##### 🚢 نسب تحقيق طلبيات التصدير")
                st.caption("الكرت فيه نسبة التحقيق الكلية لكل الطلبيات، وتحته بخط خفيف نسبة تحقيق كل طلبية لوحدها (بلد + شركة)")
                kpi_row(export_cards_c)

                export_cards_html_c = f"""
                <div style="margin-top:10px; page-break-inside:avoid;">
                    <h2 style="background:#0b1f3a; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                        🚢 نسب تحقيق طلبيات التصدير — شهر {selected_month}
                    </h2>
                    <div style="display:flex; flex-wrap:wrap;">{_pack_cards_html_c(export_cards_c)}</div>
                </div>
                """

            # ---------- اتجاه الإنتاج آخر 7 أيام (كل المصانع) + خط التارجت ----------
            trend_days_c = [d for d in range(max(1, day_pick - 6), day_pick + 1)]
            trend_cols_c = [f"يوم {d}" for d in trend_days_c]
            trend_weights_c_raw = [
            ...
            trend_days_c_nz = [d for d, w in zip(trend_days_c, trend_weights_c_raw) if w > 0]
            trend_weights_c_nz = [w for w in trend_weights_c_raw if w > 0]
            if trend_days_c_nz:
                trend_days_c, trend_weights_c = trend_days_c_nz, trend_weights_c_nz
            else:
                trend_weights_c = trend_weights_c_raw
            target_weights_c = [daily_target_weight_c] * len(trend_days_c)

            def _build_trend_fig_c(figsize=(7.2, 3.2), title_size=12):
                fig_tc, ax_tc = plt.subplots(figsize=figsize)
                fig_tc.patch.set_facecolor("#ffffff")
                ax_tc.plot(trend_days_c, trend_weights_c, marker="o", linewidth=2.4, color=BRAND_GREEN,
                           markerfacecolor="#ffffff", markeredgecolor=BRAND_GREEN, markeredgewidth=2,
                           markersize=7, zorder=3, label=ar_text("الفعلي (كجم)"))
                ax_tc.fill_between(trend_days_c, trend_weights_c, color=BRAND_GREEN, alpha=0.08, zorder=2)
                ax_tc.plot(trend_days_c, target_weights_c, linewidth=2, color="#dc2626", linestyle="--",
                           zorder=2, label=ar_text("التارجت اليومي (كجم)"))
                for x, y in zip(trend_days_c, trend_weights_c):
                    ax_tc.annotate(f"{y:,.0f}", xy=(x, y), xytext=(0, 8), textcoords="offset points",
                                   ha="center", fontsize=8, fontweight="bold", color="#1a1d21")
                ax_tc.set_xticks(trend_days_c)
                ax_tc.set_xticklabels([ar_text(f"يوم {d}") for d in trend_days_c], fontsize=9, fontweight="bold")
                ...
                ax_tc.set_title(ar_text("اتجاه الإنتاج — آخر 7 أيام (إجمالي الوزن كجم) — كل المصانع"),
                                 fontsize=title_size, fontweight="bold", color=BRAND_GREEN, pad=10)
                fig_tc.tight_layout()
                return fig_tc

            trend_chart_img_html_c = fig_to_base64_img(_build_trend_fig_c())
            ...
            if pm_single_page_c:
                ...
                single_page_height_mm_c = 95 + (main_rows_c * 9) + (cat_rows_c * 6) + (low_rows_c * 7) + (export_rows_c * 4) + (prod_lines_rows_c * 6) + 35 + 75
            ...
            st.caption(f"نفس تقرير الشيفتات بالظبط، بس تعبئة الشيفتات مجمّعة من تلات المصانع ليوم {day_pick} (نفس يوم تقرير مدير الإنتاج المجمّع) — وجدول إنتاج الخطوط بيجمع خطوط الكاندي والويفر مع بعض")
            ...
            for sh in SHIFT_OPTIONS_P:
                ...
                if sh_combined.empty:
                    st.caption("مفيش تعبئة مسجلة في الشيفت ده من أي مصنع")
                    ...
                    <table class="layout-wrapper" width="100%">
                        <tr>
                            <td valign="top" style="width:52%; padding-left:10px;">
                                <h3 style="margin:0 0 4px; font-size:11px; color:#374151;">📦 تعبئة الشيفت — كل المصانع</h3>
                                {pack_html_c}
                            </td>
            ...
            # ---------- كروت KPI: تعبئة كل شيفت (كرتونة + وزن في نفس الكرت) وتفاصيل كل مصنع بخط خفيف ----------
            shift_cards_c = []
            for sh in SHIFT_OPTIONS_P:
            ...
            st.download_button(
                ...
                mime="text/html",
                help="سكرول واحد متصل يجمع تعبئة الشيفتات من تلات المصانع + إنتاج خطوط الكاندي — افتحه ودوس Ctrl+P واختار 'Save as PDF'",
        ...
        if planning_df.empty:
            ...
            planning_html = build_printable_html(
                f"تقرير التخطيط — تعبئة يوم {day_pick} — {pack_factory_pick}",
    ...
    if report_grid_ex.empty:
        ...
        def _export_status_badge(status):
            if "✅" in str(status):
                return "#16a34a"
            elif "🔴" in str(status):
                return "#dc2626"
        ...
        row_badges_ex = [_export_status_badge(s) for s in report_grid_ex["حالة الطلبية"]]

        # نسخة الطباعة من غير عمود الشركة (زي ما طلب)
        print_cols_ex = [c for c in detail_df_display_ex.columns if c != "الشركة"]
        detail_df_print_ex = detail_df_display_ex[print_cols_ex]

        # جدول مخصص للطباعة: خط فاصل واضح (على كل خلية) بين كل طلبية والتانية،
        # وشريط لوني جانبي حسب حالة الطلبية، وهيدر وتباعد أوضح لشكل احترافي
        n_rows_ex = len(detail_df_print_ex)
        detail_header_html = "".join(f"<th>{c}</th>" for c in print_cols_ex)
        detail_rows_html = ""
        for i, (_, row) in enumerate(detail_df_print_ex.iterrows()):
            is_last_ex = (i == n_rows_ex - 1)
            row_border = "none" if is_last_ex else "2.5px solid #013e37"
            cells = []
            for j, c in enumerate(print_cols_ex):
                extra_border = f"border-right:5px solid {row_badges_ex[i]};" if j == 0 else ""
                cells.append(
                    f'<td style="background-color:{row_colors_ex[i]}; border-bottom:{row_border}; {extra_border}">'
                    f'{row[c]}</td>'
                )
            detail_rows_html += f"<tr>{''.join(cells)}</tr>"
        detail_table_html = (
            f'<table class="export-detail-table">'
            f'<thead><tr>{detail_header_html}</tr></thead>'
            f'<tbody>{detail_rows_html}</tbody></table>'
        )
        export_detail_extra_css = """
        <style>
            table.export-detail-table {
                border-collapse: collapse; width: 100%;
                border: 1.5px solid #013e37;
            }
            table.export-detail-table th {
                background: #013e37; color: #ffefb3; padding: 11px 8px; font-size: 12.5px;
                text-align: center; border-bottom: 3px solid #e0a92e; letter-spacing: 0.2px;
            }
            table.export-detail-table td {
                padding: 10px 8px; text-align: center; font-size: 12.5px; font-weight: 700; border: none;
            }
            table.export-detail-table td:first-child, table.export-detail-table th:first-child { text-align: right; }
        </style>
        """
        detail_report_title_bar = f"""
        <div style="display:flex; justify-content:space-between; align-items:center;
                    background:#f4f7fb; border:1px solid #d1d5db; border-radius:6px;
                    padding:8px 14px; margin-bottom:10px; font-size:12px; font-weight:800; color:#0b1f3a;">
            <span>📦 عدد الطلبيات: {len(detail_df_ex)}</span>
            <span style="color:#16a34a;">✅ مكتملة: {n_completed_ex}</span>
            <span style="color:#dc2626;">🔴 متأخرة: {n_delayed_ex}</span>
        </div>
        """
        ...
        detail_report_html = build_printable_html(
            ...
            f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            detail_df_display_ex,
            landscape=False,
            extra_html=export_detail_extra_css,
            top_html=f'<div style="margin-bottom:8px;">{detail_report_title_bar}{detail_table_html}</div>',
            base_font_size=13,
            bold=True,
        )
        # الجدول الافتراضي بيتحط جاهز ومنسّق في top_html فوق، فبنشيل نسخة الجدول العادي اللي بيتحط تلقائي تحته
        default_table_html_ex = detail_df_display_ex.to_html(index=False, border=0, justify="center", na_rep="-")
        detail_report_html = detail_report_html.replace(default_table_html_ex, "")
        st.download_button(
