...
if page == "الرئيسية":
    ...
    if admin_ok:
        ...
        def _factory_block_html(f_name, det, accent):
            summary_html = det["summary"].to_html(index=False, border=0, justify="center", classes="pro-table")
            return f"""
            <div style="margin-top:12px; border:1px solid #e5e7eb; border-right:5px solid {accent};
                ...
                <div class="side-small rank-list">{det['rank_html']}</div>
                <h2 style="margin-top:10px; font-size:10.5px;">📊 تفاصيل التوقف حسب الخط (من أول شهر {report_month_str} حتى اليوم)</h2>
                <div class="side-small">{summary_html}</div>
        ...
        <style>
            ...
            .pro-table {{ width:100%; border-collapse:collapse; font-size:9px; margin-top:4px; }}
            .pro-table th {{ background:#0b1f3a; color:#fff; padding:5px 6px; font-weight:700; }}
            .pro-table td {{ padding:4px 6px; border-bottom:1px solid #e5e7eb; text-align:center; }}
            .pro-table tr:nth-child(even) td {{ background:#f8fafc; }}
            .pro-table tr:last-child td {{ background:#eef2f7; font-weight:800; border-top:2px solid #0b1f3a; }}
        </style>
        {signature_html}
        {factory_blocks_html}
        ...
        daily_single_page_height_mm = (
            ...
            + sum(len(factory_details[fn]["summary"]) * 7 for fn in FACTORY_CONFIG)       # جدول التفاصيل الاحترافي لكل مصنع
