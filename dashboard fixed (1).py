...
if page == "Dashboard":
    ...
    # قراءة البيانات (أو إنشاء جدول فاضي لو أول مرة)
    if gsheet_exists(file_name):
        workers_df = read_gsheet(file_name).astype(str).fillna("")
        for c in ["الاسم", "الشيفت", "الكود", "خط السير"]:
            if c not in workers_df.columns:
                workers_df[c] = ""
        workers_df = workers_df[["الاسم", "الشيفت", "الكود", "خط السير"]]
    else:
        workers_df = pd.DataFrame(columns=["الاسم", "الشيفت", "الكود", "خط السير"])
    ...
    with st.expander("⚙️ لوحة المدير"):
        ...
        if admin_password == manager_pass:
            ...
            st.subheader("➕ إضافة أسماء دفعة واحدة (الصق قائمة أسماء)")
