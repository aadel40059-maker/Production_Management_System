...
def parse_stock_balance_file(uploaded_file):
    """بتفتح ملف إكسيل رصيد الفروع/التوكيلات، وبتدور في كل شيت على صف فيه عمود
    'اسم الصنف'، وبتجمع كل الأعمدة اللي بعده (أعمدة الفروع/التوكيلات) كإجمالي رصيد لكل صنف.
    ...
    for sheet_name in xls.sheet_names:
        ...
        header_row_idx, item_col_idx = None, None
        max_scan_rows = min(10, len(raw))
        for r in range(max_scan_rows):
            # بندوّر على 'اسم الصنف' في أول عمود بس (عمود 0) — عشان نضمن إن الشيت ده جدول بسيط
            # (صنف + أعمدة كميات للفروع)، ومش شيت تفصيلي فيه أعمدة زي الباركود/السعر/الوزن
            # ممكن تتلخبط مع أعمدة الكميات لو جمعناها غلط
            cell_val = str(raw.iat[r, 0]).strip()
            if cell_val in ITEM_NAME_HEADERS:
                header_row_idx, item_col_idx = r, 0
                break
        ...
        for _, row in data.iterrows():
            item_name_raw = row.iloc[item_col_idx]
