...
if page == "الرئيسية":
        ...
        if day_shift_log.empty:
            ...
            if shift_plan_day_col not in shift_plan_source.columns:
                ...
                if shift_plan_df.empty:
                    ...
                    st.caption("🥧 توزيع إنتاج اليوم بالفئات")
    ...
    if summary.empty:
        ...
        # ---------- جدول: الفعلي/المستهدف/نسبة التحقيق/النسبة المتوقعة بأيام الخطة — بالفئات (مصنع 1: هارد كاندي/طوفي/اكلير/فورية) ----------
        if pack_factory_pick == "مصنع 1":
            ...
            for cat in PACKING_FACTORY_CONFIG["مصنع 1"]["categories"]:
                ...
                cat_expected_kg = cat_target_kg * day_ratio
                cat_achv = (cat_actual_kg / cat_expected_kg * 100) if cat_expected_kg > 0 else None
                cat_perf_rows.append({
                    ...
                    "النسبة المتوقعة (بالنسبة لأيام الخطة) %": round(day_ratio * 100, 1),
                })
            cat_perf_df = pd.DataFrame(cat_perf_rows)
            cat_perf_row_colors = [pack_colors.get(str(v), "#f3f4f6") for v in cat_perf_df["الفئة"]]

            st.markdown("##### 📊 أداء الفئات (هارد كاندي / طوفي / اكلير / فورية) — الفعلي والمستهدف ونسبة التحقيق (بالوزن)")
            st.caption(
                f"النسبة المتوقعة محسوبة على أساس {actual_working_days} يوم عمل فعلي من أصل "
                f"{total_month_days} يوم مخطط لشهر {selected_month}"
            )
            cat_perf_display = cat_perf_df.assign(**{
                ...
                "النسبة المتوقعة (بالنسبة لأيام الخطة) %": cat_perf_df["النسبة المتوقعة (بالنسبة لأيام الخطة) %"].apply(
        ...
        if combined_summary.empty:
            ...
            def _load_export_orders_summary(month_str):
                ...
                cols_out = ["الصنف", "اسم البلد", "الشركة", "الكمية المطلوبة (KG)", "الفعلي حتى الآن (KG)", "نسبة التحقيق %"]
                if not (GSHEETS_ENABLED and gsheet_exists(sheet_name)):
                ...
                if "الكمية المطلوبة (KG)" not in df_e.columns:
                    df_e["الكمية المطلوبة (KG)"] = 0
                df_e["الكمية المطلوبة (KG)"] = pd.to_numeric(df_e["الكمية المطلوبة (KG)"], errors="coerce").fillna(0)
                ...
                if df_e.empty:
                    return pd.DataFrame(columns=cols_out)
                df_e["الفعلي حتى الآن (KG)"] = df_e[day_cols_e2].sum(axis=1)
            ...
            if not export_orders_df_c.empty:
                total_target_ex_c = export_orders_df_c["الكمية المطلوبة (KG)"].sum()
                total_actual_ex_c = export_orders_df_c["الفعلي حتى الآن (KG)"].sum()
        ...
        with cat_c1:
            st.caption("توزيع الإنتاج بين الفئات")
    ...
    # ---------- الأرقام في جدول التصدير (اللي بيتسجل فيه يوميًا) هي أصلاً بالكرتونة —
    # مفيش تحويل ولا جيب وزن من الباكينج، بناخد نفس الأرقام المسجّلة زي ما هي ----------
    report_grid_ex["الكمية المطلوبة (كرتونة)"] = report_grid_ex["الكمية المطلوبة (KG)"]
    report_grid_ex["الفعلي حتى الآن (كرتونة)"] = report_grid_ex["الفعلي حتى الآن (KG)"]
    report_grid_ex["المتبقي (كرتونة)"] = report_grid_ex["المتبقي (KG)"]
