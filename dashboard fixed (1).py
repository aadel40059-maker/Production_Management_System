...
if page == "الرئيسية":
    ...
    if admin_ok:
        ...
        def _build_factory_detail(f_name, f_lines):
            # ملحوظة مهمة: التوقف بالدقيقة/الساعة هنا بيتحسب من "السجل التفصيلي" (daily_log)
            # بالظبط زي عدد الأعطال وتكلفة الصيانة تحت — مش من الجدول الشهري المجمّع المنفصل،
            # عشان الثلاث جداول دي تفضل متطابقة دايمًا وميحصلش تعارض لو الجدول المجمّع اتأخر في المزامنة
            ...
            f_summary = (
                f_month_log.groupby("الخط")["مدة العطل (دقيقة)"].sum()
                .reindex(f_lines).fillna(0).reset_index()
                .rename(columns={"مدة العطل (دقيقة)": "إجمالي التوقف (دقيقة)"})
            )
