...
def append_gsheet(sheet_name, df_rows):
    ...
    if not existing:
        ...
        # لو الهيدر القديم في الشيت فيه أعمدة مش موجودة في البيانات الجديدة (زي أعمدة اتشالت من
        # الفورم بعدين)، أو العكس، بنظبط الأعمدة عشان القيم متتزحلقش في خانة غلط
        header = existing[0]
        aligned = df_rows.copy()
        for col in header:
            if col not in aligned.columns:
                aligned[col] = ""
        extra_cols = [c for c in aligned.columns if c not in header]
        aligned = aligned[header + extra_cols]
        if extra_cols:
            _gspread_retry(ws.update, [header + extra_cols], "A1")
    body_rows = [[_sanitize_cell_for_gsheet(v) for v in row] for row in aligned.values.tolist()]
...
def build_production_table():
    ...
    (المخطط والفعلي بس - من غير توقف أو هالك)"""
    data = []
...
if page == "Dashboard":
    ...
    # ==========================================================
    # 📊 تحليل الإنتاج الشهري - تجميع كل خط على مدار الشهر + رسومات يومية
    # ==========================================================
    st.divider()
    st.subheader("📊 تحليل الإنتاج الشهري")
    ...
    if gsheet_exists(PRODUCTION_SHEET):
        ...
        if not hist_df.empty and "Date" in hist_df.columns:
            ...
            # ---------- اختيار الشهر ----------
            today_pm = datetime.now().date()
            month_options_pm = []
            for i in range(-6, 7):
                y = today_pm.year + (today_pm.month - 1 + i) // 12
                m = (today_pm.month - 1 + i) % 12 + 1
                month_options_pm.append(f"{y}-{m:02d}")
            default_index_pm = month_options_pm.index(today_pm.strftime("%Y-%m"))
            selected_month_pm = st.selectbox(
                "📅 اختر الشهر (للتحليل)", month_options_pm,
                index=default_index_pm, key="production_month_select"
            )

            month_hist_pm = hist_df[hist_df["Date"].str.startswith(selected_month_pm)].copy()
            # الصف "All Shifts" هو إجمالي اليوم للخط، فبنعتمد عليه لتجنب تكرار عد الورديات
            month_hist_pm = month_hist_pm[month_hist_pm["Shift"] == "All Shifts"]

            if month_hist_pm.empty:
                st.info("لا توجد بيانات إنتاج مسجلة للشهر ده لسه")
            else:
                for nc in ["Target (KG)", "Actual (KG)"]:
                    month_hist_pm[nc] = pd.to_numeric(month_hist_pm[nc], errors="coerce").fillna(0)

                # ---------- ملخص كل خط على مدار الشهر ----------
                summary_pm = month_hist_pm.groupby("Line")[["Target (KG)", "Actual (KG)"]].sum()
                summary_pm["Achievement %"] = summary_pm.apply(
                    lambda r: (r["Actual (KG)"] / r["Target (KG)"] * 100) if r["Target (KG)"] > 0 else 0,
                    axis=1
                )
                st.markdown("##### إجمالي كل خط خلال الشهر")
                st.dataframe(
                    summary_pm.style.format({"Target (KG)": "{:,.0f}", "Actual (KG)": "{:,.0f}", "Achievement %": "{:.1f}%"}),
                    use_container_width=True
                )
                st.bar_chart(summary_pm[["Target (KG)", "Actual (KG)"]])

                # ---------- رسم يومي لكل خط ----------
                st.markdown("##### 📈 الأداء اليومي لكل خط")
                lines_available_pm = sorted(month_hist_pm["Line"].unique().tolist())
                cchart1, cchart2 = st.columns([2, 1])
                with cchart1:
                    selected_line_pm = st.selectbox("اختر الخط", lines_available_pm, key="production_line_chart_select")
                with cchart2:
                    chart_type_pm = st.radio(
                        "شكل الرسم", ["أعمدة", "خط بياني"],
                        horizontal=True, key="production_chart_type"
                    )

                line_daily_pm = (
                    month_hist_pm[month_hist_pm["Line"] == selected_line_pm]
                    .groupby("Date")[["Target (KG)", "Actual (KG)"]].sum()
                    .sort_index()
                )

                if line_daily_pm.empty:
                    st.info("لا توجد بيانات يومية لهذا الخط في الشهر المختار")
                else:
                    fig_pm, ax_pm = plt.subplots(figsize=(10, 4))
                    days_pm = line_daily_pm.index.tolist()
                    x_pm = np.arange(len(days_pm))

                    if chart_type_pm == "أعمدة":
                        width_pm = 0.38
                        ax_pm.bar(x_pm - width_pm / 2, line_daily_pm["Target (KG)"], width_pm,
                                  label=ar_text("المخطط"), color="#94a3b8")
                        ax_pm.bar(x_pm + width_pm / 2, line_daily_pm["Actual (KG)"], width_pm,
                                  label=ar_text("الفعلي"), color="#2f6fed")
                    else:
                        ax_pm.plot(x_pm, line_daily_pm["Target (KG)"], marker="o",
                                   label=ar_text("المخطط"), color="#94a3b8", linewidth=2)
                        ax_pm.plot(x_pm, line_daily_pm["Actual (KG)"], marker="o",
                                   label=ar_text("الفعلي"), color="#2f6fed", linewidth=2)

                    ax_pm.set_xticks(x_pm)
                    ax_pm.set_xticklabels([d[-2:] for d in days_pm], rotation=0)
                    ax_pm.set_xlabel(ar_text("يوم الشهر"))
                    ax_pm.set_ylabel(ar_text("كمية (KG)"))
                    ax_pm.set_title(ar_text(f"أداء خط {selected_line_pm.replace('🏭 ', '')} خلال {selected_month_pm}"))
                    ax_pm.legend()
                    ax_pm.grid(axis="y", linestyle="--", alpha=0.4)
                    fig_pm.tight_layout()
                    st.pyplot(fig_pm)
        else:
            st.info("لا توجد بيانات إنتاج محفوظة بعد لعرض التحليل الشهري")
    else:
        st.info("لا توجد بيانات إنتاج محفوظة بعد لعرض التحليل الشهري")
