...
if page == "الرئيسية":
    ...
    def _load_targets_df():
        ...
        return pd.DataFrame(columns=["الشهر", "المصنع", "تارجت الكهرباء", "تارجت الغاز", "تارجت المياه"])

    def _get_target(t_df, month_str, factory_name, col):
    ...
    def _energy_trend_fig(calc, factory_name, month_str, targets_map, n_days, figsize=(8, 3.2), title_size=12):
        figs = {}
        for u in ENERGY_UTILITIES:
            cons_series = calc[f"استهلاك {u['key']}"]
            daily_target = (targets_map.get(u["key"], 0) / n_days) if n_days > 0 else 0
    ...
    def _energy_kpi_cards(calc, targets_map):
        cards = []
        for u in ENERGY_UTILITIES:
            total_cons = calc[f"استهلاك {u['key']}"].sum()
            tgt = targets_map.get(u["key"], 0)
            ...
            cards.append({"icon": u["icon"], "label": f"إجمالي {u['key']} ({u['unit']})", "value": f"{total_cons:,.0f}", "color": u["color"]})
            cards.append({
                "icon": "🎯", "label": f"{u['key']} % من التارجت", "value": f"{pct:,.1f}%",
    ...
    # ==========================
    # التارجت الشهري لكل مصنع
    # ==========================
    st.divider()
    st.markdown("##### 🎯 التارجت الشهري")
    st.caption("التارجت بيتسجل لكل مصنع كل شهر لوحده، وبيستخدم في نسب المقارنة وخط التارجت في الرسم البياني والتقرير تحت")

    targets_df = _load_targets_df()
    tcol1, tcol2, tcol3 = st.columns(3)
    target_elec = tcol1.number_input(
        "⚡ تارجت الكهرباء الشهري (ك.و.س)", min_value=0.0,
        value=_get_target(targets_df, selected_month_e, energy_factory_pick, "تارجت الكهرباء"),
        step=100.0, key="energy_target_elec"
    )
    target_gas = tcol2.number_input(
        "🔥 تارجت الغاز الشهري (م³)", min_value=0.0,
        value=_get_target(targets_df, selected_month_e, energy_factory_pick, "تارجت الغاز"),
        step=100.0, key="energy_target_gas"
    )
    target_water = tcol3.number_input(
        "💧 تارجت المياه الشهري (م³)", min_value=0.0,
        value=_get_target(targets_df, selected_month_e, energy_factory_pick, "تارجت المياه"),
        step=100.0, key="energy_target_water"
    )
    ...
    if st.button("💾 حفظ التارجت", key="energy_target_save_btn"):
        ...
        new_target_row = pd.DataFrame([{
            ...
            "تارجت الكهرباء": target_elec, "تارجت الغاز": target_gas, "تارجت المياه": target_water,
        }])
    ...
    targets_map_e = {"الكهرباء": target_elec, "الغاز": target_gas, "المياه": target_water}

    # ==========================
    # KPIs ورسوم بيانية للمصنع المختار
    # ==========================
    st.divider()
    st.markdown(f"##### 📈 KPIs — {energy_factory_pick} — شهر {selected_month_e}")
    kpi_row(_energy_kpi_cards(energy_calc, targets_map_e))

    st.markdown("##### 📉 اتجاه الاستهلاك اليومي مقابل التارجت")
    energy_figs = _energy_trend_fig(energy_calc, energy_factory_pick, selected_month_e, targets_map_e, days_in_month_e)
    ...
    # ==========================
    # تقرير الطاقة الشامل — لتلات المصانع مع بعض، نفس استايل باقي التقارير في البرنامج
    # ==========================
    st.divider()
    st.markdown("#### 🖨️ تقرير الطاقة الشامل (لتلات المصانع)")
    st.caption("تقرير واحد بيجمع الكهرباء والغاز والمياه لكل المصانع الثلاثة — كروت KPI + رسوم بيانية بخط التارجت لكل مصنع")

    if st.button("📊 تجهيز تقرير الطاقة الشامل", key="energy_full_report_btn"):
        ...
        for f_name, f_conf in ENERGY_FACTORY_CONFIG.items():
            ...
            f_targets_map = {
                u["key"]: _get_target(targets_df, selected_month_e, f_name, f"تارجت {u['key']}")
                for u in ENERGY_UTILITIES
            }
            f_cards = _energy_kpi_cards(f_calc, f_targets_map)
            cards_html = "".join(
            ...
            f_figs = _energy_trend_fig(f_calc, f_name, selected_month_e, f_targets_map, days_in_month_e,
        ...
        energy_full_html = build_printable_html(
            f"تقرير الطاقة الشامل — الكهرباء والغاز والمياه — شهر {selected_month_e}",
            f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            pd.DataFrame(),
            landscape=True,
            extra_html=energy_report_blocks_html,
        )
        st.download_button(
            "🖨️ تحميل تقرير الطاقة الشامل (PDF) — لتلات المصانع",
            data=energy_full_html.encode("utf-8"),
            file_name=f"energy_report_{selected_month_e}.html",
            mime="text/html",
            help="افتح الملف بعد التحميل، ودوس Ctrl+P واختار 'Save as PDF' عشان تحوله PDF أو تطبعه",
