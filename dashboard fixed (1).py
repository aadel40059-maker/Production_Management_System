...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        # ---------- تفصيل بالوزن لكل صنف — يظهر في نسخة الطباعة بس لمصنع 2 (زي ما طلب) ----------
        m2_weight_section_html = ""
        if pack_factory_pick == "مصنع 2":
            m2_weight_df = summary[
                ["الصنف", "الوزن الفعلي (كيلو)", "المخطط بالوزن (كيلو)", "نسبة التحقيق %"]
            ].copy()
            m2_weight_df = m2_weight_df.rename(columns={
                "الوزن الفعلي (كيلو)": "إجمالي الفعلي بالوزن (كجم)",
                "المخطط بالوزن (كيلو)": "إجمالي المخطط بالوزن (كجم)",
            })
            m2_actual_sum = m2_weight_df["إجمالي الفعلي بالوزن (كجم)"].sum()
            m2_target_sum = m2_weight_df["إجمالي المخطط بالوزن (كجم)"].sum()
            m2_total_row = pd.DataFrame([{
                "الصنف": "الإجمالي",
                "إجمالي الفعلي بالوزن (كجم)": m2_actual_sum,
                "إجمالي المخطط بالوزن (كجم)": m2_target_sum,
                "نسبة التحقيق %": round(m2_actual_sum / m2_target_sum * 100, 1) if m2_target_sum > 0 else 0,
            }])
            m2_weight_display = pd.concat([m2_weight_df, m2_total_row], ignore_index=True)
            m2_row_colors = ["#f8fafc"] * (len(m2_weight_display) - 1) + ["#e5e7eb"]
            m2_weight_display_fmt = m2_weight_display.assign(**{
                "نسبة التحقيق %": m2_weight_display["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪")
            })
            m2_weight_section_html = f"""
            <div style="margin-top:10px; page-break-inside:avoid;">
                <h2 style="background:#7c3aed; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                    ⚖️ تفصيل بالوزن لكل صنف — الفعلي والمخطط ونسبة التحقيق
                </h2>
                {_df_to_html_highlighted(m2_weight_display_fmt, None, row_colors=m2_row_colors)}
            </div>
            """
        ...
        if pm_single_page:
            ...
            m2_weight_rows = len(m2_weight_display) if pack_factory_pick == "مصنع 2" else 0
            single_page_height_mm = 95 + (main_rows * 9) + (cat_rows * 6) + (prod_line_rows * 6) + (low_rows * 7) + (m2_weight_rows * 6) + 35 + 75
        ...
        if combined_summary.empty:
            ...
            if not export_orders_df_c.empty:
                ...
                orders_breakdown_ex_c = " | ".join(
                    f"{r_e['الصنف']} — {r_e['اسم البلد']}: {r_e['نسبة التحقيق %']:.1f}%"
                    for _, r_e in export_orders_df_c.iterrows()
                )
                ...
                st.caption("الكرت فيه نسبة التحقيق الكلية لكل الطلبيات، وتحته بخط خفيف نسبة تحقيق كل طلبية لوحدها")
    ...
    if report_grid_ex.empty:
        ...
        row_colors_ex = [_export_row_color(s) for s in detail_df_ex["حالة الطلبية"]]
