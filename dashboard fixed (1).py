...
def build_breakage_report_html(kpis, stage_names, stage_in, stage_out,
                                line_perf_rows, trend_dates, trend_in, trend_out, total_prod_for_pct,
                                report_title="تقرير تحليل الكسر (الداخلي والخارجي) لكل الخطوط"):
    """تقرير احترافي جدًا لتحليل الكسر حسب مرحلة العملية الإنتاجية — بنفس روح تقارير BI الجاهزة"""
    palette = ["#0d6b5c", "#e0a92e", "#4c8c7d", "#a06f0f", "#7fb5a6", "#2563eb", "#c98f1f", "#7c3aed"]

    total_in = sum(stage_in) if stage_in else 0
    total_out = sum(stage_out) if stage_out else 0
    total_break = total_in + total_out

    # ---------- جدول تحليل المراحل ----------
    stage_rows_html = ""
    for i, stg in enumerate(stage_names):
        s_in, s_out = stage_in[i], stage_out[i]
        s_total = s_in + s_out
        s_pct_of_prod = (s_total / total_prod_for_pct * 100) if total_prod_for_pct > 0 else 0
        s_kg_ton = (s_total / total_prod_for_pct * 1000) if total_prod_for_pct > 0 else 0
        s_contrib = (s_total / total_break * 100) if total_break > 0 else 0
        stage_rows_html += f"""
        <tr>
            <td class="line-cell">{stg}</td>
            <td>{s_in:,.0f}</td>
            <td>{s_out:,.0f}</td>
            <td style="font-weight:900;">{s_total:,.0f}</td>
            <td>{s_pct_of_prod:.2f}%</td>
            <td>{s_kg_ton:.2f}</td>
            <td>{s_contrib:.1f}%</td>
        </tr>
        """
    stage_rows_html += f"""
    <tr class="total-row">
        <td>الإجمالي</td><td>{total_in:,.0f}</td><td>{total_out:,.0f}</td><td>{total_break:,.0f}</td>
        <td>{(total_break/total_prod_for_pct*100) if total_prod_for_pct>0 else 0:.2f}%</td>
        <td>{(total_break/total_prod_for_pct*1000) if total_prod_for_pct>0 else 0:.2f}</td>
        <td>100%</td>
    </tr>
    """
    ...
    def _status_for_pct(pct):
        if pct <= 5:
            ...
            return "غير مقبول", "#dc2626"

    line_rows_html = ""
    for row in line_perf_rows:
        status, color = _status_for_pct(row["pct"])
        line_rows_html += f"""
        <tr>
            <td><span class="status-dot" style="background:{color};"></span>{status}</td>
            <td>{row['name']}</td>
            <td>{row['prod']:,.0f}</td>
            <td>{row['pct']:.2f}%</td>
            <td>{row['kg_ton']:.2f}</td>
            <td style="font-weight:900; color:{color};">{row['efficiency']:.2f}%</td>
        </tr>
        """
    ...
    for k in kpis:
        kpi_cards_html += f"""
        <div class="kpi" style="border-inline-start:4px solid {k['color']};">
            <div class="kpi-icon2" style="color:{k['color']};">{k['icon']}</div>
            <div class="kpi-label">{k['label']}</div>
            <div class="kpi-value">{k['value']}</div>
        </div>
        """
    ...
    <style>
        * {{ box-sizing: border-box; }}
        body {{ font-family: 'Cairo','Segoe UI',Tahoma,sans-serif; background: #fdf9ef; margin: 0; padding: 18px; color: #013e37; }}
        .report-title {{ font-size: 20px; font-weight: 900; margin-bottom: 14px; text-align: center; }}
        .grid {{ display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 18px; }}
        .kpi {{ flex: 1; min-width: 150px; background: #ffffff; border: 1px solid rgba(1,62,55,0.15);
        ...
        td {{ padding: 7px 9px; text-align: center; border-bottom: 1px solid rgba(1,62,55,0.08); font-weight: 700; }}
        .line-cell {{ font-weight: 800; }}
        tr.total-row td {{ background: #ffefb3; font-weight: 900; border-top: 2px solid #013e37; }}
        .status-dot {{ width: 9px; height: 9px; border-radius: 50%; display: inline-block; margin-left: 5px; }}
        .legend-strip {{ display: flex; gap: 6px; flex-wrap: wrap; justify-content: center; margin-top: 10px; }}
        .legend-chip {{ font-size: 11px; font-weight: 800; padding: 4px 10px; border-radius: 20px; color: #fff; }}
    </style>
    </head>
    <body>
    ...
    <div class="panel">
        <div class="panel-title">📋 تحليل الكسر حسب مراحل العملية الإنتاجية</div>
        <table>
            <tr><th>مرحلة العملية</th><th>كسر داخل (KG)</th><th>كسر خارج (KG)</th><th>إجمالي الكسر (KG)</th>
                <th>% من الإنتاج</th><th>KG/Ton</th><th>نسبة مساهمة المرحلة</th></tr>
            {stage_rows_html}
        </table>
    </div>
    ...
    <div class="row">
        <div class="col-1 panel">
            <div class="panel-title">📊 الكسر حسب مراحل العملية (داخل مقابل خارج)</div>
            <canvas id="stageBar" height="180"></canvas>
        </div>
        <div class="col-1 panel">
            <div class="panel-title">🥧 نسبة مساهمة كل مرحلة في إجمالي الكسر</div>
            <div style="position:relative; width:100%; max-width:260px; height:260px; margin:0 auto;">
                <canvas id="stageDonut"></canvas>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-1 panel">
            <div class="panel-title">📈 اتجاه الكسر (آخر 7 أيام)</div>
            <canvas id="trendChart" height="180"></canvas>
        </div>
        <div class="col-1 panel">
            <div class="panel-title">🏆 تقييم أداء الخطوط</div>
            <table>
                <tr><th>الحالة</th><th>اسم الخط</th><th>الإنتاج (KG)</th><th>نسبة الكسر %</th><th>KG/Ton</th><th>الكفاءة %</th></tr>
                {line_rows_html}
            </table>
            <div class="legend-strip">
                <span class="legend-chip" style="background:#16a34a;">ممتاز ≤ 5%</span>
                <span class="legend-chip" style="background:#0d6b5c;">جيد 5–10%</span>
                <span class="legend-chip" style="background:#e0a92e;">متوسط 10–15%</span>
                <span class="legend-chip" style="background:#ea580c;">ضعيف 15–20%</span>
                <span class="legend-chip" style="background:#dc2626;">غير مقبول &gt; 20%</span>
            </div>
        </div>
    </div>
    ...
    new Chart(document.getElementById('stageBar'), {{
        type: 'bar',
        data: {{ labels: {json.dumps(stage_names)}, datasets: [
            {{ label: 'كسر داخل', data: {json.dumps(stage_in)}, backgroundColor: '#e0a92e', borderRadius: 5 }},
            {{ label: 'كسر خارج', data: {json.dumps(stage_out)}, backgroundColor: '#dc2626', borderRadius: 5 }}
        ]}},
        options: {{
            responsive: true,
            plugins: {{ legend: {{ position: 'top', labels: {{ usePointStyle: true }} }} }},
    ...
    new Chart(document.getElementById('stageDonut'), {{
        type: 'doughnut',
        data: {{ labels: {json.dumps(stage_names)}, datasets: [{{
            data: {json.dumps([a + b for a, b in zip(stage_in, stage_out)])},
            backgroundColor: {json.dumps(palette[:len(stage_names)])}, borderColor: '#fff', borderWidth: 2
        }}]}},
        options: {{
            responsive: true, maintainAspectRatio: false,
    ...
    new Chart(document.getElementById('trendChart'), {{
        ...
        data: {{ labels: {json.dumps(trend_dates)}, datasets: [
            {{ label: 'الكسر الداخل', data: {json.dumps(trend_in)}, borderColor: '#e0a92e',
               backgroundColor: '#e0a92e22', fill: true, tension: 0.3, pointRadius: 3 }},
            {{ label: 'الكسر الخارج', data: {json.dumps(trend_out)}, borderColor: '#dc2626',
               backgroundColor: '#dc262622', fill: true, tension: 0.3, pointRadius: 3 }}
        ]}},
        options: {{
            responsive: true,
            plugins: {{ legend: {{ position: 'top', labels: {{ usePointStyle: true }} }} }},
...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        if pm_single_page:
            ...
            single_page_height_mm = 95 + (main_rows * 9) + (cat_rows * 6) + (prod_line_rows * 6) + (low_rows * 7) + 35
    ...
    page_banner("🔨", "الكسر", "تسجيل وتحليل الكسر الداخلي والخارجي لكل خط حسب مراحل العملية الإنتاجية")

    BREAKAGE_SHEET = "breakage_history"
    STAGES = ["الطباخ", "الفورمة", "سير التبريد", "الماكينات", "سير الفرز"]
    ...
    breakage_month_str = breakage_date.strftime("%Y-%m")

    all_lines_clean_b = [str(l).replace("🏭", "").strip() for l in st.session_state.production_lines]

    # ---------- هات إنتاج كل خط لنفس اليوم من صفحة Production ----------
    ...
    if GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
        ...
        day_hist_b = hist_df_b[(hist_df_b["Date"] == breakage_date_str) & (hist_df_b["Shift"] == "All Shifts")].copy()
        if not day_hist_b.empty:
            day_hist_b["Line"] = day_hist_b["Line"].astype(str).str.strip()
            day_hist_b["الفعلي (KG)"] = pd.to_numeric(day_hist_b["الفعلي (KG)"], errors="coerce").fillna(0)
    ...
    # ================= تسجيل الكسر — سهل وسلس: تاب لكل خط =================
    st.markdown(f"##### 📝 تسجيل الكسر حسب المرحلة — {breakage_date_str}")
    st.caption("اختار تاب الخط، سجّل الكسر الداخل والخارج لكل مرحلة، وبعدين دوس 'حفظ الكل' مرة واحدة تحت")

    tabs_b = st.tabs([f"{ln}  ({day_production[ln]:,.0f} KG)" for ln in all_lines_clean_b])
    editors_b = {}
    for i, ln in enumerate(all_lines_clean_b):
        with tabs_b[i]:
            stage_df = pd.DataFrame({
                "المرحلة": STAGES,
                "كسر داخل (KG)": [0] * len(STAGES),
                "كسر خارج (KG)": [0] * len(STAGES),
            })
            editors_b[ln] = st.data_editor(
                stage_df,
                column_config={
                    "المرحلة": st.column_config.TextColumn(disabled=True),
                    "كسر داخل (KG)": st.column_config.NumberColumn(min_value=0, step=1),
                    "كسر خارج (KG)": st.column_config.NumberColumn(min_value=0, step=1),
                },
                use_container_width=True,
                hide_index=True,
                key=f"breakage_editor_{ln}"
            )

    if st.button("💾 حفظ الكل — كل الخطوط مرة واحدة", type="primary"):
        all_rows_b = []
        for ln, edf in editors_b.items():
            tmp = edf.copy()
            tmp.insert(0, "الخط", ln)
            all_rows_b.append(tmp)
        to_save_b = pd.concat(all_rows_b, ignore_index=True)
        to_save_b = to_save_b[(to_save_b["كسر داخل (KG)"] != 0) | (to_save_b["كسر خارج (KG)"] != 0)]
        if to_save_b.empty:
    ...
    line_filter_b = st.selectbox("🔎 فلترة حسب خط معيّن (اختياري)", ["كل الخطوط"] + all_lines_clean_b, key="breakage_line_filter")

    if not GSHEETS_ENABLED or not gsheet_exists(BREAKAGE_SHEET):
        st.info("لسه مفيش بيانات كسر محفوظة")
    else:
        bh = read_gsheet(BREAKAGE_SHEET)
        if "المرحلة" not in bh.columns:
            st.warning("⚠ الشيت فيه بيانات كسر بالنسخة القديمة (من غير مراحل)، سجّل بيانات جديدة بالنسخة دي عشان التقرير يظهر صح")
        else:
            bh["Date"] = bh["Date"].astype(str)
            for nc in ["كسر داخل (KG)", "كسر خارج (KG)"]:
                bh[nc] = pd.to_numeric(bh[nc], errors="coerce").fillna(0)
            bh["الخط"] = bh["الخط"].astype(str).str.strip()

            day_b = bh[bh["Date"] == breakage_date_str].copy()
            if line_filter_b != "كل الخطوط":
                day_b = day_b[day_b["الخط"] == line_filter_b]

            if day_b.empty:
                st.info("لسه مفيش بيانات كسر محفوظة لنفس اليوم")
            else:
                total_prod_b = (
                    day_production.get(line_filter_b, 0) if line_filter_b != "كل الخطوط"
                    else sum(day_production.values())
                )
                total_in_b = day_b["كسر داخل (KG)"].sum()
                total_out_b = day_b["كسر خارج (KG)"].sum()
                total_break_b = total_in_b + total_out_b
                break_pct_b = (total_break_b / total_prod_b * 100) if total_prod_b > 0 else 0
                kg_per_ton_b = (total_break_b / total_prod_b * 1000) if total_prod_b > 0 else 0
                efficiency_b = max(0, 100 - break_pct_b)

                kpis_b = [
                    {"icon": "📦", "label": "إجمالي الإنتاج", "value": f"{total_prod_b:,.0f} KG", "color": "#0d6b5c"},
                    {"icon": "⬆️", "label": "إجمالي الكسر الخارجي", "value": f"{total_out_b:,.0f} KG", "color": "#dc2626"},
                    {"icon": "⬇️", "label": "إجمالي الكسر الداخلي", "value": f"{total_in_b:,.0f} KG", "color": "#e0a92e"},
                    {"icon": "🔻", "label": "إجمالي الكسر", "value": f"{total_break_b:,.0f} KG", "color": "#7c3aed"},
                    {"icon": "⚠️", "label": "نسبة الكسر من الإنتاج", "value": f"{break_pct_b:.2f}%", "color": "#dc2626"},
                    {"icon": "⚖️", "label": "الكسر لكل طن", "value": f"{kg_per_ton_b:.2f} KG/Ton", "color": "#2563eb"},
                    {"icon": "🥇", "label": "كفاءة الخطوط", "value": f"{efficiency_b:.2f}%", "color": "#0d6b5c"},
                ]

                # ---------- تحليل حسب المرحلة ----------
                stage_summary_b = day_b.groupby("المرحلة")[["كسر داخل (KG)", "كسر خارج (KG)"]].sum()
                stage_summary_b = stage_summary_b.reindex(STAGES, fill_value=0)
                stage_in_list = [float(v) for v in stage_summary_b["كسر داخل (KG)"]]
                stage_out_list = [float(v) for v in stage_summary_b["كسر خارج (KG)"]]

                # ---------- أداء الخطوط ----------
                lines_to_eval = [line_filter_b] if line_filter_b != "كل الخطوط" else all_lines_clean_b
                line_perf_rows_b = []
                for ln in lines_to_eval:
                    ln_data = day_b[day_b["الخط"] == ln] if line_filter_b == "كل الخطوط" else day_b
                    ln_in = ln_data["كسر داخل (KG)"].sum()
                    ln_out = ln_data["كسر خارج (KG)"].sum()
                    ln_prod = day_production.get(ln, 0)
                    ln_break = ln_in + ln_out
                    ln_pct = (ln_break / ln_prod * 100) if ln_prod > 0 else 0
                    ln_kgton = (ln_break / ln_prod * 1000) if ln_prod > 0 else 0
                    ln_eff = max(0, 100 - ln_pct)
                    line_perf_rows_b.append({
                        "name": ln, "prod": ln_prod, "pct": ln_pct, "kg_ton": ln_kgton, "efficiency": ln_eff
                    })

                # ---------- اتجاه آخر 7 أيام ----------
                trend_dates_b = [(breakage_date - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(6, -1, -1)]
                trend_in_b, trend_out_b = [], []
                bh_scope = bh if line_filter_b == "كل الخطوط" else bh[bh["الخط"] == line_filter_b]
                daily_totals_b = bh_scope.groupby("Date")[["كسر داخل (KG)", "كسر خارج (KG)"]].sum()
                for d in trend_dates_b:
                    if d in daily_totals_b.index:
                        trend_in_b.append(float(daily_totals_b.loc[d, "كسر داخل (KG)"]))
                        trend_out_b.append(float(daily_totals_b.loc[d, "كسر خارج (KG)"]))
                    else:
                        trend_in_b.append(0.0)
                        trend_out_b.append(0.0)

                report_title_b = (
                    f"تقرير تحليل الكسر — {line_filter_b}" if line_filter_b != "كل الخطوط"
                    else "تقرير تحليل الكسر (الداخلي والخارجي) لكل الخطوط"
                )

                breakage_html = build_breakage_report_html(
                    kpis_b, STAGES, stage_in_list, stage_out_list,
                    line_perf_rows_b, trend_dates_b, trend_in_b, trend_out_b, total_prod_b,
                    report_title=report_title_b
                )
    ...
    page_banner("📑", "Reports", "تقارير شاملة عن الإنتاج والباكينج والأعطال")
