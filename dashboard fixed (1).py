...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            total_expected_c = (total_target_c * day_ratio_c)
            avg_perf_c = (total_actual_c / total_expected_c * 100) if total_expected_c > 0 else 0
            full_plan_pct_c = (total_actual_c / total_target_c * 100) if total_target_c > 0 else 0
            ...
            kpi_cards_c = [
                {
                    "icon": "📦", "label": f"تعبئة يوم {day_pick} (كرتونة) — كل المصانع", "value": f"{total_today_c:,.0f}",
                    "color": "#0891b2",
                    "delta": _factory_line(lambda d: f"{d[day_col_pick].sum():,.0f}"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "🎯", "label": "إجمالي المخطط (كرتونة) — كل المصانع", "value": f"{total_target_c:,.0f}",
                    "color": "#2563eb",
                    "delta": _factory_line(lambda d: f"{d['المخطط بالكراتين (شهري)'].sum():,.0f}"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "📈", "label": "إجمالي الفعلي (كرتونة) — كل المصانع", "value": f"{total_actual_c:,.0f}",
                    "color": "#16a34a",
                    "delta": _factory_line(lambda d: f"{d['إجمالي الكراتين'].sum():,.0f}"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "📊", "label": "متوسط الأداء (من المتوقع) — كل المصانع", "value": f"{avg_perf_c:.1f}%",
                    "color": "#7c3aed" if avg_perf_c >= 90 else ("#d97706" if avg_perf_c >= 70 else "#dc2626"),
                    "delta": _factory_line(
                        lambda d: f"{round((d['إجمالي الكراتين'].sum() / (d['المخطط بالكراتين (شهري)'].sum() * day_ratio_c) * 100), 1) if d['المخطط بالكراتين (شهري)'].sum() * day_ratio_c > 0 else 0}%"
                    ),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "🏁", "label": "نسبة تحقيق الخطة الشهرية الكاملة — كل المصانع", "value": f"{full_plan_pct_c:.1f}%",
                    "color": "#f59e0b" if full_plan_pct_c >= 90 else ("#d97706" if full_plan_pct_c >= 70 else "#dc2626"),
                    "delta": _factory_line(
                        lambda d: f"{round((d['إجمالي الكراتين'].sum() / d['المخطط بالكراتين (شهري)'].sum() * 100), 1) if d['المخطط بالكراتين (شهري)'].sum() > 0 else 0}%"
                    ),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "⚖️", "label": "الوزن الفعلي (كجم) — كل المصانع", "value": f"{total_actual_kg_c:,.0f}",
                    "color": "#2563eb",
                    "delta": _factory_line(lambda d: f"{(d['إجمالي الكراتين'] * d['وزن الكرتونة (كيلو)']).sum():,.0f} كجم"),
                    "delta_color": "#6b7280",
                },
            ]
            kpi_row(kpi_cards_c)
            st.caption(
                f"📅 أيام العمل المخططة: {total_month_days_c} — أيام العمل الفعلية: {actual_working_days_c} — "
                f"أصناف أقل من المتوقع: {low_items_count_c} من {items_count_c}"
            )
            ...
            def _pack_cards_html_c(cards):
                ...
                for c in cards:
                    ...
                    if c.get("delta"):
                        ...
                        delta_html = f'<div style="font-size:9.5px; font-weight:600; color:{d_color}; margin-top:3px;">{c["delta"]}</div>'
                    parts.append(f"""<div style="flex:1; min-width:160px; border:1px solid #e5e7eb; border-radius:8px; padding:8px 10px; margin:3px;">
