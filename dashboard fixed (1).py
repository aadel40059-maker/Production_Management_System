...
if page == "الرئيسية":
    ...
    # ==========================
    # اعتماد الأعطال وتكلفة الصيانة (مدير الصيانة بس)
    # ==========================
    st.subheader("🛠️ اعتماد الأعطال (مدير الصيانة)")
    st.caption(
        "بعد ما مهندس الإنتاج يسجل العطل، بيفضل 'قيد مراجعة الصيانة' لحد ما مدير الصيانة يعتمده هنا "
        "ويحدد تكلفة الصيانة (أو يأكد إنه من غير تكلفة مادية)."
    )
    MAINTENANCE_PASSWORD = "8899"
    maint_pass = st.text_input("🔒 كلمة مرور مدير الصيانة", type="password", key="maint_approval_pass")

    if maint_pass and maint_pass != MAINTENANCE_PASSWORD:
        st.warning("كلمة مرور مدير الصيانة غلط")
    elif maint_pass == MAINTENANCE_PASSWORD:
        st.success("تم التحقق ✔ — تقدر تعتمد الأعطال وتحدد تكلفة الصيانة")

        HAS_DIALOG = hasattr(st, "dialog")

        def _save_fault_approval(fault_id, no_cost, cost_val, approver_name):
            ...
            full_log_now.loc[idx_match[0], "معتمد بواسطة"] = approver_name.strip() or "مدير الصيانة"
        ...
        def _render_approval_form(row, key_prefix):
            ...
            approver_name = st.text_input("👤 اسم مدير الصيانة (اختياري)", key=f"{key_prefix}_name")
            if st.button("💾 حفظ الاعتماد", key=f"{key_prefix}_save"):
                _save_fault_approval(row["معرف العطل"], no_cost, cost_val, approver_name)

        full_log_appr = load_daily_log()
        pending_df = full_log_appr[full_log_appr["الحالة"] == STATUS_PENDING].copy()

        if pending_df.empty:
            st.info("مفيش أي أعطال محتاجة اعتماد دلوقتي ✅")
        else:
            st.caption(f"فيه {len(pending_df)} عطل مستني اعتماد")
