...
def _gspread_retry(func, *args, max_attempts=5, **kwargs):
    ...
    بعد شوية ثانية (بتزيد تدريجيًا + هامش عشوائي بسيط عشان لو أكتر من مستخدم بيحاول في نفس اللحظة
    مايصطدموش في نفس التوقيت بالظبط) بدل ما البرنامج يقع فجأة برسالة خطأ حمراء طويلة. أي خطأ تاني
    (زي تاب مش موجود) بيتسيب يطلع على طول من غير تأخير."""
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1 and (status is None or status in (429, 500, 502, 503, 504)):
                time.sleep(2 * (attempt + 1) + random.uniform(0, 1.2))
                continue
            raise
...
def _read_gsheet_cached(sheet_name):
    """قراءة مؤقتة (60 ثانية) عشان نقلل عدد النداءات لجوجل شيت في نفس التحميل
...
def _remember_last_good(sheet_name, records):
    """بيحتفظ بآخر نسخة ناجحة من كل شيت جوه الجلسة، عشان لو القراءة فشلت مؤقتًا
    نقدر نعرض آخر بيانات معروفة بدل ما نقول 'مفيش بيانات' غلط"""
    st.session_state.setdefault("_gsheet_last_good", {})[sheet_name] = records


def _get_last_good(sheet_name):
    return st.session_state.get("_gsheet_last_good", {}).get(sheet_name)


def gsheet_exists(sheet_name):
    """بيرجع True لو فيه بيانات فعلية جوه الشيت ده (مش بس هيدر أو فاضي).
    بيستخدم نفس القراءة المخزّنة اللي بيستخدمها read_gsheet — مش نداء API منفصل —
    عشان النمط الشائع (gsheet_exists ثم read_gsheet) يكلّف نداء واحد بس مش اتنين"""
    if not GSHEETS_ENABLED:
        return False
    try:
        records = _read_gsheet_cached(sheet_name)
        _remember_last_good(sheet_name, records)
        return len(records) > 0
    except Exception:
        last_good = _get_last_good(sheet_name)
        return len(last_good) > 0 if last_good is not None else False
...
def read_gsheet(sheet_name):
    try:
        ...
        if last_good is not None:
            st.warning(
                f"⚠ تعذّر تحديث شيت '{sheet_name}' دلوقتي (ضغط مؤقت على الـ API) — "
                "البيانات المعروضة من آخر تحميل ناجح في الجلسة دي، مش لازم تكون أحدث حاجة."
            )
            return pd.DataFrame(last_good)
        st.warning(
...
def write_gsheet(sheet_name, df):
    ...
    if df.empty:
        ...
        _read_gsheet_cached.clear(sheet_name)
        _remember_last_good(sheet_name, [])
        return
    ...
    # نمسح كاش الشيت ده بس (مش كل الشيتات التانية) ونحدّث نسخة الاحتياط فورًا بالبيانات الطازة —
    # كده أي قراءة تانية في نفس الجلسة مش محتاجة تكلم الـ API خالص لحد ما الكاش ينتهي فعلاً
    _read_gsheet_cached.clear(sheet_name)
    fresh_records = [dict(zip(header, row)) for row in body_rows]
    _remember_last_good(sheet_name, fresh_records)


def append_gsheet(sheet_name, df_rows):
    ws = _get_worksheet(sheet_name)
    try:
        existing_records = _read_gsheet_cached(sheet_name)
    except Exception:
        existing_records = _get_last_good(sheet_name) or []
    ...
    _read_gsheet_cached.clear(sheet_name)
    new_records = [dict(zip(df_rows.columns.tolist(), row)) for row in body_rows]
    _remember_last_good(sheet_name, list(existing_records) + new_records)
            ...
            if GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
                ...
                if "Date" not in hist_df_shift.columns or hist_df_shift.empty:
                    ...
                    if day_shift_prod.empty:
                        ...
                        prod_shift_table = day_shift_prod.groupby(["Line", "Shift"])[["Target (KG)", "Actual (KG)"]].sum().reset_index()
                        prod_shift_table.columns = ["الخط", "الشيفت", "المخطط (KG)", "الفعلي (KG)"]
