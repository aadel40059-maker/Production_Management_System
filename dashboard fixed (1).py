...
if page == "الرئيسية":
    ...
    if st.button("📊 توليد ملف الإكسيل", key="reports_generate_excel_btn"):
        ...
        prod_rows_x = []
        if gsheet_exists(PRODUCTION_SHEET):
            prod_hist_x = read_gsheet(PRODUCTION_SHEET)
            prod_hist_x["Date"] = prod_hist_x["Date"].astype(str)
            prod_day_x = prod_hist_x[(prod_hist_x["Date"] == excel_day_str) & (prod_hist_x["Shift"] == "All Shifts")].copy()
            if not prod_day_x.empty:
                prod_day_x["المستهدف (KG)"] = pd.to_numeric(prod_day_x["المستهدف (KG)"], errors="coerce").fillna(0)
                prod_day_x["الفعلي (KG)"] = pd.to_numeric(prod_day_x["الفعلي (KG)"], errors="coerce").fillna(0)
                for _, r in prod_day_x.iterrows():
                    achv = round((r["الفعلي (KG)"] / r["المستهدف (KG)"] * 100), 1) if r["المستهدف (KG)"] > 0 else 0
                    prod_rows_x.append([r["Line"], r["المستهدف (KG)"], r["الفعلي (KG)"], achv])

        # ---------- الباكينج ----------
        pack_rows_x = []
        if gsheet_exists(excel_pack_sheet):
            pack_grid_x = read_gsheet(excel_pack_sheet)
            if excel_day_col in pack_grid_x.columns:
                pack_grid_x["المخطط بالكراتين (شهري)"] = pd.to_numeric(pack_grid_x.get("المخطط بالكراتين (شهري)", 0), errors="coerce").fillna(0)
                pack_grid_x[excel_day_col] = pd.to_numeric(pack_grid_x[excel_day_col], errors="coerce").fillna(0)
                pack_grid_x["وزن الكرتونة (كيلو)"] = pd.to_numeric(pack_grid_x.get("وزن الكرتونة (كيلو)", 0), errors="coerce").fillna(0)
                for _, r in pack_grid_x.iterrows():
                    ...
                    actual_cartons = r[excel_day_col]
        ...
        if gsheet_exists("faults_daily_log"):
            flog_x = read_gsheet("faults_daily_log")
            flog_x["التاريخ"] = flog_x["التاريخ"].astype(str)
            fday_x = flog_x[flog_x["التاريخ"] == excel_day_str]
            for _, r in fday_x.iterrows():
                faults_rows_x.append([
        ...
        if gsheet_exists("breakage_history"):
            blog_x = read_gsheet("breakage_history")
            blog_x["Date"] = blog_x["Date"].astype(str)
            bday_x = blog_x[blog_x["Date"] == excel_day_str]
            for _, r in bday_x.iterrows():
                ...
                breakage_rows_x.append([r.get("الخط", ""), opening_x, out_x, in_x, closing_x])
        ...
        # ---------- كتابة شيت "تراكمي" — صف لكل يوم من أول الشهر لحد اليوم المختار ----------
        cum_headers_x = [
            "اليوم", "إنتاج - تراكمي فعلي (KG)", "إنتاج - تراكمي مستهدف (KG)",
            "باكينج - تراكمي فعلي (كرتونة)", "باكينج - تراكمي مستهدف (كرتونة)",
            "أعطال - تراكمي العدد", "أعطال - تراكمي التوقف (دقيقة)",
            "كسر - تراكمي خارج (KG)", "كسر - تراكمي داخل (KG)"
        ]
        for ci, h in enumerate(cum_headers_x, start=1):
            c = ws_cum_x.cell(row=1, column=ci, value=h)
            c.font = HEADER_FONT_X
            c.fill = HEADER_FILL_X
            c.alignment = Alignment(horizontal="center")
        ...
        pack_daily_target_total_x = 0
        if gsheet_exists(excel_pack_sheet):
            pack_grid_month_x = read_gsheet(excel_pack_sheet)
            if "المخطط بالكراتين (شهري)" in pack_grid_month_x.columns and excel_days_in_month > 0:
                pack_daily_target_total_x = (
                    pd.to_numeric(pack_grid_month_x["المخطط بالكراتين (شهري)"], errors="coerce").fillna(0).sum()
                    / excel_days_in_month
                )
        ...
        cum_prod_actual_x = cum_prod_target_x = 0.0
        cum_pack_actual_x = cum_pack_target_x = 0.0
        cum_faults_n_x = 0
        cum_faults_min_x = 0.0
        cum_break_out_x = cum_break_in_x = 0.0
        ...
        for d in range(1, excel_day_num + 1):
            ...
            if not prod_hist_month_x.empty:
                day_p_x = prod_hist_month_x[prod_hist_month_x["Date"] == d_str]
                cum_prod_actual_x += day_p_x["الفعلي (KG)"].sum()
                cum_prod_target_x += day_p_x["المستهدف (KG)"].sum()

            if not pack_grid_month_x.empty and dcol in pack_grid_month_x.columns:
                pack_grid_month_x[dcol] = pd.to_numeric(pack_grid_month_x[dcol], errors="coerce").fillna(0)
                cum_pack_actual_x += pack_grid_month_x[dcol].sum()
                cum_pack_target_x += pack_daily_target_total_x

            if not faults_log_month_x.empty:
                day_f_x = faults_log_month_x[faults_log_month_x["التاريخ"] == d_str]
                cum_faults_n_x += len(day_f_x)
                cum_faults_min_x += day_f_x["مدة العطل (دقيقة)"].sum()

            if not breakage_log_month_x.empty:
                day_b_x = breakage_log_month_x[breakage_log_month_x["Date"] == d_str]
                if "كسر خارج (KG)" in day_b_x.columns:
                    cum_break_out_x += day_b_x["كسر خارج (KG)"].sum()
                if "كسر داخل (KG)" in day_b_x.columns:
                    cum_break_in_x += day_b_x["كسر داخل (KG)"].sum()

            r_idx_x = d + 1
            ws_cum_x.cell(row=r_idx_x, column=1, value=d).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=2, value=round(cum_prod_actual_x, 1)).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=3, value=round(cum_prod_target_x, 1)).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=4, value=round(cum_pack_actual_x, 1)).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=5, value=round(cum_pack_target_x, 1)).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=6, value=cum_faults_n_x).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=7, value=round(cum_faults_min_x, 1)).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=8, value=round(cum_break_out_x, 1)).font = NORMAL_FONT_X
            ws_cum_x.cell(row=r_idx_x, column=9, value=round(cum_break_in_x, 1)).font = NORMAL_FONT_X

        for col_i in range(1, 10):
            ws_cum_x.column_dimensions[get_column_letter(col_i)].width = 24
