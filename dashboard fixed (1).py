...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        planning_df = summary[summary[day_col_pick] > 0][[
            "الصنف", "الفئة", day_col_pick, "وزن الكرتونة (كيلو)", "المخطط بالكراتين (شهري)"
        ]].copy()
        ...
        if planning_df.empty:
            ...
            planning_df = planning_df[[
                "الصنف", "الفئة", "تعبئة اليوم (كرتونة)", "وزن اليوم (كجم)",
            ...
            # ---------- رسمين بيانيين: أعمدة كمية كل صنف + دائرة توزيع اليوم بالفئات ----------
            bar_labels_plan = [ar_text(n) for n in planning_df["الصنف"]]
            cat_today_totals = planning_df.groupby("الفئة")["تعبئة اليوم (كرتونة)"].sum()

            plan_chart_c1, plan_chart_c2 = st.columns([1.3, 1])
            with plan_chart_c1:
                st.caption(f"📊 كمية كل صنف المعبأة يوم {day_pick} (كرتونة)")
                fig_plan_bar, ax_plan_bar = plt.subplots(figsize=(6.6, 3.6))
                fig_plan_bar.patch.set_facecolor("#ffffff")
                bars_plan = ax_plan_bar.bar(
                    bar_labels_plan, planning_df["تعبئة اليوم (كرتونة)"],
                    color=BRAND_GREEN, edgecolor="#00201c", linewidth=0.7, zorder=3
                )
                for b in bars_plan:
                    h = b.get_height()
                    ax_plan_bar.annotate(f"{h:,.0f}", xy=(b.get_x() + b.get_width() / 2, h),
                                          xytext=(0, 4), textcoords="offset points",
                                          ha="center", va="bottom", fontsize=8, fontweight="bold", color="#1a1d21")
                ax_plan_bar.set_xticks(range(len(bar_labels_plan)))
                ax_plan_bar.set_xticklabels(bar_labels_plan, rotation=35, ha="right", fontsize=9, fontweight="bold")
                ax_plan_bar.tick_params(axis="y", labelsize=9, colors="#4b5563")
                ax_plan_bar.spines["top"].set_visible(False)
                ax_plan_bar.spines["right"].set_visible(False)
                ax_plan_bar.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
                ax_plan_bar.set_axisbelow(True)
                fig_plan_bar.tight_layout()
                st.pyplot(fig_plan_bar)
            ...
            # ---------- نفس الرسمين بجودة طباعة، يتحطوا جوه تقرير الـ PDF ----------
            fig_plan_bar_print, ax_plan_bar_print = plt.subplots(figsize=(7.4, 3.6))
            fig_plan_bar_print.patch.set_facecolor("#ffffff")
            bars_plan_p = ax_plan_bar_print.bar(
                bar_labels_plan, planning_df["تعبئة اليوم (كرتونة)"],
                color=BRAND_GREEN, edgecolor="#00201c", linewidth=0.7, zorder=3
            )
            for b in bars_plan_p:
                h = b.get_height()
                ax_plan_bar_print.annotate(f"{h:,.0f}", xy=(b.get_x() + b.get_width() / 2, h),
                                            xytext=(0, 4), textcoords="offset points",
                                            ha="center", va="bottom", fontsize=8, fontweight="bold", color="#1a1d21")
            ax_plan_bar_print.set_title(ar_text(f"كمية كل صنف المعبأة يوم {day_pick}"),
                                         fontsize=13, fontweight="bold", color=BRAND_GREEN, pad=12)
            ax_plan_bar_print.set_xticks(range(len(bar_labels_plan)))
            ax_plan_bar_print.set_xticklabels(bar_labels_plan, rotation=35, ha="right", fontsize=9, fontweight="bold")
            ax_plan_bar_print.tick_params(axis="y", labelsize=9, colors="#4b5563")
            ax_plan_bar_print.spines["top"].set_visible(False)
            ax_plan_bar_print.spines["right"].set_visible(False)
            ax_plan_bar_print.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
            ax_plan_bar_print.set_axisbelow(True)
            fig_plan_bar_print.tight_layout()
            ...
            <div style="display:flex; gap:14px; margin-top:8px; page-break-inside:avoid;">
                <div style="flex:1.3;">{fig_to_base64_img(fig_plan_bar_print)}</div>
                <div style="flex:1;">{fig_to_base64_img(fig_plan_pie_print)}</div>
            </div>
            """

            # ارتفاع الصفحة بيتحسب حسب عدد صفوف الجدول عشان التقرير يطلع صفحة واحدة مظبوطة، مش فاضية ومش مقصوصة
            planning_single_page_height_mm = 100 + (len(planning_df) * 8) + 110
