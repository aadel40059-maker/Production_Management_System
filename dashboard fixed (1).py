...
if page == "الرئيسية":
    ...
    if not prod_by_line.empty:
        ...
        fig_prod_tree = build_treemap_chart(prod_line_totals, unit=" KG")
    ...
    if not pack_by_cat.empty:
        fig_pack_tree = build_treemap_chart(pack_by_cat, unit=" كرتونة")
        if fig_pack_tree is not None:
            st.pyplot(fig_pack_tree)
    else:
        st.info("لا توجد بيانات باكينج مسجلة لهذا الشهر")

    st.markdown("###### ⚠ توزيع وقت التوقف حسب الخط (دقيقة)")
    if not faults_by_line.empty:
        fig_faults_tree = build_treemap_chart(faults_by_line, unit=" د")
        if fig_faults_tree is not None:
            st.pyplot(fig_faults_tree)
    else:
