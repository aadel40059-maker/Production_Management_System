...
if page == "الرئيسية":
    ...
    st.caption(
        "ملف إكسيل فيه شيت 'يومي' (باكينج + إنتاج الخطوط + الأعطال + الكسر لنفس اليوم، بالتارجت والمخطط "
        "وأيام العمل المخططة) وشيت 'تراكمي' بيجمع نفس البيانات يوم بيوم من أول الشهر لحد اليوم اللي تختاره"
    )
    ...
    excel_pack_sheet = f"packing_monthly_{excel_month_str}"
    ...
    if st.button("📊 توليد ملف الإكسيل", key="reports_generate_excel_btn"):
        ...
        # ---------- تحميل بيانات الشهر كله مرة واحدة (أسرع بكتير من قراءة كل يوم لوحده) ----------
        prod_hist_month_x = pd.DataFrame()
        if gsheet_exists(PRODUCTION_SHEET):
            prod_hist_month_x = read_gsheet(PRODUCTION_SHEET)
            prod_hist_month_x["Date"] = prod_hist_month_x["Date"].astype(str)
            prod_hist_month_x = prod_hist_month_x[prod_hist_month_x["Shift"] == "All Shifts"].copy()
            prod_hist_month_x["المستهدف (KG)"] = pd.to_numeric(prod_hist_month_x["المستهدف (KG)"], errors="coerce").fillna(0)
            prod_hist_month_x["الفعلي (KG)"] = pd.to_numeric(prod_hist_month_x["الفعلي (KG)"], errors="coerce").fillna(0)

        pack_grid_month_x = pd.DataFrame()
        if gsheet_exists(excel_pack_sheet):
            pack_grid_month_x = read_gsheet(excel_pack_sheet)
            pack_grid_month_x["المخطط بالكراتين (شهري)"] = pd.to_numeric(pack_grid_month_x.get("المخطط بالكراتين (شهري)", 0), errors="coerce").fillna(0)
            pack_grid_month_x["وزن الكرتونة (كيلو)"] = pd.to_numeric(pack_grid_month_x.get("وزن الكرتونة (كيلو)", 0), errors="coerce").fillna(0)
        ...
        if gsheet_exists("breakage_history"):
            ...
            for c_ in ["كسر خارج (KG)", "كسر داخل (KG)"]:
        ...
        def _gather_day_rows_x(d_str, d_num):
            ...
            if not prod_hist_month_x.empty:
                ...
                for _, r in pday.iterrows():
                    ...
                    prow.append([r["Line"], r["المستهدف (KG)"], r["الفعلي (KG)"], achv])
            ...
            if not pack_grid_month_x.empty and dcol in pack_grid_month_x.columns:
                ...
                for idx, r in pack_grid_month_x.iterrows():
                    ...
                    pkrow.append([
                        r["الصنف"], r.get("الفئة", ""), r["وزن الكرتونة (كيلو)"],
            ...
            if not faults_log_month_x.empty:
                ...
                for _, r in fday.iterrows():
                    frow.append([
                        r.get("الخط", ""), r.get("اسم الماكينة", ""), r.get("الوردية", ""),
            ...
            if not breakage_log_month_x.empty:
                ...
                for _, r in bday.iterrows():
                    ...
                    brow.append([r.get("الخط", ""), opening_x, out_x, in_x, closing_x])

            return prow, pkrow, frow, brow

        # ---------- كتابة شيت "يومي" — تفاصيل اليوم المختار بس ----------
        ...
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "🏭 إنتاج الخطوط",
            ["الخط", "المستهدف (KG)", "الفعلي (KG)", "نسبة التحقيق %"], prod_rows_x)
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "📦 الباكينج",
            ["الصنف", "الفئة", "وزن الكرتونة (كيلو)", "المخطط الشهري (كرتونة)",
             "التارجت اليومي (كرتونة)", "الفعلي (كرتونة)", "الفعلي (كيلو)"], pack_rows_x)
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "⚠ الأعطال",
            ["الخط", "الماكينة", "الوردية", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)",
             "سبب العطل", "الحالة", "تكلفة الصيانة (جنيه)"], faults_rows_x)
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "🔨 الكسر",
            ["الخط", "رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "متبقي الرصيد (KG)"], breakage_rows_x)

        for col_i in range(1, 10):
            ws_daily_x.column_dimensions[get_column_letter(col_i)].width = 20

        # ---------- كتابة شيت "تراكمي" — نفس تفاصيل شيت اليومي، بس لكل يوم من أول الشهر لحد اليوم
        # المختار، يوم تحت التاني (يوم 1، يوم 2، ...) مش أرقام مجمّعة بس ----------
        DAY_TITLE_FONT_X = Font(bold=True, size=14, color="FFFFFF", name="Arial")
        DAY_TITLE_FILL_X = PatternFill(start_color="2563EB", end_color="2563EB", fill_type="solid")

        cum_row_ptr_x = 1
        for d in range(1, excel_day_num + 1):
            d_str = f"{excel_month_str}-{d:02d}"
            d_prow, d_pkrow, d_frow, d_brow = _gather_day_rows_x(d_str, d)

            day_title_cell = ws_cum_x.cell(row=cum_row_ptr_x, column=1, value=f"📅 اليوم {d} — {d_str}")
            day_title_cell.font = DAY_TITLE_FONT_X
            for ci in range(1, 10):
                ws_cum_x.cell(row=cum_row_ptr_x, column=ci).fill = DAY_TITLE_FILL_X
            cum_row_ptr_x += 2

            cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "🏭 إنتاج الخطوط",
                ["الخط", "المستهدف (KG)", "الفعلي (KG)", "نسبة التحقيق %"], d_prow)
            cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "📦 الباكينج",
                ["الصنف", "الفئة", "وزن الكرتونة (كيلو)", "المخطط الشهري (كرتونة)",
                 "التارجت اليومي (كرتونة)", "الفعلي (كرتونة)", "الفعلي (كيلو)"], d_pkrow)
            cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "⚠ الأعطال",
                ["الخط", "الماكينة", "الوردية", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)",
                 "سبب العطل", "الحالة", "تكلفة الصيانة (جنيه)"], d_frow)
            cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "🔨 الكسر",
                ["الخط", "رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "متبقي الرصيد (KG)"], d_brow)
            cum_row_ptr_x += 1  # سطر فاضي إضافي بين كل يوم واللي بعده

        for col_i in range(1, 10):
            ws_cum_x.column_dimensions[get_column_letter(col_i)].width = 20
        ...
        st.success("اتولّد الملف ✔ — دوس تحميل تحت")
