...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            # ---------- جدول ملوّن: تعبئة اليوم بالفئات (كل المصانع) ----------
            cat_totals_c = combined_summary.groupby("الفئة")[day_col_pick].sum().reset_index()
            cat_totals_c.columns = ["الفئة", "اليومي"]
            cat_total_row_c = pd.DataFrame([{"الفئة": "الإجمالي", "اليومي": cat_totals_c["اليومي"].sum()}])
            ...
            cat_table_html_c = _df_to_html_highlighted(daily_by_cat_display_c, None, row_colors=cat_row_colors_c)

            st.markdown(f"##### 📅 تعبئة يوم {day_pick} بالفئات — كل المصانع")
            st.dataframe(daily_by_cat_display_c, use_container_width=True, hide_index=True)

            # ---------- جدول: الأصناف اللي أداؤها أقل من المتوقع ----------
            pm_low_df_c = pm_df_c[pd.to_numeric(pm_df_c["نسبة الأداء %"], errors="coerce") < 90].copy()
            pm_low_df_c = pm_low_df_c.sort_values("نسبة الأداء %").reset_index(drop=True)
            ...
            <div style="margin-top:10px; page-break-inside:avoid;">
                <h2 style="background:#0891b2; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                    📅 تعبئة يوم {day_pick} بالفئات — كل المصانع
                </h2>
                {cat_table_html_c}
            </div>
            <div style="margin-top:10px;">
            ...
            if pm_single_page_c:
                ...
                single_page_height_mm_c = 95 + (main_rows_c * 9) + (cat_rows_c * 6) + (low_rows_c * 7) + (export_rows_c * 4) + 35 + 75
