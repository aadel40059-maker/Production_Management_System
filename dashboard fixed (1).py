...
if page == "الرئيسية":
    ...
    if report_grid_ex.empty:
        ...
        # جدول مخصص للطباعة: خط فاصل واضح بين كل طلبية والتانية، وشريط لوني جانبي حسب حالة الطلبية
        detail_header_html = "".join(f"<th>{c}</th>" for c in detail_df_display_ex.columns)
        detail_rows_html = ""
        for i, (_, row) in enumerate(detail_df_display_ex.iterrows()):
            cells_html = "".join(f"<td>{row[c]}</td>" for c in detail_df_display_ex.columns)
            detail_rows_html += (
                f'<tr style="background-color:{row_colors_ex[i]}; '
                f'border-bottom:2.5px solid #013e37; border-right:5px solid {row_badges_ex[i]};">'
                f"{cells_html}</tr>"
            )
        detail_table_html = (
        ...
        <style>
            table.export-detail-table { border-collapse: separate; border-spacing: 0; width: 100%; }
            table.export-detail-table th {
                background: #013e37; color: #ffefb3; padding: 10px 8px; font-size: 12.5px;
                text-align: center; border-bottom: 3px solid #e0a92e;
            }
            table.export-detail-table td {
                padding: 9px 8px; text-align: center; font-size: 12.5px; font-weight: 700;
            }
            table.export-detail-table td:first-child, table.export-detail-table th:first-child { text-align: right; }
            table.export-detail-table tr:last-child { border-bottom: none !important; }
        </style>
        """
        ...
        detail_report_html = build_printable_html(
            ...
            top_html=f'<div style="margin-bottom:8px;">{detail_table_html}</div>',
            base_font_size=13,
            bold=True,
        )
