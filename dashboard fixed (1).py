...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            status_labels_c, row_bg_colors_c = [], []
            for pct in pm_df_c["نسبة الأداء %"]:
                label, color, bg = _pm_status_c(pct)
                status_labels_c.append(f'<span style="color:{color}; font-weight:800;">{label}</span>')
                row_bg_colors_c.append(bg)
            pm_df_c["حالة الأداء"] = status_labels_c
            ...
            st.dataframe(
                pm_df_c.assign(**{
                    "حالة الأداء": [s.split(">")[1].split("<")[0] for s in status_labels_c],
    ...
    if st.button("📊 توليد شيت الشؤون"):
        ...
        with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
            ...
            # "الإجمالي" هنا = مجموع الشيفتات + عمود "بدون شيفت" بالظبط، عشان الأرقام تتجمع صح
            # وتبقى واضحة لو فيه عمال لسه متسجلينش في شيفت
            summary_rows = []
            for line_name, df_line in all_lines_hr.items():
                ...
                unassigned = len(named[named["الشيفت"] == ""])
                summary_rows.append({
                    "الخط": line_name,
                    "الشيفت 1": s1, "الشيفت 2": s2, "الشيفت 3": s3,
                    "بدون شيفت": unassigned,
                    "الإجمالي": s1 + s2 + s3 + unassigned,
                })
            ...
            summary_df.loc[len(summary_df)] = {
                ...
                "بدون شيفت": summary_df["بدون شيفت"].sum(),
            ...
            for route in routes:
                ...
                unassigned = len(route_df[route_df["الشيفت"] == ""])
                route_rows.append({
            ...
            routes_summary_df = pd.DataFrame(route_rows) if route_rows else pd.DataFrame(
                columns=["خط السير (البلد)", "الشيفت 1", "الشيفت 2", "الشيفت 3", "بدون شيفت", "الإجمالي"]
            )
            if not routes_summary_df.empty:
                routes_summary_df.loc[len(routes_summary_df)] = {
                    ...
                    "بدون شيفت": routes_summary_df["بدون شيفت"].sum(),
        ...
        st.success("اتعمل شيت الشؤون ✔ (اتصلحت مشكلة الإجماليات، وبقى فيه عمود 'بدون شيفت' يوضح أي عامل لسه متسجلش شيفت)")
