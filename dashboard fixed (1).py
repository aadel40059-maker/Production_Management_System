    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            if pm_single_page_c:
                ...
                single_page_height_mm_c = 95 + (main_rows_c * 9) + (cat_rows_c * 6) + (low_rows_c * 7) + 35 + 75
            ...
            st.caption(f"نفس تقرير الشيفتات بالظبط، بس تعبئة الشيفتات مجمّعة من تلات المصانع ليوم {report_shift_date_str} — وجدول إنتاج الخطوط بيفضل خاص بمصنع الكاندي بس")
            ...
            factory_shift_tables_c = {
                f_name: _load_factory_shift_pack(f_conf["sheet_suffix"], report_shift_date_str, selected_month)
                for f_name, f_conf in PACKING_FACTORY_CONFIG.items()
            }
            prod_line_tables_c, prod_lines_clean_c = _load_shift_production_lines(report_shift_date_str)
            ...
            shift_html_c = build_printable_html(
                ...
                f"شهر {selected_month} — يوم {report_shift_date_str} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                pd.DataFrame(),
                landscape=True,
            ...
            st.download_button(
                ...
                file_name=f"shift_report_combined_{report_shift_date_str}.html",
                mime="text/html",
