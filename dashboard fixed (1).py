...
def normalize_arabic_text(v):
    ...
    الألف/الياء، وتحويل علامات الضرب/الفواصل والأقواس لمسافات، وفصل أي رقم ملزّق بحرف"""
    if v is None:
        return ""
    s = str(v).strip()
...
UNIT_ALIASES = {
    ...
    "قطعة": "PIECE", "قطع": "PIECE", "قطعه": "PIECE", "ق": "PIECE",
...
_PACKAGING_UNIT_PATTERN = re.compile(
    r"(\d+(?:\.\d+)?)\s*(" + "|".join(sorted(UNIT_ALIASES, key=len, reverse=True)) + r")\b"
)
...
def normalize_product_name(v):
    ...
    words = [w.replace("ة", "ه") for w in remaining.split()]
    words = [w for w in words if w not in BRAND_WORDS]
...
def parse_stock_balance_file(uploaded_file):
    ...
    بترجع (dict الاسم المطبّع -> الإجمالي، أسماء الشيتات اللي اتقرت، أسماء الشيتات اللي اتجاهلت)"""
    try:
    ...
    for sheet_name, raw in sheets_raw.items():
        ...
        return item_totals, [sheet_name], [s for s in sheets_raw if s != sheet_name]
    ...
    return item_totals, matched_sheets, skipped_sheets
    ...
    if summary.empty:
        ...
        if combined_summary.empty:
            ...
            if stock_file_c is not None:
                stock_lookup_c, matched_sheets_c, skipped_sheets_c = parse_stock_balance_file(stock_file_c)
                if stock_lookup_c:
