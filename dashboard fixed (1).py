...
if page == "الرئيسية":
    ...
    def _build_energy_cards(values, targets_map, cartons, tons, period_label):
    ...
    kpi_row(_build_energy_cards(month_values_e, monthly_targets_map_e, month_cartons_e, month_tons_e, "الشهر"))
    ...
    if not st.session_state.energy_admin_ok:
        ...
        if st.button("📊 تجهيز تقرير الطاقة اليومي", key="energy_full_report_btn"):
            ...
            combined_cards = _build_energy_cards(day_values, combined_daily_targets, cartons_day, tons_day, "اليوم — 3 مصانع")
            ...
            combined_figs = _energy_trend_fig(
                ...
                figsize=(6.6, 2.6), title_size=11
            )
            combined_charts_html = "".join(fig_to_base64_img(combined_figs[u["key"]]) for u in ENERGY_UTILITIES)

            def _energy_block(title_html, cards_html, charts_html, page_break=False):
                pb = "page-break-before: always;" if page_break else ""
                return f"""
                <div style="margin-top:16px; page-break-inside:avoid; {pb}">
                    {title_html}
                    <div style="display:flex; flex-wrap:wrap;">{cards_html}</div>
                    <div style="display:flex; flex-wrap:wrap; gap:6px; margin-top:8px;">{charts_html}</div>
                </div>
                """
            ...
            for f_name, f_conf in ENERGY_FACTORY_CONFIG.items():
                ...
                f_cards = _build_energy_cards(f_day_values, f_daily_targets, f_cartons_day, f_tons_day, "اليوم")
                f_cards_html = _energy_cards_html(f_cards)
                f_figs = _energy_trend_fig(f_calc, f_name, f_daily_targets, days_in_month_e, figsize=(6.6, 2.6), title_size=11)
                f_charts_html = "".join(fig_to_base64_img(f_figs[u["key"]]) for u in ENERGY_UTILITIES)
            ...
            continuous_height_mm = 110 + 95 + len(factory_blocks) * 95
