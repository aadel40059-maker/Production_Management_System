    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        body {{
            ...
            margin: 0; padding: 14px; color: var(--text); font-size: 13px;
        }}
        .portfolio-title {{ text-align: center; margin-bottom: 12px; }}
        .portfolio-title h1 {{ font-size: 16px; color: var(--navy); margin: 0 0 3px; }}
        .portfolio-title p {{ font-size: 9px; color: var(--muted); margin: 0; }}

        .dash-nav {{
            position: sticky; top: 0; z-index: 20; display: flex; gap: 6px; flex-wrap: wrap;
            justify-content: center; background: var(--bg); padding: 8px 0 12px; margin-bottom: 4px;
        }}
        .nav-btn {{
            border: 1px solid var(--border); background: #fff; color: var(--navy);
            font-family: inherit; font-size: 10.5px; font-weight: 800; padding: 7px 14px;
        ...
        .report-page {{ display: none; margin-bottom: 16px; }}
        ...
        .kpi-grid {{ display: flex; gap: 7px; margin-bottom: 10px; flex-wrap: wrap; }}
        .kpi-card {{
            flex: 1; min-width: 84px; background: var(--card); border: 1px solid var(--border);
            border-radius: 8px; padding: 8px 5px; text-align: center; box-shadow: 0 1px 2px rgba(0,0,0,.04);
        }}
        .kpi-icon {{
            width: 22px; height: 22px; border-radius: 6px; display: flex; align-items: center;
            justify-content: center; margin: 0 auto 4px; font-size: 11px;
        }}
        .kpi-value {{ font-size: 12.5px; font-weight: 800; color: var(--navy); line-height: 1.15; }}
        .kpi-label {{ font-size: 7.5px; color: var(--muted); font-weight: 700; margin-top: 2px; line-height: 1.2; }}

        .chart-grid {{ display: flex; gap: 8px; margin-bottom: 10px; flex-wrap: wrap; page-break-inside: avoid; }}
        .chart-card {{
            flex: 1; min-width: 200px; background: var(--card); border: 1px solid var(--border);
            border-radius: 8px; padding: 7px; box-shadow: 0 1px 2px rgba(0,0,0,.04);
        }}
        .chart-card-title {{ font-size: 9px; font-weight: 800; color: var(--navy); margin-bottom: 4px; }}
        ...
        .table-card-header {{ padding: 7px 12px; font-size: 10px; font-weight: 800; color: #fff; }}
        table.dash-table, .table-card table {{ border-collapse: collapse; width: 100%; font-size: 8.5px; }}
...
if page == "الرئيسية":
    ...
    def _trend_line_fig(x_vals, y_vals, title, color, xtick_step=5):
        fig, ax = plt.subplots(figsize=(4.6, 2.5), dpi=140)
        fig.patch.set_facecolor("#ffffff")
    ...
    def _donut_fig(labels, values, title):
        fig, ax = plt.subplots(figsize=(3.6, 3.2), dpi=140)
        fig.patch.set_facecolor("#ffffff")
        ...
        if not gsheet_exists(packing_sheet_r):
            ...
            if cat_summary_r.empty:
                ...
                report_parts["packing"] = {
                    "key": "packing",
                    "title": "PACKING REPORT (BY WEIGHT)",
