    ...
    if not st.session_state.breakage_admin_ok:
        ...
        def _load_factory_breakage_data(f_name, f_conf_b, date_str):
            ...
            if not breakage_log_x.empty:
                ...
                day_b_grouped_x = day_b_x.groupby("الخط")[["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)"]].sum()
                day_b_grouped_x = day_b_grouped_x.reindex(lines_b, fill_value=0)
            else:
                day_b_grouped_x = pd.DataFrame(0, index=lines_b, columns=["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)"])
            ...
            for ln in lines_b:
                ...
                rows_x.append({
                    "name": ln, "opening": opening_x, "out": out_x, "in": in_x,
        ...
        def _factory_metric_totals(f_name):
            ...
            return out_f, in_f, remaining_f, prod_f, break_pct_f, efficiency_f

        parts_out, parts_in, parts_remaining, parts_prod, parts_breakpct, parts_eff = [], [], [], [], [], []
        for f_name in BREAKAGE_FACTORY_CONFIG:
            o_f, i_f, r_f, p_f, bp_f, ef_f = _factory_metric_totals(f_name)
        ...
        if not any(r["opening"] or r["out"] or r["in"] for r in combined_line_rows_b):
