...
def _df_to_html_highlighted(df, highlight_mask, highlight_color="#fff3b0"):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        is_hl = bool(highlight_mask[i]) if i < len(highlight_mask) else False
...
def build_printable_html(title, subtitle, df, extra_title=None, extra_df=None, extra_tables=None,
                          ...
                          highlight_color="#fff3b0", base_font_size=10, compact=False,
    ...
    if highlight_mask is not None:
        table_html = _df_to_html_highlighted(df, list(highlight_mask), highlight_color)
    else:
...
if page == "Dashboard":
    ...
    if summary.empty:
        ...
        # ==========================
        # تقرير تاني مخصوص لمدير الإنتاج — خط كبير جدًا، ومقارنة بالمتوقع حسب أيام العمل الفعلية
        # ==========================
        st.divider()
        st.markdown("#### 🖨️ تقرير مدير الإنتاج")

        # عدد أيام العمل الفعلية = عدد الأيام (من أول الشهر لحد اليوم المختار) اللي فيها تعبئة حقيقية
        # (إجمالي كل الأصناف في اليوم ده > صفر) — مش مجرد رقم اليوم في الشهر
        ...
        total_month_days = days_in_month  # الخطة الشهرية موزّعة على أيام الشهر بالكامل

        pm_df = summary[["الصنف", "المخطط بالكراتين (شهري)", "إجمالي الكراتين"]].copy()
        pm_df = pm_df.rename(columns={
            "المخطط بالكراتين (شهري)": "المخطط الشهري (كرتونة)",
            "إجمالي الكراتين": "الفعلي حتى الآن (كرتونة)"
        })
        pm_df["المتبقي (كرتونة)"] = (pm_df["المخطط الشهري (كرتونة)"] - pm_df["الفعلي حتى الآن (كرتونة)"]).clip(lower=0)
        ...
        pm_df["المتوقع حتى الآن (كرتونة)"] = (pm_df["المخطط الشهري (كرتونة)"] * day_ratio).round(0)

        pm_df["نسبة التحقيق %"] = pm_df.apply(
            lambda r: round((r["الفعلي حتى الآن (كرتونة)"] / r["المتوقع حتى الآن (كرتونة)"] * 100), 1)
            if r["المتوقع حتى الآن (كرتونة)"] > 0 else 0,
            axis=1
        )

        pm_df = pm_df[[
            "الصنف", "المخطط الشهري (كرتونة)", "الفعلي حتى الآن (كرتونة)",
            "المتبقي (كرتونة)", "المتوقع حتى الآن (كرتونة)", "نسبة التحقيق %"
        ]]

        pm_total_row = pd.DataFrame([{
            "الصنف": "الإجمالي",
            "المخطط الشهري (كرتونة)": pm_df["المخطط الشهري (كرتونة)"].sum(),
            "الفعلي حتى الآن (كرتونة)": pm_df["الفعلي حتى الآن (كرتونة)"].sum(),
            "المتبقي (كرتونة)": pm_df["المتبقي (كرتونة)"].sum(),
            "المتوقع حتى الآن (كرتونة)": pm_df["المتوقع حتى الآن (كرتونة)"].sum(),
            "نسبة التحقيق %": round(
                pm_df["الفعلي حتى الآن (كرتونة)"].sum() / pm_df["المتوقع حتى الآن (كرتونة)"].sum() * 100, 1
            ) if pm_df["المتوقع حتى الآن (كرتونة)"].sum() > 0 else 0,
        }])
        pm_df = pd.concat([pm_df, pm_total_row], ignore_index=True)

        # الأصناف اللي متأخرة عن المتوقع (نسبة تحقيق أقل من 100% بالنسبة لأيام العمل) بتتلوّن
        pm_low_mask = (
            (pm_df["نسبة التحقيق %"] < 100) & (pm_df["الصنف"] != "الإجمالي")
        ).tolist()

        pm_top_html = f"""
        <div style="display:flex; gap:10px; margin:6px 0 10px; font-weight:700;">
            <div style="flex:1; background:#eef2f7; border-radius:6px; padding:8px; text-align:center; font-size:15px;">
                📅 أيام العمل الفعلية: {actual_working_days} من {total_month_days} يوم
            </div>
            <div style="flex:1; background:#eef2f7; border-radius:6px; padding:8px; text-align:center; font-size:15px;">
                📊 نسبة الشهر المنقضي: {(day_ratio * 100):.0f}%
            </div>
        </div>
        """

        pm_html = build_printable_html(
            f"تقرير مدير الإنتاج — Packing شهر {selected_month}",
            f"حتى يوم {day_pick} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            pm_df,
            landscape=True,
            base_font_size=19,
            bold=True,
            row_padding="12px 10px",
            top_html=pm_top_html,
            highlight_mask=pm_low_mask,
            highlight_color="#fde2e1"
        )
        st.download_button(
            ...
            mime="text/html",
            help="تقرير بخط كبير: المخطط والفعلي والمتوقع حسب أيام العمل الفعلية ونسبة التحقيق — افتحه ودوس Ctrl+P واختار 'Save as PDF'"
        )
