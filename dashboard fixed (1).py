...
def dash_kpi_card(icon, value, label, color):
    """بطاقة KPI بنفس طابع لوحات التحكم الاحترافية: أيقونة في دايرة ملونة + رقم كبير + وصف صغير"""
    return f"""
    <div class="kpi-card">
...
def build_dashboard_document(overall_title, overall_subtitle, sections):
    """بيبني صفحة HTML واحدة بطابع لوحة تحكم احترافية (Dashboard) — تنفع تتنشر كصفحة ويب مستقلة (HTML خام بدون
    أي اعتماد على إنترنت أو مكتبات خارجية) وتنفع كمان تتحول PDF من المتصفح (Ctrl+P).

    لو الأقسام أكتر من واحد (حالة البورتفوليو): بيظهر شريط أزرار (تابس) فوق، كل زرار بيورّي قسم ويخفي الباقي —
    بورتفوليو حقيقي تفاعلي مش مجرد سكرول طويل. وعند الطباعة/تصدير PDF كل الأقسام بتظهر مع بعض تلقائيًا،
    كل واحد في صفحة لوحده (page-break)، بغض النظر عن التاب المفتوح على الشاشة.
    ...
    for i, sec in enumerate(sections):
        ...
        if is_portfolio:
            ...
            nav_buttons.append(
                f"""<button class="nav-btn {active_class}" data-target="{section_id}"
                    onclick="pmsShowSection('{section_id}')"
                    style="--nav-accent:{accent};">{nav_label}</button>"""
            )
    ...
    nav_html = ""
    if is_portfolio:
        nav_html = f'<div class="dash-nav">{"".join(nav_buttons)}</div>'
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .portfolio-title {{ text-align: center; margin-bottom: 16px; }}
        .portfolio-title h1 {{ font-size: 18px; color: var(--navy); margin: 0 0 4px; }}
        .portfolio-title p {{ font-size: 10px; color: var(--muted); margin: 0; }}

        .dash-nav {{
            position: sticky; top: 0; z-index: 20; display: flex; gap: 8px; flex-wrap: wrap;
            justify-content: center; background: var(--bg); padding: 10px 0 16px; margin-bottom: 4px;
        }}
        .nav-btn {{
            border: 1px solid var(--border); background: #fff; color: var(--navy);
            font-family: inherit; font-size: 12px; font-weight: 800; padding: 10px 18px;
            border-radius: 999px; cursor: pointer; transition: all .15s ease; box-shadow: 0 1px 2px rgba(0,0,0,.04);
        }}
        .nav-btn:hover {{ border-color: var(--nav-accent, var(--navy)); transform: translateY(-1px); }}
        .nav-btn.active {{
            background: var(--nav-accent, var(--navy)); color: #fff; border-color: var(--nav-accent, var(--navy));
        }}

        .report-page {{ display: none; margin-bottom: 22px; }}
        ...
        .dash-title {{ font-size: 15px; font-weight: 800; letter-spacing: .3px; }}
        .dash-subtitle {{ font-size: 10px; color: #d7e0f2; margin-top: 3px; }}
        .dash-badge {{
            background: rgba(255,255,255,.18); padding: 6px 14px; border-radius: 8px;
            font-size: 11px; font-weight: 700; white-space: nowrap;
        }}

        .kpi-grid {{ display: flex; gap: 10px; margin-bottom: 14px; flex-wrap: wrap; }}
        .kpi-card {{
            flex: 1; min-width: 110px; background: var(--card); border: 1px solid var(--border);
            border-radius: 10px; padding: 12px 8px; text-align: center; box-shadow: 0 1px 2px rgba(0,0,0,.04);
        }}
        .kpi-icon {{
            width: 30px; height: 30px; border-radius: 8px; display: flex; align-items: center;
            justify-content: center; margin: 0 auto 6px; font-size: 14px;
        }}
        .kpi-value {{ font-size: 16px; font-weight: 800; color: var(--navy); }}
        .kpi-label {{ font-size: 9px; color: var(--muted); font-weight: 700; margin-top: 3px; }}

        .chart-grid {{ display: flex; gap: 12px; margin-bottom: 14px; flex-wrap: wrap; page-break-inside: avoid; }}
        .chart-card {{
            flex: 1; min-width: 260px; background: var(--card); border: 1px solid var(--border);
            border-radius: 10px; padding: 10px; box-shadow: 0 1px 2px rgba(0,0,0,.04);
        }}
        .chart-card-title {{ font-size: 11px; font-weight: 800; color: var(--navy); margin-bottom: 6px; }}
        ...
        .table-card-header {{ padding: 10px 14px; font-size: 12px; font-weight: 800; color: #fff; }}
        table.dash-table, .table-card table {{ border-collapse: collapse; width: 100%; font-size: 10.5px; }}
        ...
        p.dev-footer {{ text-align: center; font-size: 9px; color: #9aa5b8; margin-top: 10px; }}

        @media print {{
            body {{ padding: 0; background: #fff; }}
            .dash-nav {{ display: none; }}
            .report-page {{ display: block !important; page-break-before: always; }}
    ...
    <body>
        <div class="portfolio-title">
            <h1>🏭 {overall_title}</h1>
            <p>Production Management System — Developed by Eng. Ahmed Adel</p>
        </div>
        {nav_html}
        {''.join(blocks)}
...
if page == "الرئيسية":
    ...
    def _trend_line_fig(x_vals, y_vals, title, color, xtick_step=5):
        ...
        fig.patch.set_facecolor("#ffffff")
        ax.plot(x_vals, y_vals, marker="o", linewidth=2.2, color=color,
                markerfacecolor="#ffffff", markeredgecolor=color, markeredgewidth=1.8, markersize=5, zorder=3)
        ...
        ax.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
        ax.set_axisbelow(True)
        ax.set_title(ar_text(title), fontsize=12, fontweight="bold", color=BRAND_GREEN, pad=10)
        fig.tight_layout()
        return fig

    def _donut_fig(labels, values, title):
        fig, ax = plt.subplots()
        fig.patch.set_facecolor("#ffffff")
    ...
    def _bar_fig(labels, values, title, color):
        ...
        fig.patch.set_facecolor("#ffffff")
        bars = ax.bar(labels, values, color=color, edgecolor="#00201c", linewidth=0.7, zorder=3)
        for b in bars:
            h = b.get_height()
            ax.annotate(f"{h:,.0f}", xy=(b.get_x() + b.get_width() / 2, h), xytext=(0, 4),
    ...
    def _grouped_bar_fig(categories, actual_vals, target_vals, title):
        categories = list(categories)
        x = np.arange(len(categories))
        width = 0.35
        fig, ax = plt.subplots(figsize=(6.6, 3.6))
        fig.patch.set_facecolor("#ffffff")
        b1 = ax.bar(x - width / 2, actual_vals, width, label=ar_text("الفعلي"), color=BRAND_GREEN,
                    edgecolor="#00201c", linewidth=0.6, zorder=3)
        b2 = ax.bar(x + width / 2, target_vals, width, label=ar_text("المخطط"), color="#9ca3af",
                    edgecolor="#4b5563", linewidth=0.6, zorder=3)
        for bars in (b1, b2):
            for b in bars:
                h = b.get_height()
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([ar_text(c) for c in categories], fontsize=9, fontweight="bold")
    ...
    with st.expander("📦 تقرير الباكينج الشهري — بالوزن", expanded=False):
        ...
        if not gsheet_exists(packing_sheet_r):
            ...
            if cat_summary_r.empty:
                ...
                categories_count_w = len(cat_summary_r)
                best_cat_w = cat_summary_r.iloc[0]

                pack_kpi_defs = [
                    ("⚖️", f"{total_actual_w:,.0f} كجم", "إجمالي الوزن الفعلي", "#16a34a"),
                    ("🎯", f"{total_target_w:,.0f} كجم", "إجمالي الوزن المخطط", "#e0a92e"),
                    ("📈", f"{achievement_w:.1f}%", "نسبة التحقيق الإجمالية", "#7c3aed"),
                    ("🗂️", f"{categories_count_w}", "عدد الفئات", "#2563eb"),
                    ("🏆", str(best_cat_w["الفئة"])[:16],
                     f"أعلى فئة ({best_cat_w['الوزن الفعلي (كجم)']:,.0f} كجم)", "#0d6b5c"),
                ]
                pack_top_html = '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join(
                    _report_kpi_card(*d) for d in pack_kpi_defs
                ) + "</div>"
    ...
    with st.expander("🏭 تقرير البرودكشن الشهري", expanded=False):
        if not gsheet_exists(PRODUCTION_SHEET):
            ...
            if prod_month_r.empty:
                ...
                prod_kpi_defs = [
                    ("🏭", f"{lines_count_r}", "عدد الخطوط المنتجة", "#2563eb"),
                    ("🎯", f"{total_target_r:,.0f} KG", "إجمالي المستهدف", "#e0a92e"),
                    ("📦", f"{total_actual_r:,.0f} KG", "إجمالي الفعلي", "#0d6b5c"),
                    ("📈", f"{achievement_r:.1f}%", "نسبة التحقيق الإجمالية", "#7c3aed"),
                    ("🥇", str(best_line_r["الخط"])[:16] if best_line_r is not None else "-",
                     f"أعلى خط ({best_line_r['الفعلي (KG)']:,.0f} KG)" if best_line_r is not None else "-", "#16a34a"),
                ]
                prod_top_html = '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join(
                    _report_kpi_card(*d) for d in prod_kpi_defs
                ) + "</div>"
                ...
                st.pyplot(_donut_fig(
                    ...
                    "توزيع الإنتاج الفعلي بين الخطوط"
                ))
    ...
    with st.expander("🔨 تقرير الكسر الشهري", expanded=False):
        if not gsheet_exists("breakage_history"):
            ...
            if break_month_r.empty:
                ...
                break_kpi_defs = [
                    ("📤", f"{total_out_r:,.0f} كجم", "إجمالي الكسر خارج", "#dc2626"),
                    ("📥", f"{total_in_r:,.0f} كجم", "إجمالي الكسر داخل", "#16a34a"),
                    ("⚖️", f"{total_net_r:,.0f} كجم", "صافي الكسر", "#e0a92e"),
                    ("📉", break_ratio_txt, "نسبة صافي الكسر من الإنتاج", "#7c3aed"),
                    ("🔻", str(worst_line_break_r["الخط"])[:16] if worst_line_break_r is not None else "-",
                     ...
                     if worst_line_break_r is not None else "-", "#2563eb"),
                ]
                break_top_html = '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join(
                    _report_kpi_card(*d) for d in break_kpi_defs
                ) + "</div>"
                ...
                st.caption("📊 صافي الكسر لكل خط (كجم)")
                st.pyplot(_bar_fig([ar_text(n) for n in break_line_summary_r["الخط"]],
                                    break_line_summary_r["صافي الكسر (KG)"], "صافي الكسر لكل خط (كجم)", "#dc2626"))
                ...
                if has_pie_break:
                    ...
                    st.pyplot(_donut_fig([ar_text(n) for n in break_line_summary_r["الخط"]], pie_vals_b.values,
                                          "نسبة كل خط من إجمالي الكسر"))

                break_chart_parts = [
                    dash_chart_card("📊 صافي الكسر لكل خط (كجم)", fig_to_base64_img(
                        _bar_fig([ar_text(n) for n in break_line_summary_r["الخط"]],
                                 break_line_summary_r["صافي الكسر (KG)"], "صافي الكسر لكل خط (كجم)", "#dc2626")
                    ))
                ]
                if has_pie_break:
        ...
        if not gsheet_exists("faults_daily_log"):
            ...
            if faults_month_r.empty:
                ...
                faults_kpi_defs = [
                    ...
                    ("🛠️", f"{total_faults_count_r:,.0f}", "عدد الأعطال المسجلة", "#2563eb"),
                    ("📊", f"{avg_fault_duration_r:,.0f} د", "متوسط مدة العطل", "#e0a92e"),
                    ("⏳", f"{(total_downtime_r / 60):,.1f} س", "إجمالي التوقف بالساعات", "#7c3aed"),
                    ("🔻", str(worst_line_fault_r["الخط"])[:16] if worst_line_fault_r is not None else "-",
                     ...
                     if worst_line_fault_r is not None else "-", "#16a34a"),
                ]
                faults_top_html = '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join(
                    _report_kpi_card(*d) for d in faults_kpi_defs
                ) + "</div>"
    ...
    # ==========================
    # 🏠 صفحة النظرة العامة (Overview) — ملخص شامل، بتتحط أول صفحة في البورتفوليو قبل التقارير بالتفصيل
    # ==========================
    ...
    if report_totals:
        ...
        if "breakage" in report_totals:
            ...
            ov_kpi_defs.append(("🔨", f"{t['net']:,.0f} كجم", "صافي الكسر", "#ea580c"))
        ...
        if "breakage" in report_totals:
            t = report_totals["breakage"]
            ov_rows.append({
                "القسم": "الكسر", "المخطط": "-", "الفعلي": f"{t['net']:,.0f} كجم", "نسبة التحقيق %": None
            })
        if "faults" in report_totals:
            t = report_totals["faults"]
            ov_rows.append({
        ...
        overview_section = {
            ...
            "subtitle": f"ملخص شامل لكل الأقسام - {month_label_r}",
    ...
    # ==========================
    # 🗂️ البورتفوليو — تجميع التقارير المختارة في ملف واحد بتصميم احترافي
    # ==========================
    st.divider()
    st.markdown("### 🗂️ البورتفوليو — تجميع كل التقارير في ملف واحد")
    st.caption(
        "الصفحة الأولى في البورتفوليو بتبقى 'نظرة عامة' فيها ملخص كل الأقسام، وبعدها كل تقرير بالتفصيل في صفحة "
        "لوحده — زي داشبورد احترافي حقيقي. الملف HTML مستقل تمامًا: تقدر تنشره كصفحة ويب زي ما هو، "
    ...
    if not report_parts:
        ...
        available_keys_r = [k for k in ["production", "packing", "breakage", "faults"] if k in report_parts]
        ...
        if overview_section is not None:
            include_overview_r = st.checkbox(
                "🏠 ضيف صفحة 'نظرة عامة' في الأول (ملخص شامل لكل الأقسام)", value=True, key="portfolio_include_overview"
            )
        if chosen_keys_r:
            ...
            portfolio_html_r = build_dashboard_document(
                "البورتفوليو الشهري الشامل", month_label_r, portfolio_sections_r
            )
            st.download_button(
                "🖨️ تحميل البورتفوليو الكامل (HTML / PDF)",
                data=portfolio_html_r.encode("utf-8"),
                file_name=f"monthly_portfolio_{selected_month_r}.html",
                mime="text/html",
                help="ملف HTML واحد بتصميم لوحة تحكم احترافية — نظرة عامة أول صفحة، وبعدها كل تقرير بالتفصيل "
