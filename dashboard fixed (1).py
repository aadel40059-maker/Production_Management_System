...
if page == "الرئيسية":
    ...
    # ---------- هات وزن الكرتونة لكل صنف من شيتات الباكينج التلاتة (لتحويل أرقام التصدير من وزن لكراتين) ----------
    @st.cache_data(ttl=300, show_spinner=False)
    def _load_carton_weights_for_export(month_str):
        lookup = {}
        for f_suffix in ["", "_m2", "_m3"]:
            sheet_name = f"packing_monthly_{month_str}{f_suffix}"
            if not (GSHEETS_ENABLED and gsheet_exists(sheet_name)):
                continue
            df_w = read_gsheet(sheet_name)
            if "الصنف" not in df_w.columns or "وزن الكرتونة (كيلو)" not in df_w.columns:
                continue
            df_w["الصنف"] = df_w["الصنف"].astype(str).str.strip()
            df_w["وزن الكرتونة (كيلو)"] = pd.to_numeric(df_w["وزن الكرتونة (كيلو)"], errors="coerce").fillna(0)
            for _, row_w in df_w.iterrows():
                if row_w["الصنف"] and row_w["وزن الكرتونة (كيلو)"] > 0 and row_w["الصنف"] not in lookup:
                    lookup[row_w["الصنف"]] = row_w["وزن الكرتونة (كيلو)"]
        return lookup

    carton_weight_lookup_ex = _load_carton_weights_for_export(selected_month_ex)

    def _kg_to_cartons(item_name, kg_value):
        cw = carton_weight_lookup_ex.get(str(item_name).strip(), 0)
        return round(kg_value / cw) if cw > 0 else None

    report_grid_ex["الكمية المطلوبة (كرتونة)"] = report_grid_ex.apply(
        lambda r: _kg_to_cartons(r["الصنف"], r["الكمية المطلوبة (KG)"]), axis=1
    )
    report_grid_ex["الفعلي حتى الآن (كرتونة)"] = report_grid_ex.apply(
        lambda r: _kg_to_cartons(r["الصنف"], r["الفعلي حتى الآن (KG)"]), axis=1
    )
    report_grid_ex["المتبقي (كرتونة)"] = report_grid_ex.apply(
        lambda r: _kg_to_cartons(r["الصنف"], r["المتبقي (KG)"]), axis=1
    )
    ...
    if report_grid_ex.empty:
        ...
        detail_df_display_ex = detail_df_ex.fillna({
            "الفعلي حتى الآن (كرتونة)": "-", "الكمية المطلوبة (كرتونة)": "-", "المتبقي (كرتونة)": "-"
        })
        st.dataframe(detail_df_display_ex, use_container_width=True, hide_index=True)

        no_weight_items_ex = sorted(set(
            report_grid_ex.loc[report_grid_ex["الكمية المطلوبة (كرتونة)"].isna(), "الصنف"]
        ) - {""})
        if no_weight_items_ex:
            st.caption(
                "⚠️ الأصناف دي مالهاش وزن كرتونة مسجّل في شيت الباكينج بتاعها للشهر ده، "
                "فظهرت الكراتين بتاعتها '-': " + "، ".join(no_weight_items_ex)
            )
