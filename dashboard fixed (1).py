...
if page == "الرئيسية":
    ...
    if admin_ok:
        ...
        def _mttr_mtbf(log_slice, n_lines, n_days):
            n_faults = len(log_slice)
            downtime = pd.to_numeric(log_slice["مدة العطل (دقيقة)"], errors="coerce").fillna(0).sum()
            if n_faults == 0 or n_days <= 0:
                return 0.0, 0.0
            total_period_min = n_lines * n_days * 24 * 60
            uptime_min = max(total_period_min - downtime, 0)
            mttr = downtime / n_faults
            mtbf = uptime_min / n_faults
            return mttr, mtbf

        all_month_log = month_log_upto[month_log_upto["الخط"].isin(ALL_FAULT_LINES)]
        mttr_today, mtbf_today = _mttr_mtbf(all_month_log, len(ALL_FAULT_LINES), daily_report_date.day)

        yesterday_log = all_month_log[all_month_log["التاريخ"] < daily_report_date_str]
        mttr_yday, mtbf_yday = _mttr_mtbf(yesterday_log, len(ALL_FAULT_LINES), max(daily_report_date.day - 1, 0))
        ...
        def _build_factory_detail(f_name, f_lines):
            if report_month_str == selected_month_f and f_lines == fault_lines:
                # نفس الشهر والمصنع المعروضين فوق أصلاً — نستخدم نفس الجدول بدل نداء تاني لجوجل شيت
                f_grid, f_day_cols = faults_grid, day_cols_f
            else:
                f_grid, _, f_day_cols = load_faults_grid(report_month_str, f_lines)
            for dc in f_day_cols:
                f_grid[dc] = pd.to_numeric(f_grid[dc], errors="coerce").fillna(0)

            cols_upto = [f"يوم {d}" for d in range(1, daily_report_date.day + 1) if f"يوم {d}" in f_day_cols]
            f_summary = f_grid[["الخط"] + cols_upto].copy()
            f_summary["إجمالي التوقف (دقيقة)"] = f_summary[cols_upto].sum(axis=1)
            f_summary = f_summary[["الخط", "إجمالي التوقف (دقيقة)"]]
            f_summary["الخط"] = f_summary["الخط"].astype(str)
            f_summary = (
                f_summary.groupby("الخط")["إجمالي التوقف (دقيقة)"].sum()
                .reindex(f_lines).fillna(0).reset_index()
            )
            f_summary["إجمالي التوقف (ساعة)"] = (f_summary["إجمالي التوقف (دقيقة)"] / 60).round(2)
            f_summary = f_summary[["الخط", "إجمالي التوقف (دقيقة)", "إجمالي التوقف (ساعة)"]]

            # رسم بياني: إجمالي التوقف لكل خط
            fig_b, ax_b = plt.subplots(figsize=(4.2, 2.1))
            fig_b.patch.set_facecolor("#ffffff")
            bar_values = f_summary["إجمالي التوقف (دقيقة)"].tolist()
            bar_labels = [ar_text(l) for l in f_summary["الخط"].tolist()]
            bar_colors = [BRAND_PALETTE[i % len(BRAND_PALETTE)] for i in range(len(bar_values))] if bar_values else [BRAND_GREEN]
            bars = ax_b.bar(bar_labels, bar_values, color=bar_colors, edgecolor="#00201c", linewidth=0.6)
            ...
            for b in bars:
                h = b.get_height()
                ax_b.text(b.get_x() + b.get_width() / 2, h + max_bar_val * 0.02, f"{h:,.0f}", ha="center", va="bottom", fontsize=7, fontweight="bold")
            ...
            f_counts = month_log_upto[month_log_upto["الخط"].isin(f_lines)].groupby("الخط").size()
            f_counts = f_counts.reindex(f_lines).fillna(0).astype(int)
            f_counts_sorted = f_counts.sort_values(ascending=False)
            ...
            # ---- تكلفة الصيانة حسب الخط (من أول الشهر حتى اليوم) — بس الأعطال المعتمدة ----
            f_cost_log = month_log_upto[month_log_upto["الخط"].isin(f_lines) & (month_log_upto["الحالة"] == STATUS_APPROVED)].copy()
            ...
            fig_c, ax_c = plt.subplots(figsize=(4.2, 2.3))
            fig_c.patch.set_facecolor("#ffffff")
            cost_values = f_cost.tolist()
            cost_labels = [ar_text(l) for l in f_cost.index.tolist()]
            ax_c.barh(cost_labels, cost_values, color="#2563eb", edgecolor="#0b1f3a", linewidth=0.6)
            ax_c.set_xlabel(ar_text("جنيه"), fontsize=8, fontweight="bold")
            ax_c.set_title(ar_text(f"{f_name} — تكلفة الصيانة حسب الخط (جنيه)"), fontsize=9.5, fontweight="bold", color=BRAND_GREEN, pad=8)
            ax_c.spines["top"].set_visible(False)
            ax_c.spines["right"].set_visible(False)
            ax_c.grid(axis="x", linestyle="--", alpha=0.3)
            ax_c.set_axisbelow(True)
            max_cost_val = max(cost_values) if cost_values and max(cost_values) > 0 else 1
            for i, v in enumerate(cost_values):
                ax_c.text(v + max_cost_val * 0.02, i, f"{v:,.0f}", va="center", fontsize=7, fontweight="bold")
            fig_c.tight_layout()
            cost_buf = io.BytesIO()
            fig_c.savefig(cost_buf, format="png", dpi=150, bbox_inches="tight")
            cost_b64 = base64.b64encode(cost_buf.getvalue()).decode()
            plt.close(fig_c)
            ...
            return {
                ...
                "cost_b64": cost_b64, "cost_total": f_cost_total,
            }
        ...
        for f_name in FACTORY_CONFIG.keys():
            ...
            st.markdown(f"#### 💰 تكلفة الصيانة حسب الخط — {f_name} (من أول شهر {report_month_str} حتى اليوم)")
            st.caption(f"إجمالي تكلفة الصيانة (المعتمدة) لـ {f_name} حتى الآن: **{result['cost_total']:,.0f} جنيه**")
            st.image(f"data:image/png;base64,{result['cost_b64']}", use_container_width=True)
            st.divider()
        ...
        def _factory_block_html(f_name, det, accent):
            ...
            <div style="margin-top:12px; border:1px solid #e5e7eb; border-right:5px solid {accent};
                        border-radius:8px; padding:10px 12px; background:#fff; page-break-inside:avoid;">
                ...
                <div style="display:flex; gap:12px;">
                    ...
                    <div class="side-small" style="flex:1;">
                        <h2 style="margin-top:0; font-size:10.5px;">📊 إجمالي التوقف الشهري (دقيقة/ساعة)</h2>
                        {summary_html}
                    </div>
                </div>
                ...
                <div style="margin-top:8px;">
                    <h2 style="margin-top:0; font-size:10.5px;">💰 تكلفة الصيانة حسب الخط (المعتمدة) — إجمالي {f_name}: {det['cost_total']:,.0f} جنيه</h2>
                    <img src="data:image/png;base64,{det['cost_b64']}" style="width:100%; max-width:400px;">
                </div>
            </div>
            """
        ...
        {signature_html}
        {factory_blocks_html}
        <div style="margin-top:10px; display:flex; gap:12px; page-break-inside:avoid;">
        ...
        # ---- ارتفاع الصفحة عشان الـ PDF يطلع متصل صفحة واحدة من غير تقطيع ----
        daily_single_page_height_mm = (
            ...
            + (280 * len(FACTORY_CONFIG))  # صور الرسوم البيانية (بار + ترند لكل مصنع)
            + (120 * len(FACTORY_CONFIG))  # صورة تكلفة الصيانة لكل مصنع
            + (len(reasons_df) * 6)
            + (len(notes_list) * 6)
            + 50  # الإمضاء
        )
        ...
        daily_report_html = build_printable_html(
            ...
            single_page_height_mm=daily_single_page_height_mm,
        )
