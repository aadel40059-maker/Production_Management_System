...
# ==========================================================
# الاتصال بجوجل شيت (مستخدم في صفحتي الأعطال والعمال بس)
# ==========================================================
...
def _get_worksheet(sheet_name):
    ...
    try:
        ...
        ws = spreadsheet.add_worksheet(title=safe_name, rows=2000, cols=60)
    return ws
...
def write_gsheet(sheet_name, df):
    ...
    try:
        ...
        if old_row_count > new_row_count:
            ws.batch_clear([f"A{new_row_count + 1}:ZZ{old_row_count}"])
    except Exception:
        pass
...
def build_printable_html(title, subtitle, df, extra_title=None, extra_df=None, extra_tables=None, landscape=False):
...
if page == "Dashboard":
    ...
    PACKING_PAGE_PASSWORD = "pack2026"
    if "packing_page_unlocked" not in st.session_state:
        st.session_state.packing_page_unlocked = False

    if not st.session_state.packing_page_unlocked:
        st.subheader("🔒 صفحة محمية")
        pw_input = st.text_input("كلمة مرور صفحة الباكينج", type="password", key="packing_page_pw")
        if st.button("دخول", key="packing_page_unlock_btn"):
            if pw_input == PACKING_PAGE_PASSWORD:
                st.session_state.packing_page_unlocked = True
                st.rerun()
            else:
    ...
    if summary.empty:
        ...
        if os.path.exists(DATA_FILE):
            hist_df_p = pd.read_csv(DATA_FILE)
    ...
    # نفس كلمات مرور الخطوط اللي في Workers بالظبط
    ...
    if allow_log:
        ...
        if st.button("💾 تسجيل عطل خطي اليوم"):
            ...
            # ---- سجل تفصيلي منفصل: كل عطل بصف لوحده، من/إلى الساعة واضحين ----
            DAILY_LOG_SHEET = "faults_daily_log"
            if gsheet_exists(DAILY_LOG_SHEET):
                daily_log = read_gsheet(DAILY_LOG_SHEET)
            else:
                daily_log = pd.DataFrame(columns=["التاريخ", "الخط", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)", "سبب العطل"])
    ...
    # ==========================
    # الجدول التجميعي (DataFrame) — عرض بس، متاح لأي حد يشوفه
    # ==========================
    ...
    DAILY_LOG_SHEET = "faults_daily_log"
    if gsheet_exists(DAILY_LOG_SHEET):
        daily_log_all = read_gsheet(DAILY_LOG_SHEET).astype(str)
        daily_log_all["مدة العطل (دقيقة)"] = pd.to_numeric(daily_log_all["مدة العطل (دقيقة)"], errors="coerce").fillna(0)
    else:
        daily_log_all = pd.DataFrame(columns=["التاريخ", "الخط", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)", "سبب العطل"])

    day_incidents = daily_log_all[daily_log_all["التاريخ"] == daily_report_date_str].copy()

    # الخطوط اللي مسجلتش أي عطل اليوم ده — تظهر بصف "لا يوجد أعطال"
    lines_with_incident = set(day_incidents["الخط"])
    no_fault_rows = pd.DataFrame([
        {"التاريخ": daily_report_date_str, "الخط": l, "من الساعة": "-", "إلى الساعة": "-",
         "مدة العطل (دقيقة)": 0, "سبب العطل": "لا يوجد أعطال"}
    ...
    daily_report_df = daily_report_df[["الخط", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)", "سبب العطل"]]
    ...
    st.download_button(
        "🖨️ تحميل التقرير اليومي (للإدارة)",
