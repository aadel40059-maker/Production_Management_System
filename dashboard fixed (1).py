...
def build_breakage_report_html(totals, line_rows, trend_dates, trend_out_pct, trend_in_pct,
    ...
    # ---------- جدول الخطوط: رصيد أول المدة → الكسر الخارج → إجمالي الكسر → الكسر الداخل → الهالك → متبقي الرصيد ----------
    table_rows_html = ""
    tot_open = tot_out = tot_in = tot_waste = tot_total = tot_remain = 0
    ...
    <tr class="total-row">
        ...
        <td>{tot_total:,.0f}</td><td>{tot_in:,.0f}</td><td>{tot_waste:,.0f}</td><td>{tot_remain:,.0f}</td><td>-</td>
    </tr>
    """
    ...
    <div class="panel">
        ...
        <table>
            <tr>
                ...
                <th>إجمالي الكسر (KG)</th><th>الكسر الداخل (KG)</th><th>الهالك (KG)</th><th>متبقي الرصيد (KG)</th><th>نسبة الكسر %</th>
            </tr>
            {table_rows_html}
        </table>
...
if page == "الرئيسية":
    ...
    # إنتاج خطوط الإنتاج (صفحة Production) موجود لمصنع الكاندي بس — مش موجود في الويفر أو الشيكولاته
    show_prod_lines = (pack_factory_pick == "مصنع 1")
    ...
    with st.expander("📊 تقرير تعبئة الشيفتات + إنتاج الخطوط لنفس اليوم"):
        ...
        if day_shift_log.empty:
            ...
            # ---------- إنتاج الخطوط لكل شيفت لنفس اليوم (من صفحة Production) — مصنع الكاندي بس (خطوط الإنتاج مش موجودة في الويفر/الشيكولاته) ----------
            if show_prod_lines:
            ...
            # رسم بياني احترافي: إجمالي إنتاج كل خط (على مستوى اليوم كله) فعلي مقابل مستهدف — مصنع الكاندي بس
            shift_chart_html = ""
    ...
    if summary.empty:
        ...
        # ---------- جدول صغير: إنتاج الخطوط لنفس اليوم (من صفحة Production) — مصنع الكاندي بس ----------
        ...
        # ---------- رسم بياني احترافي: إنتاج الخطوط الفعلي مقابل المستهدف — يتحط جوه التقرير القابل للطباعة (مصنع الكاندي بس) ----------
        prod_chart_html = ""
        ...
        # ---------- جدول إنتاج الخطوط لنفس اليوم — يتحط جوه تقرير مدير الإنتاج كمان (مصنع الكاندي بس) ----------
        ...
        if combined_summary.empty:
            ...
            # ==========================
            # تقرير الشيفتات المجمّع — تلات المصانع مع بعض، ونفس نظام "إنتاج الخطوط" (الكاندي بس)
            ...
            st.caption(f"نفس تقرير الشيفتات بالظبط، بس تعبئة الشيفتات مجمّعة من تلات المصانع ليوم {day_pick} (نفس يوم تقرير مدير الإنتاج المجمّع) — وجدول إنتاج الخطوط بيفضل خاص بمصنع الكاندي بس")
            ...
            def _load_shift_production_lines(date_str):
                all_lines_clean_x = [str(l).replace("🏭", "").strip() for l in st.session_state.production_lines]
                day_shift_prod_x = pd.DataFrame()
                if GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
                    hist_df_x = read_gsheet(PRODUCTION_SHEET)
                    if "Date" in hist_df_x.columns and not hist_df_x.empty:
                    ...
                    <table class="layout-wrapper" width="100%">
                        <tr>
                            ...
                            <td valign="top" style="width:48%;">
                                <h3 style="margin:0 0 4px; font-size:11px; color:#374151;">🏭 إنتاج الخطوط (الكاندي بس)</h3>
                                {prod_html_c}
                            </td>
                        </tr>
                    </table>
    ...
    # ---------- هات إنتاج كل خط لنفس اليوم من صفحة Production (لاستخدامه في حساب نسبة الكسر %) — الكاندي بس عنده البيانات دي ----------
    ...
    if b_conf["has_production_data"] and GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
    ...
    if breakage_log_all.empty:
        ...
        if day_b.empty:
            ...
            for ln in all_lines_clean_b:
                ...
                # نسبة الكسر بتتحسب من الكسر الخارج بس (مش الداخل ولا الهالك)
    ...
    if not st.session_state.breakage_admin_ok:
        ...
        def _load_factory_breakage_data(f_name, f_conf_b, date_str):
            ...
            daily_prod_totals_x = {}
            if f_conf_b["has_production_data"] and GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
                hist_df_x = read_gsheet(PRODUCTION_SHEET)
                hist_df_x["Date"] = hist_df_x["Date"].astype(str)
                ...
                if not day_hist_x.empty:
                    ...
                    for ln in lines_b:
                        if ln in prod_by_line_x.index:
                            day_prod_b[ln] = float(prod_by_line_x[ln])
