...
if page == "Dashboard":
    ...
    with st.expander("📊 تقرير تعبئة الشيفتات + إنتاج الخطوط لنفس اليوم"):
        ...
        if day_shift_log.empty:
            ...
            pivot_shift = day_shift_log.pivot_table(
                index="الصنف", columns="الشيفت", values="الكمية (كرتونة)", aggfunc="sum", fill_value=0
            ).reset_index()
            for sh in SHIFT_OPTIONS_P:
                if sh not in pivot_shift.columns:
                    pivot_shift[sh] = 0
            pivot_shift = pivot_shift[["الصنف"] + SHIFT_OPTIONS_P]
            pivot_shift["الإجمالي"] = pivot_shift[SHIFT_OPTIONS_P].sum(axis=1)

            st.markdown(f"##### 📦 تعبئة كل شيفت لكل صنف — {report_shift_date_str}")
            st.dataframe(pivot_shift, use_container_width=True, hide_index=True)
