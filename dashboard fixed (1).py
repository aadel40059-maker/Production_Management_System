...
if page == "الرئيسية":
    ...
    if summary.empty:
        ...
        # ==========================
        # تقرير أداء المصانع الثلاثة — مجمّع فوق (كل كرت فيه رقم المصنع كله وتحته بخط خفيف كل مصنع لوحده)
        # وتفصيلي لكل مصنع تحت (زي تقرير الأعطال)
        # ==========================
        st.divider()
        st.markdown("#### 📊 تقرير أداء المصانع الثلاثة")
        st.caption("فوق: أداء المصانع الثلاثة مجمّعين — كل كرت فيه الرقم الإجمالي وتحته بخط خفيف كل مصنع لوحده. تحت: تفاصيل كل مصنع لوحده")

        def _load_factory_packing_summary(f_suffix, month_str):
            sheet_name = f"packing_monthly_{month_str}{f_suffix}"
            empty = {"target": 0.0, "actual": 0.0, "actual_kg": 0.0, "target_kg": 0.0}
            ...
            if df.empty:
                return empty
            year_f, month_num_f = map(int, month_str.split("-"))
            days_f = calendar.monthrange(year_f, month_num_f)[1]
            day_cols_f = [f"يوم {d}" for d in range(1, days_f + 1)]
            for dc in day_cols_f:
                df[dc] = pd.to_numeric(df[dc], errors="coerce").fillna(0) if dc in df.columns else 0.0
            df["وزن الكرتونة (كيلو)"] = pd.to_numeric(df.get("وزن الكرتونة (كيلو)", 0), errors="coerce").fillna(0)
            df["المخطط بالكراتين (شهري)"] = pd.to_numeric(df.get("المخطط بالكراتين (شهري)", 0), errors="coerce").fillna(0)
            actual_per_row = df[day_cols_f].sum(axis=1)
            return {
                "target": float(df["المخطط بالكراتين (شهري)"].sum()),
                "actual": float(actual_per_row.sum()),
                "actual_kg": float((actual_per_row * df["وزن الكرتونة (كيلو)"]).sum()),
                "target_kg": float((df["المخطط بالكراتين (شهري)"] * df["وزن الكرتونة (كيلو)"]).sum()),
            }

        factory_pack_summaries = {
            f_name: _load_factory_packing_summary(f_conf["sheet_suffix"], selected_month)
            for f_name, f_conf in PACKING_FACTORY_CONFIG.items()
        }

        combined_target = sum(s["target"] for s in factory_pack_summaries.values())
        combined_actual = sum(s["actual"] for s in factory_pack_summaries.values())
        combined_target_kg = sum(s["target_kg"] for s in factory_pack_summaries.values())
        combined_actual_kg = sum(s["actual_kg"] for s in factory_pack_summaries.values())
        combined_pct = round((combined_actual / combined_target * 100) if combined_target > 0 else 0, 1)

        def _factory_breakdown_line(icon_map=None):
            """بيبني سطر خفيف اللون فيه رقم كل مصنع لوحده، يتحط تحت رقم الكرت المجمّع"""
            parts = []
            for f_name, s in factory_pack_summaries.items():
                parts.append(f"{f_name}: {icon_map(s)}")
            return " | ".join(parts)

        combined_cards = [
            {
                "icon": "🎯", "label": "إجمالي المخطط (كل المصانع)", "value": f"{combined_target:,.0f} كرتونة",
                "color": "#e0a92e",
                "delta": _factory_breakdown_line(lambda s: f"{s['target']:,.0f}"),
                "delta_color": "#6b7280",
            },
            {
                "icon": "📦", "label": "إجمالي المُنتج (كل المصانع)", "value": f"{combined_actual:,.0f} كرتونة",
                "color": "#0d6b5c",
                "delta": _factory_breakdown_line(lambda s: f"{s['actual']:,.0f}"),
                "delta_color": "#6b7280",
            },
            {
                "icon": "🥇", "label": "نسبة التحقيق (كل المصانع)", "value": f"{combined_pct}%",
                "color": "#16a34a" if combined_pct >= 90 else ("#d97706" if combined_pct >= 70 else "#dc2626"),
                "delta": _factory_breakdown_line(
                    lambda s: f"{round((s['actual'] / s['target'] * 100), 1) if s['target'] > 0 else 0}%"
                ),
                "delta_color": "#6b7280",
            },
            {
                "icon": "⚖️", "label": "الوزن الفعلي (كل المصانع)", "value": f"{combined_actual_kg:,.0f} كجم",
                "color": "#2563eb",
                "delta": _factory_breakdown_line(lambda s: f"{s['actual_kg']:,.0f} كجم"),
                "delta_color": "#6b7280",
            },
        ]
        kpi_row(combined_cards)

        # ---------- تفاصيل كل مصنع لوحده (زي تقرير الأعطال بالظبط) ----------
        st.markdown("##### 📋 تفاصيل كل مصنع لوحده")
        factory_blocks_pack = []
        for f_name, s in factory_pack_summaries.items():
            f_pct = round((s["actual"] / s["target"] * 100), 1) if s["target"] > 0 else 0
            f_cards = [
                {"icon": "🎯", "label": "إجمالي المخطط", "value": f"{s['target']:,.0f} كرتونة", "color": "#e0a92e"},
                {"icon": "📦", "label": "إجمالي المُنتج", "value": f"{s['actual']:,.0f} كرتونة", "color": "#0d6b5c"},
                {
                    "icon": "🥇", "label": "نسبة التحقيق", "value": f"{f_pct}%",
                    "color": "#16a34a" if f_pct >= 90 else ("#d97706" if f_pct >= 70 else "#dc2626")
                },
                {"icon": "⚖️", "label": "الوزن الفعلي", "value": f"{s['actual_kg']:,.0f} كجم", "color": "#2563eb"},
            ]
            with st.expander(f"🏭 {f_name}"):
                kpi_row(f_cards)
            factory_blocks_pack.append((f_name, f_cards))

        if st.button("🖨️ تجهيز تقرير أداء المصانع الثلاثة للطباعة", key="pack_perf_report_btn"):
            def _pack_cards_html(cards):
                parts = []
                for c in cards:
                    delta_html = ""
                    if c.get("delta"):
                        d_color = c.get("delta_color", "#6b7280")
                        delta_html = f'<div style="font-size:10px; font-weight:600; color:{d_color}; margin-top:3px;">{c["delta"]}</div>'
            ...
            combined_block_html = f"""
            <div style="margin-top:10px; page-break-inside:avoid;">
                <h2 style="background:{BRAND_GREEN}; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:8px;">
                    🏭 إجمالي المصانع الثلاثة — شهر {selected_month}
                </h2>
                <div style="display:flex; flex-wrap:wrap;">{_pack_cards_html(combined_cards)}</div>
            </div>
            """
            factory_blocks_html = ""
            for f_name, f_cards in factory_blocks_pack:
                factory_blocks_html += f"""
                <div style="margin-top:14px; page-break-inside:avoid; border-top:3px solid {BRAND_GREEN}; padding-top:8px;">
                    <h2 style="background:#f4f7fb; color:{BRAND_GREEN}; padding:6px 10px; border-radius:6px; margin-bottom:8px; border:1px solid {BRAND_GREEN};">
                        🏭 {f_name}
                    </h2>
                    <div style="display:flex; flex-wrap:wrap;">{_pack_cards_html(f_cards)}</div>
                </div>
                """

            pack_perf_height_mm = 110 + 90 + len(factory_blocks_pack) * 70
            pack_perf_html = build_printable_html(
                f"تقرير أداء المصانع الثلاثة — شهر {selected_month}",
                f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                pd.DataFrame(),
                landscape=True,
                extra_html=combined_block_html + factory_blocks_html,
                single_page_height_mm=pack_perf_height_mm,
            )
            st.download_button(
                "🖨️ تحميل تقرير أداء المصانع الثلاثة",
                data=pack_perf_html.encode("utf-8"),
                file_name=f"packing_performance_{selected_month}.html",
                mime="text/html",
                help="افتح الملف بعد التحميل، ودوس Ctrl+P واختار 'Save as PDF' عشان تحوله PDF أو تطبعه",
                key="pack_perf_report_download_btn"
            )
