...
if page == "Dashboard":
    ...
    with st.expander("📊 تقرير تعبئة الشيفتات + إنتاج الخطوط لنفس اليوم"):
        ...
        if day_shift_log.empty:
            ...
            shift_prod_tables = {}  # جدول إنتاج منفصل لكل شيفت، فيه كل الخطوط
            ...
            for pack_sh, prod_sh in zip(SHIFT_OPTIONS_P, SHIFTS):
                ...
                shift_prod_tables[pack_sh] = sh_prod_full

                st.markdown(f"**👷 {pack_sh}**")
                st.dataframe(sh_prod_full, use_container_width=True, hide_index=True)
            ...
            for pack_sh in SHIFT_OPTIONS_P:
                ...
                prod_html = shift_prod_tables.get(pack_sh, pd.DataFrame()).to_html(
                    index=False, border=0, justify="center", na_rep="-"
                )
            ...
            overview_df = pd.DataFrame([{
                "البيان": "إجمالي اليوم (كل الشيفتات)",
                "التعبئة (كرتونة)": f"{grand_total_cartons:,.0f}",
                "وزن التعبئة (كجم)": f"{grand_total_kg:,.1f}",
                "مستهدف الإنتاج (KG)": f"{prod_target_all:,.0f}",
                "فعلي الإنتاج (KG)": f"{prod_actual_all:,.0f}",
                "نسبة التحقيق %": f"{prod_achv_all:.1f}%",
            }])
