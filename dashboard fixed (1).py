...
.app-header-banner {
    background: linear-gradient(135deg, var(--accent) 0%, #0d6b5c 100%);
...
.nav-tile .nav-tile-icon { font-size: 30px; margin-bottom: 10px; }
.nav-tile .nav-tile-title { font-size: 17px; font-weight: 900; color: var(--accent); margin-bottom: 4px; }
.nav-tile .nav-tile-desc { font-size: 12.5px; font-weight: 600; color: #6b7280; }
...
def donut_legend_html(series, unit=""):
    """قائمة جانبية بجانب الدونات فيها لون كل خط + القيمة + النسبة، زي التصميم"""
    total = series.sum()
    rows = ""
    for i, (label, val) in enumerate(series.items()):
        ...
        rows += f"""
        <div style="display:flex; align-items:center; justify-content:space-between; padding:7px 2px;
                     border-bottom:1px solid rgba(1,62,55,0.08);">
            ...
            <div style="font-weight:800; font-size:14px;">{val:,.0f}{unit} <span style="color:#6b7280; font-weight:700; font-size:12px;">({pct:.1f}%)</span></div>
        </div>
        """
    return rows
...
def kpi_row(cards):
    """صف كروت KPI بشكل السيركل أيقونة + رقم كبير + وصف — زي التصميم المرفق"""
...
if page == "الرئيسية":
        ...
        with tile_cols[i % 4]:
            st.markdown(
                ...
                <a class="nav-tile" href="?nav={name}" target="_self">
                    ...
                    <div class="nav-tile-title">{name}</div>
    ...
    kpi_row([
        {"icon": "📦", "label": "إجمالي الإنتاج (KG)", "value": f"{prod_actual:,.0f}"},
        {"icon": "🎯", "label": "المستهدف (KG)", "value": f"{prod_target:,.0f}"},
        {"icon": "⚡", "label": "كفاءة OEE", "value": f"{oee:.1f}%"},
        {"icon": "⏰", "label": "إجمالي التوقف (ساعة)", "value": f"{faults_total_min/60:,.1f}"},
        {"icon": "🥇", "label": "نسبة تحقيق الإنتاج", "value": f"{prod_achv_dash:.1f}%"},
    ])
    ...
    with gc1:
        ...
        ax_gauge.plot([0, 0.75 * np.cos(needle_rad)], [0, 0.75 * np.sin(needle_rad)],
                      color=BRAND_GREEN, linewidth=3)
        ax_gauge.add_artist(plt.Circle((0, 0), 0.05, color=BRAND_GREEN))
        ax_gauge.text(0, -0.25, f"{oee:.1f}%", ha="center", fontsize=20, fontweight="bold", color=BRAND_GREEN)
        ax_gauge.text(0, -0.45, ar_text("OEE التقريبية"), ha="center", fontsize=11, color="#6b7280", fontweight="bold")
    ...
    with dc1:
        st.markdown("###### 🏭 إنتاج الخطوط (KG)")
        if not prod_by_line.empty:
            prod_line_totals = prod_by_line["الفعلي (KG)"] if "الفعلي (KG)" in prod_by_line.columns else prod_by_line.iloc[:, -1]
            fig_donut = build_donut_chart(prod_line_totals, title="", center_label="إجمالي (KG)")
            donut_c1, donut_c2 = st.columns([1, 1])
            with donut_c1:
                if fig_donut is not None:
                    st.pyplot(fig_donut)
            with donut_c2:
                st.markdown(donut_legend_html(prod_line_totals), unsafe_allow_html=True)
        else:
            st.info("لا توجد بيانات إنتاج مسجلة لهذا الشهر")
    with dc2:
        st.markdown("###### 📦 الباكينج حسب الفئة (كرتونة)")
        if not pack_by_cat.empty:
            fig_pack_donut = build_donut_chart(pack_by_cat, title="", center_label="كرتونة")
            pack_c1, pack_c2 = st.columns([1, 1])
            with pack_c1:
                if fig_pack_donut is not None:
                    st.pyplot(fig_pack_donut)
            with pack_c2:
                st.markdown(donut_legend_html(pack_by_cat), unsafe_allow_html=True)
        else:
            st.info("لا توجد بيانات باكينج مسجلة لهذا الشهر")

    st.markdown("###### ⚠ توزيع وقت التوقف حسب الخط")
    if not faults_by_line.empty:
        fig_f, ax_f = plt.subplots()
        fig_f.patch.set_facecolor("#ffffff")
        wedges, texts, autotexts = ax_f.pie(
            faults_by_line.values, labels=[ar_text(l) for l in faults_by_line.index],
            autopct="%1.0f%%", startangle=90,
            colors=BRAND_PALETTE, textprops={"fontsize": 9, "fontweight": "bold", "color": "#1a1d21"},
            wedgeprops={"edgecolor": "#ffffff", "linewidth": 1.5}
        )
        for at in autotexts:
            at.set_color("#ffffff")
        ax_f.axis("equal")
        fault_col1, _ = st.columns([1, 2])
        with fault_col1:
            st.pyplot(fig_f)
    else:
    ...
    kpi_row([
        {"icon": "🎯", "label": "المستهدف", "value": f"{total_target:,.0f} KG"},
        {"icon": "📦", "label": "الفعلي", "value": f"{total_actual:,.0f} KG"},
        {"icon": "🥇", "label": "نسبة التحقيق", "value": f"{achievement:.1f}%"},
    ])
    ...
    if line_summary["المستهدف (KG)"].sum() == 0 and line_summary["الفعلي (KG)"].sum() == 0:
        ...
        donut_col, bar_col = st.columns([1, 1.4])
        with donut_col:
            st.markdown("###### 🏭 توزيع الإنتاج الفعلي بين الخطوط")
            line_actual_series = line_summary.set_index("الخط")["الفعلي (KG)"]
            fig_prod_donut = build_donut_chart(line_actual_series, title="", center_label="إجمالي (KG)")
            d1, d2 = st.columns([1, 1])
            with d1:
                if fig_prod_donut is not None:
                    st.pyplot(fig_prod_donut)
            with d2:
                st.markdown(donut_legend_html(line_actual_series), unsafe_allow_html=True)
        with bar_col:
    ...
    if summary.empty:
        ...
        kpi_row([
            {"icon": "🎯", "label": "إجمالي المخطط", "value": f"{summary['المخطط بالكراتين (شهري)'].sum():,.0f} كرتونة"},
            {"icon": "📦", "label": "إجمالي المُنتج", "value": f"{summary['إجمالي الكراتين'].sum():,.0f} كرتونة"},
            {"icon": "🥇", "label": "نسبة التحقيق", "value": f"{total_pct}%"},
            {"icon": "⚖️", "label": "الوزن الفعلي", "value": f"{summary['الوزن الفعلي (كيلو)'].sum():,.1f} كجم"},
        ])
    ...
    if summary_f.empty:
        ...
        with chart_c1:
            ...
            fig_f, ax_f = plt.subplots()
            fig_f.patch.set_facecolor("#ffffff")
            wedges_f, texts_f, autotexts_f = ax_f.pie(
                line_totals.values, labels=[ar_text(l) for l in line_totals.index],
                autopct="%1.1f%%", startangle=90, colors=BRAND_PALETTE,
                textprops={"fontsize": 9, "fontweight": "bold", "color": "#1a1d21"},
                wedgeprops={"edgecolor": "#ffffff", "linewidth": 1.5}
            )
            for at in autotexts_f:
                at.set_color("#ffffff")
            ax_f.axis("equal")
            st.pyplot(fig_f)
        with chart_c2:
