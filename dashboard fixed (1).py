...
def build_breakage_report_html(kpis, line_rows, trend_dates, trend_out, trend_in,
                                report_title="تقرير تحليل الكسر لكل الخطوط"):
    """تقرير احترافي لتحليل الكسر بنظام الرصيد (رصيد أول المدة + كسر خارج = إجمالي الكسر، وبعد خصم
    الكسر الداخل بيبقى متبقي الرصيد) — لكل خط في صف لوحده تحت بعض."""
    palette = ["#0d6b5c", "#e0a92e", "#4c8c7d", "#a06f0f", "#7fb5a6", "#2563eb", "#c98f1f", "#7c3aed"]

    line_names = [r["name"] for r in line_rows]
    out_vals = [r["out"] for r in line_rows]
    in_vals = [r["in"] for r in line_rows]
    ...
    table_rows_html += f"""
    <tr class="total-row">
        <td>الإجمالي</td><td>{tot_open:,.0f}</td><td>{tot_out:,.0f}</td><td>{tot_in:,.0f}</td>
        <td>{tot_total:,.0f}</td><td>{tot_remain:,.0f}</td><td>-</td>
    </tr>
    """
    ...
    <div class="panel">
        ...
        <table>
            <tr>
                ...
                <th>الكسر الداخل (KG)</th><th>إجمالي الكسر (KG)</th><th>متبقي الرصيد (KG)</th><th>نسبة الكسر %</th>
            </tr>
            {table_rows_html}
        </table>
    ...
    <div class="row">
        <div class="col-1 panel">
            ...
            <canvas id="lineBar" height="180"></canvas>
        </div>
        <div class="col-1 panel">
    ...
    <div class="panel">
        ...
        <canvas id="trendChartB" height="150"></canvas>
    </div>
    ...
    if breakage_log_all.empty:
        ...
        if day_b.empty:
            ...
            for ln in all_lines_clean_b:
                ...
                pct_v = ((out_v + in_v) / prod_v * 100) if prod_v > 0 else 0
                line_rows_b.append({
            ...
            tot_pct = ((tot_out + tot_in) / total_prod_day * 100) if total_prod_day > 0 else 0

            kpis_b = [
                {"icon": "🏁", "label": "رصيد أول المدة", "value": f"{tot_opening:,.0f} KG", "color": "#2563eb"},
                {"icon": "⬆️", "label": "الكسر الخارج", "value": f"{tot_out:,.0f} KG", "color": "#dc2626"},
                {"icon": "⬇️", "label": "الكسر الداخل", "value": f"{tot_in:,.0f} KG", "color": "#e0a92e"},
                {"icon": "🔻", "label": "إجمالي الكسر", "value": f"{tot_total:,.0f} KG", "color": "#7c3aed"},
                {"icon": "🏦", "label": "متبقي الرصيد", "value": f"{tot_remaining:,.0f} KG", "color": "#0d6b5c"},
                {"icon": "⚠️", "label": "نسبة الكسر من الإنتاج", "value": f"{tot_pct:.2f}%", "color": "#dc2626"},
            ]

            # ---------- اتجاه آخر 7 أيام ----------
            trend_dates_b = [(breakage_date - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(6, -1, -1)]
            trend_out_b, trend_in_b = [], []
            ...
            breakage_html = build_breakage_report_html(
                kpis_b, line_rows_b, trend_dates_b, trend_out_b, trend_in_b,
                report_title=f"تقرير تحليل الكسر لكل الخطوط — {breakage_date_str}"
            )
