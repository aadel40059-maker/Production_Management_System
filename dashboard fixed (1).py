...
def _normalize_item_name(v):
    """بتوحّد اسم الصنف عشان مطابقة صحيحة بين ملف المخزون وأسماء الأصناف في التقرير
    (فراغات زيادة، اختلاف الألف/الياء/التاء المربوطة، وتجاهل كلمتي 'سولا' و'ويفر' لإنهم
    اسامي براند/خط منتج بتتحط أحيانًا وأحيانًا لأ في نفس الصنف بالظبط)"""
    s = str(v).strip()
    s = re.sub(r"\s+", " ", s)
    s = s.replace("أ", "ا").replace("إ", "ا").replace("آ", "ا")
    s = s.replace("ى", "ي").replace("ة", "ه")
    s = re.sub(r"\bسولا\b", "", s)
    s = re.sub(r"\bويفر\b", "", s)
