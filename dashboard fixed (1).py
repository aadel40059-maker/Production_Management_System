...
if page == "الرئيسية":
    ...
    if st.button("📊 توليد ملف الإكسيل", key="reports_generate_excel_btn"):
        ...
        if gsheet_exists("breakage_history"):
            breakage_log_month_x = read_gsheet("breakage_history")
            breakage_log_month_x["Date"] = breakage_log_month_x["Date"].astype(str)
            for c_ in ["كسر خارج (KG)", "كسر داخل (KG)", "رصيد أول المدة (KG)"]:
                if c_ in breakage_log_month_x.columns:
                    breakage_log_month_x[c_] = pd.to_numeric(breakage_log_month_x[c_], errors="coerce").fillna(0)
            breakage_log_month_x["المصنع"] = breakage_log_month_x.get("الخط", "").map(EXCEL_LINE_TO_FACTORY_X).fillna("-")
            ...
            if not pack_grid_month_x.empty and dcol in pack_grid_month_x.columns:
                ...
                for idx, r in pack_grid_month_x.iterrows():
                    if str(r.get("الصنف", "")).strip() == "":
                        continue
                    daily_plan_cartons = round(r["المخطط بالكراتين (شهري)"] / excel_days_in_month, 1) if excel_days_in_month > 0 else 0
        ...
        row_ptr_x += 1
        ws_daily_x.cell(row=row_ptr_x, column=1, value=f"أيام العمل المخططة هذا الشهر: {excel_days_in_month}").font = NORMAL_FONT_X
        row_ptr_x += 2
    ...
    if st.button("🖥️ توليد الداشبورد اليومي", key="reports_generate_daily_dash_btn"):
        ...
        if not pack_grid_d.empty:
            pack_grid_d["_تارجت_يومي"] = pack_grid_d["المخطط بالكراتين (شهري)"] / excel_days_in_month if excel_days_in_month > 0 else 0
        ...
        daily_dashboard_html = build_dashboard_document(
            ...
            f"Planned working days this month: {excel_days_in_month}",
            dash_sections,
        )
