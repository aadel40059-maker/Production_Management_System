...
def build_multi_report_html(overall_title, overall_subtitle, sections):
    """بيجمع كذا تقرير (كل واحد فيه عنوان + بطاقات KPI + جدول + رسوم بيانية) في ملف HTML واحد قابل للطباعة/PDF،
    كل تقرير بيبدأ في صفحة جديدة (page-break-before) — ده اللي بيتستخدم في خاصية 'البورتفوليو الشهري'.
    sections: قائمة dicts فيها: title, subtitle, df (DataFrame), top_html (بطاقات KPI), extra_html (رسوم بيانية)"""
    blocks = []
    for i, sec in enumerate(sections):
        table_html = sec["df"].to_html(index=False, border=0, justify="center", na_rep="-")
        page_break = "page-break-before: always;" if i > 0 else ""
        blocks.append(f"""
        <div style="{page_break}">
            <h1 style="font-size:13px; margin-top:6px; color:#0b1f3a;">{sec['title']}</h1>
            <p class="sub">{sec.get('subtitle', '')}</p>
            {sec.get('top_html') or ''}
            {table_html}
            {sec.get('extra_html') or ''}
        </div>
        """)
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        h2 {{ color: #0b1f3a; font-size: 12px; margin: 8px 0 4px; }}
        p.dev {{ color: #374151; margin: 0 0 4px; font-size: 10px; font-weight: 600; }}
        p.sub {{ color: #6b7280; margin: 0 0 10px; font-size: 9px; }}
        table {{ border-collapse: collapse; width: 100%; font-size: 11px; font-weight: 700; }}
        table th, table td {{ border: 1px solid #d1d5db; padding: 4px 6px; text-align: center; }}
        table tr {{ page-break-inside: avoid; }}
        table thead {{ display: table-header-group; }}
        table th {{ background-color: #f4f7fb; font-weight: 700; }}
        table tr:nth-child(even):not([style]) {{ background-color: #fafafa; }}
        table tr:last-child:not([style]) {{ font-weight: 700; background-color: #eef2f7; }}
    ...
    <body>
        <h1>🏭 Production Management System</h1>
        <p class="dev">Developed by Eng. Ahmed Adel</p>
        <h1 style="font-size:14px; margin-top:4px;">{overall_title}</h1>
        <p class="sub">{overall_subtitle}</p>
        {''.join(blocks)}
    </body>
    </html>
    """
...
if page == "الرئيسية":
    ...
    page_banner("🏭", "Production Management", "متابعة إنتاج كل خط وتسجيله بالشيفت")
    ...
    total_target = edited_df["المستهدف (KG)"].sum()
    total_actual = edited_df["الفعلي (KG)"].sum()

    achievement = (total_actual / total_target * 100) if total_target > 0 else 0

    kpi_row([
        {"icon": "🎯", "label": "المستهدف", "value": f"{total_target:,.0f} KG", "color": "#e0a92e"},
        {"icon": "📦", "label": "الفعلي", "value": f"{total_actual:,.0f} KG", "color": "#0d6b5c"},
        {"icon": "🥇", "label": "نسبة التحقيق", "value": f"{achievement:.1f}%", "color": "#7c3aed"},
    ])

    # ---------- تجميع الإنتاج لكل خط (بيدمج الشيفتات الفرعية تحت اسم الخط) ----------
    st.divider()
    st.markdown("#### 📊 إجمالي الإنتاج لكل خط")

    lines_only = edited_df[edited_df["Shift"] == "All Shifts"].copy()
    lines_only["الخط"] = lines_only["Line"].astype(str).str.replace("🏭", "", regex=False).str.strip()
    line_summary = lines_only.groupby("الخط", sort=False)[["المستهدف (KG)", "الفعلي (KG)"]].sum().reset_index()
    line_summary["نسبة التحقيق %"] = line_summary.apply(
        lambda r: round((r["الفعلي (KG)"] / r["المستهدف (KG)"] * 100), 1) if r["المستهدف (KG)"] > 0 else 0,
        axis=1
    )

    if line_summary["المستهدف (KG)"].sum() == 0 and line_summary["الفعلي (KG)"].sum() == 0:
        st.info("سجّل قيم فعلي/مستهدف في الجدول فوق عشان يظهر تجميع الإنتاج لكل خط والرسم البياني")
    else:
        st.dataframe(line_summary, use_container_width=True, hide_index=True)

        fig_lines = build_line_production_chart(line_summary)
        if fig_lines is not None:
            st.pyplot(fig_lines)

        st.markdown("###### 🏭 توزيع الإنتاج الفعلي بين الخطوط")
        line_actual_series = line_summary.set_index("الخط")["الفعلي (KG)"]
        fig_prod_tree = build_treemap_chart(line_actual_series, unit=" KG")
    ...
    if st.button("💾 Save Production"):
        ...
        # إصلاح: خانة اسم الخط فاضية في صفوف الشيفتات الفرعية (بتتعرض كده بس للعرض)،
        # فبنعبّيها بنفس اسم الخط اللي فوقها (forward-fill) قبل الحفظ عشان اسم الخط
        # يظهر صح لما نقرأ بيانات الإنتاج بالشيفت في تقرير الباكينج وأي تقرير تاني
        to_save["Line"] = to_save["Line"].astype(str).str.replace("🏭", "", regex=False).str.strip()
        to_save["Line"] = to_save["Line"].replace("", pd.NA).ffill()
    ...
    # ================= سجل الإنتاج اليومي + إجمالي الشهر حتى الآن =================
    st.divider()
    st.markdown("### 📅 سجل الإنتاج اليومي وإجمالي الشهر حتى الآن")

    all_lines_clean_p = [str(l).replace("🏭", "").strip() for l in st.session_state.production_lines]
    current_month_str_p = datetime.now().strftime("%Y-%m")
    ...
    if not GSHEETS_ENABLED or not gsheet_exists(PRODUCTION_SHEET):
        ...
        if month_data_p.empty:
            ...
            daily_pivot = daily_pivot.reindex(columns=all_lines_clean_p, fill_value=0)
            daily_pivot = daily_pivot.sort_index()
            daily_pivot["الإجمالي"] = daily_pivot.sum(axis=1)

            dates_list = list(daily_pivot.index)
            series_by_line = {ln: [float(v) for v in daily_pivot[ln]] for ln in all_lines_clean_p}

            table_header_html = "<tr><th>التاريخ</th>" + "".join(f"<th>{ln}</th>" for ln in all_lines_clean_p) + "<th>الإجمالي</th></tr>"
            table_rows_html = ""
            for d in dates_list:
                row = daily_pivot.loc[d]
                cells = "".join(f"<td>{row[ln]:,.0f}</td>" for ln in all_lines_clean_p)
                table_rows_html += f"<tr><td style='font-weight:800;'>{d}</td>{cells}<td style='font-weight:900;'>{row['الإجمالي']:,.0f}</td></tr>"
            grand_total_row = daily_pivot[all_lines_clean_p].sum()
            total_cells = "".join(f"<td>{grand_total_row[ln]:,.0f}</td>" for ln in all_lines_clean_p)
            ...
            month_by_line = month_by_line.reindex(all_lines_clean_p, fill_value=0)
            ...
            # ---------- نصيب كل شيفت من إنتاج كل خط (من بداية الشهر حتى الآن) ----------
            ...
            if shifts_month_p.empty:
                ...
                shift_matrix = {}
                for ln in all_lines_clean_p:
                    shift_matrix[ln] = {}
                    for sh in SHIFTS:
                        shift_matrix[ln][sh] = float(shift_pivot.get((ln, sh), 0))

                shift_share_html = build_shift_share_html(all_lines_clean_p, SHIFTS, shift_matrix)
    ...
    if summary.empty:
        ...
        # ---------- اتجاه الإنتاج آخر 7 أيام (مشترك: بيتحط في تقرير مدير الإنتاج وتقرير التخطيط) ----------
        trend_days = [d for d in range(max(1, day_pick - 6), day_pick + 1)]
        trend_cols = [f"يوم {d}" for d in trend_days]
        trend_totals = [summary[c].sum() if c in summary.columns else 0 for c in trend_cols]

        def _build_trend_fig(figsize=(7.2, 3.2), title_size=12):
            fig_t, ax_t = plt.subplots(figsize=figsize)
            fig_t.patch.set_facecolor("#ffffff")
            ax_t.plot(trend_days, trend_totals, marker="o", linewidth=2.4, color=BRAND_GREEN,
                      markerfacecolor="#ffffff", markeredgecolor=BRAND_GREEN, markeredgewidth=2,
                      markersize=7, zorder=3)
            ax_t.fill_between(trend_days, trend_totals, color=BRAND_GREEN, alpha=0.08, zorder=2)
            for x, y in zip(trend_days, trend_totals):
                ax_t.annotate(f"{y:,.0f}", xy=(x, y), xytext=(0, 8), textcoords="offset points",
                               ha="center", fontsize=8, fontweight="bold", color="#1a1d21")
        ...
        prod_line_table_html = prod_line_table.to_html(index=False, border=0, justify="center", na_rep="-")
    ...
    fig_daily.patch.set_facecolor("#ffffff")
    bar_values = line_summary_day["إجمالي التوقف (دقيقة)"].tolist()
    bar_labels = [ar_text(l) for l in line_summary_day["الخط"].tolist()]
    bar_colors = [BRAND_PALETTE[i % len(BRAND_PALETTE)] for i in range(len(bar_values))] if bar_values else BRAND_GREEN
    bars = ax_daily.bar(
        bar_labels,
        bar_values,
        color=bar_colors,
    ...
    ax_daily.set_ylabel(ar_text("دقيقة"), fontsize=8, fontweight="bold")
    ax_daily.set_title(ar_text(f"إجمالي التوقف الشهري لكل خط — حتى {daily_report_date_str}"), fontsize=9.5, fontweight="bold", color=BRAND_GREEN, pad=8)
    ...
    plt.setp(ax_daily.get_xticklabels(), rotation=20, ha="right", fontsize=7, fontweight="bold")
    ...
    trend_colors = [BRAND_PALETTE[i % len(BRAND_PALETTE)] for i in range(len(fault_lines))]
    ...
    ax_trend.set_xticklabels([d[5:] for d in trend_day_strs], fontsize=7, fontweight="bold")
    ax_trend.set_ylabel(ar_text("عدد الأعطال"), fontsize=8, fontweight="bold")
    ax_trend.set_title(ar_text("تطور الأعطال اليومي (عدد الأعطال)"), fontsize=9.5, fontweight="bold", color=BRAND_GREEN, pad=8)
    ...
    def _render_bar_pie(bar_labels, bar_values, bar_title, bar_color, pie_labels, pie_values, pie_title):
        """بيرسم عمود + دائرة على الشاشة، وبيرجع HTML (صور base64 بجودة طباعة) جاهز يتحط في التقرير المطبوع"""
        bar_labels = list(bar_labels)
        bar_values = list(bar_values)
        pie_labels = list(pie_labels)
        pie_values = list(pie_values)
        has_pie = sum(pie_values) > 0

        c1, c2 = st.columns([1.3, 1])
        with c1:
            st.caption(bar_title)
            fig1, ax1 = plt.subplots(figsize=(6.6, 3.6))
            fig1.patch.set_facecolor("#ffffff")
            bars = ax1.bar(bar_labels, bar_values, color=bar_color, edgecolor="#00201c", linewidth=0.7, zorder=3)
            for b in bars:
                h = b.get_height()
                ax1.annotate(f"{h:,.0f}", xy=(b.get_x() + b.get_width() / 2, h), xytext=(0, 4),
                             textcoords="offset points", ha="center", va="bottom", fontsize=8, fontweight="bold")
            ax1.set_xticks(range(len(bar_labels)))
            ax1.set_xticklabels(bar_labels, rotation=35, ha="right", fontsize=9, fontweight="bold")
            ax1.spines["top"].set_visible(False)
            ax1.spines["right"].set_visible(False)
            ax1.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
            ax1.set_axisbelow(True)
            fig1.tight_layout()
            st.pyplot(fig1)
        with c2:
            st.caption(pie_title)
            if has_pie:
                fig2, ax2 = plt.subplots()
                fig2.patch.set_facecolor("#ffffff")
                w_, t_, at_ = ax2.pie(pie_values, labels=pie_labels, autopct="%1.1f%%", startangle=90,
                                       colors=BRAND_PALETTE, textprops={"fontsize": 9, "fontweight": "bold"},
                                       wedgeprops={"edgecolor": "#ffffff", "linewidth": 1.5})
                for a in at_:
                    a.set_color("#ffffff")
                ax2.axis("equal")
                ...
                st.info("مفيش بيانات كافية لرسم الدائرة")

        fig1p, ax1p = plt.subplots(figsize=(7.2, 3.6))
        fig1p.patch.set_facecolor("#ffffff")
        bars_p = ax1p.bar(bar_labels, bar_values, color=bar_color, edgecolor="#00201c", linewidth=0.7, zorder=3)
        for b in bars_p:
            h = b.get_height()
            ax1p.annotate(f"{h:,.0f}", xy=(b.get_x() + b.get_width() / 2, h), xytext=(0, 4),
                          textcoords="offset points", ha="center", va="bottom", fontsize=8, fontweight="bold")
        ax1p.set_title(ar_text(bar_title), fontsize=12, fontweight="bold", color=BRAND_GREEN, pad=10)
        ax1p.set_xticks(range(len(bar_labels)))
        ax1p.set_xticklabels(bar_labels, rotation=35, ha="right", fontsize=9, fontweight="bold")
        ax1p.spines["top"].set_visible(False)
        ax1p.spines["right"].set_visible(False)
        ax1p.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
        ax1p.set_axisbelow(True)
        fig1p.tight_layout()
        bar_img_html = fig_to_base64_img(fig1p)

        pie_img_html = ""
        if has_pie:
            fig2p, ax2p = plt.subplots(figsize=(4.6, 4.2))
            fig2p.patch.set_facecolor("#ffffff")
            w_, t_, at_ = ax2p.pie(pie_values, labels=pie_labels, autopct="%1.1f%%", startangle=90,
                                    colors=BRAND_PALETTE, textprops={"fontsize": 9, "fontweight": "bold"},
                                    wedgeprops={"edgecolor": "#ffffff", "linewidth": 1.5})
            for a in at_:
                a.set_color("#ffffff")
            ax2p.axis("equal")
            ax2p.set_title(ar_text(pie_title), fontsize=12, fontweight="bold", color=BRAND_GREEN, pad=10)
            pie_img_html = fig_to_base64_img(fig2p)

        return f"""
        <div style="display:flex; gap:14px; margin-top:8px; page-break-inside:avoid;">
            <div style="flex:1.3;">{bar_img_html}</div>
            <div style="flex:1;">{pie_img_html}</div>
        </div>
        """

    report_parts = {}  # بنخزن هنا أجزاء كل تقرير (عنوان/بطاقات/جدول/رسوم) عشان نقدر نجمعهم في البورتفوليو

    # ==========================
    # 1) تقرير الباكينج الشهري — بالوزن
    # ==========================
    with st.expander("📦 تقرير الباكينج الشهري — بالوزن", expanded=False):
        ...
        if not gsheet_exists(packing_sheet_r):
            ...
            pgrid_r["الوزن الفعلي (كجم)"] = (pgrid_r["إجمالي الكراتين"] * pgrid_r["وزن الكرتونة (كيلو)"]).round(1)
            pgrid_r["الوزن المخطط (كجم)"] = (
                pgrid_r["المخطط بالكراتين (شهري)"] * pgrid_r["وزن الكرتونة (كيلو)"]
            ).round(1)
            pgrid_r["نسبة التحقيق %"] = pgrid_r.apply(
                lambda r: round((r["الوزن الفعلي (كجم)"] / r["الوزن المخطط (كجم)"] * 100), 1)
                if r["الوزن المخطط (كجم)"] > 0 else None,
                axis=1
            )

            pack_active_r = pgrid_r[pgrid_r["إجمالي الكراتين"] > 0].sort_values(
                "الوزن الفعلي (كجم)", ascending=False
            ).reset_index(drop=True)

            if pack_active_r.empty:
                st.info("لسه مفيش أي تعبئة اتسجلت للشهر ده")
            else:
                total_actual_w = pack_active_r["الوزن الفعلي (كجم)"].sum()
                total_target_w = pgrid_r["الوزن المخطط (كجم)"].sum()
                achievement_w = (total_actual_w / total_target_w * 100) if total_target_w > 0 else 0
                top_item_w = pack_active_r.iloc[0]
                items_count_w = len(pack_active_r)

                pack_top_html = (
                    '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                        _report_kpi_card("📋", f"{items_count_w}", "عدد الأصناف المعبأة", "#2563eb"),
                        _report_kpi_card("⚖️", f"{total_actual_w:,.0f} كجم", "إجمالي الوزن الفعلي", "#0d6b5c"),
                        _report_kpi_card("🎯", f"{total_target_w:,.0f} كجم", "إجمالي الوزن المخطط", "#e0a92e"),
                        _report_kpi_card("📈", f"{achievement_w:.1f}%", "نسبة التحقيق بالوزن", "#7c3aed"),
                        _report_kpi_card("🏆", str(top_item_w["الصنف"])[:18],
                                          f"أعلى صنف ({top_item_w['الوزن الفعلي (كجم)']:,.0f} كجم)", "#16a34a"),
                    ]) + "</div>"
                )
                st.markdown(_flat_html(pack_top_html), unsafe_allow_html=True)

                pack_table_display = pack_active_r[[
                    "الصنف", "إجمالي الكراتين", "الوزن الفعلي (كجم)", "الوزن المخطط (كجم)", "نسبة التحقيق %"
                ]]
                st.dataframe(pack_table_display, use_container_width=True, hide_index=True)

                pack_top10_r = pack_active_r.head(10)
                cat_w_totals_r = pack_active_r.groupby("الفئة")["الوزن الفعلي (كجم)"].sum().sort_values(ascending=False)

                pack_extra_html = _render_bar_pie(
                    bar_labels=[ar_text(n) for n in pack_top10_r["الصنف"]],
                    bar_values=pack_top10_r["الوزن الفعلي (كجم)"],
                    bar_title="📊 أعلى 10 أصناف بالوزن (كجم)",
                    bar_color=BRAND_GREEN,
                    pie_labels=[ar_text(l) for l in cat_w_totals_r.index],
                    pie_values=cat_w_totals_r.values,
                    pie_title="🥧 توزيع الوزن بالفئات"
                )

                pack_html_r = build_printable_html(
                    f"تقرير الباكينج الشهري (بالوزن) — {month_label_r}",
                    f"إجمالي الوزن الفعلي {total_actual_w:,.0f} كجم من {total_target_w:,.0f} كجم مخطط — "
                    f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                    pack_table_display, landscape=True, base_font_size=12, bold=True, row_padding="6px 5px",
                    top_html=pack_top_html, extra_html=pack_extra_html
                )
                st.download_button(
                    "🖨️ تحميل تقرير الباكينج بالوزن", data=pack_html_r.encode("utf-8"),
                    file_name=f"packing_weight_report_{selected_month_r}.html", mime="text/html",
                    key="dl_pack_weight_report"
                )

                report_parts["packing"] = {
                    "title": f"📦 تقرير الباكينج الشهري (بالوزن) — {month_label_r}",
                    "subtitle": f"إجمالي الوزن الفعلي {total_actual_w:,.0f} كجم من {total_target_w:,.0f} كجم مخطط",
                    "df": pack_table_display, "top_html": pack_top_html, "extra_html": pack_extra_html
                }
    ...
    with st.expander("🏭 تقرير البرودكشن الشهري", expanded=False):
        if not gsheet_exists(PRODUCTION_SHEET):
            ...
            if prod_month_r.empty:
                ...
                lines_count_r = len(prod_line_summary_r)

                prod_top_html = (
                    '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                        _report_kpi_card("🏭", f"{lines_count_r}", "عدد الخطوط المنتجة", "#2563eb"),
                        _report_kpi_card("🎯", f"{total_target_r:,.0f} KG", "إجمالي المستهدف", "#e0a92e"),
                        _report_kpi_card("📦", f"{total_actual_r:,.0f} KG", "إجمالي الفعلي", "#0d6b5c"),
                        _report_kpi_card("📈", f"{achievement_r:.1f}%", "نسبة التحقيق الإجمالية", "#7c3aed"),
                        _report_kpi_card(
                            "🥇", str(best_line_r["الخط"])[:16] if best_line_r is not None else "-",
                            f"أعلى خط ({best_line_r['الفعلي (KG)']:,.0f} KG)" if best_line_r is not None else "-",
                            "#16a34a"
                        ),
                    ]) + "</div>"
                )
                ...
                fig_prod_tree_r = build_treemap_chart(line_actual_series_r, unit=" KG")
                if fig_prod_tree_r is not None:
                    st.pyplot(fig_prod_tree_r)

                prod_extra_parts_r = []
                fig_prod_r_p = build_line_production_chart(prod_line_summary_r)
                if fig_prod_r_p is not None:
                    prod_extra_parts_r.append(fig_to_base64_img(fig_prod_r_p))
                fig_prod_tree_r_p = build_treemap_chart(line_actual_series_r, unit=" KG")
                if fig_prod_tree_r_p is not None:
                    prod_extra_parts_r.append(fig_to_base64_img(fig_prod_tree_r_p))
                prod_extra_html = f'<div style="margin-top:8px;">{"".join(prod_extra_parts_r)}</div>'

                prod_html_r = build_printable_html(
                    f"تقرير البرودكشن الشهري — {month_label_r}",
                    f"إجمالي الفعلي {total_actual_r:,.0f} KG من {total_target_r:,.0f} KG مخطط — "
                    f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                    prod_line_summary_r, landscape=False, base_font_size=13, bold=True, row_padding="8px 6px",
                    top_html=prod_top_html, extra_html=prod_extra_html
                )
                st.download_button(
                    "🖨️ تحميل تقرير البرودكشن الشهري", data=prod_html_r.encode("utf-8"),
                    file_name=f"production_report_{selected_month_r}.html", mime="text/html",
                    key="dl_prod_report"
                )

                report_parts["production"] = {
                    "title": f"🏭 تقرير البرودكشن الشهري — {month_label_r}",
                    "subtitle": f"إجمالي الفعلي {total_actual_r:,.0f} KG من {total_target_r:,.0f} KG مخطط",
                    "df": prod_line_summary_r, "top_html": prod_top_html, "extra_html": prod_extra_html
                }
            ...
            if break_month_r.empty:
                ...
                if gsheet_exists(PRODUCTION_SHEET):
                    ...
                    if not prod_for_ratio_m.empty:
                        ...
                        if total_prod_month_r > 0:
                            break_ratio_txt = f"{(total_net_r / total_prod_month_r * 100):.2f}%"

                break_top_html = (
                    '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                        _report_kpi_card("📤", f"{total_out_r:,.0f} كجم", "إجمالي الكسر خارج", "#dc2626"),
                        _report_kpi_card("📥", f"{total_in_r:,.0f} كجم", "إجمالي الكسر داخل", "#16a34a"),
                        _report_kpi_card("⚖️", f"{total_net_r:,.0f} كجم", "صافي الكسر", "#e0a92e"),
                        _report_kpi_card("📉", break_ratio_txt, "نسبة صافي الكسر من الإنتاج", "#7c3aed"),
                        _report_kpi_card(
                            "🔻", str(worst_line_break_r["الخط"])[:16] if worst_line_break_r is not None else "-",
                            f"أعلى خط كسر ({worst_line_break_r['صافي الكسر (KG)']:,.0f} كجم)"
                            if worst_line_break_r is not None else "-",
                            "#2563eb"
                        ),
                    ]) + "</div>"
                )
                ...
                pie_vals_b = break_line_summary_r["صافي الكسر (KG)"].clip(lower=0)
                break_extra_html = _render_bar_pie(
                    bar_labels=[ar_text(n) for n in break_line_summary_r["الخط"]],
                    bar_values=break_line_summary_r["صافي الكسر (KG)"],
                    bar_title="📊 صافي الكسر لكل خط (كجم)",
                    bar_color="#dc2626",
                    pie_labels=[ar_text(n) for n in break_line_summary_r["الخط"]],
                    pie_values=pie_vals_b.values,
                    pie_title="🥧 نسبة كل خط من إجمالي الكسر"
                )

                break_html_r = build_printable_html(
                    f"تقرير الكسر الشهري — {month_label_r}",
                    f"صافي الكسر {total_net_r:,.0f} كجم (خارج {total_out_r:,.0f} - داخل {total_in_r:,.0f}) — "
                    f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                    break_line_summary_r, landscape=False, base_font_size=13, bold=True, row_padding="8px 6px",
                    top_html=break_top_html, extra_html=break_extra_html
                )
                st.download_button(
                    "🖨️ تحميل تقرير الكسر الشهري", data=break_html_r.encode("utf-8"),
                    file_name=f"breakage_report_{selected_month_r}.html", mime="text/html",
                    key="dl_break_report"
                )

                report_parts["breakage"] = {
                    "title": f"🔨 تقرير الكسر الشهري — {month_label_r}",
                    "subtitle": f"صافي الكسر {total_net_r:,.0f} كجم (خارج {total_out_r:,.0f} - داخل {total_in_r:,.0f})",
                    "df": break_line_summary_r, "top_html": break_top_html, "extra_html": break_extra_html
                }
    ...
    with st.expander("⚠️ تقرير الأعطال الشهري", expanded=False):
        if not gsheet_exists("faults_daily_log"):
            ...
            if faults_month_r.empty:
                ...
                faults_top_html = (
                    '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                        _report_kpi_card("⏱️", f"{total_downtime_r:,.0f} د", "إجمالي دقائق التوقف", "#dc2626"),
                        _report_kpi_card("🛠️", f"{total_faults_count_r:,.0f}", "عدد الأعطال المسجلة", "#2563eb"),
                        _report_kpi_card("📊", f"{avg_fault_duration_r:,.0f} د", "متوسط مدة العطل", "#e0a92e"),
                        _report_kpi_card("⏳", f"{(total_downtime_r / 60):,.1f} س", "إجمالي التوقف بالساعات", "#7c3aed"),
                        _report_kpi_card(
                            ...
                            if worst_line_fault_r is not None else "-",
                            "#16a34a"
                        ),
                    ]) + "</div>"
                )
                ...
                faults_extra_html = _render_bar_pie(
                    bar_labels=[ar_text(n) for n in faults_line_summary_r["الخط"]],
                    bar_values=faults_line_summary_r["إجمالي دقائق التوقف"],
                    bar_title="📊 إجمالي دقائق التوقف لكل خط",
                    bar_color="#dc2626",
                    pie_labels=[ar_text(l) for l in top_causes_r.index] if not top_causes_r.empty else [],
                    pie_values=top_causes_r.values if not top_causes_r.empty else [],
                    pie_title="🥧 أكتر أسباب الأعطال (دقائق)"
                )

                faults_html_r = build_printable_html(
                    f"تقرير الأعطال الشهري — {month_label_r}",
                    f"إجمالي دقائق التوقف {total_downtime_r:,.0f} دقيقة ({total_faults_count_r:,.0f} عطل) — "
                    f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                    faults_line_summary_r, landscape=False, base_font_size=13, bold=True, row_padding="8px 6px",
                    top_html=faults_top_html, extra_html=faults_extra_html
                )
                st.download_button(
                    "🖨️ تحميل تقرير الأعطال الشهري", data=faults_html_r.encode("utf-8"),
                    file_name=f"faults_report_{selected_month_r}.html", mime="text/html",
                    key="dl_faults_report"
                )

                report_parts["faults"] = {
                    "title": f"⚠️ تقرير الأعطال الشهري — {month_label_r}",
                    "subtitle": f"إجمالي دقائق التوقف {total_downtime_r:,.0f} دقيقة ({total_faults_count_r:,.0f} عطل)",
                    "df": faults_line_summary_r, "top_html": faults_top_html, "extra_html": faults_extra_html
                }

    # ==========================
    # 🗂️ البورتفوليو — تجميع التقارير المختارة في ملف واحد
    # ==========================
    st.divider()
    st.markdown("### 🗂️ البورتفوليو — تجميع كل التقارير في ملف واحد")

    if not report_parts:
        st.info("مفيش تقارير اتولدت لسه — لازم يكون فيه بيانات مسجلة للشهر المختار عشان تقدر تجمعها في بورتفوليو")
    else:
        labels_map_r = {
            "packing": "📦 تقرير الباكينج (بالوزن)",
            "production": "🏭 تقرير البرودكشن",
            "breakage": "🔨 تقرير الكسر",
            "faults": "⚠️ تقرير الأعطال",
        }
        available_keys_r = [k for k in ["packing", "production", "breakage", "faults"] if k in report_parts]
        chosen_keys_r = st.multiselect(
            "اختار التقارير اللي عايز تجمعها في البورتفوليو",
        ...
        if chosen_keys_r:
            ...
            portfolio_html_r = build_multi_report_html(
                f"البورتفوليو الشهري الشامل — {month_label_r}",
                f"يضم: {' + '.join(labels_map_r[k] for k in chosen_keys_r)} — "
                f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                portfolio_sections_r
            )
            st.download_button(
                "🖨️ تحميل البورتفوليو الكامل (PDF)",
                data=portfolio_html_r.encode("utf-8"),
                file_name=f"monthly_portfolio_{selected_month_r}.html",
                mime="text/html",
                help="ملف واحد فيه كل التقارير اللي اخترتها، كل تقرير في صفحة لوحده — "
                     "افتحه ودوس Ctrl+P واختار 'Save as PDF'",
                key="dl_portfolio"
            )
        else:
