...
def build_line_production_chart(line_summary_df, line_col="الخط",
    ...
    """رسم بياني احترافي (أعمدة مزدوجة) يقارن الفعلي بالمستهدف لكل خط إنتاج"""
    ...
    bars_t = ax.bar(x - width / 2, line_summary_df[target_col], width,
                     label=ar_text("المستهدف"), color="#93c5fd", edgecolor="#2563eb", linewidth=0.6)
    bars_a = ax.bar(x + width / 2, line_summary_df[actual_col], width,
                     label=ar_text("الفعلي"), color="#2563eb", edgecolor="#1e3a8a", linewidth=0.6)
        ...
        for b in bars:
            ...
            if h > 0:
                ax.annotate(f"{h:,.0f}", xy=(b.get_x() + b.get_width() / 2, h),
                            ...
                            ha="center", va="bottom", fontsize=7.5, color="#1a1d21")

    ax.set_title(ar_text(chart_title), fontsize=12, fontweight="bold", color="#0b1f3a")
    ax.set_xticks(x)
...
if page == "Dashboard":
    ...
    with gc2:
        ...
        m4.metric("🥇 نسبة تحقيق الإنتاج", f"{prod_achv_dash:.1f}%")
    ...
    with st.expander("📊 تقرير تعبئة الشيفتات + إنتاج الخطوط لنفس اليوم"):
        ...
        if day_shift_log.empty:
            ...
            grand_total_kg = 0.0
            all_shifts_pack_rows = []  # هنجمع فيها كل شيفت عشان نستخدمها في التقرير القابل للطباعة
            ...
            for sh in SHIFT_OPTIONS_P:
                ...
                sh_data_for_print = sh_data.copy()
                sh_data_for_print.insert(0, "الشيفت", sh)
                all_shifts_pack_rows.append(sh_data_for_print)
            ...
            all_shifts_pack_table = (
                pd.concat(all_shifts_pack_rows, ignore_index=True)
                if all_shifts_pack_rows else
                pd.DataFrame(columns=["الشيفت", "الصنف", "الكمية (كرتونة)", "الوزن (كيلو)"])
            )

            # إنتاج الخطوط لكل شيفت لنفس اليوم (من صفحة Production)
            st.markdown(f"##### 🏭 إنتاج خطوط الإنتاج لكل شيفت — {report_shift_date_str}")
            prod_shift_table = pd.DataFrame(columns=["الخط", "الشيفت", "المستهدف (KG)", "الفعلي (KG)"])
            if GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
                ...
                if "Date" not in hist_df_shift.columns or hist_df_shift.empty:
                    ...
                    if day_shift_prod.empty:
                        st.info("مفيش بيانات إنتاج مسجلة بالشيفت لنفس اليوم")
                    else:
                        for nc in ["المستهدف (KG)", "الفعلي (KG)"]:
                        ...
                        prod_shift_table = day_shift_prod.groupby(["Line", "Shift"])[["المستهدف (KG)", "الفعلي (KG)"]].sum().reset_index()
                        prod_shift_table.columns = ["الخط", "الشيفت", "المستهدف (KG)", "الفعلي (KG)"]
                        st.dataframe(prod_shift_table, use_container_width=True, hide_index=True)
            else:
                st.info("مفيش بيانات إنتاج متاحة (تأكد من تسجيل الإنتاج بالشيفت في صفحة Production)")

            # ---------- تقرير قابل للطباعة يجمع تعبئة الشيفتات + إنتاج الخطوط بالشيفت + رسم بياني ----------
            st.divider()
            st.markdown("##### 🖨️ تحميل تقرير الشيفتات القابل للطباعة")

            shift_chart_html = ""
            if not prod_shift_table.empty and prod_shift_table[["المستهدف (KG)", "الفعلي (KG)"]].sum().sum() > 0:
                line_totals_for_chart = prod_shift_table.groupby("الخط", sort=False)[
                    ["المستهدف (KG)", "الفعلي (KG)"]
                ].sum().reset_index()
                fig_shift_chart = build_line_production_chart(
                    line_totals_for_chart, line_col="الخط",
                    chart_title=f"إنتاج الخطوط بالشيفت — {report_shift_date_str}"
                )
                if fig_shift_chart is not None:
                    shift_chart_html = (
                        f'<h2>📊 إنتاج الخطوط — الفعلي مقابل المستهدف</h2>'
                        f'{fig_to_base64_img(fig_shift_chart)}'
                    )

            shift_top_html = (
                f'<div style="margin-bottom:8px; font-weight:700;">'
                f'📦 إجمالي كل الشيفتات: {grand_total_cartons:,.0f} كرتونة'
                f' — {grand_total_kg:,.1f} كجم</div>'
            )

            shift_printable_html = build_printable_html(
                f"تقرير الشيفتات — تعبئة وإنتاج الخطوط",
                f"يوم {report_shift_date_str} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                all_shifts_pack_table,
                landscape=True,
                top_html=shift_top_html,
                extra_tables=[
                    ("🏭 إنتاج الخطوط لكل شيفت", prod_shift_table),
                ],
                extra_html=shift_chart_html or None,
            )
            st.download_button(
    ...
    if summary.empty:
        ...
        if GSHEETS_ENABLED and gsheet_exists(PRODUCTION_SHEET):
            ...
            if not day_hist.empty:
                ...
                prod_line_table = day_hist.groupby("Line")[["المستهدف (KG)", "الفعلي (KG)"]].sum().reset_index()
        ...
        if pm_single_page:
            ...
            single_page_height_mm = 95 + (main_rows * 9) + (cat_rows * 6) + (low_rows * 7) + 25
