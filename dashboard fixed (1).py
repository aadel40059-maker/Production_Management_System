...
if page == "Dashboard":
    ...
    line_summary_day["الخط"] = pd.Categorical(line_summary_day["الخط"], categories=fault_lines, ordered=True)
    line_summary_day = line_summary_day.sort_values("الخط").reset_index(drop=True)
    ...
    ax_daily.bar(
        line_summary_day["الخط"].astype(str),
        line_summary_day["إجمالي التوقف (دقيقة)"],
        color="#2f6fed"
    )
