    ...
    if admin_ok:
        ...
        def _build_factory_detail(f_name, f_lines):
            if report_month_str == selected_month_f and f_lines == fault_lines:
                ...
                if f_grid is None:
                    st.error(
                        f"⚠️ تعذّر الاتصال بجوجل شيت وإحنا بنجيب بيانات {f_name} للتقرير (زحمة مؤقتة على الـ API). "
                        "بياناتك آمنة ومتأثرتش، بس التقرير معرفش يكمل دلوقتي. جرب تاني بعد شوية."
                    )
                    st.stop()
            for dc in f_day_cols:
            ...
            # ---- تكلفة الصيانة حسب الخط (من أول الشهر حتى اليوم، تراكمي) — بس الأعطال المعتمدة ----
