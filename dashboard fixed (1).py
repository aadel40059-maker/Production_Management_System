...
# ---------- اسم البرنامج والمطور — بيظهر في تقارير الطباعة (PDF) وملفات الإكسيل ----------
# غيّر القيمتين دول باسم البرنامج واسم المطور الحقيقيين وهما هيظهروا في كل التقارير تلقائي
PROGRAM_NAME = "PRO TRACK"
DEVELOPER_NAME = "Ahmed Adel"


def report_signature_html():
    """شريط بيظهر تحت عنوان أي تقرير طباعة (HTML/PDF) فيه اسم البرنامج والمطور"""
    return f"""
    <div style="text-align:center; font-size:11px; color:#555; margin:2px 0 10px;">
        {PROGRAM_NAME} — تطوير: {DEVELOPER_NAME}
    </div>
    """


# ============================================================
# محرك موحّد لمطابقة أسماء الأصناف بين البرنامج وملفات الإكسيل — مش مطابقة نص حرفي،
# ده بيفهم "تركيبة" الصنف (تعبئة + عدد + وحدة) بغض النظر عن اختلاف الصياغة/الاختصارات
# ============================================================
_ARABIC_DIACRITICS = re.compile(r"[\u064B-\u065F\u0670\u06D6-\u06ED]")


def normalize_arabic_text(v):
    """تنضيف عام للنص: شيل التشكيل، تحويل الأرقام العربي هندي لإنجليزي، توحيد
    الألف/الياء/التاء المربوطة، وتحويل علامات الضرب/الفواصل والأقواس لمسافات،
    وفصل أي رقم ملزّق بحرف (زي '12علبه' أو '240قطعه')"""
    if v is None:
        return ""
    s = str(v).strip()
    ...
    s = s.replace("ى", "ي")
    s = s.replace("ة", "ه")   # توحيد التاء المربوطة والهاء (علبة=علبه، قطعة=قطعه...)
    # كل علامات الضرب/الفاصل الممكنة (بما فيها التطويل العربي ـ ورموز الضرب المختلفة) لمسافة
    s = re.sub(r"[×xX✕✖\*\-–—_/\\(),.:؛,ـ]", " ", s)
    s = re.sub(r"(\d)([^\d\s])", r"\1 \2", s)   # "39ك"   -> "39 ك"
    s = re.sub(r"([^\d\s])(\d)", r"\1 \2", s)   # "ك39"   -> "ك 39"
    s = re.sub(r"\s+", " ", s).strip()
    return s


def normalize_numbers(v):
    """تحويل الأرقام العربي هندي لإنجليزي بس"""
    return str(v).translate(str.maketrans("٠١٢٣٤٥٦٧٨٩", "0123456789"))


# قاموس توحيد وحدات التعبئة: أي اختصار أو صيغة بيتحول لرمز إنجليزي موحّد.
# المفاتيح بالتاء المربوطة للوضوح، وبيتم توحيدها تلقائيًا مع نص متطبّع بالهاء (شوف تحت)
UNIT_ALIASES = {
    "كيس": "BAG", "اكياس": "BAG", "كياس": "BAG", "ك": "BAG",
    "علبة": "BOX", "علب": "BOX", "عب": "BOX", "ع": "BOX",
    "قطعة": "PIECE", "قطع": "PIECE", "ق": "PIECE",
    "كرتونة": "CARTON", "كرتون": "CARTON", "كراتين": "CARTON", "كرت": "CARTON",
    "كيلو": "KG", "كجم": "KG", "كج": "KG",
    "جرام": "GRAM", "جم": "GRAM", "جرامات": "GRAM",
}
# نسخة موحّدة من القاموس (كل المفاتيح بالهاء) عشان تتطابق مع النص بعد normalize_arabic_text
UNIT_ALIASES_NORM = {k.replace("ة", "ه"): v for k, v in UNIT_ALIASES.items()}

# كلمات براند/تصنيف اختيارية بتتحط في اسم الصنف في مكان (البرنامج أو الإكسيل) ومش
# لازم تكون موجودة في التاني — القايمة دي قابلة للتوسعة لأي صنف/براند جديد يتضاف بعدين
BRAND_WORDS = {"سولا", "ويفر", "طوفي", "اكلير", "ايطالي", "كونت", "ترانسلاب"}

_PACKAGING_UNIT_PATTERN = re.compile(
    r"(\d+(?:\.\d+)?)\s*(" + "|".join(sorted(UNIT_ALIASES_NORM, key=len, reverse=True)) + r")\b"
)


def extract_packaging_structure(normalized_text):
    """بتستخرج تركيبة التعبئة من نص متنضّف بالفعل (أرقام + وحداتها بالترتيب)
    زي [(39,'BAG'), (15,'PIECE')] — العلاقة بين كل رقم ووحدته بتتحفظ"""
    pairs = []
    for num_str, unit_raw in _PACKAGING_UNIT_PATTERN.findall(normalized_text):
        unit = UNIT_ALIASES_NORM.get(unit_raw, unit_raw)
        num = float(num_str)
        num_disp = int(num) if num == int(num) else num
        pairs.append((num_disp, unit))
    return pairs


def normalize_product_name(v):
    """بتحلل اسم الصنف بالكامل وترجع (الاسم الحقيقي بعد شيل التعبئة والبراند،
    تركيبة التعبئة) — الاتنين مع بعض هما اللي بيحددوا هوية الصنف"""
    text = normalize_arabic_text(v)
    packaging = extract_packaging_structure(text)
    remaining = _PACKAGING_UNIT_PATTERN.sub(" ", text)
    words = [w for w in remaining.split() if w not in BRAND_WORDS]
    identifier = " ".join(sorted(words))
    return identifier, tuple(packaging)


def generate_product_signature(v):
    """توقيع نصي واحد للصنف (الاسم الحقيقي + تركيبة التعبئة) يستخدم كمفتاح مطابقة
    بدل مقارنة النص الحرفي — بيتجاهل فروق الصياغة والاختصارات وكلمات البراند الاختيارية"""
    identifier, packaging = normalize_product_name(v)
    packaging_str = "|".join(f"{n}{u}" for n, u in packaging)
    return f"{identifier}::{packaging_str}"


def match_excel_product_to_application_product(excel_name, app_signature_to_name):
    """بتدي اسم صنف من الإكسيل وdict {توقيع الصنف: اسم الصنف في البرنامج}،
    وترجع (اسم الصنف المطابق أو None، درجة الثقة)"""
    sig = generate_product_signature(excel_name)
    if sig in app_signature_to_name:
        return app_signature_to_name[sig], 100
    return None, 0


def debug_product_signature(v):
    """وضع تشخيص: بيوري خطوة بخطوة إزاي البرنامج فهم اسم الصنف ده — مفيد لمعرفة
    سبب عدم تطابق أي صنف. بيرجع dict فيه: النص الأصلي، بعد التطبيع، التعبئة المكتشفة،
    الاسم الحقيقي بعد شيل التعبئة والبراند، والتوقيع النهائي"""
    text = normalize_arabic_text(v)
    packaging = extract_packaging_structure(text)
    identifier, _ = normalize_product_name(v)
    return {
        "الأصل": v,
        "بعد التطبيع": text,
        "التعبئة المكتشفة": ", ".join(f"{n} {u}" for n, u in packaging) or "—",
        "الاسم الحقيقي (بدون تعبئة/براند)": identifier or "—",
        "التوقيع النهائي": generate_product_signature(v),
    }


# ---------- قراءة ملف "رصيد الفروع والتوكيلات" (استوك المخزن) وتجميعه لكل صنف ----------
def _normalize_item_name(v):
    """مفتاح مطابقة اسم الصنف (بيستخدم توقيع التعبئة بدل مطابقة النص الحرفي —
    شوف generate_product_signature لتفاصيل التطبيع)"""
    return generate_product_signature(v)


# صفوف الإجمالي/المجموع الفرعي جوه شيتات المخزون (زي "الاجمالي بالكرتونه" أو "الاجمالى العام")
# لازم تتجاهل تمامًا وميتحسبش كصنف، وإلا الأرقام تطلع غلط
TOTAL_ROW_MARKERS = ("جمالي", "جمالى", "مجموع")


def _is_total_row(item_name):
    normalized = item_name.replace("أ", "ا").replace("إ", "ا").replace("آ", "ا")
    return any(m in normalized for m in TOTAL_ROW_MARKERS)


def _normalize_number(v):
    """بتحوّل أي قيمة كميه جايه من إكسيل (نص فيه فاصلة آلاف، نقطة، أرقام عربي هندي، فراغات،
    فاضي، نص زي 'nan') لرقم عشري صحيح، وترجع صفر لو مش قابلة للتحويل خالص"""
    if v is None:
        return 0.0
...
ITEM_NAME_HEADERS = ("اسم الصنف", "إسم الصنف", "الصنف")
TOTAL_COL_HEADERS = ("الاجمالي", "الإجمالي")
BARCODE_COL_HEADERS = ("باركود", "الباركود", "Barcode", "barcode", "BARCODE")


def _normalize_barcode(v):
    """بتنضّف قيمة الباركود من غير ما تحوّلها لرقم (عشان الأصفار الأولى في الأول متضيعش):
    شيل المسافات، شيل '.0' الزيادة لو اتقرا غلط كرقم عشري، ورجّع فاضي لو مافيش باركود"""
    if v is None:
        return ""
    s = str(v).strip()
...
def _extract_stock_rows(uploaded_file):
    """بتفتح ملف إكسيل رصيد الفروع/التوكيلات وترجع صفوف خام (اسم الصنف + باركود لو موجود
    + الإجمالي) من غير أي تجميع أو مطابقة — الأساس المشترك اللي بيتبنى عليه أي عرض/مطابقة
    تانية (تجميع بالتوقيع، أو مطابقة بالباركود، إلخ) عشان منكررش نفس منطق القراءة.
    نفس منطق 'شيت فيه عمود الاجمالي جاهز = المصدر المعتمد' من قبل، زائد قراءة الباركود كنص
    صريح (dtype=str) عشان الأصفار الأولى في الباركود متضيعش."""
    try:
    ...
    for sheet_name in xls.sheet_names:
        try:
            sheets_raw[sheet_name] = pd.read_excel(xls, sheet_name=sheet_name, header=None)
        except Exception:
            continue

    def _find_header(raw):
        """بتدور على صف فيه عمود اسم الصنف (في أي عمود)، وترجع (رقم الصف، عمود اسم الصنف،
        عمود الإجمالي الجاهز لو موجود، عمود الباركود لو موجود)"""
        max_scan_rows = min(10, len(raw))
        for r in range(max_scan_rows):
            item_col, total_col, barcode_col = None, None, None
            for c in range(raw.shape[1]):
                cell_val = str(raw.iat[r, c]).strip()
                if cell_val in ITEM_NAME_HEADERS and item_col is None:
                    item_col = c
                if cell_val in TOTAL_COL_HEADERS and total_col is None:
                    total_col = c
                if cell_val in BARCODE_COL_HEADERS and barcode_col is None:
                    barcode_col = c
            if item_col is not None:
                return r, item_col, total_col, barcode_col
        return None, None, None, None

    rows = []

    # ---- الخطوة 1: هل فيه شيت فيه عمود "الاجمالي" جاهز؟ لو آه، ده المصدر الوحيد المعتمد ----
    for sheet_name, raw in sheets_raw.items():
        header_row_idx, item_col_idx, total_col_idx, barcode_col_idx = _find_header(raw)
        if header_row_idx is None or total_col_idx is None:
            continue

        raw_str = None
        if barcode_col_idx is not None:
            try:
                raw_str = pd.read_excel(xls, sheet_name=sheet_name, header=None, dtype=str)
            except Exception:
                raw_str = None

        data = raw.iloc[header_row_idx + 1:].reset_index(drop=True)
        data_str = raw_str.iloc[header_row_idx + 1:].reset_index(drop=True) if raw_str is not None else None
        for i in range(len(data)):
            item_name = str(data.iat[i, item_col_idx]).strip()
            if item_name == "" or item_name.lower() == "nan" or _is_total_row(item_name):
                continue
            total_val = _normalize_number(data.iat[i, total_col_idx])
            barcode = ""
            if barcode_col_idx is not None and data_str is not None and i < len(data_str):
                barcode = _normalize_barcode(data_str.iat[i, barcode_col_idx])
            rows.append({"اسم الصنف": item_name, "باركود": barcode, "الإجمالي": total_val, "الشيت": sheet_name})

        return rows, [sheet_name], [s for s in sheets_raw if s != sheet_name]

    # ---- الخطوة 2: مفيش أي شيت فيه عمود إجمالي جاهز، نرجع نجمع أعمدة الفروع بنفسنا ----
    matched_sheets, skipped_sheets = [], []
    for sheet_name, raw in sheets_raw.items():
        header_row_idx, item_col_idx, _, barcode_col_idx = _find_header(raw)
        # هنا بس بنشترط إن اسم الصنف يكون في أول عمود، عشان نضمن إن الشيت جدول بسيط
        # (صنف + أعمدة كميات فروع)، مش شيت فيه أعمدة زي السعر ممكن تتلخبط لو جمعناها
        if header_row_idx is None or item_col_idx != 0:
            skipped_sheets.append(sheet_name)
            continue

        matched_sheets.append(sheet_name)
        data = raw.iloc[header_row_idx + 1:].reset_index(drop=True)
        qty_col_idxs = [c for c in range(raw.shape[1]) if c != item_col_idx and c != barcode_col_idx]

        for _, row in data.iterrows():
            item_name = str(row.iloc[item_col_idx]).strip()
            if item_name == "" or item_name.lower() == "nan" or _is_total_row(item_name):
                continue
            row_total = sum(_normalize_number(row.iloc[c]) for c in qty_col_idxs)
            barcode = _normalize_barcode(row.iloc[barcode_col_idx]) if barcode_col_idx is not None else ""
            rows.append({"اسم الصنف": item_name, "باركود": barcode, "الإجمالي": row_total, "الشيت": sheet_name})

    return rows, matched_sheets, skipped_sheets


def parse_stock_balance_file(uploaded_file):
    """نسخة مبسّطة (بدون باركود) فوق _extract_stock_rows — بترجع (dict التوقيع -> الإجمالي،
    أسماء الشيتات اللي اتقرت، أسماء الشيتات اللي اتجاهلت، dict التوقيع -> أول اسم أصلي)
    محتفظ بيها لأي كود قديم بيستخدمها؛ للمطابقة بالباركود استخدم match_stock_against_report"""
    rows, matched_sheets, skipped_sheets = _extract_stock_rows(uploaded_file)
    item_totals, item_names = {}, {}
    for row in rows:
        key = _normalize_item_name(row["اسم الصنف"])
        item_totals[key] = item_totals.get(key, 0.0) + row["الإجمالي"]
        item_names.setdefault(key, row["اسم الصنف"])
    return item_totals, matched_sheets, skipped_sheets, item_names


# ============================================================
# نظام "الباركود الرئيسي" — ربط دائم بين الباركود واسم الصنف في البرنامج، محفوظ في جوجل
# شيت عشان يعيش بعد إعادة تشغيل البرنامج. أول مرة: مطابقة بالاسم (أو يدوي) وحفظ الباركود.
# المرات الجاية: الباركود نفسه يبقى المفتاح الأساسي، حتى لو الاسم في الإكسيل اتغيّر تمامًا
# ============================================================
BARCODE_MASTER_SHEET = "barcode_master_mapping"
BARCODE_MASTER_COLUMNS = [
    "الباركود", "توقيع الصنف", "اسم الصنف في البرنامج",
    "تاريخ الإنشاء", "تاريخ آخر تحديث", "المصدر", "الحالة",
]


def load_barcode_master():
    """بيقرا جدول الباركود الرئيسي من جوجل شيت — بيرجع جدول فاضي بالأعمدة الصحيحة لو
    الشيت مش موجود لسه أو ناقص أعمدة أو حصل أي خطأ اتصال، عشان باقي الكود يتعامل معاه
    بشكل موحّد وآمن من غير ما يكراش لو جوجل شيت مؤقتًا مش مستجيب"""
    try:
        if GSHEETS_ENABLED and gsheet_exists(BARCODE_MASTER_SHEET):
            df = read_gsheet(BARCODE_MASTER_SHEET)
            if "الباركود" in df.columns and "توقيع الصنف" in df.columns:
        ...
        pass
    return pd.DataFrame(columns=BARCODE_MASTER_COLUMNS)


def _append_barcode_mappings(new_rows):
    """بتضيف صفوف ربط باركود جديدة لجوجل شيت من غير إعادة كتابة الجدول كله.
    بترجع True لو نجحت، وFalse لو فشل الاتصال بجوجل شيت (مؤقتًا أو غيره) — عشان أي
    مشكلة شبكة/كوتة ماتكراشش الصفحة كلها، وتظهر رسالة واضحة للمستخدم بدل كراش خام"""
    if not GSHEETS_ENABLED or not new_rows:
        return False
    df_new = pd.DataFrame(new_rows, columns=BARCODE_MASTER_COLUMNS)
    try:
        if gsheet_exists(BARCODE_MASTER_SHEET):
            return append_gsheet(BARCODE_MASTER_SHEET, df_new)
        else:
            return write_gsheet(BARCODE_MASTER_SHEET, df_new)
    except Exception:
        return False


def save_barcode_mapping_manual(barcode, app_product_name):
    """حفظ ربط باركود يدوي (من شاشة 'محتاج مطابقة يدوية') — بترفض تكتب فوق ربط موجود
    بالفعل لنفس الباركود مربوط بصنف مختلف (تعارض باركود) من غير تأكيد صريح من المستخدم"""
    barcode = _normalize_barcode(barcode)
    if not barcode:
        return False, "لا يوجد باركود لحفظه لهذا الصف"
    master = load_barcode_master()
    sig = generate_product_signature(app_product_name)
    existing = master[master["الباركود"] == barcode]
    if not existing.empty:
        active_existing = existing[existing.get("الحالة", "نشط") == "نشط"]
        if not active_existing.empty and active_existing.iloc[0]["توقيع الصنف"] != sig:
            return False, f"⚠ تعارض باركود: الباركود ده متربط بالفعل بـ«{active_existing.iloc[0]['اسم الصنف في البرنامج']}»"
    today = pd.Timestamp.today().strftime("%Y-%m-%d")
    saved = _append_barcode_mappings([{
        "الباركود": barcode, "توقيع الصنف": sig, "اسم الصنف في البرنامج": app_product_name,
        "تاريخ الإنشاء": today, "تاريخ آخر تحديث": today, "المصدر": "يدوي", "الحالة": "نشط",
    }])
    if not saved:
        return False, "⚠ تعذّر الاتصال بجوجل شيت مؤقتًا — جرب تاني بعد كام ثانية"
    return True, "تم حفظ الربط بنجاح"


def match_stock_against_report(uploaded_file, app_item_names):
    """محرك المطابقة الكامل حسب الأولوية: 1) الباركود لو معروف من قبل، 2) مطابقة الاسم
    الذكية، وأي مطابقة بالاسم معاها باركود جديد بتتحفظ تلقائي (مصدر: تلقائي) عشان
    المرات الجاية تتطابق بالباركود على طول من غير أي تحليل اسم. بيرجع:
    (dict اسم الصنف في البرنامج -> الإجمالي، dict ملخص أرقام الاستيراد،
    لسته الصفوف اللي محتاجة مطابقة يدوية، أسماء الشيتات اللي اتقرت/اتجاهلت)"""
    rows, matched_sheets, skipped_sheets = _extract_stock_rows(uploaded_file)

    barcode_master = load_barcode_master()
    if not barcode_master.empty and "الحالة" in barcode_master.columns:
        active_master = barcode_master[barcode_master["الحالة"] == "نشط"]
    else:
        active_master = barcode_master
    barcode_to_sig = dict(zip(active_master["الباركود"], active_master["توقيع الصنف"])) if not active_master.empty else {}
    app_sig_to_name = {generate_product_signature(n): n for n in app_item_names}

    totals_by_app_name = {}
    unmatched_rows = []
    new_mapping_rows = []
    report = {
        "إجمالي الصفوف": 0, "تطابق بالباركود": 0, "تطابق بالاسم": 0,
        "باركود جديد اتحفظ": 0, "محتاج مطابقة يدوية": 0, "بدون باركود": 0, "تعارض باركود": 0,
    }

    for row in rows:
        report["إجمالي الصفوف"] += 1
        name, barcode, total = row["اسم الصنف"], row["باركود"], row["الإجمالي"]
        sig = generate_product_signature(name)
        matched_name, method = None, None

        if barcode and barcode in barcode_to_sig:
            matched_name = app_sig_to_name.get(barcode_to_sig[barcode])
            if matched_name:
                method = "باركود"
                report["تطابق بالباركود"] += 1

        if matched_name is None and sig in app_sig_to_name:
            matched_name = app_sig_to_name[sig]
            method = "اسم الصنف"
            report["تطابق بالاسم"] += 1
            if barcode:
                if barcode in barcode_to_sig and barcode_to_sig[barcode] != sig:
                    report["تعارض باركود"] += 1  # الباركود ده متربط بصنف تاني بالفعل — ماتلمسوش تلقائي
                else:
                    today = pd.Timestamp.today().strftime("%Y-%m-%d")
                    new_mapping_rows.append({
                        "الباركود": barcode, "توقيع الصنف": sig, "اسم الصنف في البرنامج": matched_name,
                        "تاريخ الإنشاء": today, "تاريخ آخر تحديث": today, "المصدر": "تلقائي", "الحالة": "نشط",
                    })
                    barcode_to_sig[barcode] = sig  # عشان صفوف تانية بنفس الباركود في نفس الملف تستفيد فورًا
                    report["باركود جديد اتحفظ"] += 1
            else:
                report["بدون باركود"] += 1

        if matched_name:
            totals_by_app_name[matched_name] = totals_by_app_name.get(matched_name, 0.0) + total
        else:
            report["محتاج مطابقة يدوية"] += 1
            unmatched_rows.append(row)

    if new_mapping_rows:
        if not _append_barcode_mappings(new_mapping_rows):
            # فشل حفظ الباركودات الجديدة (مؤقتًا) — التقرير الحالي سليم برضو لأن المطابقة
            # والإجماليات لحظة الاستيراد ده صحيحة، بس الباركودات الجديدة مش هتتفتكر للمرة الجاية
            report["باركود جديد اتحفظ"] = 0
            report["تعذّر حفظ باركود جديد"] = len(new_mapping_rows)

    return totals_by_app_name, report, unmatched_rows, matched_sheets, skipped_sheets
...
def gsheet_exists(sheet_name):
    ...
    النتيجة متخزنة لمدة قصيرة (كاش) عشان الصفحة مش تعمل نداء حي لجوجل شيت من جديد
    مع كل حركة/كليك في أي حتة تانية بالصفحة — ده كان بيبطّئ التحميل وبيستهلك الكوتة"""
    if not GSHEETS_ENABLED:
        return False
    return _gsheet_exists_cached(sheet_name)


@st.cache_data(ttl=40, show_spinner=False)
def _gsheet_exists_cached(sheet_name):
    try:
...
def _clear_gsheet_caches():
    _read_gsheet_cached.clear()
    _gsheet_exists_cached.clear()


def write_gsheet(sheet_name, df):
    """بتكتب جدول كامل على جوجل شيت. لو فشل الاتصال، بترمي الاستثناء زي ما كانت
    (كتير من صفحات البرنامج عندها try/except خاص بيها حوالين النداء ده برسائل مخصصة
    و st.stop() — فمينفعش نبلع الخطأ هنا وإلا هي مش هتعرف إن الحفظ فشل)"""
    ws = _get_worksheet(sheet_name)
    header = [str(c) for c in df.columns]
    if df.empty:
        _gspread_retry(ws.clear)
        _gspread_retry(ws.update, [header])
        _clear_gsheet_caches()
        return True
...
def append_gsheet(sheet_name, df_rows):
    """بتضيف صفوف لجوجل شيت من غير إعادة كتابة الجدول كله. بترمي الاستثناء لو فشل
    الاتصال (زي write_gsheet بالظبط) عشان نفس السبب"""
    ws = _get_worksheet(sheet_name)
...
.kpi-card {
    background: linear-gradient(180deg, var(--card-bg) 0%, rgba(1,62,55,0.025) 100%);
    border: 1px solid rgba(1,62,55,0.12);
    border-radius: 16px; padding: 18px 20px;
    box-shadow: 0 6px 16px rgba(1,62,55,0.09);
...
.kpi-card .kpi-label { font-size: 12.5px; font-weight: 700; color: #6b7280; margin-bottom: 5px; letter-spacing: 0.1px; }
.kpi-card .kpi-value { font-size: 24px; font-weight: 900; color: var(--accent); line-height: 1.3; }
...
if "production_lines_m2" not in st.session_state:
    st.session_state.production_lines_m2 = [
        "ويفر ١",
        "ويفر ٢",
        "ويفر ٣",
    ]
...
def build_breakage_report_html(totals, line_rows, trend_dates, trend_out_pct, trend_in_pct,
                                ...
                                page_height_mm=None, factory_breakdown=None):
    ...
    factory_breakdown (اختياري): dict {مفتاح الكرت: نص HTML خفيف} بيتحط تحت كرت الـ KPI بتاعه — مستخدم بس
    في التقرير المجمّع لعرض تفصيل كل مصنع لوحده تحت كل كرت، من غير ما يأثر على التقرير العادي لمصنع واحد."""
    palette = ["#0d6b5c", "#e0a92e", "#4c8c7d", "#a06f0f", "#7fb5a6", "#2563eb", "#c98f1f", "#7c3aed"]
    fb = factory_breakdown or {}

    def _fb_line(key):
        return (
            f'<div class="kpi-sub" style="font-size:9px; color:#9ca3af; margin-top:4px;">{fb[key]}</div>'
            if key in fb else ""
        )
    ...
    waste_vals = [float(r.get("waste", 0)) for r in line_rows]

    def _pct_pill(pct_value, has_prod):
        """بيرجع HTML لـ pill ملوّن للنسبة — لو مفيش إنتاج مربوط للخط ده بيوضح '—' بدل ما يوهم إن النسبة صفر."""
        if not has_prod:
            return '<span class="pct-pill" style="background:#f3f4f6; color:#9ca3af;" title="لا يوجد إنتاج مربوط بهذا الخط">—</span>'
        pct_color = "#16a34a" if pct_value <= 5 else ("#0d6b5c" if pct_value <= 10 else
                     ("#e0a92e" if pct_value <= 15 else ("#ea580c" if pct_value <= 20 else "#dc2626")))
        return f'<span class="pct-pill" style="background:{pct_color}1f; color:{pct_color};">{pct_value:.2f}%</span>'

    # ---------- جدول الخطوط: إنتاج اليوم → رصيد أول المدة → الكسر الخارج → إجمالي الكسر → الكسر الداخل → الهالك → متبقي الرصيد → نسبة خارج/داخل لوحدهم ----------
    table_rows_html = ""
    tot_open = tot_out = tot_in = tot_waste = tot_total = tot_remain = tot_prod_rows = 0
    for i, r in enumerate(line_rows):
        r_waste = r.get("waste", 0)
        r_prod = float(r.get("production", 0))
        has_prod_line = r_prod > 0
        out_pill = _pct_pill(r["pct"], has_prod_line)
        in_pill = _pct_pill(r.get("in_pct", 0), has_prod_line)
        table_rows_html += f"""
        <tr>
            <td class="line-cell"><span class="dot" style="background:{palette[i % len(palette)]}"></span>{r['name']}</td>
            <td style="color:#2563eb; font-weight:800;">{r_prod:,.0f}</td>
            <td>{r['opening']:,.0f}</td>
            <td>{r['out']:,.0f}</td>
            <td style="font-weight:900;">{r['total']:,.0f}</td>
            <td>{r['in']:,.0f}</td>
            <td style="color:#7c2d12;">{r_waste:,.0f}</td>
            <td style="font-weight:900; color:#013e37;">{r['remaining']:,.0f}</td>
            <td>{out_pill}</td>
            <td>{in_pill}</td>
        </tr>
        """
        tot_open += r["opening"]; tot_out += r["out"]; tot_in += r["in"]; tot_waste += r_waste
        tot_total += r["total"]; tot_remain += r["remaining"]; tot_prod_rows += r_prod
    tot_has_prod = tot_prod_rows > 0
    tot_out_pill = _pct_pill((tot_out / tot_prod_rows * 100) if tot_has_prod else 0, tot_has_prod)
    tot_in_pill = _pct_pill((tot_in / tot_prod_rows * 100) if tot_has_prod else 0, tot_has_prod)
    table_rows_html += f"""
    <tr class="total-row">
        <td>الإجمالي</td><td>{tot_prod_rows:,.0f}</td><td>{tot_open:,.0f}</td><td>{tot_out:,.0f}</td>
        <td>{tot_total:,.0f}</td><td>{tot_in:,.0f}</td><td>{tot_waste:,.0f}</td><td>{tot_remain:,.0f}</td>
        <td>{tot_out_pill}</td><td>{tot_in_pill}</td>
    </tr>
    """
    ...
    <div class="kpi" style="border-inline-start:4px solid #7c2d12;">
        <div class="kpi-icon2" style="color:#7c2d12;">🗑️</div>
        <div class="kpi-label">إجمالي الهالك (إعدام)</div>
        <div class="kpi-value">{totals.get('waste', 0):,.0f} KG</div>
        <div class="kpi-sub">{totals.get('waste_pct', 0):.2f}% من الإنتاج</div>
        {_fb_line('waste')}
    </div>
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .line-cell {{ text-align: right; display: flex; align-items: center; gap: 8px; font-weight: 800; }}
        .dot {{ width: 10px; height: 10px; border-radius: 50%; display: inline-block; }}
        .pct-pill {{ padding: 4px 12px; border-radius: 20px; font-weight: 800; font-size: 12px; display: inline-block; min-width: 52px; }}
        tr.total-row td {{ background: #ffefb3; font-weight: 900; border-top: 2px solid #013e37; }}
        .breakage-table th:first-child, .breakage-table td:first-child {{ text-align: right; }}
        .breakage-table tbody tr:not(.total-row):nth-child(odd) td {{ background: rgba(1,62,55,0.025); }}
        .breakage-table tbody tr:not(.total-row):hover td {{ background: rgba(224,169,46,0.10); }}
        .breakage-table th {{ border-bottom: 2px solid #e0a92e; }}
    ...
    <body>
    <div class="app-brand">
        <div class="app-name">🏭 Pro Track</div>
    ...
    <div class="panel">
        <div class="panel-title">📋 جدول الكسر لكل الخطوط (إنتاج اليوم → رصيد أول المدة → متبقي الرصيد)</div>
        <table class="breakage-table">
            <tr>
                <th>اسم الخط</th><th>إنتاج اليوم (KG)</th><th>رصيد أول المدة (KG)</th><th>الكسر الخارج (KG)</th>
                <th>إجمالي الكسر (KG)</th><th>الكسر الداخل (KG)</th><th>الهالك (KG)</th><th>متبقي الرصيد (KG)</th>
                <th>نسبة كسر خارج %</th><th>نسبة كسر داخل %</th>
            </tr>
            {table_rows_html}
        </table>
        <div style="font-size:11.5px; color:#6b7280; font-weight:700; margin-top:8px; text-align:center;">
            إجمالي الكسر = رصيد أول المدة + الكسر الخارج &nbsp;&nbsp;|&nbsp;&nbsp; متبقي الرصيد = إجمالي الكسر − الكسر الداخل &nbsp;&nbsp;|&nbsp;&nbsp; الهالك بيتسجل لوحده كمرجع، ومش بيتخصم من الرصيد
            <br>نسبة كسر خارج % = الكسر الخارج ÷ إنتاج اليوم لنفس الخط &nbsp;&nbsp;|&nbsp;&nbsp; نسبة كسر داخل % = الكسر الداخل ÷ إنتاج اليوم لنفس الخط &nbsp;&nbsp;|&nbsp;&nbsp; "—" تعني مفيش إنتاج مسجّل لهذا الخط في نفس اليوم
        </div>
    </div>
    ...
    new Chart(document.getElementById('lineBar'), {{
        ...
        data: {{ labels: {json.dumps(line_names)}, datasets: [
            ...
            {{ label: 'الهالك', data: {json.dumps(waste_vals)}, backgroundColor: '#7c2d12', borderRadius: 5 }}
        ]}},
        options: {{
            responsive: true, maintainAspectRatio: false,
            plugins: {{ legend: {{ position: 'top', labels: {{ usePointStyle: true }} }} }},
    ...
    new Chart(document.getElementById('lineDonutB'), {{
        type: 'doughnut',
        data: {{ labels: {json.dumps(line_names)}, datasets: [{{
            data: {json.dumps([o + i + w for o, i, w in zip(out_vals, in_vals, waste_vals)])},
...
def kpi_row(cards):
    ...
    for c in cards:
        ...
        extra_html = ""
        if c.get("extra"):
            extra_html = f'<div style="font-size:10.5px; font-weight:400; color:#9ca3af; margin-top:3px;">{c["extra"]}</div>'
        icon_color = c.get("color", "#013e37")
        icon_style = (
            f'background:{icon_color}22; color:{icon_color};'
        )
        card_html = f"""
        <div class="kpi-card" style="flex:1; min-width:180px; border-right: 4px solid {icon_color};">
...
def _df_to_html_highlighted(df, highlight_mask, highlight_color="#fff3b0", row_colors=None, row_top_borders=None):
    ...
    row_top_borders: قائمة اختيارية (نفس عدد الصفوف) لإضافة خط فاصل واضح فوق صف معيّن
    (مفيد للفصل بين مجموعات، زي الفصل بين كل بلد والتاني، من غير ما نغيّر شكل باقي الجدول)."""
    ...
    row_top_borders = list(row_top_borders) if row_top_borders is not None else None
    for i, (_, row) in enumerate(df.iterrows()):
        style_parts = []
        if row_colors is not None and i < len(row_colors) and row_colors[i]:
            style_parts.append(f"background-color:{row_colors[i]};")
        else:
        ...
        if row_top_borders is not None and i < len(row_top_borders) and row_top_borders[i]:
            style_parts.append(f"border-top:{row_top_borders[i]};")
...
def build_printable_html(title, subtitle, df, extra_title=None, extra_df=None, extra_tables=None,
                          ...
                          highlight_color="#fff3b0", row_colors=None, row_top_borders=None, base_font_size=10, compact=False,
    ...
    row_top_borders: قائمة اختيارية لإضافة خط فاصل فوق صفوف معيّنة (للفصل بين مجموعات زي الدول)
    ...
    if highlight_mask is not None or row_colors is not None or row_top_borders is not None:
        table_html = _df_to_html_highlighted(df, highlight_mask, highlight_color, row_colors, row_top_borders)
    else:
...
def build_dashboard_document(overall_title, overall_subtitle, sections):
    ...
    brand_strip_html = f"""
        <div class="brand-strip">
            <div>🏭 Pro Track &nbsp;•&nbsp; Developed by Eng. Ahmed Adel</div>
        ...
        <div class="dash-layout">
            <div class="dash-sidebar">
                <div class="sidebar-logo">
                    ...
                    <div>
                        <div class="sidebar-logo-text">Pro Track</div>
                        <div class="sidebar-logo-sub">Production Tracking</div>
                    </div>
                </div>
            ...
            <div class="dash-main">
                {''.join(blocks)}
                <p class="dev-footer">🏭 Pro Track &nbsp;•&nbsp; Developed by Eng. Ahmed Adel</p>
            </div>
        </div>
        {script_html}
    </body>
...
if page == "الرئيسية":
    ...
    HOME_TILES = [
        ...
        ("Packing", "📦", "تسجيل إنتاج الأصناف وتقارير الشيفتات"),
    ...
    # ---------- تقسيم المصانع: كل مصنع له خطوطه الخاصة وشيتاته الخاصة (بيانات كل مصنع منفصلة عن التانية) ----------
    PRODUCTION_FACTORY_CONFIG = {
        "مصنع 1": {"lines_key": "production_lines", "sheet_suffix": ""},
        "مصنع 2": {"lines_key": "production_lines_m2", "sheet_suffix": "_m2"},
    }
    # نفس كلمة مرور كل مصنع بالظبط زي صفحة الباكينج، عشان كل مصنع له دخول مستقل بنفس الباسورد بتاعه
    PRODUCTION_FACTORY_PASSWORDS = {
        "مصنع 1": "candy1",
        "مصنع 2": "wafer2",
    }
    production_factory_pick = st.radio(
        "🏭 اختار المصنع", list(PRODUCTION_FACTORY_CONFIG.keys()), horizontal=True, key="production_factory_pick"
    )
    prod_f_conf = PRODUCTION_FACTORY_CONFIG[production_factory_pick]
    prod_suffix = prod_f_conf["sheet_suffix"]
    PRODUCTION_SHEET = f"production_history_v2{prod_suffix}"

    prod_unlock_key = f"production_unlocked_{production_factory_pick}"
    if prod_unlock_key not in st.session_state:
        st.session_state[prod_unlock_key] = False
    if not st.session_state[prod_unlock_key]:
        st.subheader(f"🔒 {production_factory_pick} — صفحة محمية")
        pw_input = st.text_input("كلمة مرور المصنع", type="password", key=f"production_page_pw_{production_factory_pick}")
        if st.button("دخول", key=f"production_page_unlock_btn_{production_factory_pick}"):
            if pw_input == PRODUCTION_FACTORY_PASSWORDS[production_factory_pick]:
                st.session_state[prod_unlock_key] = True
                st.rerun()
            else:
    ...
    all_lines_clean_pr = [str(l).replace("🏭", "").strip() for l in st.session_state[prod_f_conf["lines_key"]]]

    # مصنع 2 (الويفر) بيغيّر المنتج على نفس الخط أكتر من مرة في الشهر، فالتارجت مش ثابت زي مصنع 1 —
    # عايزين تارجت لكل يوم لوحده بدل رقم واحد بيتكرر على طول الشهر
    use_daily_target_pr = (production_factory_pick == "مصنع 2")
    ...
    def _empty_production_rows(lines_for_default, p_day_cols):
        ...
        for l in lines_for_default:
            target_fields = (
                {_target_col(dc): 0 for dc in p_day_cols} if use_daily_target_pr
                else {"التارجت اليومي (KG)": 0}
            )
            rows.append({
                "الخط": l, "الشيفت": TOTAL_SHIFT_LABEL,
                **target_fields, **{dc: 0 for dc in p_day_cols}
            })
            for sh in SHIFTS:
                rows.append({
                    "الخط": l, "الشيفت": sh,
    ...
    def load_production_month_grid(month_str, lines_for_default):
        """الجدول الشهري: كل خط وتحته شيفتاته الثلاثة.
        لمصنع 1: عمود تارجت يومي واحد (بيتكتب مرة واحدة وينطبق على كل أيام الشهر تلقائي).
        لمصنع 2: عمود تارجت لكل يوم لوحده (لإن المنتج بيتغيّر على نفس الخط، فالتارجت بيختلف يوم عن يوم).
        وعمود لكل يوم في الشهر للإنتاج الفعلي، في الحالتين.
        ...
        target_cols = [_target_col(dc) for dc in p_day_cols] if use_daily_target_pr else ["التارجت اليومي (KG)"]
        p_cols = ["الخط", "الشيفت"] + target_cols + p_day_cols
        p_path = f"production_monthly_{month_str}{prod_suffix}"
        ...
        if grid_raw.empty:
            ...
            # توافق: لو كان فيه تارجت يومي ثابت قديم واتحول الخط بقى بتارجت يومي منفصل لكل يوم،
            # نستخدم نفس القيمة كبداية لكل الأيام بدل ما تتصفر فجأة
            if use_daily_target_pr and "التارجت اليومي (KG)" in grid.columns and not all(tc in grid.columns for tc in target_cols):
                old_daily = pd.to_numeric(grid["التارجت اليومي (KG)"], errors="coerce").fillna(0)
                for tc in target_cols:
                    if tc not in grid.columns:
                        grid[tc] = old_daily
            for c in p_cols:
        ...
        missing_rows = [
            {"الخط": l, "الشيفت": sh, **{tc: 0 for tc in target_cols}, **{dc: 0 for dc in p_day_cols}}
            for l in lines_for_default
        ...
        for tc in target_cols:
            grid[tc] = pd.to_numeric(grid[tc], errors="coerce").fillna(0)
    ...
    if use_daily_target_pr:
        st.caption(
            "📌 مصنع 2: المنتج بيتغيّر على نفس الخط، فسجّل 'التارجت' لكل يوم لوحده (عمود 🎯 جنب كل يوم)، "
            "وسجّل الفعلي في عموده جنبه. البيانات بتتحفظ ومتتمسحش، وتقدر تعدّل فيها في أي وقت وتحفظ تاني. "
    ...
    if use_daily_target_pr:
        # كل يوم بعمودين جنب بعض: التارجت بتاعه، وبعده الفعلي بتاعه
        display_cols_pr = ["الخط", "الشيفت"]
        for dc in visible_day_cols_pr:
            display_cols_pr += [_target_col(dc), dc]
        target_col_config = {
            _target_col(dc): st.column_config.NumberColumn(
                f"🎯 {dc}", format="%.0f", step=1, min_value=0,
                help="التارجت المستهدف ليوم ده بالذات (ممكن يختلف عن باقي الأيام حسب المنتج)"
            )
            for dc in visible_day_cols_pr
        }
    else:
    ...
    if use_daily_target_pr:
        target_cols_pr_all = [_target_col(dc) for dc in day_cols_pr]
        target_cols_pr_elapsed = [_target_col(dc) for dc in elapsed_day_cols_pr]
        total_target_pr = lines_only_pr[target_cols_pr_all].sum().sum() if target_cols_pr_all else 0
        # المتوقع حتى الآن = مجموع تارجت الأيام اللي فاتت فعلاً بالظبط (مش تقدير بالنسبة)، لإن التارجت بيختلف يوم عن يوم
        total_expected_pr = lines_only_pr[target_cols_pr_elapsed].sum().sum() if target_cols_pr_elapsed else 0
        total_daily_target_pr = total_target_pr / days_in_month_pr if days_in_month_pr else 0
        target_kpi_label_pr = "متوسط التارجت اليومي"
    else:
        ...
        target_kpi_label_pr = "إجمالي التارجت اليومي"
    ...
    kpi_row([
        {"icon": "🎯", "label": target_kpi_label_pr, "value": f"{total_daily_target_pr:,.0f} KG", "color": "#e0a92e"},
    ...
    if use_daily_target_pr:
        line_summary_pr = lines_only_pr[["الخط"]].copy().reset_index(drop=True)
        line_summary_pr["المستهدف (KG)"] = lines_only_pr[target_cols_pr_all].sum(axis=1).values if target_cols_pr_all else 0
        line_summary_pr["المتوقع حتى الآن (KG)"] = (
            lines_only_pr[target_cols_pr_elapsed].sum(axis=1).values if target_cols_pr_elapsed else 0
        )
    else:
    ...
    if st.button("💾 حفظ بيانات الشهر", key="production_save_month_btn"):
        ...
        # من غير ما نلمسهم. مصنع 1: بننسخ "التارجت اليومي" الثابت على كل أيام الشهر.
        # مصنع 2: بناخد تارجت كل يوم من عموده لوحده (لإنه بيختلف حسب المنتج).
        ...
        for _, r in to_save_pr.iterrows():
            ...
            for day_num, dc in enumerate(day_cols_pr, start=1):
                ...
                if use_daily_target_pr:
                    tc = _target_col(dc)
                    daily_target = float(r[tc]) if pd.notna(r[tc]) else 0.0
    ...
    page_banner("📦", "Packing", "تسجيل ومتابعة إنتاج الأصناف بالشيفت والفئة")
    ...
    # تقسيم المصانع: كل مصنع له فئاته الخاصة وشيته الخاص (بيانات كل مصنع منفصلة عن التانية)
    # مصنع 1 بياناته في نفس الشيتات القديمة (من غير سوفكس) عشان مانفقدش البيانات المسجلة قبل كده
    PACKING_FACTORY_CONFIG = {
        "مصنع 1": {
            "categories": ["هارد كاندي", "طوفي", "اكلير", "فورية"],
            "sheet_suffix": "",
            "colors": {"هارد كاندي": "#e0f2fe", "طوفي": "#fef3c7", "اكلير": "#fce7f3", "فورية": "#dcfce7"},
        },
        "مصنع 2": {
            "categories": ["ويفر"],
            "sheet_suffix": "_m2",
        ...
        "مصنع 3": {
            "categories": ["شيكولاته", "شيكوهند"],
            "sheet_suffix": "_m3",
            "colors": {"شيكولاته": "#fde68a", "شيكوهند": "#f5d0fe"},
        },
    }
    # كلمة مرور مستقلة لكل مصنع لوحده (منفصلة عن باسورد صفحة Production)
    PACKING_FACTORY_PASSWORDS = {
        "مصنع 1": "candy1",
        "مصنع 2": "wafer2",
        "مصنع 3": "choco3",
    }
    PACKING_ADMIN_PASSWORD = "9999"

    # نفس أسماء خطوط الأعطال بتاعة كل مصنع (زي صفحة الأعطال بالظبط) — مستخدمة هنا بس عشان
    # نفلتر جدول "الأعطال لنفس اليوم" جوه تقرير الباكينج، عشان كل مصنع يشوف أعطال خطوطه بس
    PACKING_FAULT_LINES_MAP = {
        "مصنع 1": ["الاكلير ص٣", "المستمر", "الإيطالي", "الاكلير ص١", "الطوفي", "الترانسلاب"],
        "مصنع 2": ["خط ويفر ١", "خط ويفر ٢", "خط ويفر ٣", "المطحنة", "المحطه"],
        "مصنع 3": ["شيكوهند بار", "خط شيكولاته ١ (وان شوت)", "خط شيكولاته ٢ (ميني باك)"],
    }

    pack_factory_pick = st.radio(
        "🏭 اختار المصنع", list(PACKING_FACTORY_CONFIG.keys()), horizontal=True, key="packing_factory_pick"
    )
    # إنتاج خطوط الإنتاج (صفحة Production) موجود لمصنع الكاندي والويفر — مش موجود في الشيكولاته لسه
    PACKING_PRODUCTION_SOURCE = {
        "مصنع 1": {"lines_key": "production_lines", "sheet_suffix": ""},
        "مصنع 2": {"lines_key": "production_lines_m2", "sheet_suffix": "_m2"},
    }
    show_prod_lines = pack_factory_pick in PACKING_PRODUCTION_SOURCE
    if show_prod_lines:
        active_prod_lines_key = PACKING_PRODUCTION_SOURCE[pack_factory_pick]["lines_key"]
        ACTIVE_PRODUCTION_SHEET = f"production_history_v2{PACKING_PRODUCTION_SOURCE[pack_factory_pick]['sheet_suffix']}"
    else:
        active_prod_lines_key = None
        ACTIVE_PRODUCTION_SHEET = None

    pack_unlock_key = f"packing_unlocked_{pack_factory_pick}"
    if pack_unlock_key not in st.session_state:
        st.session_state[pack_unlock_key] = False
    if not st.session_state[pack_unlock_key]:
        st.subheader(f"🔒 {pack_factory_pick} — صفحة محمية")
        pw_input = st.text_input("كلمة مرور المصنع", type="password", key=f"packing_page_pw_{pack_factory_pick}")
        if st.button("دخول", key=f"packing_page_unlock_btn_{pack_factory_pick}"):
            if pw_input == PACKING_FACTORY_PASSWORDS[pack_factory_pick]:
                st.session_state[pack_unlock_key] = True
                st.rerun()
            else:
                st.error("كلمة المرور غلط")
        st.stop()

    pack_cats = PACKING_FACTORY_CONFIG[pack_factory_pick]["categories"]
    pack_suffix = PACKING_FACTORY_CONFIG[pack_factory_pick]["sheet_suffix"]
    pack_colors = PACKING_FACTORY_CONFIG[pack_factory_pick]["colors"]
    pack_fault_lines = PACKING_FAULT_LINES_MAP[pack_factory_pick]
    ...
    PACKING_SHEET = f"packing_monthly_{selected_month}{pack_suffix}"
    ...
    def _suggest_category(row):
        ...
        if existing and existing.lower() != "nan" and existing in pack_cats:
            return existing
        auto = categorize_product(row["الصنف"])
        return auto if auto in pack_cats else pack_cats[-1]
    ...
    st.caption("سجل إنتاج كل يوم في عموده — البيانات بتتحفظ تلقائي ومش بتتمسح. تقدر تضيف صنف جديد في آخر الجدول من (+)")
    ...
    with st.expander("➕ إضافة صنف في مكان معين جوه الجدول (مش في الآخر بس)"):
        ...
        if insert_clicked:
            ...
            if not clean_name:
                ...
                auto_cat = auto_cat if auto_cat in pack_cats else pack_cats[-1]
    ...
    # ==========================
    # تسجيل إنتاج حسب الشيفت — سجل تفصيلي منفصل، وبيتجمّع تلقائي في عمود اليوم بالجدول الشهري
    # ==========================
    st.divider()
    st.markdown("#### 👷 تسجيل إنتاج حسب الشيفت")
    st.caption(
        "بدل ما تسجل إنتاج اليوم كله مرة واحدة، تقدر تسجل كل شيفت لوحده هنا. "
        "إجمالي الشيفتات بيتجمّع تلقائي في عمود اليوم بالجدول الشهري فوق."
    )

    SHIFT_LOG_SHEET = f"packing_shift_log{pack_suffix}"
    ...
    if st.button(f"💾 حفظ إنتاج {shift_log_pick} — {shift_log_date_str}"):
        ...
        st.success(f"اتحفظت إنتاج {shift_log_pick} ليوم {shift_log_date_str} ✔ (اتزامن عمود اليوم تلقائي)")
        st.rerun()

    # ---------- تقرير: إنتاج الشيفتات + إنتاج الخطوط لنفس اليوم ----------
    with st.expander("📊 تقرير إنتاج الشيفتات + إنتاج الخطوط لنفس اليوم"):
        ...
        if day_shift_log.empty:
            st.info("مفيش أي إنتاج مسجل بالشيفت لليوم ده لسه")
        else:
            weight_map = edited_grid.set_index("الصنف")["وزن الكرتونة (كيلو)"].to_dict()

            grand_total_cartons = 0.0
            grand_total_kg = 0.0
            shift_pack_tables = {}  # جدول إنتاج منفصل لكل شيفت (للعرض والطباعة)

            st.markdown(f"##### 📦 إنتاج كل شيفت — {report_shift_date_str}")
            for sh in SHIFT_OPTIONS_P:
                ...
                st.markdown(f"**👷 {sh}**")
                if sh_data.empty:
                    st.caption("مفيش إنتاج مسجل في الشيفت ده")
            ...
            # ---------- إنتاج الخطوط لكل شيفت لنفس اليوم (من صفحة Production) — الكاندي والويفر (خطوط الإنتاج مش موجودة في الشيكولاته لسه) ----------
            if show_prod_lines:
                st.markdown(f"##### 🏭 إنتاج خطوط الإنتاج لكل شيفت — {report_shift_date_str}")

                all_lines_clean = [
                    str(l).replace("🏭", "").strip() for l in st.session_state[active_prod_lines_key]
                ]
                ...
                if GSHEETS_ENABLED and gsheet_exists(ACTIVE_PRODUCTION_SHEET):
                    hist_df_shift = read_gsheet(ACTIVE_PRODUCTION_SHEET)
            ...
            # ---------- تقرير قابل للطباعة احترافي: كروت KPI + جدول لكل شيفت (إنتاج الشيفت + إنتاج الخطوط) + رسم بياني ----------
            st.divider()
            ...
            shift_top_html = '<div style="display:flex; gap:8px; margin:8px 0 14px;">' + "".join([
                _shift_kpi_card("📦", f"{grand_total_cartons:,.0f}", "إجمالي الإنتاج (كرتونة)", "#0891b2"),
                _shift_kpi_card("⚖️", f"{grand_total_kg:,.1f} كجم", "إجمالي الإنتاج بالوزن", "#0891b2"),
                    ...
                    <table class="layout-wrapper" width="100%">
                        <tr>
                            <td class="main-cell" valign="top" style="width:52%; padding-left:10px;">
                                <h3 style="margin:0 0 4px; font-size:11px; color:#374151;">📦 إنتاج الشيفت</h3>
                                {pack_html}
                            </td>
            ...
            # رسم بياني احترافي: إجمالي إنتاج كل خط (على مستوى اليوم كله) فعلي مقابل مستهدف — الكاندي والويفر
            shift_chart_html = ""
            line_totals_for_chart = pd.DataFrame({"الخط": all_lines_clean, "المستهدف (KG)": 0, "الفعلي (KG)": 0})
            if show_prod_lines and shift_prod_tables:
            ...
            if show_prod_lines and line_totals_for_chart[["المستهدف (KG)", "الفعلي (KG)"]].sum().sum() > 0:
            ...
            shift_printable_html = build_printable_html(
                f"تقرير الشيفتات — إنتاج الشيفت وإنتاج الخطوط — {pack_factory_pick}",
            ...
            st.download_button(
                ...
                file_name=f"shift_report_{report_shift_date_str}{pack_suffix}.html",
                mime="text/html",
            ...
            # ==========================
            st.divider()
            st.markdown("##### 🗓️ تقرير التخطيط — إنتاج اليوم (نفس يوم تقرير الشيفتات)")
            st.caption(
            ...
            if shift_plan_day_col not in shift_plan_source.columns:
                ...
                if shift_plan_df.empty:
                    ...
                    shift_plan_df = shift_plan_df.rename(columns={shift_plan_day_col: "إنتاج اليوم (كرتونة)"})
                    shift_plan_df["وزن اليوم (كجم)"] = (
                        shift_plan_df["إنتاج اليوم (كرتونة)"] * shift_plan_df["وزن الكرتونة (كيلو)"]
                    ).round(1)
                    shift_plan_df["نسبة من الخطة الشهرية %"] = shift_plan_df.apply(
                        lambda r: round((r["إنتاج اليوم (كرتونة)"] / r["المخطط بالكراتين (شهري)"] * 100), 1)
                        if r["المخطط بالكراتين (شهري)"] > 0 else None,
                        axis=1
                    )
                    shift_plan_df = shift_plan_df.sort_values("إنتاج اليوم (كرتونة)", ascending=False).reset_index(drop=True)
                    ...
                    shift_plan_df = shift_plan_df[[
                        "الصنف", "إنتاج اليوم (كرتونة)", "إجمالي الكراتين", "وزن اليوم (كجم)",
                        "المخطط بالكراتين (شهري)", "نسبة من الخطة الشهرية %"
                    ]]

                    # ---------- بطاقات KPI للتخطيط ----------
                    sp_items_today = len(shift_plan_df)
                    sp_total_qty_today = shift_plan_df["إنتاج اليوم (كرتونة)"].sum()
                    ...
                    shift_plan_top_html = (
                        '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                            ...
                            _shift_kpi_card("🎯", f"{sp_today_share:.1f}%",
                                            "نسبة إنتاج اليوم من إجمالي الخطة الشهرية", "#e0a92e"),
                            _shift_kpi_card("🏆", str(sp_top_item["الصنف"])[:18],
                                            f"أعلى صنف ({sp_top_item['إنتاج اليوم (كرتونة)']:,.0f} كرتونة)", "#7c3aed"),
                    ...
                    # ---------- توزيع الفئات (يستخدم لرسم الـ PDF بس، مفيش رسوم بيانية على الشاشة) ----------
                    sp_cat_totals = shift_plan_df_full.groupby("الفئة")["إنتاج اليوم (كرتونة)"].sum()

                    # ---------- نفس الرسمين بجودة طباعة، يتحطوا جوه تقرير الـ PDF بس (فوق بعض عشان الصفحة بالطول) ----------
                    ...
                    ax_sp_pie_print.set_title(ar_text("توزيع إنتاج اليوم بالفئات"),
                    ...
                    shift_plan_html = build_printable_html(
                        f"تقرير التخطيط — إنتاج يوم {shift_plan_day} — {pack_factory_pick}",
                    ...
                    st.download_button(
                        ...
                        file_name=f"packing_planning_{selected_month}{pack_suffix}_day{shift_plan_day}.html",
                        mime="text/html",
    ...
    if "packing_admin_ok" not in st.session_state:
        st.session_state.packing_admin_ok = False
    ...
    if summary.empty:
        ...
        if not st.session_state.packing_admin_ok:
            st.caption("🔒 ملخص الشهر التفصيلي (جدول + كروت + تحميل CSV) متاح للأدمن بس")
            admin_pw_pack_a = st.text_input("كلمة مرور الأدمن", type="password", key="packing_admin_pw_a")
            if st.button("دخول كأدمن", key="packing_admin_unlock_a"):
                if admin_pw_pack_a == PACKING_ADMIN_PASSWORD:
                    st.session_state.packing_admin_ok = True
                    st.rerun()
                else:
                    st.error("كلمة المرور غلط")
        else:
            ...
            st.download_button(
                ...
                file_name=f"packing_{selected_month}{pack_suffix}.csv",
                mime="text/csv"
            )
        ...
        print_df[f"إنتاج يوم {day_pick}"] = summary[day_col_pick]
        ...
        total_row_print = pd.DataFrame([{
            "الصنف": "الإجمالي",
            f"إنتاج يوم {day_pick}": print_df[f"إنتاج يوم {day_pick}"].sum(),
        ...
        # ---------- جدول صغير: إنتاج الخطوط لنفس اليوم (من صفحة Production) — الكاندي والويفر ----------
        ...
        prod_line_table = pd.DataFrame(columns=["الخط", "المستهدف (KG)", "الفعلي (KG)"])
        if show_prod_lines:
            all_lines_clean_pm = [str(l).replace("🏭", "").strip() for l in st.session_state[active_prod_lines_key]]
            ...
            if GSHEETS_ENABLED and gsheet_exists(ACTIVE_PRODUCTION_SHEET):
                hist_df_p = read_gsheet(ACTIVE_PRODUCTION_SHEET)
                hist_df_p["Date"] = hist_df_p["Date"].astype(str)
        ...
        # ---------- جدول صغير: الأعطال لنفس اليوم (من جوجل شيت) — بس خطوط المصنع المختار ----------
        ...
        if GSHEETS_ENABLED and gsheet_exists(faults_sheet_name):
            ...
            if day_col_pick in faults_df_p.columns and "الخط" in faults_df_p.columns:
                ...
                faults_table = faults_table[faults_table["الخط"].isin(pack_fault_lines)].reset_index(drop=True)

        # ---------- رسم بياني احترافي: إنتاج الخطوط الفعلي مقابل المستهدف — يتحط جوه التقرير القابل للطباعة (الكاندي والويفر) ----------
        prod_chart_html = ""
        if show_prod_lines and not prod_line_table.empty and prod_line_table[["المستهدف (KG)", "الفعلي (KG)"]].sum().sum() > 0:
        ...
        daily_report_extra_tables = [
            (f"📅 إنتاج يوم {day_pick} بالفئات", daily_by_cat_display),
        ]
        if show_prod_lines:
            daily_report_extra_tables.append(("🏭 إنتاج الخطوط لنفس اليوم", prod_line_table))
        daily_report_extra_tables.append(("⚠ الأعطال لنفس اليوم", faults_table))

        printable_html = build_printable_html(
            f"تقرير Packing اليومي — {pack_factory_pick} — شهر {selected_month}",
            f"يوم {day_pick} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            print_df,
            landscape=True,
            extra_tables=daily_report_extra_tables,
            extra_html=prod_chart_html or None,
        )
        st.download_button(
            "🖨️ تحميل نسخة قابلة للطباعة (PDF) — للإدارة",
            data=printable_html.encode("utf-8"),
            file_name=f"packing_{selected_month}{pack_suffix}_day{day_pick}.html",
            mime="text/html",
            help="افتح الملف بعد التحميل، ودوس Ctrl+P واختار 'Save as PDF' عشان تحوله PDF أو تطبعه"
        )
        ...
        PACKING_SETTINGS_SHEET = f"packing_settings{pack_suffix}"
        ...
        def load_planned_days(month_str, default_days):
            if GSHEETS_ENABLED and gsheet_exists(PACKING_SETTINGS_SHEET):
                ...
                if "الشهر" in settings_df.columns and "أيام العمل المخططة" in settings_df.columns:
        ...
        def save_planned_days(month_str, value):
            if GSHEETS_ENABLED:
                if gsheet_exists(PACKING_SETTINGS_SHEET):
                    settings_df = read_gsheet(PACKING_SETTINGS_SHEET)
                    if "الشهر" in settings_df.columns:
        ...
        # عدد أيام العمل الفعلية = عدد الأيام (من أول الشهر لحد اليوم المختار) اللي فيها إنتاج حقيقية
        ...
        pm_df = pm_df.rename(columns={
            day_col_pick: f"إنتاج يوم {day_pick}",
        ...
        if pack_factory_pick == "مصنع 2":
            # أعمدة إضافية لمصنع 2 بس: الإجمالي بالطن (فعلي/مخطط) + نسبة التحقيق (الفعلي ÷ المخطط الكلي، من غير تعديل بأيام العمل)
            pm_df["إجمالي الفعلي بالطن"] = (
                pm_df["إجمالي الفعلي (كرتونة)"] * pm_df["وزن الكرتونة (كيلو)"] / 1000
            ).round(2)
            pm_df["إجمالي المخطط بالطن"] = (
                pm_df["إجمالي المخطط (كرتونة)"] * pm_df["وزن الكرتونة (كيلو)"] / 1000
            ).round(2)
            pm_df["نسبة التحقيق %"] = pm_df.apply(
                lambda r: round((r["إجمالي الفعلي (كرتونة)"] / r["إجمالي المخطط (كرتونة)"] * 100), 1)
                if r["إجمالي المخطط (كرتونة)"] > 0 else None,
                axis=1
            )
            # الترتيب المطلوب لمصنع 2: إنتاج اليوم، الفعلي/المخطط كرتونة، الفعلي/المخطط بالطن، نسبة الأداء، حالة الأداء، نسبة التحقيق
            pm_df = pm_df[[
                "الصنف", f"إنتاج يوم {day_pick}", "إجمالي الفعلي (كرتونة)", "إجمالي المخطط (كرتونة)",
                "إجمالي الفعلي بالطن", "إجمالي المخطط بالطن", "نسبة الأداء %", "حالة الأداء", "نسبة التحقيق %"
            ]]
        else:
            # الترتيب المطلوب: الصنف، إنتاج اليوم، الإجمالي، المخطط، نسبة الأداء، حالة الأداء
            pm_df = pm_df[[
                "الصنف", f"إنتاج يوم {day_pick}", "إجمالي الفعلي (كرتونة)",
        ...
        pm_top_html = (
            '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                ...
                kpi_card("📦", f"{total_today_pm:,.0f}", f"إنتاج يوم {day_pick} (كرتونة)", "#0891b2"),
            ...
            + '<div style="display:flex; gap:8px; margin:0 0 12px;">' + "".join([
                ...
                kpi_card("⚖️", f"{total_today_kg:,.0f} كجم", f"إنتاج يوم {day_pick} بالوزن", "#0891b2"),
        ...
        if "نسبة التحقيق %" in pm_df.columns:
            pm_df_display_extra["نسبة التحقيق %"] = pm_df["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-")
        st.dataframe(
            pm_df.assign(**pm_df_display_extra),
            use_container_width=True, hide_index=True
        )

        # ---------- جدول ملوّن: إنتاج اليوم بالفئات (حسب فئات المصنع المختار) ----------
        cat_row_colors = [pack_colors.get(str(v), "#f3f4f6") for v in daily_by_cat_display["اليومي"]]
        ...
        st.markdown(f"##### 📅 إنتاج يوم {day_pick} بالفئات")
        st.dataframe(daily_by_cat_display, use_container_width=True, hide_index=True)

        # ---------- جدول: الفعلي/المستهدف/نسبة التحقيق — بالفئات (مصنع 1: هارد كاندي/طوفي/اكلير/فورية) ----------
        # ده جدول للـ PDF بس (مش بيتعرض في الصفحة) — بيتحط بعد الرسمة البيانية في تقرير مدير الإنتاج تحت.
        # نسبة التحقيق = الفعلي ÷ المستهدف الشهري الكامل × 100 (مش مقسومة على نسبة أيام الشهر اللي فاتت،
        # عشان تفضل نسبة واضحة ومفهومة: "قد ايه حققنا من المستهدف الشهري لحد دلوقتي")
        cat_perf_table_html = ""
        if pack_factory_pick == "مصنع 1":
            cat_perf_rows = []
            for cat in PACKING_FACTORY_CONFIG["مصنع 1"]["categories"]:
                cat_df = summary[summary["الفئة"] == cat]
                # بالوزن (كيلو) — لازم نضرب لكل صنف على حدة بوزن كرتونته هو (مش وزن موحّد)
                # قبل ما نجمع، عشان الأصناف المختلفة في نفس الفئة أوزانها مختلفة
                cat_actual_kg = (cat_df["إجمالي الكراتين"] * cat_df["وزن الكرتونة (كيلو)"]).sum()
                cat_target_kg = (cat_df["المخطط بالكراتين (شهري)"] * cat_df["وزن الكرتونة (كيلو)"]).sum()
                cat_achv = (cat_actual_kg / cat_target_kg * 100) if cat_target_kg > 0 else None
                cat_perf_rows.append({
                    "الفئة": cat,
                    "الإنتاج الفعلي (كيلو)": round(cat_actual_kg, 1),
                    "المستهدف (كيلو)": round(cat_target_kg, 1),
                    "نسبة التحقيق %": round(cat_achv, 1) if cat_achv is not None else None,
                })
            cat_perf_df = pd.DataFrame(cat_perf_rows)
            cat_perf_row_colors = [pack_colors.get(str(v), "#f3f4f6") for v in cat_perf_df["الفئة"]]

            cat_perf_display = cat_perf_df.assign(**{
                "الإنتاج الفعلي (كيلو)": cat_perf_df["الإنتاج الفعلي (كيلو)"].apply(lambda v: f"{v:,.1f}"),
                "المستهدف (كيلو)": cat_perf_df["المستهدف (كيلو)"].apply(lambda v: f"{v:,.1f}"),
                "نسبة التحقيق %": cat_perf_df["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-"),
            })
            cat_perf_table_inner_html = _df_to_html_highlighted(cat_perf_display, None, row_colors=cat_perf_row_colors)
            cat_perf_table_html = f"""
            <div style="margin-top:10px; page-break-inside:avoid;">
                <h2 style="background:#7c3aed; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                    📊 أداء الفئات (هارد كاندي / طوفي / اكلير / فورية) — الفعلي والمستهدف ونسبة التحقيق (بالوزن)
                </h2>
                {cat_perf_table_inner_html}
            </div>
            """
        ...
        if "نسبة التحقيق %" in pm_df.columns:
            pm_df["نسبة التحقيق %"] = pm_df["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-")
        ...
        # ---------- جدول إنتاج الخطوط لنفس اليوم — يتحط جوه تقرير مدير الإنتاج كمان (الكاندي والويفر) ----------
        ...
        if pm_single_page:
            ...
            cat_perf_rows_n = len(PACKING_FACTORY_CONFIG["مصنع 1"]["categories"]) if pack_factory_pick == "مصنع 1" else 0
            single_page_height_mm = 95 + (main_rows * 9) + (cat_rows * 6) + (prod_line_rows * 6) + (low_rows * 7) + (cat_perf_rows_n * 7) + 35 + 75

        pm_html = build_printable_html(
            f"تقرير متابعة الإنتاج — لمدير الإنتاج (Packing) — {pack_factory_pick}",
        ...
        st.download_button(
            ...
            file_name=f"packing_pm_{selected_month}{pack_suffix}_day{day_pick}.html",
            mime="text/html",
        ...
        if not st.session_state.packing_admin_ok:
            st.divider()
            st.caption(
                "🔒 باقي التقارير والتفاصيل (تخطيط الأصناف، تفاصيل الفئات، تقرير أداء المصانع الثلاثة) متاحة للأدمن بس"
            )
            admin_pw_pack_b = st.text_input("كلمة مرور الأدمن", type="password", key="packing_admin_pw_b")
            if st.button("دخول كأدمن", key="packing_admin_unlock_b"):
                if admin_pw_pack_b == PACKING_ADMIN_PASSWORD:
                    st.session_state.packing_admin_ok = True
                    st.rerun()
                else:
                    st.error("كلمة المرور غلط")
        ...
        # ==========================
        # تقرير مدير الإنتاج المجمّع — تلات المصانع مع بعض، بالظبط زي تقرير مدير الإنتاج العادي:
        # نفس الجدول (كل الأصناف من كل المصانع)، ونفس الكروت — بس بيتعاملوا مع تلات المصانع كأنهم مصنع واحد،
        # وتحت كل كرت تفاصيل كل مصنع بخط خفيف شوية (من غير قسم منفصل لتفاصيل كل مصنع)
        # ==========================
        st.divider()
        st.markdown("#### 🖨️ تقرير مدير الإنتاج المجمّع — تلات المصانع")
        st.caption(
            "نفس تقرير مدير الإنتاج بالظبط، بس بيجمع أصناف تلات المصانع في جدول واحد، "
            "وكروت الـ KPI بتتعامل مع تلات المصانع كأنهم مصنع واحد، وتحت كل كرت تفاصيل كل مصنع بخط خفيف"
        )

        def _load_factory_full_summary(f_suffix, month_str, n_days):
            sheet_name = f"packing_monthly_{month_str}{f_suffix}"
            day_cols_f = [f"يوم {d}" for d in range(1, n_days + 1)]
            cols = ["الصنف", "الفئة", "وزن الكرتونة (كيلو)", "المخطط بالكراتين (شهري)"] + day_cols_f
            if not (GSHEETS_ENABLED and gsheet_exists(sheet_name)):
                return pd.DataFrame(columns=cols + ["إجمالي الكراتين"])
            df = read_gsheet(sheet_name)
            for c in cols:
                if c not in df.columns:
                    df[c] = 0
            df = df[cols].copy()
            df = df[df["الصنف"].astype(str).str.strip() != ""]
            for dc in day_cols_f:
                df[dc] = pd.to_numeric(df[dc], errors="coerce").fillna(0)
            df["وزن الكرتونة (كيلو)"] = pd.to_numeric(df["وزن الكرتونة (كيلو)"], errors="coerce").fillna(0.0)
            df["المخطط بالكراتين (شهري)"] = pd.to_numeric(df["المخطط بالكراتين (شهري)"], errors="coerce").fillna(0)
            df["إجمالي الكراتين"] = df[day_cols_f].sum(axis=1)
            return df

        combined_colors_c = {}
        for f_conf_c in PACKING_FACTORY_CONFIG.values():
            combined_colors_c.update(f_conf_c["colors"])

        factory_full_summaries_c = {
            f_name: _load_factory_full_summary(f_conf["sheet_suffix"], selected_month, days_in_month)
            for f_name, f_conf in PACKING_FACTORY_CONFIG.items()
        }
        combined_summary = pd.concat(factory_full_summaries_c.values(), ignore_index=True)

        if combined_summary.empty:
            st.info("مفيش أي بيانات مسجلة في أي مصنع للشهر ده لحد دلوقتي")
        else:
            PACKING_SETTINGS_SHEET_C = "packing_settings_combined"

            def load_planned_days_c(month_str, default_days):
                if GSHEETS_ENABLED and gsheet_exists(PACKING_SETTINGS_SHEET_C):
                    settings_df = read_gsheet(PACKING_SETTINGS_SHEET_C)
                    if "الشهر" in settings_df.columns and "أيام العمل المخططة" in settings_df.columns:
                        match = settings_df[settings_df["الشهر"] == month_str]
                        if not match.empty:
                            val = pd.to_numeric(match.iloc[0]["أيام العمل المخططة"], errors="coerce")
            ...
            def save_planned_days_c(month_str, value):
                if GSHEETS_ENABLED:
                    if gsheet_exists(PACKING_SETTINGS_SHEET_C):
                        settings_df = read_gsheet(PACKING_SETTINGS_SHEET_C)
                        if "الشهر" in settings_df.columns:
                            settings_df = settings_df[settings_df["الشهر"] != month_str]
                        else:
                    ...
                    new_row = pd.DataFrame([{"الشهر": month_str, "أيام العمل المخططة": value}])
                    settings_df = pd.concat([settings_df, new_row], ignore_index=True)
                    write_gsheet(PACKING_SETTINGS_SHEET_C, settings_df)

            saved_planned_days_c = load_planned_days_c(selected_month, days_in_month)
            pdc1c, pdc2c = st.columns([2, 1])
            with pdc1c:
                total_month_days_c = st.number_input(
                    f"📅 عدد أيام العمل المخططة لشهر {selected_month} (لتلات المصانع)",
                    min_value=1, max_value=31, value=int(saved_planned_days_c), step=1,
                    key=f"planned_days_combined_{selected_month}"
                )
            with pdc2c:
                st.write("")
                st.write("")
                if st.button("💾 حفظ عدد الأيام", key="save_planned_days_combined_btn"):
                    save_planned_days_c(selected_month, total_month_days_c)
                    st.success("اتحفظ ✔")

            working_days_upto_c = [f"يوم {d}" for d in range(1, day_pick + 1)]
            actual_working_days_c = sum(1 for dc in working_days_upto_c if combined_summary[dc].sum() > 0)
            day_ratio_c = (actual_working_days_c / total_month_days_c) if total_month_days_c > 0 else 0

            pm_df_c = combined_summary[["الصنف", day_col_pick, "المخطط بالكراتين (شهري)", "إجمالي الكراتين",
                                         "وزن الكرتونة (كيلو)"]].copy()
            pm_df_c = pm_df_c.rename(columns={
                day_col_pick: f"إنتاج يوم {day_pick}",
                "المخطط بالكراتين (شهري)": "إجمالي المخطط (كرتونة)",
                "إجمالي الكراتين": "إجمالي الفعلي (كرتونة)"
            })
            pm_df_c = pm_df_c[pm_df_c["الصنف"].astype(str).str.strip() != ""].reset_index(drop=True)

            pm_df_c["_متوقع"] = (pm_df_c["إجمالي المخطط (كرتونة)"] * day_ratio_c)
            pm_df_c["نسبة الأداء %"] = pm_df_c.apply(
                lambda r: round((r["إجمالي الفعلي (كرتونة)"] / r["_متوقع"] * 100), 1)
                if r["_متوقع"] > 0 else None,
                axis=1
            )

            def _pm_status_c(pct):
                if pct is None or pd.isna(pct):
                    return "➖ بدون خطة", "#6b7280", "#f3f4f6"
                if pct >= 90:
                    return "⬆ تحقق المستهدف", "#16a34a", "#eafaf0"
            ...
            # لتلوين صفوف الجدول في تقرير الطباعة بس (حسب الأداء) — عمود "حالة الأداء" نفسه
            # بقى بيعرض الرصيد الفعلي بالمخزن بدل النص ده (تحت)
            row_bg_colors_c = [_pm_status_c(pct)[2] for pct in pm_df_c["نسبة الأداء %"]]

            # ---------- رصيد المخزون الفعلي (بدل عمود "حالة الأداء") ----------
            st.markdown("##### 📦 رصيد المخزون الفعلي (بدل عمود حالة الأداء)")
            stock_file_c = st.file_uploader(
                "ارفع ملف إكسيل رصيد الفروع والتوكيلات",
                type=["xlsx", "xls"], key="stock_balance_upload_combined",
                help="بيتقرا تلقائي أي شيت فيه عمود 'اسم الصنف' + 'الاجمالي' (وباركود لو موجود). "
                     "أول مرة بيطابق بالاسم، ولو معاه باركود بيحفظه تلقائي — المرات الجاية "
                     "بيطابق بالباركود مباشرة حتى لو الاسم في الإكسيل اتغيّر تمامًا"
            )
            totals_by_app_name_c = {}
            unmatched_rows_c = []
            if stock_file_c is not None:
                # الملف بيفضل "مرفوع" في الواجهة، وستريمليت بيعيد تشغيل الصفحة كلها مع أي
                # حركة تانية (كتابة في مربع بحث، دوس أي زرار في أي حتة بالصفحة) — فمن غير
                # الكاش ده، تحليل الإكسيل والقراءة/الكتابة على جوجل شيت كانت بتتكرر كل مرة،
                # وده اللي كان بيبطّئ الصفحة ويخليها توقع. دلوقتي بيتعمل مرة واحدة بس لكل ملف
                file_key = f"{stock_file_c.name}_{stock_file_c.size}"
                cache_key = f"stock_match_cache_{file_key}"
                if cache_key not in st.session_state:
                    with st.spinner("بيتم تحليل ومطابقة ملف المخزون..."):
                        st.session_state[cache_key] = match_stock_against_report(
                            stock_file_c, pm_df_c["الصنف"].tolist()
                        )
                    # نمسح كاش أي ملف قديم غير ده عشان الذاكرة ما تتكدسش
                    for k in list(st.session_state.keys()):
                        if k.startswith("stock_match_cache_") and k != cache_key:
                            del st.session_state[k]

                    # ---------- نحفظ رصيد المخزون ده بشكل دائم (Google Sheet) عشان يبقى متاح
                    # في ملف الإكسيل اليومي اللي بيتولّد من صفحة التقارير — من غير الحفظ ده،
                    # الرصيد كان بيفضل موجود بس في الجلسة دي، ويختفي تمامًا لو حد فتح صفحة التقارير
                    # لوحدها من غير ما يرفع نفس ملف المخزون تاني هناك ----------
                    _totals_to_save_c = st.session_state[cache_key][0]
                    if _totals_to_save_c:
                        _stock_date_str_c = f"{selected_month}-{day_pick:02d}"
                        _stock_save_df_c = pd.DataFrame([
                            {"Date": _stock_date_str_c, "الصنف": k, "الرصيد الفعلي بالمخزن": v}
                            for k, v in _totals_to_save_c.items()
                        ])
                        try:
                            if gsheet_exists("stock_balance_log"):
                                _existing_stock_c = read_gsheet("stock_balance_log")
                                _existing_stock_c = _existing_stock_c[_existing_stock_c["Date"] != _stock_date_str_c]
                                _combined_stock_c = pd.concat([_existing_stock_c, _stock_save_df_c], ignore_index=True)
                                write_gsheet("stock_balance_log", _combined_stock_c)
                            else:
                                write_gsheet("stock_balance_log", _stock_save_df_c)
                        except Exception:
                            st.info(
                                "⚠ الأرقام في التقرير ده صحيحة، بس تعذّر حفظ رصيد المخزون بشكل دائم "
                                "(اتصال جوجل شيت مؤقتًا) — عشان يظهر في ملف الإكسيل اليومي، ارفع نفس "
                                "الملف تاني هنا بعد شوية"
                            )

                (totals_by_app_name_c, import_report_c, unmatched_rows_c,
                 matched_sheets_c, skipped_sheets_c) = st.session_state[cache_key]
                if import_report_c["إجمالي الصفوف"] == 0:
                    st.warning("متلقاش في الملف ده أي شيت فيه عمود 'اسم الصنف' و'الاجمالي' يتقرا منه")
                else:
                    st.success(f"اتقرا {import_report_c['إجمالي الصفوف']} صنف من: {', '.join(matched_sheets_c)}")
                    rc1, rc2, rc3, rc4, rc5 = st.columns(5)
                    rc1.metric("تطابق بالباركود", import_report_c["تطابق بالباركود"])
                    rc2.metric("تطابق بالاسم", import_report_c["تطابق بالاسم"])
                    rc3.metric("باركود جديد اتحفظ", import_report_c["باركود جديد اتحفظ"])
                    rc4.metric("محتاج مطابقة يدوية", import_report_c["محتاج مطابقة يدوية"])
                    rc5.metric("تعارض باركود", import_report_c["تعارض باركود"])
                    if import_report_c.get("تعذّر حفظ باركود جديد", 0) > 0:
                        st.warning(
                            f"⚠ {import_report_c['تعذّر حفظ باركود جديد']} باركود جديد اتطابق بالاسم لكن "
                            "تعذّر حفظه بشكل دائم (اتصال جوجل شيت مؤقتًا) — الإجماليات في التقرير ده صحيحة، "
                            "بس ارفع نفس الملف تاني بعد شوية عشان الباركودات تتحفظ للمرات الجاية"
                        )

            pm_df_c["حالة الأداء"] = [
                (f"{totals_by_app_name_c[name]:,.0f}" if name in totals_by_app_name_c else "-")
                for name in pm_df_c["الصنف"]
            ]

            # ---------- أصناف من ملف المخزون محتاجة مطابقة يدوية (مرة واحدة بس لكل صنف جديد) ----------
            if unmatched_rows_c:
                with st.expander(f"🛠️ محتاج مطابقة يدوية — {len(unmatched_rows_c)} صنف من ملف المخزون"):
                    st.caption("اختار الصنف الصحيح من التقرير، واحفظ الربط — المرات الجاية هيتطابق تلقائي بالباركود")
                    app_names_list_c = [""] + pm_df_c["الصنف"].tolist()
                    for idx, row in enumerate(unmatched_rows_c):
                        info = debug_product_signature(row["اسم الصنف"])
                        st.markdown(f"**{row['اسم الصنف']}**  (باركود: {row['باركود'] or 'غير موجود'})")
                        st.caption(
                            f"تعبئة مكتشفة: {info['التعبئة المكتشفة']} | "
                            f"الاسم الحقيقي: {info['الاسم الحقيقي (بدون تعبئة/براند)'] or '—'}"
                        )
                        pick_col, btn_col = st.columns([3, 1])
                        picked = pick_col.selectbox(
                            "الصنف الصحيح في التقرير", app_names_list_c,
                            key=f"unmatched_pick_c_{idx}_{row['اسم الصنف']}"
                        )
                        if btn_col.button("💾 حفظ الربط", key=f"unmatched_save_c_{idx}_{row['اسم الصنف']}"):
                            if not picked:
                                st.warning("اختار الصنف الأول")
                            elif not row["باركود"]:
                                st.info("مافيش باركود للصف ده، فمش ممكن نحفظ ربط دائم — الإكسيل نفسه لازم يكون فيه عمود باركود لهذا الصنف الأول")
                            else:
                                ok, msg = save_barcode_mapping_manual(row["باركود"], picked)
                                (st.success if ok else st.error)(msg)
                                if ok:
                                    # نمسح كاش المطابقة بتاع الملف ده عشان التقرير يتحدث فورًا
                                    # بالربط الجديد (الرصيد يظهر، والصنف يختفي من "محتاج مطابقة يدوية")
            ...
            pm_df_c = pm_df_c.rename(columns={"حالة الأداء": "الرصيد الفعلي بالمخزن"})

            # ---------- إدارة الباركود الرئيسي: عرض/بحث/تعطيل أي ربط محفوظ ----------
            with st.expander("🏷️ إدارة الباركود الرئيسي (الربط الدائم بين الباركود والصنف)"):
                barcode_master_view = load_barcode_master()
                if barcode_master_view.empty:
                    st.caption("لسه مافيش أي روابط باركود محفوظة")
                else:
                    search_bc = st.text_input("بحث بالباركود أو اسم الصنف", key="barcode_master_search_c")
                    view_df = barcode_master_view.copy()
                    if search_bc.strip():
                        mask = (
                            view_df["الباركود"].astype(str).str.contains(search_bc, case=False, na=False)
                            | view_df["اسم الصنف في البرنامج"].astype(str).str.contains(search_bc, case=False, na=False)
                        )
                        view_df = view_df[mask]
                    st.dataframe(
                        view_df[["الباركود", "اسم الصنف في البرنامج", "المصدر", "الحالة", "تاريخ الإنشاء"]],
                        use_container_width=True, hide_index=True
                    )
                    deactivate_bc = st.selectbox(
                        "تعطيل ربط باركود (لو غلط أو محتاج يتغيّر)",
                        [""] + view_df["الباركود"].tolist(), key="barcode_master_deactivate_c"
                    )
                    if deactivate_bc and st.button("🚫 تعطيل الربط ده", key="barcode_master_deactivate_btn_c"):
                        full_master = load_barcode_master()
                        full_master.loc[full_master["الباركود"] == deactivate_bc, "الحالة"] = "غير نشط"
                        full_master.loc[full_master["الباركود"] == deactivate_bc, "تاريخ آخر تحديث"] = pd.Timestamp.today().strftime("%Y-%m-%d")
                        try:
                            write_gsheet(BARCODE_MASTER_SHEET, full_master)
                            st.success("تم تعطيل الربط")
                        except Exception:
                            st.error("⚠ تعذّر الاتصال بجوجل شيت مؤقتًا — جرب تاني بعد كام ثانية")

                # ---------- إضافة/تعديل باركود لأي صنف من البرنامج (مش لازم يكون جه من ملف إكسيل) ----------
                # ده بيغطي كل أصناف البرنامج (تلات المصانع)، مش بس اللي ظهرت في آخر ملف إكسيل اترفع.
                # بمجرد ما تحفظ الباركود هنا، أي ملف إكسيل جاي فيه نفس الباركود هيتطابق تلقائي بالصنف ده
                # على طول، حتى لو اسم الصنف في الإكسيل مختلف تمامًا — والربط ده بيفضل محفوظ دايمًا
                # (مش بس للشهر ده) لحد ما تتعطله بنفسك من فوق.
                st.markdown("---")
                st.markdown("##### ✍️ إضافة/تعديل باركود لأصناف البرنامج")
                st.caption(
                    "جدول بكل أصناف البرنامج (تلات المصانع) — اكتب الباركود جنب أي صنف، أو الصق "
                    "(Copy/Paste) عمود باركودات كامل من إكسيل مرة واحدة، وبعدين دوس 'حفظ'. "
                    "أي صنف ملوش باركود لسه، عمود الباركود بتاعه بيفضل فاضي لحد ما تكتبه."
                )
                all_app_products_bc = sorted(pm_df_c["الصنف"].dropna().astype(str).unique().tolist())
                active_master_names_bc = {}
                if not barcode_master_view.empty and "الحالة" in barcode_master_view.columns:
                    active_rows_bc = barcode_master_view[barcode_master_view["الحالة"] == "نشط"]
                    # لو فيه أكتر من باركود نشط لنفس الصنف (نادر)، ناخد آخر واحد اتسجل
                    for _, r_bc in active_rows_bc.iterrows():
                        active_master_names_bc[r_bc["اسم الصنف في البرنامج"]] = r_bc["الباركود"]

                manual_bc_show_all = st.checkbox(
                    "اعرض كل الأصناف (مش بس اللي بدون باركود)", value=False, key="manual_bc_show_all_c"
                )
                if manual_bc_show_all:
                    products_for_bc_table = all_app_products_bc
                else:
                    products_for_bc_table = [p for p in all_app_products_bc if p not in active_master_names_bc]
                    if products_for_bc_table:
                        st.caption(f"📋 {len(products_for_bc_table)} صنف من البرنامج لسه ملوش باركود محفوظ")
                    else:
                        st.success("كل أصناف البرنامج ليها باركود محفوظ ✔")

                bc_table_df = pd.DataFrame({
                    "الصنف": products_for_bc_table,
                    "الباركود": [active_master_names_bc.get(p, "") for p in products_for_bc_table],
                })
                bc_table_edited = st.data_editor(
                    bc_table_df,
                    use_container_width=True,
                    hide_index=True,
                    num_rows="fixed",
                    key="manual_bc_table_editor_c",
                    column_config={
                        "الصنف": st.column_config.TextColumn("📦 الصنف", disabled=True),
                        "الباركود": st.column_config.TextColumn("🏷️ الباركود (اكتب أو الصق هنا)"),
                    },
                )

                if st.button("💾 حفظ كل الباركودات اللي اتكتبت/اتغيّرت", key="manual_bc_save_btn_c"):
                    saved_bc, skipped_bc, conflicts_bc = 0, 0, []
                    for _, row_bc in bc_table_edited.iterrows():
                        product_bc = str(row_bc["الصنف"]).strip()
                        barcode_bc = str(row_bc["الباركود"]).strip()
                        if not barcode_bc:
                            continue
                        # لو نفس الباركود المحفوظ بالفعل لنفس الصنف، تجاهل (من غير كتابة مكررة)
                        if active_master_names_bc.get(product_bc, "") == barcode_bc:
                            continue
                        ok_bc, msg_bc = save_barcode_mapping_manual(barcode_bc, product_bc)
                        if ok_bc:
                            saved_bc += 1
                        else:
                            skipped_bc += 1
                            conflicts_bc.append(f"{product_bc}: {msg_bc}")
                    if saved_bc:
                        st.success(f"اتحفظ {saved_bc} باركود بنجاح ✔")
                    if skipped_bc:
                        st.warning("فيه صفوف متحفظتش (تعارض باركود):\n" + "\n".join(conflicts_bc))
                    if not saved_bc and not skipped_bc:
                        st.info("مفيش حاجة جديدة اتكتبت عشان تتحفظ")
                    if saved_bc:
                        st.rerun()

            # ---------- كروت KPI مجمّعة (كأنهم مصنع واحد)، وتحت كل كرت تفاصيل كل مصنع بخط خفيف ----------
            total_target_c = combined_summary["المخطط بالكراتين (شهري)"].sum()
            total_actual_c = combined_summary["إجمالي الكراتين"].sum()
            total_today_c = combined_summary[day_col_pick].sum()
            total_today_kg_c = (combined_summary[day_col_pick] * combined_summary["وزن الكرتونة (كيلو)"]).sum()
            items_count_c = len(pm_df_c[pm_df_c["إجمالي المخطط (كرتونة)"] > 0])
            low_items_count_c = sum(1 for pct in pm_df_c["نسبة الأداء %"] if pd.notna(pct) and pct < 90)
            remaining_days_c = max(total_month_days_c - actual_working_days_c, 0)

            # الأداء بيتحسب بالوزن (كجم) مش بالكراتين — عشان الجمع بين أصناف مختلفة الوزن يبقى له معنى فعلي
            total_target_kg_c = (combined_summary["المخطط بالكراتين (شهري)"] * combined_summary["وزن الكرتونة (كيلو)"]).sum()
            total_actual_kg_c = (combined_summary["إجمالي الكراتين"] * combined_summary["وزن الكرتونة (كيلو)"]).sum()
            total_expected_kg_c = total_target_kg_c * day_ratio_c
            avg_perf_kg_c = (total_actual_kg_c / total_expected_kg_c * 100) if total_expected_kg_c > 0 else 0
            full_plan_pct_kg_c = (total_actual_kg_c / total_target_kg_c * 100) if total_target_kg_c > 0 else 0

            def _factory_line(metric_fn):
                return " | ".join(f"{f_name}: {metric_fn(f_df)}" for f_name, f_df in factory_full_summaries_c.items())

            def _factory_target_kg(f_df):
                return (f_df["المخطط بالكراتين (شهري)"] * f_df["وزن الكرتونة (كيلو)"]).sum()

            def _factory_actual_kg(f_df):
                return (f_df["إجمالي الكراتين"] * f_df["وزن الكرتونة (كيلو)"]).sum()

            def _factory_perf_kg(f_df):
                f_target_kg = _factory_target_kg(f_df)
                f_expected_kg = f_target_kg * day_ratio_c
                f_actual_kg = _factory_actual_kg(f_df)
                return round((f_actual_kg / f_expected_kg * 100), 1) if f_expected_kg > 0 else 0

            def _factory_fullplan_kg(f_df):
                f_target_kg = _factory_target_kg(f_df)
                f_actual_kg = _factory_actual_kg(f_df)
                return round((f_actual_kg / f_target_kg * 100), 1) if f_target_kg > 0 else 0

            def _factory_low_count(f_df):
                if f_df.empty:
                    return 0
                mask_has_plan = f_df["المخطط بالكراتين (شهري)"] > 0
                if not mask_has_plan.any():
                    return 0
                f_expected = f_df["المخطط بالكراتين (شهري)"] * day_ratio_c
                f_actual = f_df["إجمالي الكراتين"]
                with np.errstate(divide="ignore", invalid="ignore"):
                    pct_series = (f_actual / f_expected * 100).where(f_expected > 0)
                return int((pct_series[mask_has_plan] < 90).sum())

            # النسبة المتوقع تحقيقها لحد النهاردة من الخطة الشهرية، حسب نسبة أيام العمل اللي فاتت
            expected_pct_today_c = day_ratio_c * 100

            # ---------- ملخص طلبيات التصدير (هنضيفه كارت في نفس صف الـ KPI الرئيسي تحت) ----------
            def _load_export_orders_summary(month_str):
                sheet_name = f"export_monthly_{month_str}"
                cols_out = ["الصنف", "اسم البلد", "الشركة", "الكمية المطلوبة (كرتونة)", "الفعلي حتى الآن (كرتونة)", "نسبة التحقيق %"]
                if not (GSHEETS_ENABLED and gsheet_exists(sheet_name)):
                    return pd.DataFrame(columns=cols_out)
                df_e = read_gsheet(sheet_name)
                # توافق مع بيانات شهور قديمة كانت متسجلة بعمود اسمه "(KG)" قبل ما نحوّله لـ "(كرتونة)"
                if "الكمية المطلوبة (كرتونة)" not in df_e.columns and "الكمية المطلوبة (KG)" in df_e.columns:
                    df_e = df_e.rename(columns={"الكمية المطلوبة (KG)": "الكمية المطلوبة (كرتونة)"})
                id_cols_e = ["الصنف", "اسم البلد", "الشركة"]
                for c in id_cols_e:
                    if c not in df_e.columns:
                        df_e[c] = ""
                    df_e[c] = df_e[c].astype(str).str.strip()
                if "الكمية المطلوبة (كرتونة)" not in df_e.columns:
                    df_e["الكمية المطلوبة (كرتونة)"] = 0
                df_e["الكمية المطلوبة (كرتونة)"] = pd.to_numeric(df_e["الكمية المطلوبة (كرتونة)"], errors="coerce").fillna(0)
                year_e2, month_num_e2 = map(int, month_str.split("-"))
                days_e2 = calendar.monthrange(year_e2, month_num_e2)[1]
                day_cols_e2 = [f"يوم {d}" for d in range(1, days_e2 + 1)]
                for dc in day_cols_e2:
                    if dc not in df_e.columns:
                        df_e[dc] = 0
                    df_e[dc] = pd.to_numeric(df_e[dc], errors="coerce").fillna(0)
                df_e = df_e[(df_e["الصنف"] != "") | (df_e["اسم البلد"] != "")].reset_index(drop=True)
                if df_e.empty:
                    return pd.DataFrame(columns=cols_out)
                df_e["الفعلي حتى الآن (كرتونة)"] = df_e[day_cols_e2].sum(axis=1)
                df_e["نسبة التحقيق %"] = df_e.apply(
                    lambda r: round((r["الفعلي حتى الآن (كرتونة)"] / r["الكمية المطلوبة (كرتونة)"] * 100), 1)
                    if r["الكمية المطلوبة (كرتونة)"] > 0 else 0,
                    axis=1
                )
                return df_e[cols_out]

            export_orders_df_c = _load_export_orders_summary(selected_month)
            export_kpi_card_c = None
            if not export_orders_df_c.empty:
                total_target_ex_c = export_orders_df_c["الكمية المطلوبة (كرتونة)"].sum()
                total_actual_ex_c = export_orders_df_c["الفعلي حتى الآن (كرتونة)"].sum()
                overall_pct_ex_c = round((total_actual_ex_c / total_target_ex_c * 100), 1) if total_target_ex_c > 0 else 0
                overall_color_ex_c = "#16a34a" if overall_pct_ex_c >= 90 else ("#d97706" if overall_pct_ex_c >= 70 else "#dc2626")

                # كل طلبية = بلد + شركة (ممكن تحتوي على أكتر من صنف)، فبنجمع كل أصناف نفس الطلبية مع بعض
                # ونطلع نسبة تحقيق واحدة للطلبية، من غير ما نسرد الأصناف واحد واحد
                orders_grouped_ex_c = export_orders_df_c.groupby(["اسم البلد", "الشركة"], sort=False).agg(
                    **{
                        "الكمية المطلوبة (كرتونة)": ("الكمية المطلوبة (كرتونة)", "sum"),
                        "الفعلي حتى الآن (كرتونة)": ("الفعلي حتى الآن (كرتونة)", "sum"),
                    }
                ).reset_index()
                orders_grouped_ex_c["نسبة التحقيق %"] = orders_grouped_ex_c.apply(
                    lambda r: round((r["الفعلي حتى الآن (كرتونة)"] / r["الكمية المطلوبة (كرتونة)"] * 100), 1)
                    if r["الكمية المطلوبة (كرتونة)"] > 0 else 0,
                    axis=1
                )
                orders_breakdown_ex_c = " | ".join(
                    f"{r_e['اسم البلد']} — {r_e['الشركة']}: {r_e['نسبة التحقيق %']:.1f}%"
                    for _, r_e in orders_grouped_ex_c.iterrows()
                )
                export_kpi_card_c = {
                    "icon": "🚢", "label": "نسبة تحقيق طلبيات التصدير (كل الطلبيات)",
                    "value": f"{overall_pct_ex_c}%", "color": overall_color_ex_c,
                    "delta": orders_breakdown_ex_c, "delta_color": "#6b7280",
                }

            # كرت "أيام العمل": مخططة / فعلية / متبقي تحت بعض في نفس الكرت
            work_days_value_html = f"""
            <div style="display:flex; flex-direction:column; gap:3px; margin-top:2px;">
                <div style="font-size:15px; font-weight:800; color:#1f2937;">📅 مخططة: {total_month_days_c}</div>
                <div style="font-size:15px; font-weight:800; color:#16a34a;">✅ فعلية: {actual_working_days_c}</div>
                <div style="font-size:15px; font-weight:800; color:#d97706;">⏳ متبقي: {remaining_days_c}</div>
            </div>
            """

            # كرت "إنتاج اليوم": كراتين ووزن تحت بعض في نفس الكرت
            today_value_html = f"""
            <div style="display:flex; flex-direction:column; gap:2px; margin-top:2px;">
                <div style="font-size:20px; font-weight:900; color:#0891b2;">📦 {total_today_c:,.0f} كرتونة</div>
                <div style="font-size:14px; font-weight:700; color:#6b7280;">⚖️ {total_today_kg_c:,.0f} كجم</div>
            </div>
            """
            today_delta = " | ".join(
                f"{f_name}: {f_df[day_col_pick].sum():,.0f} كرتونة / {(f_df[day_col_pick] * f_df['وزن الكرتونة (كيلو)']).sum():,.0f} كجم"
                for f_name, f_df in factory_full_summaries_c.items()
            )

            # كرت "نسبة تحقيق الخطة الشهرية": النسبة الفعلية دلوقتي + جنبها النسبة المفروض توصلها لحد النهاردة
            full_plan_color_c = "#16a34a" if full_plan_pct_kg_c >= 90 else ("#d97706" if full_plan_pct_kg_c >= 70 else "#dc2626")
            full_plan_value_html_c = f"""
            <div style="display:flex; flex-direction:column; gap:2px; margin-top:2px;">
                <div style="font-size:20px; font-weight:900; color:{full_plan_color_c};">🏁 {full_plan_pct_kg_c:.1f}%</div>
                <div style="font-size:13px; font-weight:700; color:#6b7280;">📌 المتوقع: {expected_pct_today_c:.1f}%</div>
            </div>
            """

            kpi_cards_c = [
                {
                    "icon": "🗓️", "label": "أيام العمل بالشهر", "value": work_days_value_html,
                    "color": "#2563eb",
                },
                {
                    "icon": "📦", "label": f"إنتاج يوم {day_pick} — كل المصانع", "value": today_value_html,
                    "color": "#0891b2",
                    "delta": today_delta,
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "📊", "label": "متوسط الأداء بالوزن (من المتوقع) — كل المصانع", "value": f"{avg_perf_kg_c:.1f}%",
                    "color": "#16a34a" if avg_perf_kg_c >= 90 else ("#d97706" if avg_perf_kg_c >= 70 else "#dc2626"),
                    "delta": _factory_line(lambda d: f"{_factory_perf_kg(d)}%"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "🏁", "label": "نسبة تحقيق الخطة الشهرية بالوزن — كل المصانع", "value": full_plan_value_html_c,
                    "color": full_plan_color_c,
                    "delta": _factory_line(lambda d: f"{_factory_fullplan_kg(d)}%"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "🔻", "label": "أصناف أداؤها أقل من المتوقع", "value": f"{low_items_count_c} من {items_count_c}",
                    "color": "#dc2626" if low_items_count_c > 0 else "#16a34a",
                    "delta": _factory_line(lambda d: f"{_factory_low_count(d)}"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "🎯", "label": "إجمالي المخطط بالوزن (كجم) — كل المصانع", "value": f"{total_target_kg_c:,.0f}",
                    "color": "#2563eb",
                    "delta": _factory_line(lambda d: f"{_factory_target_kg(d):,.0f} كجم"),
                    "delta_color": "#6b7280",
                },
                {
                    "icon": "📈", "label": "إجمالي الفعلي بالوزن (كجم) — كل المصانع", "value": f"{total_actual_kg_c:,.0f}",
                    "color": "#0d6b5c",
                    "delta": _factory_line(lambda d: f"{_factory_actual_kg(d):,.0f} كجم"),
                    "delta_color": "#6b7280",
                },
            ]
            if export_kpi_card_c is not None:
                kpi_cards_c.append(export_kpi_card_c)
            kpi_row(kpi_cards_c)

            # ---------- جدول كل الأصناف من كل المصانع ----------
            st.dataframe(
                pm_df_c.assign(**{
                    "نسبة الأداء %": pm_df_c["نسبة الأداء %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-"),
                }),
                use_container_width=True, hide_index=True
            )

            # ---------- جدول ملوّن: إنتاج اليوم بالفئات (كل المصانع) — كرتونة ووزن ونسبة تحقيق كل فئة مقابل التارجت اليومي بالوزن ----------
            combined_summary["_يومي_وزن_ك"] = combined_summary[day_col_pick] * combined_summary["وزن الكرتونة (كيلو)"]
            combined_summary["_تارجت_يومي_وزن_ك"] = (
                (combined_summary["المخطط بالكراتين (شهري)"] / total_month_days_c) * combined_summary["وزن الكرتونة (كيلو)"]
                if total_month_days_c > 0 else 0
            )
            cat_totals_c = combined_summary.groupby("الفئة", sort=False).agg(**{
                "اليومي (كرتونة)": (day_col_pick, "sum"),
                "اليومي (وزن كجم)": ("_يومي_وزن_ك", "sum"),
                "_تارجت_يومي": ("_تارجت_يومي_وزن_ك", "sum"),
            }).reset_index()
            cat_totals_c["نسبة التحقيق %"] = cat_totals_c.apply(
                lambda r: round((r["اليومي (وزن كجم)"] / r["_تارجت_يومي"] * 100), 1) if r["_تارجت_يومي"] > 0 else 0,
                axis=1
            )
            cat_totals_c = cat_totals_c.drop(columns=["_تارجت_يومي"])

            cat_total_target_c = combined_summary["_تارجت_يومي_وزن_ك"].sum()
            cat_total_row_c = pd.DataFrame([{
                "الفئة": "الإجمالي",
                "اليومي (كرتونة)": cat_totals_c["اليومي (كرتونة)"].sum(),
                "اليومي (وزن كجم)": cat_totals_c["اليومي (وزن كجم)"].sum(),
                "نسبة التحقيق %": round((cat_totals_c["اليومي (وزن كجم)"].sum() / cat_total_target_c * 100), 1)
                if cat_total_target_c > 0 else 0,
            }])
            daily_by_cat_display_c = pd.concat([cat_totals_c, cat_total_row_c], ignore_index=True)
            daily_by_cat_display_c["اليومي (كرتونة)"] = daily_by_cat_display_c["اليومي (كرتونة)"].round(0)
            daily_by_cat_display_c["اليومي (وزن كجم)"] = daily_by_cat_display_c["اليومي (وزن كجم)"].round(1)
            cat_row_colors_c = [combined_colors_c.get(str(v), "#f3f4f6") for v in daily_by_cat_display_c["الفئة"]]
            cat_row_colors_c[-1] = "#e5e7eb"
            daily_by_cat_display_c_fmt = daily_by_cat_display_c.assign(**{
                "نسبة التحقيق %": daily_by_cat_display_c["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪")
            })
            cat_table_html_c = _df_to_html_highlighted(daily_by_cat_display_c_fmt, None, row_colors=cat_row_colors_c)

            st.markdown(f"##### 🏭 إنتاج يوم {day_pick} — كل المصانع")
            st.dataframe(daily_by_cat_display_c_fmt, use_container_width=True, hide_index=True)

            # ---------- جدول: إنتاج الخطوط لنفس اليوم من كل المصانع (فعلي، تارجت، نسبة) ----------
            @st.cache_data(ttl=60, show_spinner=False)
            def _load_combined_prod_lines(report_date_str, lines_m1, lines_m2):
                prod_factory_defs = [
                    (lines_m1, "production_history_v2"),
                    (lines_m2, "production_history_v2_m2"),
                ]
                tables = []
                for lines_raw, sheet_name in prod_factory_defs:
                    lines_clean = [str(l).replace("🏭", "").strip() for l in lines_raw]
                    if not lines_clean:
                        continue
                    f_table = pd.DataFrame({"الخط": lines_clean, "المستهدف (KG)": 0.0, "الفعلي (KG)": 0.0})
                    if GSHEETS_ENABLED and gsheet_exists(sheet_name):
                        hist_df_cp = read_gsheet(sheet_name)
                        hist_df_cp["Date"] = hist_df_cp["Date"].astype(str)
                        day_hist_cp = hist_df_cp[(hist_df_cp["Date"] == report_date_str) & (hist_df_cp["Shift"] == "All Shifts")]
                        if "Line" in day_hist_cp.columns:
                            day_hist_cp = day_hist_cp[day_hist_cp["Line"].astype(str).str.strip() != ""]
                        if not day_hist_cp.empty:
                            for nc in ["المستهدف (KG)", "الفعلي (KG)"]:
                                day_hist_cp[nc] = pd.to_numeric(day_hist_cp[nc], errors="coerce").fillna(0)
                            day_grouped_cp = day_hist_cp.groupby("Line")[["المستهدف (KG)", "الفعلي (KG)"]].sum()
                            f_table = day_grouped_cp.reindex(lines_clean, fill_value=0).reset_index()
                            f_table.columns = ["الخط", "المستهدف (KG)", "الفعلي (KG)"]
                    tables.append(f_table)
                if not tables:
                    return pd.DataFrame(columns=["الخط", "المستهدف (KG)", "الفعلي (KG)", "نسبة التحقيق %"])
                out_df = pd.concat(tables, ignore_index=True)
                out_df["نسبة التحقيق %"] = out_df.apply(
                    lambda r: round((r["الفعلي (KG)"] / r["المستهدف (KG)"] * 100), 1) if r["المستهدف (KG)"] > 0 else 0,
                    axis=1
                )
                return out_df

            combined_prod_lines_df = _load_combined_prod_lines(
                report_date, st.session_state.production_lines, st.session_state.production_lines_m2
            )
            prod_lines_table_html_c = ""
            if not combined_prod_lines_df.empty:
                cpl_total_row = pd.DataFrame([{
                    "الخط": "الإجمالي",
                    "المستهدف (KG)": combined_prod_lines_df["المستهدف (KG)"].sum(),
                    "الفعلي (KG)": combined_prod_lines_df["الفعلي (KG)"].sum(),
                    "نسبة التحقيق %": round(
                        combined_prod_lines_df["الفعلي (KG)"].sum() / combined_prod_lines_df["المستهدف (KG)"].sum() * 100, 1
                    ) if combined_prod_lines_df["المستهدف (KG)"].sum() > 0 else 0,
                }])
                combined_prod_lines_display = pd.concat([combined_prod_lines_df, cpl_total_row], ignore_index=True)
                cpl_row_colors = ["#f8fafc"] * (len(combined_prod_lines_display) - 1) + ["#e5e7eb"]
                combined_prod_lines_display_fmt = combined_prod_lines_display.assign(**{
                    "نسبة التحقيق %": combined_prod_lines_display["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪")
                })

                st.markdown(f"##### 🏭 إنتاج الخطوط لنفس اليوم {day_pick} — كل المصانع")
                st.dataframe(combined_prod_lines_display_fmt, use_container_width=True, hide_index=True)

                prod_lines_table_html_c = f"""
                <div style="margin-top:10px; page-break-inside:avoid;">
                    <h2 style="background:#2563eb; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                        🏭 إنتاج الخطوط لنفس اليوم {day_pick} — كل المصانع
                    </h2>
                    {_df_to_html_highlighted(combined_prod_lines_display_fmt, None, row_colors=cpl_row_colors)}
                </div>
                """

            # ---------- جدول: الأصناف اللي أداؤها أقل من المتوقع — بنفس ترتيب المصانع الأصلي (مصنع 1 ثم 2 ثم 3)
            # من غير أي فرز بينهم يخلي أصناف المصانع تتداخل مع بعض ----------
            pm_low_df_c = pm_df_c[pd.to_numeric(pm_df_c["نسبة الأداء %"], errors="coerce") < 90].copy().reset_index(drop=True)
            low_row_colors_sorted_c = [_pm_status_c(p)[2] for p in pm_low_df_c["نسبة الأداء %"]]
            pm_low_df_c["نسبة الأداء %"] = pm_low_df_c["نسبة الأداء %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-")
            pm_bottom_table_html_c = _df_to_html_highlighted(pm_low_df_c, None, row_colors=low_row_colors_sorted_c) if not pm_low_df_c.empty else \
                "<p style='font-size:12px; color:#6b7280;'>كل الأصناف حققت 90% أو أكتر من المتوقع 🎉</p>"

            pm_df_c["نسبة الأداء %"] = pm_df_c["نسبة الأداء %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-")

            def _pack_cards_html_c(cards):
                parts = []
                for c in cards:
                    delta_html = ""
                    if c.get("delta"):
                        d_color = c.get("delta_color", "#6b7280")
                        delta_html = f'<div style="font-size:9.5px; font-weight:600; color:{d_color}; margin-top:4px;">{c["delta"]}</div>'
                    parts.append(f"""<div style="flex:1; min-width:170px; background:#fff; border:1px solid #e5e7eb; border-right:4px solid {c['color']}; border-radius:10px; padding:10px 12px; margin:4px; box-shadow:0 2px 6px rgba(0,0,0,0.05);">
                        <div style="font-size:11px; color:#6b7280; font-weight:600; margin-bottom:4px;">{c['icon']} {c['label']}</div>
                        <div style="font-size:16px; font-weight:800; color:{c['color']}; line-height:1.3;">{c['value']}</div>
                        {delta_html}
                    </div>""")
                return "".join(parts)

            # ---------- اتجاه الإنتاج آخر 7 أيام (كل المصانع) + خط التارجت ----------
            trend_days_window_c = [d for d in range(max(1, day_pick - 6), day_pick + 1)]
            trend_cols_c = [f"يوم {d}" for d in trend_days_window_c]
            trend_weights_c_raw = [
                (combined_summary[c] * combined_summary["وزن الكرتونة (كيلو)"]).sum() if c in combined_summary.columns else 0
                for c in trend_cols_c
            ]
            daily_target_weight_c = (
                (combined_summary["المخطط بالكراتين (شهري)"] / total_month_days_c * combined_summary["وزن الكرتونة (كيلو)"]).sum()
                if total_month_days_c > 0 else 0
            )
            # نشيل أيام الإجازة (اللي مفيهاش إنتاج خالص) من الرسم، عشان الخط ميبقاش نازل لصفر في يوم مفيهوش شغل أصلاً
            trend_days_c_nz = [d for d, w in zip(trend_days_window_c, trend_weights_c_raw) if w > 0]
            trend_weights_c_nz = [w for w in trend_weights_c_raw if w > 0]
            if trend_days_c_nz:
                trend_days_c, trend_weights_c = trend_days_c_nz, trend_weights_c_nz
            else:
                trend_days_c, trend_weights_c = trend_days_window_c, trend_weights_c_raw
            target_weights_c = [daily_target_weight_c] * len(trend_days_c)

            def _build_trend_fig_generic(days_list, weights_list, targets_list, title_text, figsize=(7.2, 3.2), title_size=12):
                fig_tc, ax_tc = plt.subplots(figsize=figsize)
                fig_tc.patch.set_facecolor("#ffffff")
                ax_tc.plot(days_list, weights_list, marker="o", linewidth=2.4, color=BRAND_GREEN,
                           markerfacecolor="#ffffff", markeredgecolor=BRAND_GREEN, markeredgewidth=2,
                           markersize=7, zorder=3, label=ar_text("الفعلي (كجم)"))
                ax_tc.fill_between(days_list, weights_list, color=BRAND_GREEN, alpha=0.08, zorder=2)
                ax_tc.plot(days_list, targets_list, linewidth=2, color="#dc2626", linestyle="--",
                           zorder=2, label=ar_text("التارجت اليومي (كجم)"))
                for x, y in zip(days_list, weights_list):
                    ax_tc.annotate(f"{y:,.0f}", xy=(x, y), xytext=(0, 8), textcoords="offset points",
                                   ha="center", fontsize=8, fontweight="bold", color="#1a1d21")
                ax_tc.set_xticks(days_list)
                ax_tc.set_xticklabels([ar_text(f"يوم {d}") for d in days_list], fontsize=9, fontweight="bold")
                ax_tc.tick_params(axis="y", labelsize=9, colors="#4b5563")
                ax_tc.spines["top"].set_visible(False)
                ax_tc.spines["right"].set_visible(False)
                ax_tc.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
                ax_tc.set_axisbelow(True)
                ax_tc.legend(fontsize=8, loc="upper left", frameon=False)
                ax_tc.set_title(ar_text(title_text), fontsize=title_size, fontweight="bold", color=BRAND_GREEN, pad=10)
                fig_tc.tight_layout()
                return fig_tc

            trend_chart_img_html_c = fig_to_base64_img(_build_trend_fig_generic(
                trend_days_c, trend_weights_c, target_weights_c,
                "اتجاه الإنتاج — آخر 7 أيام (إجمالي الوزن كجم) — كل المصانع"
            ))

            # ---------- نفس الرسم بس لكل مصنع لوحده، يتحط تحت الرسم المجمّع مباشرة ----------
            per_factory_trend_html_c = ""
            for f_name_tr, f_df_tr in factory_full_summaries_c.items():
                f_weights_raw_tr = [
                    (f_df_tr[c] * f_df_tr["وزن الكرتونة (كيلو)"]).sum() if c in f_df_tr.columns else 0
                    for c in trend_cols_c
                ]
                f_daily_target_tr = (
                    (f_df_tr["المخطط بالكراتين (شهري)"] / total_month_days_c * f_df_tr["وزن الكرتونة (كيلو)"]).sum()
                    if total_month_days_c > 0 else 0
                )
                f_days_nz_tr = [d for d, w in zip(trend_days_window_c, f_weights_raw_tr) if w > 0]
                f_weights_nz_tr = [w for w in f_weights_raw_tr if w > 0]
                if f_days_nz_tr:
                    f_days_tr, f_weights_tr = f_days_nz_tr, f_weights_nz_tr
                else:
                    f_days_tr, f_weights_tr = trend_days_window_c, f_weights_raw_tr
                f_targets_tr = [f_daily_target_tr] * len(f_days_tr)

                f_chart_img_tr = fig_to_base64_img(_build_trend_fig_generic(
                    f_days_tr, f_weights_tr, f_targets_tr,
                    f"اتجاه الإنتاج — آخر 7 أيام (بالوزن) — {f_name_tr}",
                    figsize=(7.2, 2.9), title_size=11
                ))
                per_factory_trend_html_c += f"""
                <div style="margin-top:8px; page-break-inside:avoid;">
                    {f_chart_img_tr}
                </div>
                """

            trend_chart_section_html_c = f"""
            <div style="margin-top:10px; page-break-inside:avoid;">
                <h2 style="background:{BRAND_GREEN}; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                    📈 اتجاه الإنتاج — آخر 7 أيام (بالوزن) — كل المصانع
                </h2>
                {trend_chart_img_html_c}
            </div>
            <div style="margin-top:6px;">
                <h2 style="background:#0d6b5c; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                    📊 اتجاه الإنتاج آخر 7 أيام — لكل مصنع على حدة
                </h2>
                {per_factory_trend_html_c}
            </div>
            """

            pm_top_html_c = f'<div style="display:flex; flex-wrap:wrap; margin:8px 0 14px;">{_pack_cards_html_c(kpi_cards_c)}</div>'

            pm_extra_html_c = f"""
            <div style="margin-top:10px; page-break-inside:avoid;">
                <h2 style="background:#0891b2; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                    🏭 إنتاج يوم {day_pick} — كل المصانع
                </h2>
                {cat_table_html_c}
            </div>
            {prod_lines_table_html_c}
            <div style="margin-top:10px;">
                ...
                {pm_bottom_table_html_c}
            </div>
            <p style="font-size:9px; color:#6b7280; margin-top:10px;">
                ℹ️ يتم احتساب "المتوقع" و"نسبة الأداء" بناءً على عدد أيام العمل الفعلية
                ({actual_working_days_c} يوم) من إجمالي أيام العمل المخططة ({total_month_days_c} يوم).
            </p>
            {trend_chart_section_html_c}
            """

            pm_single_page_c = st.checkbox(
                "📄 نسخة صفحة واحدة متصلة (للإرسال أونلاين فقط — مش مقاس ورق طابعة عادي)",
                value=False,
                key="pm_single_page_toggle_combined",
                help="لو مفعّل: هيطلع PDF بصفحة واحدة طويلة بدون تقسيم، مناسب للإرسال بالإيميل/واتساب بس. "
                     "لو عايز تطبعه فعليًا على ورق A4، سيبه مطفي."
            )

            single_page_height_mm_c = None
            if pm_single_page_c:
                main_rows_c = len(pm_df_c)
                cat_rows_c = len(daily_by_cat_display_c)
                low_rows_c = max(len(pm_low_df_c), 1)
                export_rows_c = len(export_orders_df_c) if not export_orders_df_c.empty else 0
                prod_lines_rows_c = len(combined_prod_lines_df) if not combined_prod_lines_df.empty else 0
                n_factory_charts_c = len(factory_full_summaries_c)
                single_page_height_mm_c = (
                    95 + (main_rows_c * 9) + (cat_rows_c * 6) + (low_rows_c * 7) + (export_rows_c * 4)
                    + (prod_lines_rows_c * 6) + (n_factory_charts_c * 60) + 35 + 75
                )

            pm_html_c = build_printable_html(
                "تقرير متابعة الإنتاج — لمدير الإنتاج (Packing) — إجمالي المصانع الثلاثة",
                f"شهر {selected_month} — يوم {day_pick} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                pm_df_c,
                landscape=True,
                base_font_size=15,
                bold=True,
                row_padding="10px 8px",
                top_html=pm_top_html_c,
                row_colors=row_bg_colors_c,
                extra_html=pm_extra_html_c,
                single_page_height_mm=single_page_height_mm_c
            )

            st.download_button(
                "🖨️ تحميل تقرير مدير الإنتاج المجمّع (خط كبير)",
                data=pm_html_c.encode("utf-8"),
                file_name=f"packing_pm_combined_{selected_month}_day{day_pick}.html",
                mime="text/html",
                help="تقرير بطاقات + جدول ملوّن بالحالة + جدول فئات — افتحه ودوس Ctrl+P واختار 'Save as PDF'"
            )

            # ==========================
            # تقرير الشيفتات المجمّع — تلات المصانع مع بعض، ونفس نظام "إنتاج الخطوط" (الكاندي والويفر)
            # بيمشي بنفس يوم تقرير مدير الإنتاج المجمّع (day_pick) مش بتاريخ منفصل
            # ==========================
            combined_shift_date_str = pd.Timestamp(year=year, month=month_num, day=day_pick).strftime("%Y-%m-%d")

            st.markdown("##### 🖨️ تقرير الشيفتات المجمّع — تلات المصانع")
            st.caption(f"نفس تقرير الشيفتات بالظبط، بس إنتاج الشيفتات مجمّعة من تلات المصانع ليوم {day_pick} (نفس يوم تقرير مدير الإنتاج المجمّع) — وجدول إنتاج الخطوط بيجمع خطوط الكاندي والويفر مع بعض")

            def _load_factory_shift_pack(f_suffix, date_str, month_str):
                sheet_name = f"packing_shift_log{f_suffix}"
                weight_sheet = f"packing_monthly_{month_str}{f_suffix}"
                weight_map_f = {}
                if GSHEETS_ENABLED and gsheet_exists(weight_sheet):
                    wdf = read_gsheet(weight_sheet)
                    if "الصنف" in wdf.columns and "وزن الكرتونة (كيلو)" in wdf.columns:
                        wdf["وزن الكرتونة (كيلو)"] = pd.to_numeric(wdf["وزن الكرتونة (كيلو)"], errors="coerce").fillna(0)
                        weight_map_f = dict(zip(wdf["الصنف"], wdf["وزن الكرتونة (كيلو)"]))
                day_log = pd.DataFrame(columns=["التاريخ", "الشيفت", "الصنف", "الكمية (كرتونة)"])
                if GSHEETS_ENABLED and gsheet_exists(sheet_name):
                    log_df = read_gsheet(sheet_name)
                    for c in ["التاريخ", "الشيفت", "الصنف", "الكمية (كرتونة)"]:
                        if c not in log_df.columns:
                            log_df[c] = 0 if c == "الكمية (كرتونة)" else ""
                    log_df["الكمية (كرتونة)"] = pd.to_numeric(log_df["الكمية (كرتونة)"], errors="coerce").fillna(0)
                    day_log = log_df[log_df["التاريخ"] == date_str]
                tables = {}
                for sh in SHIFT_OPTIONS_P:
                    sh_data = day_log[day_log["الشيفت"] == sh][["الصنف", "الكمية (كرتونة)"]].copy()
                    sh_data = sh_data[sh_data["الكمية (كرتونة)"] > 0].reset_index(drop=True)
                    if sh_data.empty:
                        tables[sh] = pd.DataFrame(columns=["الصنف", "الكمية (كرتونة)", "الوزن (كيلو)"])
                        continue
                    sh_data["الوزن (كيلو)"] = sh_data.apply(
                        lambda r: r["الكمية (كرتونة)"] * weight_map_f.get(r["الصنف"], 0), axis=1
                    ).round(1)
                    tables[sh] = sh_data
                return tables

            def _load_shift_production_lines(date_str):
                combined_lines_x = []
                combined_hist_parts = []
                for src in PACKING_PRODUCTION_SOURCE.values():
                    lines_src = [str(l).replace("🏭", "").strip() for l in st.session_state[src["lines_key"]]]
                    combined_lines_x += lines_src
                    sheet_src = f"production_history_v2{src['sheet_suffix']}"
                    if GSHEETS_ENABLED and gsheet_exists(sheet_src):
                        h = read_gsheet(sheet_src)
                        if "Date" in h.columns and not h.empty:
                            combined_hist_parts.append(h)

                all_lines_clean_x = combined_lines_x
                day_shift_prod_x = pd.DataFrame()
                if combined_hist_parts:
                    hist_df_x = pd.concat(combined_hist_parts, ignore_index=True)
                    hist_df_x["Date"] = hist_df_x["Date"].astype(str)
                    day_shift_prod_x = hist_df_x[
                        (hist_df_x["Date"] == date_str) & (hist_df_x["Shift"] != "All Shifts")
                    ].copy()
                    if not day_shift_prod_x.empty:
                        for nc in ["المستهدف (KG)", "الفعلي (KG)"]:
                            day_shift_prod_x[nc] = pd.to_numeric(day_shift_prod_x[nc], errors="coerce").fillna(0)
                        day_shift_prod_x["Line"] = day_shift_prod_x["Line"].astype(str).str.strip()
                        day_shift_prod_x = day_shift_prod_x[day_shift_prod_x["Line"] != ""]
                tables = {}
                for pack_sh, prod_sh in zip(SHIFT_OPTIONS_P, SHIFTS):
                    if not day_shift_prod_x.empty:
                        sh_prod = day_shift_prod_x[day_shift_prod_x["Shift"] == prod_sh]
                        grouped = sh_prod.groupby("Line")[["المستهدف (KG)", "الفعلي (KG)"]].sum()
                    else:
                        grouped = pd.DataFrame(columns=["المستهدف (KG)", "الفعلي (KG)"])
                    full = grouped.reindex(all_lines_clean_x, fill_value=0).reset_index()
                    full.columns = ["الخط", "المستهدف (KG)", "الفعلي (KG)"]
                    full["نسبة التحقيق %"] = full.apply(
                        lambda r: round((r["الفعلي (KG)"] / r["المستهدف (KG)"] * 100), 1) if r["المستهدف (KG)"] > 0 else 0,
                        axis=1
                    )
                    tables[pack_sh] = full
                return tables, all_lines_clean_x

            factory_shift_tables_c = {
                f_name: _load_factory_shift_pack(f_conf["sheet_suffix"], combined_shift_date_str, selected_month)
                for f_name, f_conf in PACKING_FACTORY_CONFIG.items()
            }
            prod_line_tables_c, prod_lines_clean_c = _load_shift_production_lines(combined_shift_date_str)

            grand_total_cartons_c = 0.0
            grand_total_kg_c = 0.0
            shift_blocks_html_c = ""
            for sh in SHIFT_OPTIONS_P:
                sh_combined = pd.concat(
                    [factory_shift_tables_c[f_name][sh] for f_name in PACKING_FACTORY_CONFIG], ignore_index=True
                )
                st.markdown(f"**👷 {sh}**")
                if sh_combined.empty:
                    st.caption("مفيش إنتاج مسجل في الشيفت ده من أي مصنع")
                    sh_display_c = pd.DataFrame(columns=["الصنف", "الكمية (كرتونة)", "الوزن (كيلو)"])
                else:
                    sh_total_cartons_c = sh_combined["الكمية (كرتونة)"].sum()
                    sh_total_kg_c = sh_combined["الوزن (كيلو)"].sum()
                    grand_total_cartons_c += sh_total_cartons_c
                    grand_total_kg_c += sh_total_kg_c
                    total_row_c = pd.DataFrame([{
                        "الصنف": "الإجمالي", "الكمية (كرتونة)": sh_total_cartons_c,
                        "الوزن (كيلو)": round(sh_total_kg_c, 1)
                    }])
                    sh_display_c = pd.concat([sh_combined, total_row_c], ignore_index=True)
                    st.dataframe(sh_display_c, use_container_width=True, hide_index=True)

                pack_html_c = sh_display_c.to_html(index=False, border=0, justify="center", na_rep="-")
                prod_html_c = prod_line_tables_c[sh].to_html(index=False, border=0, justify="center", na_rep="-")
                shift_blocks_html_c += f"""
                <div style="margin-top:14px; page-break-inside:avoid;">
                    <h2 style="background:#0b1f3a; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:6px;">
                        👷 {sh}
                    </h2>
                    <table class="layout-wrapper" width="100%">
                        <tr>
                            <td valign="top" style="width:52%; padding-left:10px;">
                                <h3 style="margin:0 0 4px; font-size:11px; color:#374151;">📦 إنتاج الشيفت — كل المصانع</h3>
                                {pack_html_c}
                            </td>
                            <td valign="top" style="width:48%;">
                                <h3 style="margin:0 0 4px; font-size:11px; color:#374151;">🏭 إنتاج الخطوط (كاندي + ويفر)</h3>
                                {prod_html_c}
                            </td>
                        </tr>
                    </table>
            ...
            # ---------- كروت KPI: إنتاج كل شيفت (كرتونة + وزن في نفس الكرت) وتفاصيل كل مصنع بخط خفيف ----------
            shift_cards_c = []
            for sh in SHIFT_OPTIONS_P:
                sh_total_cartons_x = 0.0
                sh_total_kg_x = 0.0
                per_factory_lines_x = []
                for f_name in PACKING_FACTORY_CONFIG:
                    f_sh_df = factory_shift_tables_c[f_name][sh]
                    f_cartons_x = f_sh_df["الكمية (كرتونة)"].sum() if not f_sh_df.empty else 0.0
                    f_kg_x = f_sh_df["الوزن (كيلو)"].sum() if not f_sh_df.empty else 0.0
                    sh_total_cartons_x += f_cartons_x
                    sh_total_kg_x += f_kg_x
                    per_factory_lines_x.append(f"{f_name}: {f_cartons_x:,.0f} كرتونة / {f_kg_x:,.0f} كجم")
                sh_value_html = f"""
                <div style="display:flex; flex-direction:column; gap:2px; margin-top:2px;">
                    <div style="font-size:18px; font-weight:900; color:#0b1f3a;">📦 {sh_total_cartons_x:,.0f} كرتونة</div>
                    <div style="font-size:13px; font-weight:700; color:#6b7280;">⚖️ {sh_total_kg_x:,.0f} كجم</div>
                </div>
                """
                shift_cards_c.append({
                    "icon": "👷", "label": sh, "value": sh_value_html, "color": "#0b1f3a",
                    "delta": " | ".join(per_factory_lines_x), "delta_color": "#6b7280",
                })

            grand_cards_c = [
                {"icon": "📦", "label": "إجمالي كل الشيفتات (كرتونة) — كل المصانع", "value": f"{grand_total_cartons_c:,.0f}", "color": "#0891b2"},
                {"icon": "⚖️", "label": "إجمالي كل الشيفتات (كيلو) — كل المصانع", "value": f"{grand_total_kg_c:,.1f}", "color": "#2563eb"},
            ]
            kpi_row(shift_cards_c + grand_cards_c)

            shift_top_html_c = f"""
            <div style="display:flex; flex-wrap:wrap; margin:8px 0 14px;">
                {_pack_cards_html_c(shift_cards_c + grand_cards_c)}
            </div>
            """

            shift_report_height_mm_c = 90 + len(SHIFT_OPTIONS_P) * 90 + len(prod_lines_clean_c) * 5

            shift_html_c = build_printable_html(
                "تقرير الشيفتات المجمّع — تلات المصانع",
                f"شهر {selected_month} — يوم {combined_shift_date_str} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                pd.DataFrame(),
                landscape=True,
                top_html=shift_top_html_c,
                extra_html=shift_blocks_html_c,
                single_page_height_mm=shift_report_height_mm_c,
            )
            st.download_button(
                "🖨️ تحميل تقرير الشيفتات المجمّع (صفحة واحدة متصلة)",
                data=shift_html_c.encode("utf-8"),
                file_name=f"shift_report_combined_{combined_shift_date_str}.html",
                mime="text/html",
                help="سكرول واحد متصل يجمع إنتاج الشيفتات من تلات المصانع + إنتاج خطوط الكاندي — افتحه ودوس Ctrl+P واختار 'Save as PDF'",
                key="shift_report_combined_download_btn"
            )
        ...
        # ==========================
        st.divider()
        st.markdown("#### 🗓️ تقرير التخطيط — إنتاج اليوم")
        st.caption(
        ...
        if planning_df.empty:
            ...
            planning_df = planning_df.rename(columns={day_col_pick: "إنتاج اليوم (كرتونة)"})
            planning_df["وزن اليوم (كجم)"] = (
                planning_df["إنتاج اليوم (كرتونة)"] * planning_df["وزن الكرتونة (كيلو)"]
            ).round(1)
            ...
            planning_df = planning_df.sort_values("إنتاج اليوم (كرتونة)", ascending=False).reset_index(drop=True)
            ...
            total_qty_today = planning_df["إنتاج اليوم (كرتونة)"].sum()
            ...
            planning_top_html = (
                '<div style="display:flex; gap:8px; margin:8px 0 8px;">' + "".join([
                    ...
                    kpi_card("🎯", f"{today_share_of_plan:.1f}%", "نسبة إنتاج اليوم من إجمالي الخطة الشهرية", "#e0a92e"),
                    kpi_card("🏆", str(top_item_row["الصنف"])[:18],
                             f"أعلى صنف ({top_item_row['إنتاج اليوم (كرتونة)']:,.0f} كرتونة)", "#7c3aed"),
            ...
            cat_today_totals = planning_df_full.groupby("الفئة")["إنتاج اليوم (كرتونة)"].sum()
            ...
            ax_plan_pie_print.set_title(ar_text("توزيع إنتاج اليوم بالفئات"),
                                         fontsize=12, fontweight="bold", color=BRAND_GREEN, pad=10)
            ...
            planning_html = build_printable_html(
                f"تقرير التخطيط — إنتاج يوم {day_pick} — {pack_factory_pick}",
            ...
            st.download_button(
                ...
                file_name=f"packing_planning_{selected_month}{pack_suffix}_day{day_pick}.html",
                mime="text/html",
        ...
        if cat_df.empty:
            ...
            st.caption(f"إنتاج كل يوم بالشهر لفئة {category_choice}")
            daily_trend = cat_df[day_cols].sum(axis=0)
            daily_trend_df = pd.DataFrame({
                "اليوم": range(1, len(day_cols) + 1),
                "إجمالي الإنتاج (كرتونة)": daily_trend.values
            })
            st.dataframe(daily_trend_df, use_container_width=True, hide_index=True)
    ...
    base_cols_ex = id_cols_ex + ["الكمية المطلوبة (كرتونة)"] + day_cols_ex
    ...
    if grid_raw_ex.empty:
        month_grid_ex = pd.DataFrame([{
            "الصنف": "", "اسم البلد": "", "الشركة": "", "الكمية المطلوبة (كرتونة)": 0,
            **{dc: 0 for dc in day_cols_ex}
        }])
    else:
        month_grid_ex = grid_raw_ex
        # توافق مع بيانات شهور قديمة كانت متسجلة بعمود اسمه "(KG)" قبل ما نحوّله لـ "(كرتونة)"
        if "الكمية المطلوبة (كرتونة)" not in month_grid_ex.columns and "الكمية المطلوبة (KG)" in month_grid_ex.columns:
            month_grid_ex = month_grid_ex.rename(columns={"الكمية المطلوبة (KG)": "الكمية المطلوبة (كرتونة)"})
    ...
    month_grid_ex["الكمية المطلوبة (كرتونة)"] = pd.to_numeric(month_grid_ex["الكمية المطلوبة (كرتونة)"], errors="coerce").fillna(0)
    for dc in day_cols_ex:
        month_grid_ex[dc] = pd.to_numeric(month_grid_ex[dc], errors="coerce").fillna(0)

    st.caption(
        "📌 سجّل كل طلبية تصدير في صف: الصنف، اسم البلد، الشركة، والكمية المطلوبة (كرتونة). "
    ...
    edited_ex = st.data_editor(
        ...
        column_config={
            ...
            "الكمية المطلوبة (كرتونة)": st.column_config.NumberColumn("🎯 الكمية المطلوبة (كرتونة)", format="%.0f", min_value=0),
    ...
    report_grid_ex["الفعلي حتى الآن (كرتونة)"] = report_grid_ex[day_cols_ex].sum(axis=1)
    report_grid_ex["نسبة التحقيق %"] = report_grid_ex.apply(
        lambda r: round((r["الفعلي حتى الآن (كرتونة)"] / r["الكمية المطلوبة (كرتونة)"] * 100), 1)
        if r["الكمية المطلوبة (كرتونة)"] > 0 else 0,
        axis=1
    )
    report_grid_ex["المتبقي (كرتونة)"] = (report_grid_ex["الكمية المطلوبة (كرتونة)"] - report_grid_ex["الفعلي حتى الآن (كرتونة)"]).clip(lower=0)
    ...
    report_grid_ex["المتوقع حتى الآن (كرتونة)"] = (report_grid_ex["الكمية المطلوبة (كرتونة)"] * day_ratio_ex).round(0)
    report_grid_ex["حالة الطلبية"] = report_grid_ex.apply(
        lambda r: "✅ مكتملة" if r["نسبة التحقيق %"] >= 100
        else ("🟢 على المسار" if r["الفعلي حتى الآن (كرتونة)"] >= r["المتوقع حتى الآن (كرتونة)"]
              else "🔴 متأخرة"),
        axis=1
    )

    total_target_ex = report_grid_ex["الكمية المطلوبة (كرتونة)"].sum()
    total_actual_ex = report_grid_ex["الفعلي حتى الآن (كرتونة)"].sum()
    total_pct_ex = round((total_actual_ex / total_target_ex * 100), 1) if total_target_ex > 0 else 0
    total_remaining_ex = report_grid_ex["المتبقي (كرتونة)"].sum()
    ...
    kpi_row([
        ...
        {"icon": "🎯", "label": "إجمالي الكمية المطلوبة", "value": f"{total_target_ex:,.0f} كرتونة", "color": "#e0a92e"},
        {"icon": "📦", "label": "إجمالي الفعلي حتى الآن", "value": f"{total_actual_ex:,.0f} كرتونة", "color": "#0d6b5c"},
        {"icon": "🥇", "label": "نسبة التحقيق الإجمالية", "value": f"{total_pct_ex}%", "color": "#7c3aed"},
    ])
    kpi_row([
        {"icon": "⏳", "label": "المتبقي لإتمام كل الطلبيات", "value": f"{total_remaining_ex:,.0f} كرتونة", "color": "#ea580c"},
    ...
    if report_grid_ex.empty:
        ...
        country_summary_ex = country_summary_ex.sort_values("الكمية المطلوبة (كرتونة)", ascending=False).reset_index(drop=True)
    ...
    if report_grid_ex.empty:
        ...
        company_summary_ex = company_summary_ex.sort_values("الكمية المطلوبة (كرتونة)", ascending=False).reset_index(drop=True)
        st.dataframe(company_summary_ex, use_container_width=True, hide_index=True)

    # ==========================
    # جداول إضافية تفيد التخطيط (من غير رسوم بيانية)
    # ==========================
    st.divider()
    st.subheader("📋 جداول تفيد التخطيط")

    if not report_grid_ex.empty:
        table_col1, table_col2 = st.columns(2)
        with table_col1:
            st.caption("🏆 أعلى 10 أصناف من حيث الكمية المطلوبة (كرتونة)")
            top_items_ex = (
                report_grid_ex.groupby("الصنف")["الكمية المطلوبة (كرتونة)"].sum()
                .sort_values(ascending=False).head(10)
            )
            if not top_items_ex.empty:
                st.dataframe(top_items_ex.rename("الكمية المطلوبة (كرتونة)"), use_container_width=True)
            else:
        ...
        with table_col2:
            ...
            st.dataframe(status_counts_ex.rename("عدد الطلبيات"), use_container_width=True)

        st.caption("📈 إجمالي الكمية المُصدَّرة كل يوم على مدار الشهر (كرتونة)")
        daily_trend_ex = report_grid_ex[day_cols_ex].sum(axis=0)
        daily_trend_ex_df = pd.DataFrame({
            "اليوم": range(1, len(day_cols_ex) + 1),
            "إجمالي المُصدَّر (كرتونة)": daily_trend_ex.values
        })
        st.dataframe(daily_trend_ex_df, use_container_width=True, hide_index=True)
    ...
    if report_grid_ex.empty:
        ...
        detail_cols_ex = [
            "الصنف", "اسم البلد", "الشركة",
            "الفعلي حتى الآن (كرتونة)", "الكمية المطلوبة (كرتونة)", "المتبقي (كرتونة)", "نسبة التحقيق %"
        ]
        # كل طلبية في صف لوحدها بالترتيب اللي اتسجلت بيه، من غير أي فرز أو تجميع تحت أي تصنيف
        detail_df_ex = report_grid_ex[detail_cols_ex].reset_index(drop=True)

        detail_df_display_ex = detail_df_ex.copy()
        for c in ["الفعلي حتى الآن (كرتونة)", "الكمية المطلوبة (كرتونة)", "المتبقي (كرتونة)"]:
            detail_df_display_ex[c] = detail_df_display_ex[c].apply(lambda v: f"{v:,.0f}")
        detail_df_display_ex["نسبة التحقيق %"] = detail_df_display_ex["نسبة التحقيق %"].apply(lambda v: f"{v:.1f} ٪" if pd.notna(v) else "-")
        st.dataframe(detail_df_display_ex, use_container_width=True, hide_index=True)

        def _export_row_color(status):
            if "✅" in str(status):
                return "#eafaf0"
            elif "🔴" in str(status):
                return "#fde2e1"
        ...
        row_colors_ex = [_export_row_color(s) for s in report_grid_ex["حالة الطلبية"]]

        # نسخة الطباعة من غير عمود الشركة (زي ما طلب)، وبنفس ستايل باقي التقارير في البرنامج بالظبط
        print_cols_ex = [c for c in detail_df_display_ex.columns if c != "الشركة"]
        detail_df_print_ex = detail_df_display_ex[print_cols_ex].copy()
        detail_df_print_ex["_status"] = report_grid_ex["حالة الطلبية"].values
        detail_df_print_ex["_color"] = row_colors_ex

        hide_completed_ex = st.checkbox(
            "🚫 شيل الطلبيات اللي تمّت من التقرير",
            value=False,
            key="export_hide_completed_toggle",
            help="لو مفعّل، الطلبيات المكتملة (✅) مش هتظهر في نسخة الطباعة"
        )
        if hide_completed_ex:
            detail_df_print_ex = detail_df_print_ex[~detail_df_print_ex["_status"].astype(str).str.contains("✅")]

        # ترتيب وتجميع حسب البلد، مع خط فاصل واضح بين كل بلد والتاني (بنفس شكل باقي التقارير تمامًا)
        detail_df_print_ex = detail_df_print_ex.reset_index(drop=True)
        detail_df_print_ex["_orig_order"] = range(len(detail_df_print_ex))
        detail_df_print_ex = detail_df_print_ex.sort_values(
            ["اسم البلد", "_orig_order"], kind="stable"
        ).reset_index(drop=True)

        row_top_borders_ex = []
        prev_country_ex = None
        for c_val in detail_df_print_ex["اسم البلد"]:
            if prev_country_ex is not None and c_val != prev_country_ex:
                row_top_borders_ex.append("3px solid #013e37")
            else:
                row_top_borders_ex.append(None)
            prev_country_ex = c_val

        row_colors_ex_sorted = detail_df_print_ex["_color"].tolist()
        detail_df_print_final_ex = detail_df_print_ex.drop(columns=["_status", "_color", "_orig_order"])

        detail_report_html = build_printable_html(
            f"التقرير المفصل للتصدير — شهر {selected_month_ex}",
            f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')} — عدد الطلبيات: {len(detail_df_print_final_ex)} — "
            f"متأخرة: {n_delayed_ex} — مكتملة: {n_completed_ex}",
            detail_df_print_final_ex,
            landscape=False,
            row_colors=row_colors_ex_sorted,
            row_top_borders=row_top_borders_ex,
            base_font_size=13,
            bold=True,
            row_padding="9px 8px",
        )
    ...
    fault_line_passwords = {
        ...
        "المحطه": "7575",
        "شيكوهند بار": "6060",
        "خط شيكولاته ١ (وان شوت)": "6161",
        "خط شيكولاته ٢ (ميني باك)": "6262",
    }
    ...
    FACTORY_CONFIG = {
        ...
        "مصنع 2": {
            "lines": ["خط ويفر ١", "خط ويفر ٢", "خط ويفر ٣", "المطحنة", "المحطه"],
            "maint_password": "9988",
            "maint_name": "م. عبدالرحمن",
        },
        "مصنع 3": {
            "lines": ["شيكوهند بار", "خط شيكولاته ١ (وان شوت)", "خط شيكولاته ٢ (ميني باك)"],
            "maint_password": "7799",
            "maint_name": "م. محمد حسين",
        },
    }
    ...
    ALL_FAULT_LINES = [line for f_conf in FACTORY_CONFIG.values() for line in f_conf["lines"]]
    st.divider()
    ...
    if admin_ok:
        ...
        st.caption("تقرير جاهز للطباعة والعرض على الإدارة — لكل المصانع مع بعض، وكل خط ماسجلش عطل بيظهر جنبه 'لا يوجد عطل'")
        ...
        # الخطوط اللي مسجلتش أي عطل اليوم ده — تظهر بصف "لا يوجد عطل" (لكل خطوط المصانع مع بعض)
        ...
        daily_report_df["__order"] = daily_report_df["الخط"].apply(
            lambda l: ALL_FAULT_LINES.index(l) if l in ALL_FAULT_LINES else len(ALL_FAULT_LINES)
        )
        daily_report_df = daily_report_df.sort_values("__order").drop(columns="__order").reset_index(drop=True)
        ...
        def _build_signature_block(font_size_label, font_size_name, margin_top, extra_style=""):
            ...
            if len(factories_approved_today) == 1:
                label = f"✅ تم الاعتماد من قبل مدير صيانة {factories_approved_today[0]}"
            else:
                factories_line = " و ".join(factories_approved_today)
                label = f"✅ تم الاعتماد من قبل مديري صيانة {factories_line}"
            return f"""
        ...
        FACTORY_ACCENTS = {"مصنع 1": "#2563eb", "مصنع 2": "#7c3aed", "مصنع 3": "#059669"}
        factory_details = {}
    ...
    # نفس تقسيم المصانع الثلاثة، ونفس خطوطهم، ونفس باسورد الباكينج لكل مصنع بالظبط
    BREAKAGE_FACTORY_CONFIG = {
        "مصنع 1": {
            "lines": ["الاكلير ص٣", "المستمر", "الإيطالي", "الاكلير ص١", "الطوفي"],
            "sheet_suffix": "",
            "password": "candy1",
            "has_production_data": True,  # عنده بيانات إنتاج بالكيلو من صفحة Production
            "production_sheet_suffix": "",  # نفس suffix شيت صفحة الإنتاج بتاعة نفس المصنع
        },
        "مصنع 2": {
            "lines": ["خط ويفر ١", "خط ويفر ٢", "خط ويفر ٣"],
            "sheet_suffix": "_m2",
            "password": "wafer2",
            "has_production_data": True,  # بياناته موجودة في صفحة الإنتاج (production_history_v2_m2)
            "production_sheet_suffix": "_m2",
        },
        "مصنع 3": {
            "lines": ["شيكوهند بار", "خط شيكولاته ١ (وان شوت)", "خط شيكولاته ٢ (ميني باك)"],
            "sheet_suffix": "_m3",
            "password": "choco3",
            "has_production_data": False,  # لسه مفيش صفحة إنتاج مربوطة لمصنع 3
            "production_sheet_suffix": None,
        },
    }
    BREAKAGE_ADMIN_PASSWORD = "9999"

    def _norm_line_name(name):
        """بيوحّد اسم الخط للمقارنة بين صفحة الإنتاج وصفحة الكسر (مثلاً 'ويفر ١' في صفحة الإنتاج
        مقابل 'خط ويفر ١' في صفحة الكسر) — بيشيل كلمة 'خط' والمسافات الزيادة بس، من غير ما يأثر
        على الاسم المعروض للمستخدم في أي مكان."""
        return str(name).replace("خط", "").strip()

    def _build_line_name_lookup(target_lines, source_names):
        """بيرجع dict {اسم مصدر: اسم هدف} بمطابقة الأسماء بعد التوحيد، عشان نجمع إنتاج كل خط صح
        حتى لو الاسم مكتوب بصيغة مختلفة شوية بين صفحة الإنتاج وصفحة الكسر."""
        norm_targets = {_norm_line_name(t): t for t in target_lines}
        lookup = {}
        for s in source_names:
            t = norm_targets.get(_norm_line_name(s))
            if t:
                lookup[s] = t
        return lookup

    breakage_factory_pick = st.radio(
        "🏭 اختار المصنع", list(BREAKAGE_FACTORY_CONFIG.keys()), horizontal=True, key="breakage_factory_pick"
    )
    b_conf = BREAKAGE_FACTORY_CONFIG[breakage_factory_pick]

    breakage_unlock_key = f"breakage_unlocked_{breakage_factory_pick}"
    if breakage_unlock_key not in st.session_state:
        st.session_state[breakage_unlock_key] = False
    if not st.session_state[breakage_unlock_key]:
        st.subheader(f"🔒 {breakage_factory_pick} — صفحة محمية")
        bpw_input = st.text_input("كلمة مرور المصنع", type="password", key=f"breakage_pw_{breakage_factory_pick}")
        if st.button("دخول", key=f"breakage_unlock_btn_{breakage_factory_pick}"):
            if bpw_input == b_conf["password"]:
                st.session_state[breakage_unlock_key] = True
                st.rerun()
            else:
                st.error("كلمة المرور غلط")
        st.stop()

    BREAKAGE_SHEET = f"breakage_history{b_conf['sheet_suffix']}"
    ...
    all_lines_clean_b = b_conf["lines"]

    # ---------- هات إنتاج كل خط لنفس اليوم من صفحة Production (لاستخدامه في حساب نسبة الكسر %) ----------
    # كل مصنع بيقرا من شيت الإنتاج بتاعه هو (بالـ suffix بتاعه)، ومطابقة اسم الخط بتتم بعد توحيد الاسم
    # (زي "ويفر ١" في صفحة الإنتاج مقابل "خط ويفر ١" في صفحة الكسر) عشان الإنتاج يتجمع صح.
    ...
    b_prod_sheet = f"production_history_v2{b_conf.get('production_sheet_suffix', '')}" \
        if b_conf.get("production_sheet_suffix") is not None else None
    if b_conf["has_production_data"] and b_prod_sheet and GSHEETS_ENABLED and gsheet_exists(b_prod_sheet):
        hist_df_b = read_gsheet(b_prod_sheet)
        ...
        if not day_hist_b.empty:
            ...
            name_lookup_b = _build_line_name_lookup(all_lines_clean_b, prod_by_line_day.index)
            for src_name, target_line in name_lookup_b.items():
                day_production[target_line] += float(prod_by_line_day[src_name])
    ...
    if GSHEETS_ENABLED and gsheet_exists(BREAKAGE_SHEET):
        ...
        if needed_cols.issubset(set(breakage_log_all.columns)):
            ...
            if "هالك (KG)" not in breakage_log_all.columns:
                breakage_log_all["هالك (KG)"] = 0  # توافق مع سجلات قديمة اتسجلت قبل إضافة الهالك
            for nc in ["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "هالك (KG)"]:
            ...
            breakage_log_all["متبقي الرصيد (KG)"] = (
                breakage_log_all["رصيد أول المدة (KG)"] + breakage_log_all["كسر خارج (KG)"]
                - breakage_log_all["كسر داخل (KG)"]
            )
    ...
    st.caption("رصيد أول المدة بييجي تلقائي من متبقي رصيد آخر يوم سجّلت فيه، وتقدر تعدّله يدوي لو حابب. الهالك = الكمية اللي بتتعدم نهائيًا")

    entry_rows = [
        {"الخط": ln, "رصيد أول المدة (KG)": carry_forward[ln], "كسر خارج (KG)": 0, "كسر داخل (KG)": 0, "هالك (KG)": 0}
        for ln in all_lines_clean_b
    ]
    ...
    edited_entry = st.data_editor(
        ...
        column_config={
            ...
            "هالك (KG)": st.column_config.NumberColumn("🗑️ هالك (إعدام) (KG)", min_value=0, step=1),
        },
        use_container_width=True,
        hide_index=True,
    ...
    if breakage_log_all.empty:
        ...
        if day_b.empty:
            ...
            day_b_grouped = day_b.groupby("الخط")[["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "هالك (KG)"]].sum()
            ...
            for ln in all_lines_clean_b:
                ...
                waste_v = day_b_grouped.loc[ln, "هالك (KG)"]
                total_v = opening + out_v
                remaining_v = total_v - in_v
                prod_v = day_production.get(ln, 0)
                # نسبة الكسر بتتحسب من الكسر الخارج بس (مش الداخل ولا الهالك) — وبنحسب نسبة الداخل لوحدها كمان
                pct_v = (out_v / prod_v * 100) if prod_v > 0 else 0
                in_pct_v = (in_v / prod_v * 100) if prod_v > 0 else 0
                line_rows_b.append({
                    "name": ln, "opening": opening, "out": out_v, "in": in_v, "waste": waste_v,
                    "total": total_v, "remaining": remaining_v, "pct": pct_v,
                    "production": prod_v, "in_pct": in_pct_v,
                })

            tot_opening = sum(r["opening"] for r in line_rows_b)
            tot_out = sum(r["out"] for r in line_rows_b)
            tot_in = sum(r["in"] for r in line_rows_b)
            tot_waste = sum(r["waste"] for r in line_rows_b)
            ...
            tot_waste_pct = (tot_waste / total_prod_day * 100) if total_prod_day > 0 else 0
            ...
            totals_b = {
                ...
                "waste": tot_waste, "waste_pct": tot_waste_pct,
            ...
            breakage_html = build_breakage_report_html(
                ...
                breakage_date_str, report_title=f"تحليل الكسر لخطوط إنتاج {breakage_factory_pick}",
                page_height_mm=breakage_page_height_mm
            )
            ...
            st.download_button(
                ...
                file_name=f"breakage_report_{breakage_factory_pick}_{breakage_date_str}.html",
                mime="text/html",
                help="افتح الملف بعد التحميل، ودوس Ctrl+P واختار 'Save as PDF' عشان تحوله PDF أو تطبعه"
            )

    # ==========================
    # تقرير الكسر المجمّع — تلات المصانع، للأدمن بس (نفس التقرير بالظبط، بس مجمّع وتحت كل كرت تفصيل كل مصنع بخط خفيف)
    # ==========================
    st.divider()
    st.markdown("#### 📊 تقرير الكسر المجمّع — تلات المصانع")
    st.caption("قسم التقرير المجمّع ده للأدمن بس")

    if "breakage_admin_ok" not in st.session_state:
        st.session_state.breakage_admin_ok = False

    if not st.session_state.breakage_admin_ok:
        admin_pw_breakage = st.text_input("كلمة مرور الأدمن", type="password", key="breakage_admin_pw")
        if st.button("دخول كأدمن", key="breakage_admin_unlock_btn"):
            if admin_pw_breakage == BREAKAGE_ADMIN_PASSWORD:
                st.session_state.breakage_admin_ok = True
                st.rerun()
            else:
                st.error("كلمة المرور غلط")
    else:
        combined_breakage_date = st.date_input("📅 اختر اليوم", value=datetime.now(), key="breakage_combined_date_pick")
        combined_breakage_date_str = combined_breakage_date.strftime("%Y-%m-%d")

        def _load_factory_breakage_data(f_name, f_conf_b, date_str):
            suffix_b = f_conf_b["sheet_suffix"]
            lines_b = f_conf_b["lines"]
            sheet_name_b = f"breakage_history{suffix_b}"

            day_prod_b = {ln: 0.0 for ln in lines_b}
            daily_prod_totals_x = {}
            prod_suffix_x = f_conf_b.get("production_sheet_suffix")
            prod_sheet_x = f"production_history_v2{prod_suffix_x}" if prod_suffix_x is not None else None
            if f_conf_b["has_production_data"] and prod_sheet_x and GSHEETS_ENABLED and gsheet_exists(prod_sheet_x):
                hist_df_x = read_gsheet(prod_sheet_x)
                hist_df_x["Date"] = hist_df_x["Date"].astype(str)
                all_shifts_x = hist_df_x[hist_df_x["Shift"] == "All Shifts"].copy()
                all_shifts_x["الفعلي (KG)"] = pd.to_numeric(all_shifts_x["الفعلي (KG)"], errors="coerce").fillna(0)
                daily_prod_totals_x = all_shifts_x.groupby("Date")["الفعلي (KG)"].sum().to_dict()
                day_hist_x = all_shifts_x[all_shifts_x["Date"] == date_str].copy()
                if not day_hist_x.empty:
                    day_hist_x["Line"] = day_hist_x["Line"].astype(str).str.strip()
                    prod_by_line_x = day_hist_x.groupby("Line")["الفعلي (KG)"].sum()
                    name_lookup_x = _build_line_name_lookup(lines_b, prod_by_line_x.index)
                    for src_name, target_line in name_lookup_x.items():
                        day_prod_b[target_line] += float(prod_by_line_x[src_name])

            breakage_log_x = pd.DataFrame()
            if GSHEETS_ENABLED and gsheet_exists(sheet_name_b):
                breakage_log_x = read_gsheet(sheet_name_b)
                needed_x = {"Date", "الخط", "رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)"}
                if needed_x.issubset(set(breakage_log_x.columns)):
                    breakage_log_x["Date"] = breakage_log_x["Date"].astype(str)
                    if "هالك (KG)" not in breakage_log_x.columns:
                        breakage_log_x["هالك (KG)"] = 0
                    for nc in ["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "هالك (KG)"]:
                        breakage_log_x[nc] = pd.to_numeric(breakage_log_x[nc], errors="coerce").fillna(0)
                    breakage_log_x["الخط"] = breakage_log_x["الخط"].astype(str).str.strip()
                else:
                    breakage_log_x = pd.DataFrame()

            if not breakage_log_x.empty:
                day_b_x = breakage_log_x[breakage_log_x["Date"] == date_str]
                day_b_grouped_x = day_b_x.groupby("الخط")[["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "هالك (KG)"]].sum()
                day_b_grouped_x = day_b_grouped_x.reindex(lines_b, fill_value=0)
            else:
                day_b_grouped_x = pd.DataFrame(0, index=lines_b, columns=["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "هالك (KG)"])

            rows_x = []
            for ln in lines_b:
                opening_x = day_b_grouped_x.loc[ln, "رصيد أول المدة (KG)"]
                out_x = day_b_grouped_x.loc[ln, "كسر خارج (KG)"]
                in_x = day_b_grouped_x.loc[ln, "كسر داخل (KG)"]
                waste_x = day_b_grouped_x.loc[ln, "هالك (KG)"]
                total_x = opening_x + out_x
                remaining_x = total_x - in_x
                prod_x = day_prod_b.get(ln, 0)
                pct_x = (out_x / prod_x * 100) if prod_x > 0 else 0
                in_pct_x = (in_x / prod_x * 100) if prod_x > 0 else 0
                rows_x.append({
                    "name": ln, "opening": opening_x, "out": out_x, "in": in_x, "waste": waste_x,
                    "total": total_x, "remaining": remaining_x, "pct": pct_x,
                    "production": prod_x, "in_pct": in_pct_x,
                })
            total_prod_day_x = sum(day_prod_b.values())
            return rows_x, total_prod_day_x, breakage_log_x, daily_prod_totals_x

        factory_data_c = {
            f_name: dict(zip(
                ["line_rows", "total_prod", "log", "daily_prod_totals"],
                _load_factory_breakage_data(f_name, f_conf_b, combined_breakage_date_str)
            ))
            for f_name, f_conf_b in BREAKAGE_FACTORY_CONFIG.items()
        }

        combined_line_rows_b = []
        for f_name in BREAKAGE_FACTORY_CONFIG:
            combined_line_rows_b += factory_data_c[f_name]["line_rows"]

        combined_total_prod_day = sum(factory_data_c[f]["total_prod"] for f in factory_data_c)
        tot_out_c = sum(r["out"] for r in combined_line_rows_b)
        tot_in_c = sum(r["in"] for r in combined_line_rows_b)
        tot_waste_c = sum(r["waste"] for r in combined_line_rows_b)
        tot_remaining_c = sum(r["remaining"] for r in combined_line_rows_b)
        tot_break_pct_c = (tot_out_c / combined_total_prod_day * 100) if combined_total_prod_day > 0 else 0
        tot_in_pct_c = (tot_in_c / combined_total_prod_day * 100) if combined_total_prod_day > 0 else 0
        tot_waste_pct_c = (tot_waste_c / combined_total_prod_day * 100) if combined_total_prod_day > 0 else 0
        tot_out_kgton_c = (tot_out_c / combined_total_prod_day * 1000) if combined_total_prod_day > 0 else 0
        tot_in_kgton_c = (tot_in_c / combined_total_prod_day * 1000) if combined_total_prod_day > 0 else 0
        tot_efficiency_c = max(0, 100 - tot_break_pct_c)

        totals_combined_b = {
            "out": tot_out_c, "out_pct": tot_break_pct_c, "out_kgton": tot_out_kgton_c,
            "in": tot_in_c, "in_pct": tot_in_pct_c, "in_kgton": tot_in_kgton_c,
            "waste": tot_waste_c, "waste_pct": tot_waste_pct_c,
            "remaining": tot_remaining_c, "production": combined_total_prod_day,
            "break_pct": tot_break_pct_c, "efficiency": tot_efficiency_c,
        }

        def _factory_metric_totals(f_name):
            rows_f = factory_data_c[f_name]["line_rows"]
            prod_f = factory_data_c[f_name]["total_prod"]
            out_f = sum(r["out"] for r in rows_f)
            in_f = sum(r["in"] for r in rows_f)
            waste_f = sum(r["waste"] for r in rows_f)
            remaining_f = sum(r["remaining"] for r in rows_f)
            break_pct_f = (out_f / prod_f * 100) if prod_f > 0 else 0
            efficiency_f = max(0, 100 - break_pct_f)
            return out_f, in_f, waste_f, remaining_f, prod_f, break_pct_f, efficiency_f

        parts_out, parts_in, parts_waste, parts_remaining, parts_prod, parts_breakpct, parts_eff = [], [], [], [], [], [], []
        for f_name in BREAKAGE_FACTORY_CONFIG:
            o_f, i_f, w_f, r_f, p_f, bp_f, ef_f = _factory_metric_totals(f_name)
            parts_out.append(f"{f_name}: {o_f:,.0f}")
            parts_in.append(f"{f_name}: {i_f:,.0f}")
            parts_waste.append(f"{f_name}: {w_f:,.0f}")
            parts_remaining.append(f"{f_name}: {r_f:,.0f}")
            parts_prod.append(f"{f_name}: {p_f:,.0f}")
            parts_breakpct.append(f"{f_name}: {bp_f:.2f}%")
            parts_eff.append(f"{f_name}: {ef_f:.2f}%")

        factory_breakdown_b = {
            "out": " | ".join(parts_out),
            "in": " | ".join(parts_in),
            "waste": " | ".join(parts_waste),
            "remaining": " | ".join(parts_remaining),
            "production": " | ".join(parts_prod),
            "break_pct": " | ".join(parts_breakpct),
            "efficiency": " | ".join(parts_eff),
        }

        # ---------- اتجاه نسبة الكسر (داخل/خارج) آخر 7 أيام — مجمّع لتلات المصانع ----------
        trend_dates_c = [(combined_breakage_date - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(6, -1, -1)]
        trend_out_pct_c, trend_in_pct_c = [], []

        logs_nonempty_c = [factory_data_c[f]["log"] for f in factory_data_c if not factory_data_c[f]["log"].empty]
        combined_daily_totals_c = pd.DataFrame()
        if logs_nonempty_c:
            all_logs_c = pd.concat(logs_nonempty_c, ignore_index=True)
            combined_daily_totals_c = all_logs_c.groupby("Date")[["كسر خارج (KG)", "كسر داخل (KG)"]].sum()

        combined_daily_prod_totals_c = {}
        for f_name in factory_data_c:
            for d, v in factory_data_c[f_name]["daily_prod_totals"].items():
                combined_daily_prod_totals_c[d] = combined_daily_prod_totals_c.get(d, 0) + v

        for d in trend_dates_c:
            d_prod_c = combined_daily_prod_totals_c.get(d, 0)
            if not combined_daily_totals_c.empty and d in combined_daily_totals_c.index and d_prod_c > 0:
                trend_out_pct_c.append(round(float(combined_daily_totals_c.loc[d, "كسر خارج (KG)"]) / d_prod_c * 100, 2))
                trend_in_pct_c.append(round(float(combined_daily_totals_c.loc[d, "كسر داخل (KG)"]) / d_prod_c * 100, 2))
            else:
                trend_out_pct_c.append(0.0)
                trend_in_pct_c.append(0.0)

        if not any(r["opening"] or r["out"] or r["in"] or r["waste"] for r in combined_line_rows_b):
            st.info("لسه مفيش بيانات كسر محفوظة لنفس اليوم من أي مصنع")
        else:
            num_lines_c = len(combined_line_rows_b)
            combined_height_mm = 130 + (num_lines_c * 8) + 90 + 75 + 30

            combined_breakage_html = build_breakage_report_html(
                totals_combined_b, combined_line_rows_b, trend_dates_c, trend_out_pct_c, trend_in_pct_c,
                combined_breakage_date_str, report_title="تحليل الكسر لخطوط إنتاج المصانع الثلاثة",
                page_height_mm=combined_height_mm, factory_breakdown=factory_breakdown_b
            )
            components.html(combined_breakage_html, height=1200, scrolling=False)

            st.download_button(
                "🖨️ تحميل التقرير المجمّع (HTML) — يتفتح ويتطبع أو يتحفظ PDF",
                data=combined_breakage_html.encode("utf-8"),
                file_name=f"breakage_report_combined_{combined_breakage_date_str}.html",
                mime="text/html",
                help="افتح الملف بعد التحميل، ودوس Ctrl+P واختار 'Save as PDF' عشان تحوله PDF أو تطبعه",
                key="breakage_combined_download_btn"
            )

elif page == "Energy Management":
    page_banner("⚡", "Energy Management", "متابعة استهلاك الكهرباء والغاز والمياه لكل مصنع، ومقارنتها بالتارجت والإنتاج")

    ENERGY_PASSWORD = "energy2026"
    if "energy_unlocked" not in st.session_state:
        st.session_state.energy_unlocked = False
    if not st.session_state.energy_unlocked:
        st.subheader("🔒 صفحة محمية")
        pw_input_e = st.text_input("كلمة مرور الصفحة", type="password", key="energy_page_pw")
        if st.button("دخول", key="energy_page_unlock_btn"):
            if pw_input_e == ENERGY_PASSWORD:
                st.session_state.energy_unlocked = True
                st.rerun()
            else:
                st.error("كلمة المرور غلط")
        st.stop()
    ...
    # نفس تقسيم المصانع المستخدم في صفحتي الأعطال والباكينج بالظبط — بيانات كل مصنع منفصلة عن التانية
    ENERGY_FACTORY_CONFIG = {
        "مصنع 1": {"sheet_suffix": "", "packing_suffix": ""},
        "مصنع 2": {"sheet_suffix": "_m2", "packing_suffix": "_m2"},
        "مصنع 3": {"sheet_suffix": "_m3", "packing_suffix": "_m3"},
    }
    ENERGY_UTILITIES = [
        {"key": "الكهرباء", "unit": "ك.و.س", "color": "#e0a92e", "icon": "⚡"},
        {"key": "الغاز", "unit": "م³", "color": "#dc2626", "icon": "🔥"},
        {"key": "المياه", "unit": "م³", "color": "#2563eb", "icon": "💧"},
    ]
    ENERGY_TARGETS_SHEET = "energy_targets"

    energy_factory_pick = st.radio(
        "🏭 اختار المصنع", list(ENERGY_FACTORY_CONFIG.keys()), horizontal=True, key="energy_factory_pick"
    )
    e_suffix = ENERGY_FACTORY_CONFIG[energy_factory_pick]["sheet_suffix"]
    e_pack_suffix = ENERGY_FACTORY_CONFIG[energy_factory_pick]["packing_suffix"]

    today_e = datetime.now().date()
    month_options_e = []
    for i in range(-6, 7):
        y = today_e.year + (today_e.month - 1 + i) // 12
        m = (today_e.month - 1 + i) % 12 + 1
        month_options_e.append(f"{y}-{m:02d}")
    default_index_e = month_options_e.index(today_e.strftime("%Y-%m"))
    selected_month_e = st.selectbox("📅 اختر الشهر", month_options_e, index=default_index_e, key="energy_month_select")

    year_e, month_num_e = map(int, selected_month_e.split("-"))
    days_in_month_e = calendar.monthrange(year_e, month_num_e)[1]

    def _load_targets_df():
        if GSHEETS_ENABLED and gsheet_exists(ENERGY_TARGETS_SHEET):
            return read_gsheet(ENERGY_TARGETS_SHEET)
        return pd.DataFrame(columns=[
            "الشهر", "المصنع", "تارجت الكهرباء اليومي", "تارجت الغاز اليومي", "تارجت المياه اليومي", "أيام العمل"
        ])

    def _get_target_field(t_df, month_str, factory_name, col, default=0.0):
        match = t_df[(t_df["الشهر"].astype(str) == month_str) & (t_df["المصنع"].astype(str) == factory_name)]
        if not match.empty:
            v = pd.to_numeric(match.iloc[0].get(col, default), errors="coerce")
            return float(v) if pd.notna(v) else default
        return default

    def _get_daily_targets(t_df, month_str, factory_name):
        """بيرجع تارجت يومي لكل مادة + عدد أيام التشغيل بتاعة المصنع ده الشهر ده"""
        daily = {
            u["key"]: _get_target_field(t_df, month_str, factory_name, f"تارجت {u['key']} اليومي")
            for u in ENERGY_UTILITIES
        }
        workdays = _get_target_field(t_df, month_str, factory_name, "أيام العمل", default=0.0)
        return daily, workdays

    def _load_energy_grid(factory_name, suffix, month_str, n_days):
        """بيحمل شيت قراءات المصنع ده للشهر ده، أو يبني جدول فاضي بعدد أيام الشهر،
        وبيطبّق التسلسل التلقائي: قراءة أول اليوم = قراءة آخر اليوم اللي قبله لو لسه متسجلتش"""
        sheet_name = f"energy_monthly_{month_str}{suffix}"
        cols = ["اليوم"]
        for u in ENERGY_UTILITIES:
            cols += [f"{u['key']} - أول اليوم", f"{u['key']} - آخر اليوم"]

        if GSHEETS_ENABLED and gsheet_exists(sheet_name):
            grid = read_gsheet(sheet_name)
            for c in cols:
                if c not in grid.columns:
                    grid[c] = 0
            grid = grid[cols]
        else:
            grid = pd.DataFrame({"اليوم": list(range(1, n_days + 1))})
            for c in cols[1:]:
                grid[c] = 0.0

        grid["اليوم"] = pd.to_numeric(grid["اليوم"], errors="coerce").fillna(0).astype(int)
        for c in cols[1:]:
            grid[c] = pd.to_numeric(grid[c], errors="coerce").fillna(0)
        full_days_df = pd.DataFrame({"اليوم": list(range(1, n_days + 1))})
        grid = full_days_df.merge(grid, on="اليوم", how="left").fillna(0)
        grid = grid.sort_values("اليوم").reset_index(drop=True)

        for u in ENERGY_UTILITIES:
            start_c, end_c = f"{u['key']} - أول اليوم", f"{u['key']} - آخر اليوم"
            for i in range(1, len(grid)):
                if grid.loc[i, start_c] == 0 and grid.loc[i - 1, end_c] > 0:
                    grid.loc[i, start_c] = grid.loc[i - 1, end_c]
        return grid, sheet_name

    def _compute_energy(grid, factory_name, pack_suffix, month_str, n_days):
        """بيحسب الاستهلاك اليومي لكل مادة، وبيربطها بإنتاج نفس اليوم (كراتين وطن) من شيت الباكينج بتاع نفس المصنع"""
        calc = grid.copy()
        for u in ENERGY_UTILITIES:
            start_c, end_c, cons_c = f"{u['key']} - أول اليوم", f"{u['key']} - آخر اليوم", f"استهلاك {u['key']}"
            calc[cons_c] = np.where(
                (calc[end_c] > 0) & (calc[end_c] >= calc[start_c]), calc[end_c] - calc[start_c], 0.0
            )

        pack_sheet = f"packing_monthly_{month_str}{pack_suffix}"
        prod_cartons = pd.Series(0.0, index=range(1, n_days + 1))
        prod_tons = pd.Series(0.0, index=range(1, n_days + 1))
        if GSHEETS_ENABLED and gsheet_exists(pack_sheet):
            pack_df = read_gsheet(pack_sheet)
            if "وزن الكرتونة (كيلو)" in pack_df.columns:
                pack_df["وزن الكرتونة (كيلو)"] = pd.to_numeric(pack_df["وزن الكرتونة (كيلو)"], errors="coerce").fillna(0)
                for d in range(1, n_days + 1):
                    dc = f"يوم {d}"
                    if dc in pack_df.columns:
                        cartons_d = pd.to_numeric(pack_df[dc], errors="coerce").fillna(0)
                        prod_cartons.loc[d] = cartons_d.sum()
                        prod_tons.loc[d] = (cartons_d * pack_df["وزن الكرتونة (كيلو)"]).sum() / 1000

        calc["الإنتاج (كراتين)"] = calc["اليوم"].map(prod_cartons).fillna(0)
        calc["الإنتاج (طن)"] = calc["اليوم"].map(prod_tons).fillna(0)
        for u in ENERGY_UTILITIES:
            cons_c = f"استهلاك {u['key']}"
            calc[f"{u['key']} / كرتونة"] = np.where(
                calc["الإنتاج (كراتين)"] > 0, calc[cons_c] / calc["الإنتاج (كراتين)"], 0
            ).round(3)
            calc[f"{u['key']} / طن"] = np.where(
                calc["الإنتاج (طن)"] > 0, calc[cons_c] / calc["الإنتاج (طن)"], 0
            ).round(2)
        return calc

    def _energy_trend_fig(calc, factory_name, daily_targets_map, n_days, figsize=(8, 3.2), title_size=12):
        figs = {}
        for u in ENERGY_UTILITIES:
            cons_series = calc[f"استهلاك {u['key']}"]
            daily_target = daily_targets_map.get(u["key"], 0)
            fig_e, ax_e = plt.subplots(figsize=figsize)
            fig_e.patch.set_facecolor("#ffffff")
            ax_e.plot(calc["اليوم"], cons_series, marker="o", linewidth=2.2, color=u["color"],
                      markerfacecolor="#ffffff", markeredgecolor=u["color"], markeredgewidth=1.6, markersize=5,
                      zorder=3, label=ar_text(f"الفعلي ({u['unit']})"))
            ax_e.fill_between(calc["اليوم"], cons_series, color=u["color"], alpha=0.08, zorder=2)
            if daily_target > 0:
                ax_e.axhline(daily_target, color="#111827", linestyle="--", linewidth=1.6, label=ar_text("التارجت اليومي"))
            ax_e.set_xticks(calc["اليوم"][::max(1, n_days // 10)])
            ax_e.tick_params(axis="x", labelsize=7)
            ax_e.tick_params(axis="y", labelsize=8)
            ax_e.spines["top"].set_visible(False)
            ax_e.spines["right"].set_visible(False)
            ax_e.grid(axis="y", linestyle="--", alpha=0.4, color="#c9cfd6", zorder=0)
            ax_e.set_axisbelow(True)
            ax_e.legend(fontsize=8, loc="upper left", frameon=False)
            ax_e.set_title(ar_text(f"استهلاك {u['key']} اليومي — {factory_name}"),
                            fontsize=title_size, fontweight="bold", color=BRAND_GREEN, pad=10)
            fig_e.tight_layout()
            figs[u["key"]] = fig_e
        return figs

    def _build_energy_cards(values, targets_map, cartons, tons, period_label, prices_map=None):
        """كروت KPI مختصرة: كرت إجمالي (وتحته % التحقيق ملوّن أخضر/أحمر) وكرت لكل وحدة إنتاج (كرتونة وتحتها طن) — لكل مادة، بدون ما تكتر الكروت.
        لو فيه سعر وحدة (prices_map) بيضيف سطر خفيف تحت كرت الوحدة بتكلفة الطاقة لكل كرتونة وطن"""
        cards = []
        for u in ENERGY_UTILITIES:
            actual = values.get(u["key"], 0.0)
            tgt = targets_map.get(u["key"], 0.0)
            pct = (actual / tgt * 100) if tgt > 0 else 0
            good = (tgt == 0) or (pct <= 100)
            pct_color = "#16a34a" if good else "#dc2626"
            per_carton = (actual / cartons) if cartons > 0 else 0
            per_ton = (actual / tons) if tons > 0 else 0

            cards.append({
                "icon": u["icon"],
                "label": f"إجمالي {u['key']} {period_label} ({u['unit']})",
                "value": f"{actual:,.1f}",
                "color": u["color"],
                "delta": f"{'✅' if good else '⚠️'} {pct:,.1f}% من التارجت",
                "delta_color": pct_color,
            })
            unit_card = {
                "icon": "📦",
                "label": f"{u['key']} / كرتونة",
                "value": f"{per_carton:,.3f}",
                "color": u["color"],
                "delta": f"🏋️ / طن: {per_ton:,.2f}",
                "delta_color": "#6b7280",
            }
            price = (prices_map or {}).get(u["key"], 0.0)
            if price > 0:
                cost_carton = per_carton * price
                cost_ton = per_ton * price
                unit_card["extra"] = f"💰 التكلفة: {cost_carton:,.2f} ج/كرتونة — {cost_ton:,.1f} ج/طن"
            cards.append(unit_card)
        return cards

    def _energy_cards_html(cards):
        """بيحول قائمة كروت (زي اللي بترجع من _build_energy_cards) لـ HTML جاهز للتقرير القابل للطباعة"""
        parts = []
        for c in cards:
            delta_html = ""
            if c.get("delta"):
                d_color = c.get("delta_color", "#16a34a")
                delta_html = f'<div style="font-size:11px; font-weight:800; color:{d_color}; margin-top:3px;">{c["delta"]}</div>'
            extra_html = ""
            if c.get("extra"):
                extra_html = f'<div style="font-size:9.5px; font-weight:400; color:#9ca3af; margin-top:2px;">{c["extra"]}</div>'
            parts.append(f"""<div style="flex:1; min-width:150px; border:1px solid #e5e7eb; border-radius:8px; padding:8px 10px; margin:3px;">
                <div style="font-size:11px; color:#6b7280;">{c['icon']} {c['label']}</div>
    ...
    def _combined_calc_and_totals(factory_calcs, day, n_days):
        """بيجمع بيانات كذا مصنع مع بعض: سلسلة الشهر كله (للرسم) + قيم اليوم المحدد (للكروت)"""
        combined_trend = pd.DataFrame({"اليوم": list(range(1, n_days + 1))})
        day_values = {}
        for u in ENERGY_UTILITIES:
            col = f"استهلاك {u['key']}"
            total_series = pd.Series(0.0, index=range(1, n_days + 1))
            for c in factory_calcs:
                total_series = total_series.add(c.set_index("اليوم")[col], fill_value=0)
            combined_trend[col] = combined_trend["اليوم"].map(total_series).fillna(0)
            day_values[u["key"]] = float(total_series.get(day, 0.0))

        cartons_series = pd.Series(0.0, index=range(1, n_days + 1))
        tons_series = pd.Series(0.0, index=range(1, n_days + 1))
        for c in factory_calcs:
            cartons_series = cartons_series.add(c.set_index("اليوم")["الإنتاج (كراتين)"], fill_value=0)
            tons_series = tons_series.add(c.set_index("اليوم")["الإنتاج (طن)"], fill_value=0)
        combined_trend["الإنتاج (كراتين)"] = combined_trend["اليوم"].map(cartons_series).fillna(0)
        combined_trend["الإنتاج (طن)"] = combined_trend["اليوم"].map(tons_series).fillna(0)
        cartons_day = float(cartons_series.get(day, 0.0))
        tons_day = float(tons_series.get(day, 0.0))
        return combined_trend, day_values, cartons_day, tons_day
        combined_trend = pd.DataFrame({"اليوم": list(range(1, n_days + 1))})
        day_values = {}
        for u in ENERGY_UTILITIES:
            col = f"استهلاك {u['key']}"
    ...
    # ==========================
    # تسجيل قراءات المصنع المختار
    # ==========================
    energy_grid, ENERGY_SHEET = _load_energy_grid(energy_factory_pick, e_suffix, selected_month_e, days_in_month_e)

    st.markdown(f"#### 📋 تسجيل قراءات العدادات — {energy_factory_pick} — شهر {selected_month_e}")
    st.caption(
        "سجّل قراءة العداد أول ما تفتحه الصبح وآخر ما تقفله بالليل — استهلاك اليوم بيتحسب لوحده تلقائي، "
        "وقراءة 'أول اليوم' بتاعة بكرة بتتسحب تلقائي من قراءة 'آخر اليوم' بتاعة النهاردة. "
        "كل حاجة بتتسجل شهري لوحدها في شيت المصنع ده تحت"
    )

    utility_by_key = {u["key"]: u for u in ENERGY_UTILITIES}
    tabs_order = ["المياه", "الكهرباء", "الغاز"]
    reg_tabs = st.tabs([f"{utility_by_key[k]['icon']} استهلاك {k}" for k in tabs_order])

    merged_edit_df = energy_grid[["اليوم"]].copy()
    for tab, k in zip(reg_tabs, tabs_order):
        u = utility_by_key[k]
        with tab:
            start_c, end_c = f"{k} - أول اليوم", f"{k} - آخر اليوم"
            sub_df = energy_grid[["اليوم", start_c, end_c]].copy()
            sub_col_config = {
                "اليوم": st.column_config.NumberColumn("اليوم", disabled=True),
                start_c: st.column_config.NumberColumn(f"{u['icon']} أول اليوم ({u['unit']})", min_value=0.0, format="%.1f"),
                end_c: st.column_config.NumberColumn(f"{u['icon']} آخر اليوم ({u['unit']})", min_value=0.0, format="%.1f"),
            }
            edited_sub = st.data_editor(
                sub_df, use_container_width=True, hide_index=True, num_rows="fixed",
                column_config=sub_col_config,
                key=f"energy_editor_{k}_{energy_factory_pick}_{selected_month_e}"
            )
            merged_edit_df[start_c] = edited_sub[start_c]
            merged_edit_df[end_c] = edited_sub[end_c]

    edited_energy = merged_edit_df

    if st.button("💾 حفظ القراءات", key="energy_save_btn"):
        write_gsheet(ENERGY_SHEET, edited_energy)
        st.success(f"اتحفظت قراءات {energy_factory_pick} لشهر {selected_month_e} ✔")
        st.rerun()

    energy_calc = _compute_energy(edited_energy, energy_factory_pick, e_pack_suffix, selected_month_e, days_in_month_e)

    st.markdown("##### 📊 الاستهلاك اليومي ومربوط بالإنتاج")
    display_cols_e = ["اليوم"]
    for u in ENERGY_UTILITIES:
        display_cols_e += [f"استهلاك {u['key']}", f"{u['key']} / كرتونة", f"{u['key']} / طن"]
    display_cols_e += ["الإنتاج (كراتين)", "الإنتاج (طن)"]
    st.dataframe(energy_calc[display_cols_e], use_container_width=True, hide_index=True)

    # ==========================
    # التارجت اليومي لكل مصنع
    # ==========================
    st.divider()
    st.markdown("##### 🎯 التارجت اليومي")
    st.caption(
        "سجّل التارجت اليومي وعدد أيام التشغيل بالشهر — البرنامج بيحسب التارجت الشهري لوحده "
        "(التارجت اليومي × عدد أيام التشغيل)"
    )

    targets_df = _load_targets_df()
    daily_targets_saved, workdays_saved = _get_daily_targets(targets_df, selected_month_e, energy_factory_pick)

    tcol1, tcol2, tcol3, tcol4 = st.columns(4)
    target_elec_daily = tcol1.number_input(
        "⚡ تارجت الكهرباء اليومي (ك.و.س)", min_value=0.0,
        value=daily_targets_saved.get("الكهرباء", 0.0), step=10.0, key="energy_target_elec_daily"
    )
    target_gas_daily = tcol2.number_input(
        "🔥 تارجت الغاز اليومي (م³)", min_value=0.0,
        value=daily_targets_saved.get("الغاز", 0.0), step=10.0, key="energy_target_gas_daily"
    )
    target_water_daily = tcol3.number_input(
        "💧 تارجت المياه اليومي (م³)", min_value=0.0,
        value=daily_targets_saved.get("المياه", 0.0), step=10.0, key="energy_target_water_daily"
    )
    workdays_e = tcol4.number_input(
        "📆 عدد أيام التشغيل بالشهر", min_value=0, max_value=31,
        value=int(workdays_saved) if workdays_saved else days_in_month_e, step=1, key="energy_workdays"
    )

    monthly_elec = target_elec_daily * workdays_e
    monthly_gas = target_gas_daily * workdays_e
    monthly_water = target_water_daily * workdays_e
    st.caption(
        f"📌 التارجت الشهري المحسوب: ⚡ {monthly_elec:,.0f} ك.و.س — 🔥 {monthly_gas:,.0f} م³ — 💧 {monthly_water:,.0f} م³ "
        f"(= التارجت اليومي × {workdays_e:.0f} يوم تشغيل)"
    )

    if st.button("💾 حفظ التارجت", key="energy_target_save_btn"):
        targets_df = targets_df[
            ~((targets_df["الشهر"].astype(str) == selected_month_e) & (targets_df["المصنع"].astype(str) == energy_factory_pick))
        ]
        new_target_row = pd.DataFrame([{
            "الشهر": selected_month_e, "المصنع": energy_factory_pick,
            "تارجت الكهرباء اليومي": target_elec_daily, "تارجت الغاز اليومي": target_gas_daily,
            "تارجت المياه اليومي": target_water_daily, "أيام العمل": workdays_e,
        }])
        targets_df = pd.concat([targets_df, new_target_row], ignore_index=True)
        write_gsheet(ENERGY_TARGETS_SHEET, targets_df)
        st.success("اتحفظ التارجت ✔")
        st.rerun()

    daily_targets_map_e = {"الكهرباء": target_elec_daily, "الغاز": target_gas_daily, "المياه": target_water_daily}
    monthly_targets_map_e = {"الكهرباء": monthly_elec, "الغاز": monthly_gas, "المياه": monthly_water}

    # ==========================
    # تسعيرة الوحدات — لحساب تكلفة الطاقة لكل كرتونة وطن
    # ==========================
    st.divider()
    st.markdown("##### 💰 تسعيرة الوحدات")
    st.caption("سعر الوحدة (كيلو وات / متر مكعب) بيتسجل شهري، وبيستخدم لحساب تكلفة الطاقة لكل كرتونة وطن جوه الكروت والتقرير")

    ENERGY_PRICES_SHEET = "energy_prices"

    def _load_prices_df():
        if GSHEETS_ENABLED and gsheet_exists(ENERGY_PRICES_SHEET):
            return read_gsheet(ENERGY_PRICES_SHEET)
        return pd.DataFrame(columns=["الشهر", "سعر الكهرباء", "سعر الغاز", "سعر المياه"])

    def _get_price_field(p_df, month_str, col):
        match = p_df[p_df["الشهر"].astype(str) == month_str]
        if not match.empty:
            v = pd.to_numeric(match.iloc[0].get(col, 0), errors="coerce")
            return float(v) if pd.notna(v) else 0.0
        return 0.0

    prices_df = _load_prices_df()
    pcol1, pcol2, pcol3 = st.columns(3)
    price_elec = pcol1.number_input(
        "⚡ سعر الكيلو وات (كهرباء)", min_value=0.0,
        value=_get_price_field(prices_df, selected_month_e, "سعر الكهرباء"),
        step=0.05, format="%.3f", key="energy_price_elec"
    )
    price_gas = pcol2.number_input(
        "🔥 سعر وحدة الغاز (م³)", min_value=0.0,
        value=_get_price_field(prices_df, selected_month_e, "سعر الغاز"),
        step=0.05, format="%.3f", key="energy_price_gas"
    )
    price_water = pcol3.number_input(
        "💧 سعر وحدة المياه (م³)", min_value=0.0,
        value=_get_price_field(prices_df, selected_month_e, "سعر المياه"),
        step=0.05, format="%.3f", key="energy_price_water"
    )

    if st.button("💾 حفظ التسعيرة", key="energy_price_save_btn"):
        prices_df = prices_df[prices_df["الشهر"].astype(str) != selected_month_e]
        new_price_row = pd.DataFrame([{
            "الشهر": selected_month_e, "سعر الكهرباء": price_elec, "سعر الغاز": price_gas, "سعر المياه": price_water,
        }])
        prices_df = pd.concat([prices_df, new_price_row], ignore_index=True)
        write_gsheet(ENERGY_PRICES_SHEET, prices_df)
        st.success("اتحفظت التسعيرة ✔")
        st.rerun()

    prices_map_e = {"الكهرباء": price_elec, "الغاز": price_gas, "المياه": price_water}

    # ==========================
    # KPIs ورسوم بيانية للمصنع المختار (تراكمي للشهر)
    # ==========================
    st.divider()
    st.markdown(f"##### 📈 KPIs (تراكمي للشهر) — {energy_factory_pick} — شهر {selected_month_e}")
    month_values_e = {u["key"]: energy_calc[f"استهلاك {u['key']}"].sum() for u in ENERGY_UTILITIES}
    month_cartons_e = energy_calc["الإنتاج (كراتين)"].sum()
    month_tons_e = energy_calc["الإنتاج (طن)"].sum()
    kpi_row(_build_energy_cards(month_values_e, monthly_targets_map_e, month_cartons_e, month_tons_e, "الشهر", prices_map=prices_map_e))

    st.markdown("##### 📉 اتجاه الاستهلاك اليومي مقابل التارجت اليومي")
    energy_figs = _energy_trend_fig(energy_calc, energy_factory_pick, daily_targets_map_e, days_in_month_e)
    energy_chart_tabs = st.tabs([f"{u['icon']} {u['key']}" for u in ENERGY_UTILITIES])
    for tab, u in zip(energy_chart_tabs, ENERGY_UTILITIES):
        with tab:
            st.pyplot(energy_figs[u["key"]])

    # ==========================
    # تقرير الطاقة اليومي — إجمالي المصانع الثلاثة فوق، وتفاصيل كل مصنع تحت (زي تقرير الأعطال بالظبط) — للأدمن بس
    # ==========================
    st.divider()
    st.markdown("#### 🖨️ تقرير الطاقة اليومي")
    st.caption("قسم التقرير والطباعة ده للأدمن بس")

    if "energy_admin_ok" not in st.session_state:
        st.session_state.energy_admin_ok = False

    if not st.session_state.energy_admin_ok:
        admin_pw_e = st.text_input("كلمة مرور الأدمن لعرض التقرير", type="password", key="energy_admin_pw")
        if st.button("دخول كأدمن", key="energy_admin_unlock_btn"):
            if admin_pw_e == "9999":
                st.session_state.energy_admin_ok = True
                st.rerun()
            else:
                st.error("كلمة المرور غلط")
    else:
        st.caption(
            "تقرير ليوم واحد: فوق كروت KPI ورسم بياني لإجمالي المصانع الثلاثة ومربوطين بإجمالي إنتاج المصانع الثلاثة، "
            "وتحته تفاصيل كل مصنع لوحده (زي تقرير الأعطال) مربوطة بإنتاج نفس المصنع"
        )

        report_day_e = st.selectbox(
            "📅 اختار يوم التقرير", list(range(1, days_in_month_e + 1)),
            index=min(today_e.day, days_in_month_e) - 1, key="energy_report_day"
        )

        if st.button("📊 تجهيز تقرير الطاقة اليومي", key="energy_full_report_btn"):
            factory_calcs = {}
            factory_daily_targets = {}
            for f_name, f_conf in ENERGY_FACTORY_CONFIG.items():
                f_grid, _ = _load_energy_grid(f_name, f_conf["sheet_suffix"], selected_month_e, days_in_month_e)
                f_calc = _compute_energy(f_grid, f_name, f_conf["packing_suffix"], selected_month_e, days_in_month_e)
                f_daily_targets, _ = _get_daily_targets(targets_df, selected_month_e, f_name)
                factory_calcs[f_name] = f_calc
                factory_daily_targets[f_name] = f_daily_targets

            # ---------- الجزء العلوي: إجمالي المصانع الثلاثة ----------
            combined_trend, day_values, cartons_day, tons_day = _combined_calc_and_totals(
                list(factory_calcs.values()), report_day_e, days_in_month_e
            )
            combined_daily_targets = {
                u["key"]: sum(factory_daily_targets[f_name].get(u["key"], 0) for f_name in factory_daily_targets)
                for u in ENERGY_UTILITIES
            }
            combined_cards = _build_energy_cards(day_values, combined_daily_targets, cartons_day, tons_day, "اليوم — 3 مصانع", prices_map=prices_map_e)
            combined_cards_html = _energy_cards_html(combined_cards)
            combined_figs = _energy_trend_fig(
                combined_trend, "إجمالي المصانع الثلاثة", combined_daily_targets, days_in_month_e,
                figsize=(4.6, 2.0), title_size=10
            )
            REPORT_CHART_IMG_STYLE = "width:31%; max-width:230px; display:inline-block; margin:3px 1%; vertical-align:top;"
            combined_charts_html = "".join(
                fig_to_base64_img(combined_figs[u["key"]], style=REPORT_CHART_IMG_STYLE) for u in ENERGY_UTILITIES
            )

            def _energy_block(title_html, cards_html, charts_html, page_break=False):
                pb = "page-break-before: always;" if page_break else ""
                return f"""
                <div style="margin-top:12px; page-break-inside:avoid; {pb}">
                    {title_html}
                    <div style="display:flex; flex-wrap:wrap;">{cards_html}</div>
                    <div style="text-align:center; margin-top:6px;">{charts_html}</div>
                </div>
                """

            combined_title_html = f"""<h2 style="background:{BRAND_GREEN}; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:8px;">
                🏭 إجمالي المصانع الثلاثة — يوم {report_day_e}
            </h2>"""

            # ---------- الجزء السفلي: تفاصيل كل مصنع لوحده (زي تقرير الأعطال بالظبط) ----------
            factory_blocks = []
            for f_name, f_conf in ENERGY_FACTORY_CONFIG.items():
                f_calc = factory_calcs[f_name]
                f_daily_targets = factory_daily_targets[f_name]
                row_df = f_calc[f_calc["اليوم"] == report_day_e]
                row = row_df.iloc[0] if not row_df.empty else None
                f_day_values = {u["key"]: (float(row[f"استهلاك {u['key']}"]) if row is not None else 0.0) for u in ENERGY_UTILITIES}
                f_cartons_day = float(row["الإنتاج (كراتين)"]) if row is not None else 0.0
                f_tons_day = float(row["الإنتاج (طن)"]) if row is not None else 0.0
                f_cards = _build_energy_cards(f_day_values, f_daily_targets, f_cartons_day, f_tons_day, "اليوم", prices_map=prices_map_e)
                f_cards_html = _energy_cards_html(f_cards)
                f_figs = _energy_trend_fig(f_calc, f_name, f_daily_targets, days_in_month_e, figsize=(4.6, 2.0), title_size=10)
                f_charts_html = "".join(
                    fig_to_base64_img(f_figs[u["key"]], style=REPORT_CHART_IMG_STYLE) for u in ENERGY_UTILITIES
                )
                f_title_html = f"""<h2 style="background:#f4f7fb; color:{BRAND_GREEN}; padding:6px 10px; border-radius:6px; margin-bottom:8px; border:1px solid {BRAND_GREEN};">
                    🏭 {f_name}
                </h2>"""
                factory_blocks.append((f_title_html, f_cards_html, f_charts_html))

            # ---------- نسخة 1: صفحة واحدة متصلة (للعرض والمشاركة أونلاين) ----------
            continuous_html_body = (
                _energy_block(combined_title_html, combined_cards_html, combined_charts_html)
                + "<h2 style='margin-top:14px;'>📋 تفاصيل كل مصنع لوحده</h2>"
                + "".join(_energy_block(t, c, ch) for t, c, ch in factory_blocks)
            )
            # تقدير سخي لارتفاع الصفحة عشان محتوى الكروت والرسوم كلها تدخل من غير ما تتقسم على أكتر من صفحة
            continuous_height_mm = 100 + 155 * (1 + len(factory_blocks))

            energy_continuous_html = build_printable_html(
                f"تقرير الطاقة اليومي — يوم {report_day_e} — شهر {selected_month_e}",
                f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                pd.DataFrame(),
                landscape=True,
                extra_html=continuous_html_body,
                single_page_height_mm=continuous_height_mm,
            )

            # ---------- نسخة 2: للطباعة الورقية — 4 صفحات (إجمالي + كل مصنع في صفحة) ----------
            print_html_body = (
                _energy_block(combined_title_html, combined_cards_html, combined_charts_html)
                + "".join(
                    _energy_block(t, c, ch, page_break=True) for t, c, ch in factory_blocks
                )
            )
            energy_print_html = build_printable_html(
                f"تقرير الطاقة اليومي — يوم {report_day_e} — شهر {selected_month_e}",
                f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                pd.DataFrame(),
                landscape=True,
                extra_html=print_html_body,
            )

            dl_col1, dl_col2 = st.columns(2)
            dl_col1.download_button(
                "🖨️ تحميل نسخة صفحة واحدة متصلة",
                data=energy_continuous_html.encode("utf-8"),
                file_name=f"energy_report_{selected_month_e}_day{report_day_e}_page.html",
                mime="text/html",
                help="نسخة سكرول متصلة، مناسبة للعرض أو المشاركة أونلاين",
                key="energy_continuous_download_btn"
            )
            dl_col2.download_button(
                "🖨️ تحميل نسخة للطباعة (4 صفحات)",
                data=energy_print_html.encode("utf-8"),
                file_name=f"energy_report_{selected_month_e}_day{report_day_e}_print.html",
                mime="text/html",
                help="صفحة إجمالي المصانع + صفحة لكل مصنع لوحده — افتح الملف ودوس Ctrl+P واختار 'Save as PDF' أو اطبعها على الورق",
                key="energy_print_download_btn"
            )
    ...
    def normalize_shift(v):
        """بتوحّد شكل قيمة الشيفت مهما جت من جوجل شيت (نص، رقم صحيح، رقم عشري زي '1.0'،
        فراغات زيادة، أو None) عشان مقارنة == '1' / '2' / '3' متفشلش وتسبب فرق في الإجماليات."""
        s = str(v).strip()
    ...
    if save_btn:
        ...
        edited_df_norm_shift = edited_df["الشيفت"].apply(normalize_shift)
        shift1 = edited_df[edited_df_norm_shift == "1"]
        shift2 = edited_df[edited_df_norm_shift == "2"]
        shift3 = edited_df[edited_df_norm_shift == "3"]
    ...
    """ + report_signature_html() + """

    """

    edited_df_report_norm = edited_df["الشيفت"].apply(normalize_shift)
        ...
        for i, shift in enumerate(["1", "2", "3"]):
            ...
            for line_name, df_line in all_lines_data.items():
                ...
                if line_name == "السولا" and shift != "1":
                    continue
                names = df_line[df_line["الشيفت"].apply(normalize_shift) == shift]["الاسم"].tolist()
            ...
            combined_html += report_signature_html()
    ...
    if st.button("📊 توليد شيت الشؤون"):
        ...
        HDR_FILL = PatternFill("solid", fgColor="013E37")   # أخضر البراند
        TOTAL_FILL = PatternFill("solid", fgColor="E0A92E")  # دهبي البراند
        STRIPE_FILL = PatternFill("solid", fgColor="F2F2F2")  # رمادي فاتح للصفوف الزوجية
        HDR_FONT = Font(bold=True, color="FFFFFF", size=11)
        TOTAL_FONT = Font(bold=True, color="013E37", size=11)
        CENTER = Alignment(horizontal="center", vertical="center", wrap_text=True)
        THIN = Side(style="thin", color="BBBBBB")
        BORDER = Border(left=THIN, right=THIN, top=THIN, bottom=THIN)

        def style_ws(ws, df, total_row_label=None):
            """بتلوّن أي شيت: هيدر أخضر بخط أبيض، تظليل متبادل للصفوف، صف الإجمالي دهبي،
            حدود لكل الخلايا، وعرض أعمدة مناسب للمحتوى — كله بيتطبق تلقائي على أي جدول."""
            ...
            for r in range(2, n_rows + 2):
                is_total = total_row_label is not None and first_col_vals[r - 2] == total_row_label
            ...
            for col_idx in range(1, n_cols + 1):
                ...
                max_len = max([header_len] + [len(str(v)) for v in df.iloc[:, col_idx - 1]]) if n_rows else header_len
                ws.column_dimensions[get_column_letter(col_idx)].width = max(12, min(30, max_len + 4))
        ...
        for line_name, fpath in worker_files.items():
            ...
            # توحيد شكل عمود الشيفت هنا مرة واحدة (يحل مشكلة "1.0" أو فراغات جايه من جوجل شيت
            # واللي كانت بتخلي مقارنة == "1" تفشل ويطلع الإجمالي غلط)
            df_line["الشيفت"] = df_line["الشيفت"].apply(normalize_shift)
            all_lines_hr[line_name] = df_line

        excel_buffer = io.BytesIO()
        with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:

            # ---- شيت معلومات: اسم البرنامج والمطور وتاريخ التوليد ----
            info_df = pd.DataFrame([
                {"البيان": "اسم البرنامج", "القيمة": PROGRAM_NAME},
                {"البيان": "المطور", "القيمة": DEVELOPER_NAME},
                {"البيان": "تاريخ التوليد", "القيمة": pd.Timestamp.today().strftime("%Y-%m-%d")},
            ])
            info_df.to_excel(writer, sheet_name="معلومات", index=False)
            style_ws(writer.sheets["معلومات"], info_df)
            ...
            for shift in ["1", "2", "3"]:
                ...
                sheet_name_sh = f"الشيفت {shift}"
                shift_sheet.to_excel(writer, sheet_name=sheet_name_sh, index=False)
                style_ws(writer.sheets[sheet_name_sh], shift_sheet)

            # ---- شيت ملخص الأعداد لكل خط ----
            # "الإجمالي" هنا = مجموع الشيفتات التلاتة بس (اللي معاهم شيفت متسجل)
            summary_rows = []
            for line_name, df_line in all_lines_hr.items():
                named = df_line[df_line["الاسم"].str.strip() != ""]
                s1 = len(named[named["الشيفت"] == "1"])
                s2 = len(named[named["الشيفت"] == "2"])
                s3 = len(named[named["الشيفت"] == "3"])
                summary_rows.append({
                    "الخط": line_name,
            ...
            style_ws(writer.sheets["ملخص الأعداد"], summary_df, total_row_label="الإجمالي الكلي")
            ...
            for route in routes:
                ...
                route_rows.append({
                    "خط السير (البلد)": route,
                    "الشيفت 1": s1, "الشيفت 2": s2, "الشيفت 3": s3,
                    "الإجمالي": s1 + s2 + s3,
                })
            routes_summary_df = pd.DataFrame(route_rows) if route_rows else pd.DataFrame(
                columns=["خط السير (البلد)", "الشيفت 1", "الشيفت 2", "الشيفت 3", "الإجمالي"]
            )
            ...
            style_ws(writer.sheets["ملخص البلاد بالشيفت"], routes_summary_df, total_row_label="الإجمالي الكلي")

        st.success("اتعمل شيت الشؤون ✔ (اتصلحت مشكلة الإجماليات)")
    ...
    # ==========================
    # 📥 توليد ملف إكسيل يومي شامل (باكينج + إنتاج + أعطال + كسر لنفس اليوم) + شيت تراكمي يجمع يوم بيوم
    # ==========================
    st.divider()
    st.markdown("### 📥 توليد ملف إكسيل يومي شامل")
    st.caption(
        "ملف إكسيل فيه شيت 'يومي' (بيانات نفس اليوم فقط لكل الأصناف/الخطوط) وشيت 'تراكمي' "
        "(بيانات مجمّعة/مجمّوعة من أول الشهر لحد اليوم اللي تختاره — رقم واحد إجمالي لكل خط/صنف، "
        "مش يوم 1 لوحده ويوم 2 لوحده) — يغطي الثلاث مصانع: برودكشن وباكينج بالورديات وأعطال وكسر"
    )

    excel_day = st.date_input("📅 اختر اليوم", value=today_r, key="reports_excel_day")
    excel_day_str = excel_day.strftime("%Y-%m-%d")
    excel_month_str = excel_day.strftime("%Y-%m")
    excel_year, excel_month_num = map(int, excel_month_str.split("-"))
    excel_days_in_month = calendar.monthrange(excel_year, excel_month_num)[1]
    excel_day_num = excel_day.day
    excel_day_col = f"يوم {excel_day_num}"
    excel_pack_sheet = f"packing_monthly_{excel_month_str}"  # مستخدم في نسخة الداشبورد اليومي (HTML) تحت — مصنع 1 بس

    # ---------- خطوط/شيتات كل مصنع من الثلاثة (زي صفحات Production/Packing/الأعطال/الكسر بالظبط) ----------
    EXCEL_PRODUCTION_FACTORIES_X = {
        "مصنع 1": "production_history_v2",
        "مصنع 2": "production_history_v2_m2",
        # مصنع 3 (شيكولاتة) مفيهوش تتبع خطوط إنتاج منفصل في صفحة Production لسه
    }
    EXCEL_PACKING_FACTORIES_X = {
        "مصنع 1": f"packing_monthly_{excel_month_str}",
        "مصنع 2": f"packing_monthly_{excel_month_str}_m2",
        "مصنع 3": f"packing_monthly_{excel_month_str}_m3",
    }
    EXCEL_BREAKAGE_FACTORIES_X = {
        "مصنع 1": "breakage_history",
        "مصنع 2": "breakage_history_m2",
        "مصنع 3": "breakage_history_m3",
    }
    EXCEL_PACKING_SETTINGS_SHEETS_X = {
        "مصنع 1": "packing_settings",
        "مصنع 2": "packing_settings_m2",
        "مصنع 3": "packing_settings_m3",
    }

    def _load_planned_days_x(settings_sheet, month_str, default_days):
        """بيرجع 'عدد أيام العمل المخططة' اللي اتسجلت فعليًا لشهر معيّن (زي ما هي محفوظة في صفحة الباكينج)،
        مش عدد أيام الشهر التقويمية — عشان التارجت اليومي يتحسب صح حتى لو الشهر فيه إجازات/أيام متعملش فيها إنتاج"""
        if GSHEETS_ENABLED and gsheet_exists(settings_sheet):
            settings_df = read_gsheet(settings_sheet)
            if "الشهر" in settings_df.columns and "أيام العمل المخططة" in settings_df.columns:
                match = settings_df[settings_df["الشهر"] == month_str]
                if not match.empty:
                    val = pd.to_numeric(match.iloc[0]["أيام العمل المخططة"], errors="coerce")
    ...
    EXCEL_PLANNED_DAYS_X = {
        fac: _load_planned_days_x(sheet, excel_month_str, excel_days_in_month)
        for fac, sheet in EXCEL_PACKING_SETTINGS_SHEETS_X.items()
    }
    EXCEL_LINE_TO_FACTORY_X = {}
    for _f, _lines in {
        "مصنع 1": ["الاكلير ص٣", "المستمر", "الإيطالي", "الاكلير ص١", "الطوفي", "الترانسلاب"],
        "مصنع 2": ["خط ويفر ١", "خط ويفر ٢", "خط ويفر ٣", "المطحنة", "المحطه"],
        "مصنع 3": ["شيكوهند بار", "خط شيكولاته ١ (وان شوت)", "خط شيكولاته ٢ (ميني باك)"],
        ...
        for _l in _lines:
            EXCEL_LINE_TO_FACTORY_X[_l] = _f

    if st.button("📊 توليد ملف الإكسيل", key="reports_generate_excel_btn"):
        import io as _io
        ...
        wb_x = Workbook()
        ws_daily_x = wb_x.active
        ws_daily_x.title = "يومي"
        ws_cum_x = wb_x.create_sheet("تراكمي")

        HEADER_FILL_X = PatternFill(start_color="0B1F3A", end_color="0B1F3A", fill_type="solid")
        HEADER_FONT_X = Font(color="FFFFFF", bold=True, name="Arial")
        SECTION_FONT_X = Font(bold=True, size=13, color="0B1F3A", name="Arial")
        TITLE_FONT_X = Font(bold=True, size=15, color="0B1F3A", name="Arial")
        NORMAL_FONT_X = Font(name="Arial")

        def _write_table_x(ws, start_row, title, headers, rows):
            ws.cell(row=start_row, column=1, value=title).font = SECTION_FONT_X
            hr = start_row + 1
            for ci, h in enumerate(headers, start=1):
                c = ws.cell(row=hr, column=ci, value=h)
                c.font = HEADER_FONT_X
                c.fill = HEADER_FILL_X
                c.alignment = Alignment(horizontal="center")
            if not rows:
                ws.cell(row=hr + 1, column=1, value="لا توجد بيانات").font = NORMAL_FONT_X
                return hr + 3
            for ri, row_vals in enumerate(rows, start=hr + 1):
                for ci, v in enumerate(row_vals, start=1):
                    ws.cell(row=ri, column=ci, value=v).font = NORMAL_FONT_X
            return hr + len(rows) + 2

        # ---------- تحميل بيانات الشهر كله مرة واحدة لكل الثلاث مصانع (أسرع بكتير من قراءة كل يوم لوحده) ----------
        prod_hist_month_x = pd.DataFrame()
        for _fac, _sheet in EXCEL_PRODUCTION_FACTORIES_X.items():
            if gsheet_exists(_sheet):
                _df = read_gsheet(_sheet)
                _df["Date"] = _df["Date"].astype(str)
                _df = _df[_df["Shift"] == "All Shifts"].copy()
                _df["المستهدف (KG)"] = pd.to_numeric(_df["المستهدف (KG)"], errors="coerce").fillna(0)
                _df["الفعلي (KG)"] = pd.to_numeric(_df["الفعلي (KG)"], errors="coerce").fillna(0)
                _df["المصنع"] = _fac
                prod_hist_month_x = pd.concat([prod_hist_month_x, _df], ignore_index=True)

        pack_grid_month_x = pd.DataFrame()
        for _fac, _sheet in EXCEL_PACKING_FACTORIES_X.items():
            if gsheet_exists(_sheet):
                _df = read_gsheet(_sheet)
                _df["المخطط بالكراتين (شهري)"] = pd.to_numeric(_df.get("المخطط بالكراتين (شهري)", 0), errors="coerce").fillna(0)
                _df["وزن الكرتونة (كيلو)"] = pd.to_numeric(_df.get("وزن الكرتونة (كيلو)", 0), errors="coerce").fillna(0)
                _df["المصنع"] = _fac
                pack_grid_month_x = pd.concat([pack_grid_month_x, _df], ignore_index=True)

        export_grid_month_x = pd.DataFrame()
        _export_sheet_x = f"export_monthly_{excel_month_str}"
        if gsheet_exists(_export_sheet_x):
            export_grid_month_x = read_gsheet(_export_sheet_x)
            if "الكمية المطلوبة (كرتونة)" not in export_grid_month_x.columns and "الكمية المطلوبة (KG)" in export_grid_month_x.columns:
                export_grid_month_x = export_grid_month_x.rename(columns={"الكمية المطلوبة (KG)": "الكمية المطلوبة (كرتونة)"})
            for _c in ["الصنف", "اسم البلد", "الشركة"]:
                if _c not in export_grid_month_x.columns:
                    export_grid_month_x[_c] = ""
            export_grid_month_x["الكمية المطلوبة (كرتونة)"] = pd.to_numeric(
                export_grid_month_x.get("الكمية المطلوبة (كرتونة)", 0), errors="coerce"
            ).fillna(0)
            export_grid_month_x = export_grid_month_x[
                (export_grid_month_x["الصنف"].astype(str).str.strip() != "") |
                (export_grid_month_x["اسم البلد"].astype(str).str.strip() != "")
            ].reset_index(drop=True)

        faults_log_month_x = pd.DataFrame()
        if gsheet_exists("faults_daily_log"):
            faults_log_month_x = read_gsheet("faults_daily_log")
            faults_log_month_x["التاريخ"] = faults_log_month_x["التاريخ"].astype(str)
            faults_log_month_x["مدة العطل (دقيقة)"] = pd.to_numeric(faults_log_month_x["مدة العطل (دقيقة)"], errors="coerce").fillna(0)
            # ملاحظة: المهندسين بيدخلوا مدة العطل بالساعة فعليًا في حقل اسمه "دقيقة" (خطأ إدخال قديم بالبرنامج،
            # مش حاجة غيّرناها إحنا) — فبنحوّلها هنا لدقايق فعلية (×60) عشان تظهر صح في الإكسيل والداشبورد.
            # لو يوم من الأيام العلاج الحقيقي (تصحيح حقل الإدخال نفسه) يتعمل، امسح السطر ده.
            FAULT_DURATION_ENTERED_AS_HOURS_X = True
            if FAULT_DURATION_ENTERED_AS_HOURS_X:
                faults_log_month_x["مدة العطل (دقيقة)"] = faults_log_month_x["مدة العطل (دقيقة)"] * 60
            faults_log_month_x["تكلفة الصيانة (جنيه)"] = pd.to_numeric(faults_log_month_x.get("تكلفة الصيانة (جنيه)", 0), errors="coerce").fillna(0)
            faults_log_month_x["المصنع"] = faults_log_month_x.get("الخط", "").map(EXCEL_LINE_TO_FACTORY_X).fillna("-")

        breakage_log_month_x = pd.DataFrame()
        for _fac, _sheet in EXCEL_BREAKAGE_FACTORIES_X.items():
            if gsheet_exists(_sheet):
                _df = read_gsheet(_sheet)
                _df["Date"] = _df["Date"].astype(str)
                for c_ in ["كسر خارج (KG)", "كسر داخل (KG)", "رصيد أول المدة (KG)"]:
                    if c_ in _df.columns:
                        _df[c_] = pd.to_numeric(_df[c_], errors="coerce").fillna(0)
                _df["المصنع"] = _fac  # بناخد اسم المصنع من مصدر الشيت مباشرة (مش من اسم الخط)، أدق وأضمن
                breakage_log_month_x = pd.concat([breakage_log_month_x, _df], ignore_index=True)

        def _gather_day_rows_x(d_str, d_num):
            """بترجع صفوف الإنتاج/الباكينج/الأعطال/الكسر ليوم واحد بالظبط، من البيانات المحمّلة فوق —
            بتاعة الثلاث مصانع مع بعض وعمود 'المصنع' بيوضح كل صف بتاع مصنع مين"""
            dcol = f"يوم {d_num}"

            prow = []
            if not prod_hist_month_x.empty:
                pday = prod_hist_month_x[prod_hist_month_x["Date"] == d_str]
                for _, r in pday.iterrows():
                    achv = round((r["الفعلي (KG)"] / r["المستهدف (KG)"] * 100), 1) if r["المستهدف (KG)"] > 0 else 0
                    prow.append([r["المصنع"], r["Line"], r["المستهدف (KG)"], r["الفعلي (KG)"], achv])

            pkrow = []
            if not pack_grid_month_x.empty and dcol in pack_grid_month_x.columns:
                day_vals = pd.to_numeric(pack_grid_month_x[dcol], errors="coerce").fillna(0)
                for idx, r in pack_grid_month_x.iterrows():
                    if str(r.get("الصنف", "")).strip() == "":
                        continue
                    _plan_days_fac = EXCEL_PLANNED_DAYS_X.get(r["المصنع"], excel_days_in_month)
                    daily_plan_cartons = round(r["المخطط بالكراتين (شهري)"] / _plan_days_fac, 1) if _plan_days_fac > 0 else 0
                    actual_cartons = day_vals.loc[idx]
                    actual_weight = round(actual_cartons * r["وزن الكرتونة (كيلو)"], 1)
                    pkrow.append([
                        r["المصنع"], r["الصنف"], r.get("الفئة", ""), r["وزن الكرتونة (كيلو)"],
                        r["المخطط بالكراتين (شهري)"], daily_plan_cartons, actual_cartons, actual_weight
                    ])

            frow = []
            if not faults_log_month_x.empty:
                fday = faults_log_month_x[faults_log_month_x["التاريخ"] == d_str]
                for _, r in fday.iterrows():
                    frow.append([
                        r.get("المصنع", "-"), r.get("الخط", ""), r.get("اسم الماكينة", ""), r.get("الوردية", ""),
                        r.get("من الساعة", ""), r.get("إلى الساعة", ""), r.get("مدة العطل (دقيقة)", 0),
                        r.get("سبب العطل", ""), r.get("الحالة", ""), r.get("تكلفة الصيانة (جنيه)", 0)
                    ])

            brow = []
            if not breakage_log_month_x.empty:
                bday = breakage_log_month_x[breakage_log_month_x["Date"] == d_str]
                for _, r in bday.iterrows():
                    opening_x = pd.to_numeric(r.get("رصيد أول المدة (KG)", 0), errors="coerce") or 0
                    out_x = pd.to_numeric(r.get("كسر خارج (KG)", 0), errors="coerce") or 0
                    in_x = pd.to_numeric(r.get("كسر داخل (KG)", 0), errors="coerce") or 0
                    closing_x = opening_x + out_x - in_x
                    brow.append([r.get("المصنع", "-"), r.get("الخط", ""), opening_x, out_x, in_x, closing_x])

            return prow, pkrow, frow, brow

        # ---------- كتابة شيت "يومي" — تفاصيل اليوم المختار بس، الثلاث مصانع مع بعض بعمود "المصنع" ----------
        prod_rows_x, pack_rows_x, faults_rows_x, breakage_rows_x = _gather_day_rows_x(excel_day_str, excel_day_num)

        row_ptr_x = 1
        ws_daily_x.cell(row=row_ptr_x, column=1, value=f"التقرير اليومي الشامل — {excel_day_str}").font = TITLE_FONT_X
        row_ptr_x += 2

        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "🏭 إنتاج الخطوط (مصنع 1 و 2)",
            ["المصنع", "الخط", "المستهدف (KG)", "الفعلي (KG)", "نسبة التحقيق %"], prod_rows_x)
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "📦 الباكينج (الثلاث مصانع)",
            ["المصنع", "الصنف", "الفئة", "وزن الكرتونة (كيلو)", "المخطط الشهري (كرتونة)",
             "التارجت اليومي (كرتونة)", "الفعلي (كرتونة)", "الفعلي (كيلو)"], pack_rows_x)
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "⚠ الأعطال (الثلاث مصانع)",
            ["المصنع", "الخط", "الماكينة", "الوردية", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)",
             "سبب العطل", "الحالة", "تكلفة الصيانة (جنيه)"], faults_rows_x)
        row_ptr_x = _write_table_x(ws_daily_x, row_ptr_x, "🔨 الكسر (الثلاث مصانع)",
            ["المصنع", "الخط", "رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "متبقي الرصيد (KG)"], breakage_rows_x)

        for col_i in range(1, 11):
            ws_daily_x.column_dimensions[get_column_letter(col_i)].width = 20

        # ---------- كتابة شيت "تراكمي" — بيانات مُجمّعة (رقم واحد إجمالي) من أول الشهر لحد اليوم المختار،
        # مش يوم 1 لوحده ويوم 2 لوحده وهكذا — كل خط/صنف له صف واحد بالإجمالي التراكمي ----------
        cum_days = list(range(1, excel_day_num + 1))
        cum_day_strs = [f"{excel_month_str}-{d:02d}" for d in cum_days]

        # -- إنتاج الخطوط: مجموع المستهدف والفعلي لكل (مصنع, خط) على مدار الأيام دي --
        cum_prod_rows_x = []
        if not prod_hist_month_x.empty:
            prod_to_date = prod_hist_month_x[prod_hist_month_x["Date"].isin(cum_day_strs)]
            if not prod_to_date.empty:
                g = prod_to_date.groupby(["المصنع", "Line"], as_index=False).agg(
                    **{"المستهدف (KG)": ("المستهدف (KG)", "sum"), "الفعلي (KG)": ("الفعلي (KG)", "sum")}
                )
                for _, r in g.iterrows():
                    achv = round((r["الفعلي (KG)"] / r["المستهدف (KG)"] * 100), 1) if r["المستهدف (KG)"] > 0 else 0
                    cum_prod_rows_x.append([r["المصنع"], r["Line"], round(r["المستهدف (KG)"], 1), round(r["الفعلي (KG)"], 1), achv])

        # -- الباكينج: مجموع الفعلي (كرتونة) لكل (مصنع, صنف) على مدار الأيام دي، مقابل المخطط الشهري الكامل --
        cum_pack_rows_x = []
        if not pack_grid_month_x.empty:
            valid_day_cols = [f"يوم {d}" for d in cum_days if f"يوم {d}" in pack_grid_month_x.columns]
            for _, r in pack_grid_month_x.iterrows():
                if str(r.get("الصنف", "")).strip() == "":
                    continue
                actual_cartons_cum = pd.to_numeric(r[valid_day_cols], errors="coerce").fillna(0).sum() if valid_day_cols else 0
                actual_weight_cum = round(actual_cartons_cum * r["وزن الكرتونة (كيلو)"], 1)
                achv_pk = round((actual_cartons_cum / r["المخطط بالكراتين (شهري)"] * 100), 1) if r["المخطط بالكراتين (شهري)"] > 0 else 0
                cum_pack_rows_x.append([
                    r["المصنع"], r["الصنف"], r.get("الفئة", ""), r["وزن الكرتونة (كيلو)"],
                    r["المخطط بالكراتين (شهري)"], round(actual_cartons_cum, 1), actual_weight_cum, achv_pk
                ])

        # -- الأعطال: عدد الأعطال وإجمالي مدتها وتكلفتها لكل (مصنع, خط) على مدار الأيام دي --
        cum_faults_rows_x = []
        if not faults_log_month_x.empty:
            faults_to_date = faults_log_month_x[faults_log_month_x["التاريخ"].isin(cum_day_strs)]
            if not faults_to_date.empty:
                g = faults_to_date.groupby(["المصنع", "الخط"], as_index=False).agg(
                    **{
                        "عدد الأعطال": ("الخط", "count"),
                        "إجمالي مدة الأعطال (دقيقة)": ("مدة العطل (دقيقة)", "sum"),
                        "إجمالي تكلفة الصيانة (جنيه)": ("تكلفة الصيانة (جنيه)", "sum"),
                    }
                )
                for _, r in g.iterrows():
                    cum_faults_rows_x.append([
                        r["المصنع"], r["الخط"], int(r["عدد الأعطال"]),
                        round(r["إجمالي مدة الأعطال (دقيقة)"], 1), round(r["إجمالي تكلفة الصيانة (جنيه)"], 1)
                    ])

        # -- الكسر: رصيد أول المدة (أول يوم فيه بيانات)، إجمالي كسر خارج/داخل، والرصيد الحالي لكل (مصنع, خط) --
        cum_breakage_rows_x = []
        if not breakage_log_month_x.empty:
            brk_to_date = breakage_log_month_x[breakage_log_month_x["Date"].isin(cum_day_strs)].sort_values("Date")
            if not brk_to_date.empty:
                for (fac_b, line_b), grp_b in brk_to_date.groupby(["المصنع", "الخط"]):
                    opening_first = pd.to_numeric(grp_b.iloc[0].get("رصيد أول المدة (KG)", 0), errors="coerce") or 0
                    out_sum = pd.to_numeric(grp_b.get("كسر خارج (KG)", 0), errors="coerce").sum()
                    in_sum = pd.to_numeric(grp_b.get("كسر داخل (KG)", 0), errors="coerce").sum()
                    closing_now = opening_first + out_sum - in_sum
                    cum_breakage_rows_x.append([
                        fac_b, line_b, round(opening_first, 1), round(out_sum, 1), round(in_sum, 1), round(closing_now, 1)
                    ])

        cum_row_ptr_x = 1
        ws_cum_x.cell(row=cum_row_ptr_x, column=1,
            value=f"التقرير التراكمي — من يوم 1 لحد يوم {excel_day_num} ({excel_month_str})").font = TITLE_FONT_X
        cum_row_ptr_x += 1
        ws_cum_x.cell(row=cum_row_ptr_x, column=1,
            value="البيانات مجمّعة/مجموعة لكل خط أو صنف (رقم إجمالي واحد)، مش مفصّلة يوم بيوم").font = NORMAL_FONT_X
        cum_row_ptr_x += 2

        cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "🏭 إنتاج الخطوط (مصنع 1 و 2) — إجمالي تراكمي",
            ["المصنع", "الخط", "المستهدف الكلي (KG)", "الفعلي الكلي (KG)", "نسبة التحقيق %"], cum_prod_rows_x)
        cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "📦 الباكينج (الثلاث مصانع) — إجمالي تراكمي",
            ["المصنع", "الصنف", "الفئة", "وزن الكرتونة (كيلو)", "المخطط الشهري (كرتونة)",
             "الفعلي التراكمي (كرتونة)", "الفعلي التراكمي (كيلو)", "نسبة التحقيق %"], cum_pack_rows_x)
        cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "⚠ الأعطال (الثلاث مصانع) — إجمالي تراكمي",
            ["المصنع", "الخط", "عدد الأعطال", "إجمالي مدة الأعطال (دقيقة)", "إجمالي تكلفة الصيانة (جنيه)"], cum_faults_rows_x)
        cum_row_ptr_x = _write_table_x(ws_cum_x, cum_row_ptr_x, "🔨 الكسر (الثلاث مصانع) — إجمالي تراكمي",
            ["المصنع", "الخط", "رصيد أول المدة (KG)", "إجمالي كسر خارج (KG)", "إجمالي كسر داخل (KG)", "الرصيد الحالي (KG)"], cum_breakage_rows_x)

        for col_i in range(1, 11):
            ws_cum_x.column_dimensions[get_column_letter(col_i)].width = 22

        # ---------- شيتات بيانات مسطّحة (Data_*) — مخصصة عشان الداشبورد التفاعلي يقرأها بسهولة،
        # كل شيت جدول عادي بس (صف واحد لكل خط/صنف) من غير عناوين قسم متداخلة ----------
        ws_meta = wb_x.create_sheet("Data_Meta")
        ws_meta.append(["اليوم المختار", excel_day_str])
        ws_meta.append(["الشهر", excel_month_str])
        ws_meta.append(["رقم اليوم في الشهر", excel_day_num])
        ws_meta.append(["أيام الشهر التقويمية", excel_days_in_month])
        for _fac, _pdays in EXCEL_PLANNED_DAYS_X.items():
            ws_meta.append([f"أيام العمل المخططة - {_fac}", _pdays])

        # -- Production: صف واحد لكل (مصنع, خط) فيه اليومي والتراكمي مع بعض --
        prod_daily_map = {(r[0], r[1]): r for r in prod_rows_x}
        ws_prod_data = wb_x.create_sheet("Data_Production")
        ws_prod_data.append(["المصنع", "الخط", "المستهدف اليومي (KG)", "الفعلي اليومي (KG)", "نسبة التحقيق اليومي %",
                              "المستهدف التراكمي (KG)", "الفعلي التراكمي (KG)", "نسبة التحقيق التراكمي %"])
        for r in cum_prod_rows_x:
            fac, line, tgt_c, act_c, achv_c = r
            d = prod_daily_map.get((fac, line), [fac, line, 0, 0, 0])
            ws_prod_data.append([fac, line, d[2], d[3], d[4], tgt_c, act_c, achv_c])

        # -- Packing: صف واحد لكل (مصنع, صنف) فيه اليومي والتراكمي مع بعض --
        pack_daily_map = {(r[0], r[1]): r for r in pack_rows_x}
        ws_pack_data = wb_x.create_sheet("Data_Packing")
        ws_pack_data.append(["المصنع", "الصنف", "الفئة", "وزن الكرتونة (كيلو)", "المخطط الشهري (كرتونة)",
                              "التارجت اليومي (كرتونة)", "الفعلي اليومي (كرتونة)", "الفعلي اليومي (كيلو)",
                              "الفعلي التراكمي (كرتونة)", "الفعلي التراكمي (كيلو)", "نسبة التحقيق التراكمي %"])
        for r in cum_pack_rows_x:
            fac, item, cat, cw, plan_m, act_cum_c, act_cum_kg, achv_c = r
            d = pack_daily_map.get((fac, item), [fac, item, cat, cw, plan_m, 0, 0, 0])
            ws_pack_data.append([fac, item, cat, cw, plan_m, d[5], d[6], d[7], act_cum_c, act_cum_kg, achv_c])

        # -- Faults: شيت لأحداث اليوم، وشيت تاني للإجمالي التراكمي لكل خط --
        ws_faults_daily = wb_x.create_sheet("Data_Faults_Daily")
        ws_faults_daily.append(["المصنع", "الخط", "الماكينة", "الوردية", "من الساعة", "إلى الساعة",
                                 "مدة العطل (دقيقة)", "سبب العطل", "الحالة", "تكلفة الصيانة (جنيه)"])
        for r in faults_rows_x:
            ws_faults_daily.append(r)

        ws_faults_cum = wb_x.create_sheet("Data_Faults_Cumulative")
        ws_faults_cum.append(["المصنع", "الخط", "عدد الأعطال", "إجمالي مدة الأعطال (دقيقة)", "إجمالي تكلفة الصيانة (جنيه)"])
        for r in cum_faults_rows_x:
            ws_faults_cum.append(r)

        # -- Breakage: شيت لبيانات اليوم، وشيت تاني للإجمالي التراكمي لكل خط --
        ws_brk_daily = wb_x.create_sheet("Data_Breakage_Daily")
        ws_brk_daily.append(["المصنع", "الخط", "رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)", "متبقي الرصيد (KG)"])
        for r in breakage_rows_x:
            ws_brk_daily.append(r)

        ws_brk_cum = wb_x.create_sheet("Data_Breakage_Cumulative")
        ws_brk_cum.append(["المصنع", "الخط", "رصيد أول المدة (KG)", "إجمالي كسر خارج (KG)", "إجمالي كسر داخل (KG)", "الرصيد الحالي (KG)"])
        for r in cum_breakage_rows_x:
            ws_brk_cum.append(r)

        # -- التصدير (طلبيات التصدير لكل بلد/شركة — بالكرتونة) --
        ws_exp_daily = wb_x.create_sheet("Data_Export_Daily")
        ws_exp_daily.append(["الصنف", "اسم البلد", "الشركة", "الكمية المطلوبة (كرتونة)", "المُصدَّر اليوم (كرتونة)"])
        ws_exp_cum = wb_x.create_sheet("Data_Export_Cumulative")
        ws_exp_cum.append(["الصنف", "اسم البلد", "الشركة", "الكمية المطلوبة (كرتونة)", "المُصدَّر التراكمي (كرتونة)", "نسبة التحقيق %"])
        if not export_grid_month_x.empty:
            _exp_day_col_x = f"يوم {excel_day_num}"
            _exp_cum_cols_x = [f"يوم {d}" for d in range(1, excel_day_num + 1) if f"يوم {d}" in export_grid_month_x.columns]
            for _, r in export_grid_month_x.iterrows():
                target_r = r["الكمية المطلوبة (كرتونة)"]
                daily_r = pd.to_numeric(r.get(_exp_day_col_x, 0), errors="coerce") or 0
                ws_exp_daily.append([r["الصنف"], r["اسم البلد"], r["الشركة"], target_r, daily_r])
                cum_r = pd.to_numeric(r[_exp_cum_cols_x], errors="coerce").fillna(0).sum() if _exp_cum_cols_x else 0
                achv_r = round((cum_r / target_r * 100), 1) if target_r > 0 else 0
                ws_exp_cum.append([r["الصنف"], r["اسم البلد"], r["الشركة"], target_r, round(cum_r, 1), achv_r])

        # -- المخزون (جرد المواد الخام — ملف محلي مش جوجل شيت) --
        ws_inv = wb_x.create_sheet("Data_Inventory")
        ws_inv.append(["المادة الخام", "رصيد أول المدة", "الوارد", "المرتجع", "الهالك", "المستخدم", "الجرد الدفتري", "الجرد الفعلي", "الفرق"])
        INV_RAW_FILE_X = "raw_inventory.csv"
        inv_row_count_x = 0
        inv_status_x = ""  # نص تشخيصي بيوضح ليه المخزون فاضي لو فاضي، يظهر تحت بعد التوليد
        if not os.path.exists(INV_RAW_FILE_X):
            inv_status_x = f"ملف \"{INV_RAW_FILE_X}\" مش موجود على السيرفر — تأكد إنك دخلت شاشة الجرد وحفظت بيانات المخزون قبل كده"
        else:
            try:
                inv_log_x = pd.read_csv(INV_RAW_FILE_X, encoding="utf-8-sig")
                inv_log_x["Date"] = inv_log_x["Date"].astype(str)
                inv_day_x = inv_log_x[inv_log_x["Date"] == excel_day_str]
                inv_cols_x = ["المادة الخام", "رصيد اول المدة", "الوارد", "المرتجع", "الهالك", "المستخدم", "الجرد الدفتري", "الجرد الفعلي", "الفرق"]
                for _, r in inv_day_x.iterrows():
                    ws_inv.append([r.get(c, 0) if c == "المادة الخام" else pd.to_numeric(r.get(c, 0), errors="coerce") or 0 for c in inv_cols_x])
                    inv_row_count_x += 1
                if inv_row_count_x == 0:
                    inv_status_x = f"الملف موجود بس مفيهوش صفوف بتاريخ {excel_day_str} بالظبط — تأكد إنك سجّلت جرد المخزون لنفس اليوم ده من شاشة الجرد"
            except Exception as _inv_err_x:
                inv_status_x = f"حصل خطأ وإحنا بنقرا ملف الجرد: {_inv_err_x}"

        # -- رصيد المخزون الفعلي بالفروع/التوكيلات (اللي بيتحفظ من صفحة الباكينج → التقرير المجمّع
        # → رفع ملف المخزون هناك) — ده غير جرد المواد الخام اللي فوق ده خالص --
        ws_stock = wb_x.create_sheet("Data_StockBalance")
        ws_stock.append(["الصنف", "الرصيد الفعلي بالمخزن"])
        stock_row_count_x = 0
        stock_status_x = ""
        if gsheet_exists("stock_balance_log"):
            try:
                stock_log_x = read_gsheet("stock_balance_log")
                stock_log_x["Date"] = stock_log_x["Date"].astype(str)
                stock_day_x = stock_log_x[stock_log_x["Date"] == excel_day_str]
                for _, r in stock_day_x.iterrows():
                    ws_stock.append([r.get("الصنف", ""), pd.to_numeric(r.get("الرصيد الفعلي بالمخزن", 0), errors="coerce") or 0])
                    stock_row_count_x += 1
                if stock_row_count_x == 0:
                    stock_status_x = (
                        f"مفيش رصيد مخزون متسجّل بتاريخ {excel_day_str} بالظبط — لازم ترفع ملف إكسيل "
                        "رصيد الفروع والتوكيلات من صفحة الباكينج ← تقرير مدير الإنتاج المجمّع، لنفس اليوم ده بالتحديد"
                    )
            except Exception as _stock_err_x:
                stock_status_x = f"حصل خطأ وإحنا بنقرا رصيد المخزون: {_stock_err_x}"
        else:
            stock_status_x = (
                "لسه معندكش أي رصيد مخزون محفوظ خالص — ارفع ملف إكسيل رصيد الفروع والتوكيلات مرة واحدة من "
                "صفحة الباكينج ← تقرير مدير الإنتاج المجمّع، وبعدها هيبقى متاح هنا تلقائيًا كل يوم بترفعه فيه"
            )

        buf_x = _io.BytesIO()
        wb_x.save(buf_x)
        st.session_state["reports_excel_bytes"] = buf_x.getvalue()
        st.session_state["reports_excel_filename"] = f"daily_report_{excel_day_str}.xlsx"
        st.success("اتولّد الملف ✔ — دوس تحميل تحت. الملف فيه شيتات 'يومي'/'تراكمي' للطباعة، وشيتات Data_* للداشبورد التفاعلي")

        # ---------- ملخص واضح: إيه اللي فعلاً اتضمّن في الملف ده، عشان تعرف قبل ما تفتحه
        # لو حاجة زي المخزون نزلت معاه ولا لأ، من غير ما تدخل تفتح الإكسيل وتدور ----------
        st.markdown("##### 📋 إيه اللي اتضمّن في الملف ده")
        summary_rows_x = [
            ("🏭 إنتاج الخطوط", len(prod_rows_x)),
            ("📦 الباكينج", len(pack_rows_x)),
            ("⚠️ الأعطال", len(faults_rows_x)),
            ("🔨 الكسر", len(breakage_rows_x)),
            ("🚢 التصدير", len(export_grid_month_x)),
            ("🗃️ جرد المواد الخام", inv_row_count_x),
            ("📦 رصيد المخزون بالفروع", stock_row_count_x),
        ]
        for _label_x, _count_x in summary_rows_x:
            if _count_x > 0:
                st.markdown(f"- ✅ {_label_x}: **{_count_x}** صف")
            else:
                st.markdown(f"- ⚠️ {_label_x}: **مفيش بيانات** نزلت في الملف ده")
        if inv_status_x:
            st.caption(f"🗃️ جرد المواد الخام: {inv_status_x}")
        if stock_status_x:
            st.caption(f"📦 رصيد المخزون بالفروع: {stock_status_x}")

    if "reports_excel_bytes" in st.session_state:
        st.download_button(
            "⬇️ تحميل ملف الإكسيل",
            data=st.session_state["reports_excel_bytes"],
            file_name=st.session_state.get("reports_excel_filename", "daily_report.xlsx"),
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="reports_excel_download_btn"
        )

    # ==========================
    # 🖥️ نفس البيانات، لكن كداشبورد HTML سهل القراءة (بنفس طابع التقرير الشهري الشامل)
    # ==========================
    st.markdown("#### 🖥️ نسخة داشبورد يومي (HTML) — نفس بيانات الإكسيل بس أسهل في القراءة")
    st.caption("بطاقات KPI ورسومات ورسم بياني، بنفس تصميم التقرير الشهري الشامل — لنفس اليوم المختار فوق")

    if st.button("🖥️ توليد الداشبورد اليومي", key="reports_generate_daily_dash_btn"):
        prod_hist_d = pd.DataFrame()
        if gsheet_exists(PRODUCTION_SHEET):
            prod_hist_d = read_gsheet(PRODUCTION_SHEET)
            prod_hist_d["Date"] = prod_hist_d["Date"].astype(str)
            prod_hist_d = prod_hist_d[(prod_hist_d["Date"] == excel_day_str) & (prod_hist_d["Shift"] == "All Shifts")].copy()
            prod_hist_d["المستهدف (KG)"] = pd.to_numeric(prod_hist_d["المستهدف (KG)"], errors="coerce").fillna(0)
            prod_hist_d["الفعلي (KG)"] = pd.to_numeric(prod_hist_d["الفعلي (KG)"], errors="coerce").fillna(0)

        pack_grid_d = pd.DataFrame()
        if gsheet_exists(excel_pack_sheet):
            pack_grid_d = read_gsheet(excel_pack_sheet)
            if excel_day_col in pack_grid_d.columns:
                pack_grid_d["المخطط بالكراتين (شهري)"] = pd.to_numeric(pack_grid_d.get("المخطط بالكراتين (شهري)", 0), errors="coerce").fillna(0)
                pack_grid_d[excel_day_col] = pd.to_numeric(pack_grid_d[excel_day_col], errors="coerce").fillna(0)
                pack_grid_d["وزن الكرتونة (كيلو)"] = pd.to_numeric(pack_grid_d.get("وزن الكرتونة (كيلو)", 0), errors="coerce").fillna(0)
                pack_grid_d = pack_grid_d[pack_grid_d["الصنف"].astype(str).str.strip() != ""]
            else:
                pack_grid_d = pd.DataFrame()

        faults_log_d = pd.DataFrame()
        if gsheet_exists("faults_daily_log"):
            faults_log_d = read_gsheet("faults_daily_log")
            faults_log_d["التاريخ"] = faults_log_d["التاريخ"].astype(str)
            faults_log_d = faults_log_d[faults_log_d["التاريخ"] == excel_day_str].copy()
            faults_log_d["مدة العطل (دقيقة)"] = pd.to_numeric(faults_log_d["مدة العطل (دقيقة)"], errors="coerce").fillna(0)
            faults_log_d["تكلفة الصيانة (جنيه)"] = pd.to_numeric(faults_log_d.get("تكلفة الصيانة (جنيه)", 0), errors="coerce").fillna(0)

        breakage_log_d = pd.DataFrame()
        if gsheet_exists("breakage_history"):
            breakage_log_d = read_gsheet("breakage_history")
            breakage_log_d["Date"] = breakage_log_d["Date"].astype(str)
            breakage_log_d = breakage_log_d[breakage_log_d["Date"] == excel_day_str].copy()
            for c_ in ["رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)"]:
                if c_ in breakage_log_d.columns:
                    breakage_log_d[c_] = pd.to_numeric(breakage_log_d[c_], errors="coerce").fillna(0)

        def _simple_bar_fig_d(labels, values, title, color):
            fig, ax = plt.subplots(figsize=(3.6, 2.4), dpi=140)
            fig.patch.set_facecolor("#ffffff")
            if not values or sum(v for v in values if v) <= 0:
                ax.text(0.5, 0.5, ar_text("لا توجد بيانات"), ha="center", va="center",
                        fontsize=9, fontweight="bold", color="#9aa5b8", transform=ax.transAxes)
                ax.axis("off")
            else:
                bars = ax.bar([ar_text(l) for l in labels], values, color=color, edgecolor="#00201c", linewidth=0.5)
                ax.tick_params(labelsize=6.5)
                plt.setp(ax.get_xticklabels(), rotation=25, ha="right", fontsize=6.5, fontweight="bold")
                ax.spines["top"].set_visible(False)
                ax.spines["right"].set_visible(False)
                ...
                for b in bars:
                    h = b.get_height()
                    ax.text(b.get_x() + b.get_width() / 2, h + max_v * 0.02, f"{h:,.0f}",
        ...
        # ---------- Production section ----------
        prod_target_sum = prod_hist_d["المستهدف (KG)"].sum()
        prod_actual_sum = prod_hist_d["الفعلي (KG)"].sum()
        prod_achv = round((prod_actual_sum / prod_target_sum * 100), 1) if prod_target_sum > 0 else 0
        prod_kpis = "".join([
            dash_kpi_card("🎯", f"{prod_target_sum:,.0f} KG", "إجمالي المستهدف", "#e0a92e"),
            dash_kpi_card("📦", f"{prod_actual_sum:,.0f} KG", "إجمالي الفعلي", "#0d6b5c"),
            dash_kpi_card("🥇", f"{prod_achv}%", "نسبة التحقيق", "#7c3aed"),
        ])
        prod_chart = dash_chart_card(
            "المستهدف مقابل الفعلي لكل خط",
            fig_to_base64_img(_simple_bar_fig_d(
                prod_hist_d["Line"].tolist() if not prod_hist_d.empty else [],
                prod_hist_d["الفعلي (KG)"].tolist() if not prod_hist_d.empty else [],
                "الفعلي لكل خط (KG)", "#2563eb"
            ))
        )
        prod_table_d = prod_hist_d[["Line", "المستهدف (KG)", "الفعلي (KG)"]].rename(columns={"Line": "الخط"}) if not prod_hist_d.empty else pd.DataFrame(columns=["الخط", "المستهدف (KG)", "الفعلي (KG)"])
        dash_sections.append({
            "key": "production", "title": "🏭 إنتاج الخطوط", "subtitle": f"يوم {excel_day_str}",
            "kpi_html": prod_kpis, "chart_html": prod_chart, "table_title": "تفاصيل الخطوط", "table_df": prod_table_d,
        })

        # ---------- Packing section ----------
        pack_target_sum = 0
        pack_actual_sum = 0
        if not pack_grid_d.empty:
            _plan_days_fac1 = EXCEL_PLANNED_DAYS_X.get("مصنع 1", excel_days_in_month)
            pack_grid_d["_تارجت_يومي"] = pack_grid_d["المخطط بالكراتين (شهري)"] / _plan_days_fac1 if _plan_days_fac1 > 0 else 0
            pack_target_sum = pack_grid_d["_تارجت_يومي"].sum()
            pack_actual_sum = pack_grid_d[excel_day_col].sum()
        pack_achv = round((pack_actual_sum / pack_target_sum * 100), 1) if pack_target_sum > 0 else 0
        pack_kpis = "".join([
            dash_kpi_card("🎯", f"{pack_target_sum:,.0f}", "تارجت اليوم (كرتونة)", "#e0a92e"),
            dash_kpi_card("📦", f"{pack_actual_sum:,.0f}", "الفعلي (كرتونة)", "#0d6b5c"),
            dash_kpi_card("🥇", f"{pack_achv}%", "نسبة التحقيق", "#7c3aed"),
        ])
        pack_chart = dash_chart_card(
            "الفعلي لكل صنف (كرتونة)",
            fig_to_base64_img(_simple_bar_fig_d(
                pack_grid_d["الصنف"].tolist() if not pack_grid_d.empty else [],
                pack_grid_d[excel_day_col].tolist() if not pack_grid_d.empty else [],
                "الفعلي لكل صنف", "#16a34a"
            ))
        )
        pack_table_d = (
            pack_grid_d[["الصنف", "الفئة", "المخطط بالكراتين (شهري)", excel_day_col]].rename(columns={excel_day_col: "الفعلي (كرتونة)"})
            if not pack_grid_d.empty else pd.DataFrame(columns=["الصنف", "الفئة", "المخطط بالكراتين (شهري)", "الفعلي (كرتونة)"])
        )
        dash_sections.append({
            "key": "packing", "title": "📦 الباكينج", "subtitle": f"يوم {excel_day_str}",
            "kpi_html": pack_kpis, "chart_html": pack_chart, "table_title": "تفاصيل الأصناف", "table_df": pack_table_d,
        })

        # ---------- Faults section ----------
        faults_count = len(faults_log_d)
        faults_downtime = faults_log_d["مدة العطل (دقيقة)"].sum() if not faults_log_d.empty else 0
        faults_cost = faults_log_d["تكلفة الصيانة (جنيه)"].sum() if not faults_log_d.empty else 0
        faults_kpis = "".join([
            dash_kpi_card("⚠", f"{faults_count}", "عدد الأعطال", "#dc2626"),
            dash_kpi_card("⏱", f"{faults_downtime:,.0f} د", "إجمالي التوقف", "#2563eb"),
            dash_kpi_card("💰", f"{faults_cost:,.0f} ج", "تكلفة الصيانة", "#c026d3"),
        ])
        faults_by_line_d = faults_log_d.groupby("الخط")["مدة العطل (دقيقة)"].sum() if not faults_log_d.empty else pd.Series(dtype=float)
        faults_chart = dash_chart_card(
            "التوقف حسب الخط (دقيقة)",
            fig_to_base64_img(_simple_bar_fig_d(
                list(faults_by_line_d.index), list(faults_by_line_d.values), "التوقف حسب الخط", "#dc2626"
            ))
        )
        faults_table_cols = ["الخط", "اسم الماكينة", "الوردية", "من الساعة", "إلى الساعة", "مدة العطل (دقيقة)", "سبب العطل", "الحالة"]
        faults_table_d = faults_log_d[faults_table_cols] if not faults_log_d.empty else pd.DataFrame(columns=faults_table_cols)
        dash_sections.append({
            "key": "faults", "title": "⚠ الأعطال", "subtitle": f"يوم {excel_day_str}",
            "kpi_html": faults_kpis, "chart_html": faults_chart, "table_title": "تفاصيل الأعطال", "table_df": faults_table_d,
        })

        # ---------- Breakage section ----------
        break_out_sum = breakage_log_d["كسر خارج (KG)"].sum() if "كسر خارج (KG)" in breakage_log_d.columns and not breakage_log_d.empty else 0
        break_in_sum = breakage_log_d["كسر داخل (KG)"].sum() if "كسر داخل (KG)" in breakage_log_d.columns and not breakage_log_d.empty else 0
        break_net = break_out_sum - break_in_sum
        break_kpis = "".join([
            dash_kpi_card("📤", f"{break_out_sum:,.0f} KG", "كسر خارج", "#dc2626"),
            dash_kpi_card("📥", f"{break_in_sum:,.0f} KG", "كسر داخل", "#16a34a"),
            dash_kpi_card("⚖️", f"{break_net:,.0f} KG", "صافي الكسر", "#7c3aed"),
        ])
        break_chart = dash_chart_card(
            "صافي الكسر حسب الخط (KG)",
            fig_to_base64_img(_simple_bar_fig_d(
                breakage_log_d["الخط"].tolist() if not breakage_log_d.empty else [],
                (breakage_log_d["كسر خارج (KG)"] - breakage_log_d["كسر داخل (KG)"]).tolist() if not breakage_log_d.empty else [],
                "صافي الكسر لكل خط", "#ea580c"
            ))
        )
        break_table_cols = ["الخط", "رصيد أول المدة (KG)", "كسر خارج (KG)", "كسر داخل (KG)"]
        break_table_d = breakage_log_d[break_table_cols] if not breakage_log_d.empty else pd.DataFrame(columns=break_table_cols)
        dash_sections.append({
            "key": "breakage", "title": "🔨 الكسر", "subtitle": f"يوم {excel_day_str}",
            "kpi_html": break_kpis, "chart_html": break_chart, "table_title": "تفاصيل الكسر", "table_df": break_table_d,
        })

        daily_dashboard_html = build_dashboard_document(
            f"Daily Dashboard — {excel_day_str}",
            "",
            dash_sections,
        )
        st.session_state["reports_daily_dash_bytes"] = daily_dashboard_html.encode("utf-8")
        st.session_state["reports_daily_dash_filename"] = f"daily_dashboard_{excel_day_str}.html"
        st.success("اتولّد الداشبورد ✔ — دوس تحميل تحت")

    if "reports_daily_dash_bytes" in st.session_state:
        st.download_button(
            "⬇️ تحميل الداشبورد اليومي (HTML)",
            data=st.session_state["reports_daily_dash_bytes"],
            file_name=st.session_state.get("reports_daily_dash_filename", "daily_dashboard.html"),
            mime="text/html",
            help="افتح الملف في المتصفح، أو دوس Ctrl+P لتحويله PDF",
            key="reports_daily_dash_download_btn"
        )
    ...
    def _donut_fig(labels, values, title, center_text=None):
        ...
        fig.patch.set_facecolor("#ffffff")
        if not values or sum(v for v in values if v and v > 0) <= 0:
            ax.text(0.5, 0.5, ar_text("لا توجد بيانات"), ha="center", va="center",
                    fontsize=10, fontweight="bold", color="#9aa5b8", transform=ax.transAxes)
            ax.axis("off")
            ax.set_title(ar_text(title), fontsize=8.5, fontweight="bold", color=BRAND_GREEN, pad=6)
            fig.tight_layout(pad=0.6)
            return fig
