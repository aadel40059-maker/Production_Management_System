...
def build_printable_html(title, subtitle, df, extra_title=None, extra_df=None, extra_tables=None,
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        table:not(.layout-wrapper) {{ border-collapse: collapse; width: 100%; font-size: {base_font_size}px; page-break-inside: avoid; font-weight: {main_font_weight}; }}
...
if page == "Dashboard":
    ...
    if summary.empty:
        ...
        # ==========================
        # تقرير تاني مخصوص لمدير الإنتاج — بطاقات KPI + جدول ملوّن بالسهم/الحالة + رسم واحد + جدول تحت للمقصّرين
        # ==========================
        st.divider()
        ...
        pm_df = summary[["الصنف", "المخطط بالكراتين (شهري)", "إجمالي الكراتين"]].copy()
        pm_df = pm_df.rename(columns={
            "المخطط بالكراتين (شهري)": "إجمالي المخطط (كرتونة)",
            "إجمالي الكراتين": "إجمالي الفعلي (كرتونة)"
        })
        pm_df = pm_df[pm_df["إجمالي المخطط (كرتونة)"] > 0].reset_index(drop=True)

        # المتوقع تحقيقه حتى الآن = المخطط الشهري × (أيام العمل الفعلية / أيام الشهر)
        pm_df["المتوقع حتى الآن"] = (pm_df["إجمالي المخطط (كرتونة)"] * day_ratio).round(0)
        pm_df["الفارق (كرتونة)"] = (pm_df["إجمالي الفعلي (كرتونة)"] - pm_df["المتوقع حتى الآن"]).round(0)
        pm_df["نسبة الأداء %"] = pm_df.apply(
            lambda r: round((r["إجمالي الفعلي (كرتونة)"] / r["المتوقع حتى الآن"] * 100), 1)
            if r["المتوقع حتى الآن"] > 0 else 100.0,
            axis=1
        )
        ...
        status_labels, status_colors, row_bg_colors = [], [], []
        ...
        pm_df = pm_df[[
            "الصنف", "إجمالي المخطط (كرتونة)", "إجمالي الفعلي (كرتونة)",
            "الفارق (كرتونة)", "نسبة الأداء %", "حالة الأداء"
        ]]

        # ---------- بطاقات KPI ----------
        total_target_pm = pm_df["إجمالي المخطط (كرتونة)"].sum()
        total_actual_pm = pm_df["إجمالي الفعلي (كرتونة)"].sum()
        total_expected_pm = (total_target_pm * day_ratio)
        total_gap_pm = total_actual_pm - total_expected_pm
        avg_perf_pm = (total_actual_pm / total_expected_pm * 100) if total_expected_pm > 0 else 0
        items_count_pm = len(pm_df)
        low_items_count_pm = sum(1 for pct in pm_df["نسبة الأداء %"] if pct < 90)
        ...
        def kpi_card(icon, value, label, color):
            ...
            <div style="flex:1; background:#fff; border:1px solid #e5e7eb; border-right:5px solid {color};
                ...
                <div style="font-size:19px; font-weight:800; color:#0b1f3a;">{value}</div>
                <div style="font-size:10px; color:#6b7280; font-weight:700;">{label}</div>
            </div>
            """

        pm_top_html = '<div style="display:flex; gap:8px; margin:8px 0 12px;">' + "".join([
            ...
            kpi_card("⚖", f"{total_gap_pm:,.0f}", "الفارق عن المتوقع (كرتونة)", "#dc2626" if total_gap_pm < 0 else "#16a34a"),
            kpi_card("📊", f"{avg_perf_pm:.1f}%", "متوسط الأداء", "#7c3aed"),
            kpi_card("🔻", f"{low_items_count_pm} من {items_count_pm}", "أصناف أقل من المتوقع", "#dc2626"),
        ]) + "</div>"

        # ---------- رسم بياني واحد: نسبة الأداء لكل صنف ----------
        chart_df = pm_df.copy()
        chart_df["نسبة_رقمية"] = pd.to_numeric(chart_df["نسبة الأداء %"], errors="coerce").fillna(0)
        chart_df = chart_df.sort_values("نسبة_رقمية", ascending=True)
        bar_colors_pm = [_pm_status(p)[1] for p in chart_df["نسبة_رقمية"]]

        fig_pm, ax_pm = plt.subplots(figsize=(10, max(2.6, 0.32 * len(chart_df))))
        y_labels = [ar_text(l) for l in chart_df["الصنف"].tolist()]
        ax_pm.barh(y_labels, chart_df["نسبة_رقمية"], color=bar_colors_pm, edgecolor="#333", linewidth=0.4)
        ax_pm.axvline(100, color="#0b1f3a", linestyle="--", linewidth=1.2)
        ax_pm.set_xlabel(ar_text("نسبة الأداء %"), fontsize=11, fontweight="bold")
        ax_pm.set_title(ar_text("نسبة تحقيق كل صنف بالنسبة للمتوقع حتى الآن"), fontsize=13, fontweight="bold", pad=10)
        ax_pm.spines["top"].set_visible(False)
        ax_pm.spines["right"].set_visible(False)
        ax_pm.grid(axis="x", linestyle="--", alpha=0.3)
        ax_pm.set_axisbelow(True)
        for i, v in enumerate(chart_df["نسبة_رقمية"]):
            ax_pm.text(v + 1.5, i, f"{v:.0f}%", va="center", fontsize=9, fontweight="bold")
        fig_pm.tight_layout()

        pm_chart_buf = io.BytesIO()
        fig_pm.savefig(pm_chart_buf, format="png", dpi=150, bbox_inches="tight")
        plt.close(fig_pm)
        pm_chart_b64 = base64.b64encode(pm_chart_buf.getvalue()).decode()
        ...
        low_row_colors_sorted = [
            _pm_status(p)[2] for p in pm_low_df["نسبة الأداء %"]
        ]
        ...
        <div style="margin:14px 0; text-align:center;">
            <img src="data:image/png;base64,{pm_chart_b64}" style="width:100%; max-width:900px;">
        </div>
        ...
        <p style="font-size:9px; color:#6b7280; margin-top:10px;">
            ℹ️ يتم احتساب "المتوقع حتى الآن" و"نسبة الأداء" بناءً على عدد أيام العمل الفعلية
            ({actual_working_days} يوم) من إجمالي أيام الشهر المخططة ({total_month_days} يوم).
        </p>
        """

        pm_html = build_printable_html(
            "تقرير متابعة الإنتاج — لمدير الإنتاج (Packing)",
            f"شهر {selected_month} — حتى يوم {day_pick} — تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            pm_df,
            landscape=True,
            base_font_size=15,
            bold=True,
        ...
        st.download_button(
            ...
            mime="text/html",
            help="تقرير بطاقات + جدول ملوّن بالحالة + رسم بياني للأداء لكل صنف — افتحه ودوس Ctrl+P واختار 'Save as PDF'"
        )
