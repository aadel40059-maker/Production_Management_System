...
def build_dashboard_document(overall_title, overall_subtitle, sections):
    ...
    <style>
        :root {{
            --navy: #0b1f3a; --navy-light: #13294f; --bg: #f4f6fb;
            --card: #ffffff; --border: #e5e7eb; --text: #1a1d21; --muted: #6b7280;
        }}
        * {{ box-sizing: border-box; }}
        ...
        .dash-sidebar {{
            direction: rtl; width: 168px; flex-shrink: 0; background: linear-gradient(180deg, var(--navy), var(--navy-light));
        ...
        .dash-main {{ direction: rtl; flex: 1; min-width: 0; padding: 10px; }}
        ...
        .kpi-card {{
            ...
            border-radius: 7px; padding: 6px 5px; text-align: center; box-shadow: 0 1px 2px rgba(0,0,0,.04);
            page-break-inside: avoid; break-inside: avoid;
        }}
        .kpi-icon {{
        ...
        .kpi-value {{ font-size: 11.5px; font-weight: 800; color: var(--navy); line-height: 1.1; }}
        ...
        .chart-card {{
            ...
            border-radius: 7px; padding: 5px; box-shadow: 0 1px 2px rgba(0,0,0,.04);
            page-break-inside: avoid; break-inside: avoid;
        }}
        .chart-card-title {{ font-size: 8.5px; font-weight: 800; color: var(--navy); margin-bottom: 3px; }}
        ...
        .table-card table th, .table-card table td {{
            padding: 4px 5px; text-align: center; border-bottom: 1px solid var(--border);
        }}
        .table-card table th {{ background: #f4f7fb; color: var(--navy); font-weight: 800; }}
        .table-card table tr {{ page-break-inside: avoid; break-inside: avoid; }}
        .table-card table tr:last-child {{ font-weight: 800; background: #eef2f7; }}

        p.dev-footer {{ text-align: center; font-size: 8px; color: #9aa5b8; margin-top: 8px; }}
...
if page == "الرئيسية":
    ...
    def _empty_production_rows(lines_for_default, p_day_cols):
        ...
        for l in lines_for_default:
            rows.append({
                "الخط": l, "الشيفت": TOTAL_SHIFT_LABEL, "مخفي": False,
                "المخطط الشهري (KG)": 0, **{dc: 0 for dc in p_day_cols}
            })
            for sh in SHIFTS:
                rows.append({
                    "الخط": l, "الشيفت": sh, "مخفي": False,
                    "المخطط الشهري (KG)": 0, **{dc: 0 for dc in p_day_cols}
                })
        return rows

    def load_production_month_grid(month_str, lines_for_default):
        """الجدول الشهري: كل خط وتحته شيفتاته الثلاثة، عمود مخطط شهري، وعمود لكل يوم في الشهر.
        ...
        p_cols = ["الخط", "الشيفت", "مخفي", "المخطط الشهري (KG)"] + p_day_cols
        p_path = f"production_monthly_{month_str}"

        if gsheet_exists(p_path):
            grid = read_gsheet(p_path)
            for c in p_cols:
                if c not in grid.columns:
                    if c == "مخفي":
                        grid[c] = False
                    elif c in ["الخط", "الشيفت"]:
                        grid[c] = ""
                    else:
                        grid[c] = 0
            grid = grid[p_cols]
        ...
        grid = grid[grid.apply(lambda r: (r["الخط"], r["الشيفت"]) in valid_pairs, axis=1)].reset_index(drop=True)

        existing_pairs = set(zip(grid["الخط"], grid["الشيفت"]))
        missing_rows = [
            {"الخط": l, "الشيفت": sh, "مخفي": False, "المخطط الشهري (KG)": 0, **{dc: 0 for dc in p_day_cols}}
            for l in lines_for_default
        ...
        grid["__order"] = grid.apply(lambda r: order_map.get((r["الخط"], r["الشيفت"]), 9999), axis=1)
        ...
        grid["مخفي"] = (
            grid["مخفي"].astype(str).str.strip().str.lower().isin(["true", "1", "yes", "✔"])
        )
        grid["المخطط الشهري (KG)"] = pd.to_numeric(grid["المخطط الشهري (KG)"], errors="coerce").fillna(0)
    ...
    st.caption(
        "📌 سجّل إنتاج كل يوم في عموده تحت كل خط وشيفته — البيانات بتتحفظ ومتتمسحش، وتقدر تعدّل فيها في أي وقت وتحفظ تاني. "
    ...
    show_hidden_pr = st.checkbox("👁️ إظهار الصفوف المخفية", value=False, key="production_show_hidden")
    mask_hidden_pr = month_grid_pr["مخفي"].astype(bool)
    display_grid_pr = month_grid_pr if show_hidden_pr else month_grid_pr[~mask_hidden_pr]

    if display_grid_pr.empty:
        st.info("كل الصفوف متخفية دلوقتي. فعّل 'إظهار الصفوف المخفية' عشان تقدر تعدّل أو تظهرها تاني.")
    ...
    edited_display_pr = st.data_editor(
        ...
        column_config={
            ...
            "مخفي": st.column_config.CheckboxColumn("🙈 إخفاء الصف", help="بتخفي الصف من العرض الافتراضي من غير ما تمسح بياناته — تقدر تظهره تاني وقت ما تحب"),
            "المخطط الشهري (KG)": st.column_config.NumberColumn("🎯 المخطط الشهري (KG)", format="%.0f", step=1, min_value=0),
            **{dc: st.column_config.NumberColumn(dc, format="%.0f", step=1, min_value=0) for dc in day_cols_pr},
        }
    )

    # نرجّع الصفوف المعدّلة (اللي كانت ظاهرة بس) لمكانها في الجدول الكامل، والصفوف المخفية تفضل زي ما هي
    month_grid_pr.loc[edited_display_pr.index] = edited_display_pr
    month_grid_pr["مخفي"] = month_grid_pr["مخفي"].astype(bool)

    st.divider()

    lines_only_pr = month_grid_pr[month_grid_pr["الشيفت"] == TOTAL_SHIFT_LABEL].copy()
    total_target_pr = lines_only_pr["المخطط الشهري (KG)"].sum()
    total_actual_pr = lines_only_pr[day_cols_pr].sum(axis=1).sum() if day_cols_pr else 0
    achievement_pr = (total_actual_pr / total_target_pr * 100) if total_target_pr > 0 else 0

    kpi_row([
        {"icon": "🎯", "label": "المخطط الشهري", "value": f"{total_target_pr:,.0f} KG", "color": "#e0a92e"},
        {"icon": "📦", "label": "الفعلي حتى الآن", "value": f"{total_actual_pr:,.0f} KG", "color": "#0d6b5c"},
        {"icon": "🥇", "label": "نسبة التحقيق", "value": f"{achievement_pr:.1f}%", "color": "#7c3aed"},
    ])
    ...
    line_summary_pr = lines_only_pr[["الخط", "المخطط الشهري (KG)"]].rename(
        columns={"المخطط الشهري (KG)": "المستهدف (KG)"}
    ...
    if st.button("💾 حفظ بيانات الشهر", key="production_save_month_btn"):
        ...
        # عشان الداشبورد وتقرير الباكينج والتقرير الشهري وصفحة الكسر يفضلوا شغالين
        # بنفس الشكل زي ما هو من غير ما نلمسهم، المخطط الشهري بيتوزع بالتساوي على أيام
        # الشهر عشان مجموع المخطط في الشهر يفضل مطابق للرقم اللي انت داخله بالظبط
        ...
        for _, r in to_save_pr.iterrows():
            ...
            monthly_target = float(r["المخطط الشهري (KG)"]) if pd.notna(r["المخطط الشهري (KG)"]) else 0.0
            daily_target = (monthly_target / days_in_month_pr) if days_in_month_pr else 0.0
        ...
        existing_long_pr = read_gsheet(PRODUCTION_SHEET) if gsheet_exists(PRODUCTION_SHEET) else pd.DataFrame(columns=long_cols)
        if not existing_long_pr.empty and "Date" in existing_long_pr.columns:
            existing_long_pr["Date"] = existing_long_pr["Date"].astype(str)
            existing_long_pr = existing_long_pr[~existing_long_pr["Date"].str.startswith(selected_month_pr)]
        else:
            existing_long_pr = pd.DataFrame(columns=long_cols)

        new_long_pr = pd.DataFrame(long_rows, columns=long_cols)
        merged_long_pr = pd.concat([existing_long_pr, new_long_pr], ignore_index=True)
    ...
    page_banner("📈", "كابستي العمالة والإنتاجية", "طاقة العمالة الحالية مقابل المطلوب، ومتوسط الإنتاج التقديري لكل عامل")
    ...
    st.caption(
        "طاقة العمالة عبارة عن لقطة حالية (العدد المسجل دلوقتي في صفحة Workers)، أما متوسط الإنتاج لكل عامل "
        "فبيتحسب تراكمي على مستوى الشهر اللي تختاره تحت."
    )

    CAPACITY_SHEET = "capacity_required_headcount"
    worker_files_cap = {
        "الإيطالي": "workers_italy.csv",
        "المستمر": "workers_continuous.csv",
        "اللفافات": "workers_rolls.csv",
    ...
    CAPACITY_DEPTS = list(worker_files_cap.keys())
    CAPACITY_SHIFTS = ["1", "2", "3"]

    # ---------- تحميل/تجهيز جدول "المطلوب" (يتحفظ في شيت مستقل ويتعدل بباسورد المدير) ----------
    def load_capacity_targets():
        cols = ["الخط", "الشيفت", "المطلوب"]
        if gsheet_exists(CAPACITY_SHEET):
            df = read_gsheet(CAPACITY_SHEET)
            for c in cols:
                if c not in df.columns:
                    df[c] = 0 if c == "المطلوب" else ""
            df = df[cols]
        else:
            df = pd.DataFrame(columns=cols)

        df["الخط"] = df["الخط"].astype(str).str.strip()
        df["الشيفت"] = df["الشيفت"].astype(str).str.strip()
        df["المطلوب"] = pd.to_numeric(df["المطلوب"], errors="coerce").fillna(0).astype(int)
        df = df[df["الخط"].isin(CAPACITY_DEPTS) & df["الشيفت"].isin(CAPACITY_SHIFTS)]

        existing_pairs = set(zip(df["الخط"], df["الشيفت"]))
        missing = [
            {"الخط": d, "الشيفت": s, "المطلوب": 0}
            for d in CAPACITY_DEPTS for s in CAPACITY_SHIFTS
            if (d, s) not in existing_pairs
        ]
        if missing:
            df = pd.concat([df, pd.DataFrame(missing)], ignore_index=True)

        order_map = {(d, s): i for i, (d, s) in enumerate((d, s) for d in CAPACITY_DEPTS for s in CAPACITY_SHIFTS)}
        df["__order"] = df.apply(lambda r: order_map.get((r["الخط"], r["الشيفت"]), 999), axis=1)
        df = df.sort_values("__order").drop(columns="__order").reset_index(drop=True)
        return df

    targets_df = load_capacity_targets()

    # ---------- العمالة الحالية الفعلية — حي من صفحة Workers، لكل خط وشيفت ----------
    current_counts = {}
    ...
    targets_df["العمالة الحالية"] = targets_df.apply(lambda r: current_counts.get((r["الخط"], r["الشيفت"]), 0), axis=1)
    targets_df["العجز / الفائض"] = targets_df["العمالة الحالية"] - targets_df["المطلوب"]

    # ---------- ملخص عام ----------
    total_current_cap = targets_df["العمالة الحالية"].sum()
    total_required_cap = targets_df["المطلوب"].sum()
    total_gap_cap = total_current_cap - total_required_cap

    kpi_row([
        {"icon": "👥", "label": "إجمالي العمالة الحالية", "value": f"{total_current_cap} عامل", "color": "#0d6b5c"},
        {"icon": "🎯", "label": "إجمالي العمالة المطلوبة", "value": f"{total_required_cap} عامل", "color": "#e0a92e"},
        {
            "icon": "🔴" if total_gap_cap < 0 else "🟢",
            "label": "العجز / الفائض الإجمالي",
            "value": f"{total_gap_cap:+d} عامل",
            "color": "#b3261e" if total_gap_cap < 0 else "#0d6b5c",
        },
    ])

    # ---------- جدول التفصيل لكل خط/شيفت + تعديل المطلوب (محمي) ----------
    st.markdown("#### 🎯 طاقة العمالة لكل خط وشيفت")

    admin_pass_cap = st.text_input("🔒 كلمة مرور المدير للتعديل على 'المطلوب'", type="password", key="capacity_admin_pass")
    can_edit_targets = admin_pass_cap == "admin123"

    display_cols_cap = ["الخط", "الشيفت", "العمالة الحالية", "المطلوب", "العجز / الفائض"]
    bar_max_cap = int(max(targets_df["العمالة الحالية"].max(), targets_df["المطلوب"].max(), 1))

    if can_edit_targets:
        st.success("تم تسجيل دخول المدير — تقدر تعدل عمود 'المطلوب' وتحفظ")
        edited_targets_cap = st.data_editor(
            targets_df[display_cols_cap],
            use_container_width=True,
            hide_index=True,
            num_rows="fixed",
            key="capacity_targets_editor",
            disabled=["الخط", "الشيفت", "العمالة الحالية", "العجز / الفائض"],
            column_config={
                "المطلوب": st.column_config.NumberColumn("🎯 المطلوب", min_value=0, step=1),
            },
        )
        if st.button("💾 حفظ العمالة المطلوبة", key="capacity_save_targets_btn"):
            to_save_cap = targets_df[["الخط", "الشيفت"]].copy()
            to_save_cap["المطلوب"] = edited_targets_cap["المطلوب"].values
            write_gsheet(CAPACITY_SHEET, to_save_cap)
            st.success("اتحفظت العمالة المطلوبة ✔")
            st.rerun()
    else:
        st.info("العرض هنا للقراءة بس — لو عايز تعدل 'المطلوب' ادخل كلمة مرور المدير فوق")
        st.dataframe(
            targets_df[display_cols_cap],
            use_container_width=True,
            hide_index=True,
            column_config={
                "العمالة الحالية": st.column_config.ProgressColumn(
                    "العمالة الحالية", min_value=0, max_value=bar_max_cap, format="%d"
                ),
            },
        )

    # ---------- ملخص لكل قسم (إجمالي الشيفتات) ----------
    st.markdown("#### 📊 طاقة العمالة حسب القسم (إجمالي الشيفتات)")
    dept_summary_cap = targets_df.groupby("الخط", sort=False)[["العمالة الحالية", "المطلوب"]].sum().reindex(CAPACITY_DEPTS).reset_index()
    dept_summary_cap["العجز / الفائض"] = dept_summary_cap["العمالة الحالية"] - dept_summary_cap["المطلوب"]
    dept_bar_max_cap = int(max(dept_summary_cap["العمالة الحالية"].max(), dept_summary_cap["المطلوب"].max(), 1))
    st.dataframe(
        dept_summary_cap,
        use_container_width=True,
        hide_index=True,
        column_config={
            "العمالة الحالية": st.column_config.ProgressColumn(
                "العمالة الحالية", min_value=0, max_value=dept_bar_max_cap, format="%d"
            ),
        },
    )

    # ================= متوسط الإنتاج لكل عامل (تقديري) — شهري تراكمي =================
    st.divider()
    st.markdown("### 📦 متوسط الإنتاج لكل عامل (تقديري) — شهري تراكمي")
    st.caption(
        "متوسط تقديري = إجمالي إنتاج الشهر (KG) المسجل في صفحة Production للخط (أو للخطوط) ÷ عدد العمال الحاليين "
        "المسجلين عليه في صفحة Workers. قوة 'الإيطالي' في صفحة Workers هي قوة الطباخات المشتركة اللي بتغذي "
        "3 خطوط مع بعض (الإيطالي + الاكلير ص1 + الاكلير ص3)، فبنجمع إنتاج الخطوط التلاتة عليها. الأقسام اللي "
        "مالهاش رقم إنتاج بالكيلو مباشر (اللفافات والترانسلاب) هتلاقيها في تقدير تقريبي منفصل تحت، مبني على "
        "كراتين الباكينج."
    )
    ...
    selected_month_cap = st.selectbox("📅 اختر الشهر", month_options_cap, index=default_index_cap, key="capacity_prod_month")

    # الأقسام اللي بتتسجل ليها كيلوجرامات مباشرة في صفحة Production
    # ملحوظة: "الإيطالي" في صفحة Workers هي قوة "الطباخات" المشتركة اللي بتغذي 3 خطوط
    # مع بعض (الإيطالي + الاكلير ص1 + الاكلير ص3)، فبنجمع إنتاج الخطوط التلاتة عليها
    WORKERS_TO_PRODUCTION_LINE = {
        "الإيطالي": ["الإيطالي", "الاكلير ص١", "الاكلير ص٣"],
        "المستمر": ["المستمر"],
        "الطوفي": ["الطوفي"],
    }

    month_by_line_cap = pd.Series(dtype=float)
    if gsheet_exists(PRODUCTION_SHEET):
        hist_cap = read_gsheet(PRODUCTION_SHEET)
        hist_cap["Date"] = hist_cap["Date"].astype(str)
        all_shifts_cap = hist_cap[hist_cap["Shift"] == "All Shifts"].copy()
        all_shifts_cap["الفعلي (KG)"] = pd.to_numeric(all_shifts_cap["الفعلي (KG)"], errors="coerce").fillna(0)
        all_shifts_cap["Line"] = all_shifts_cap["Line"].astype(str).str.strip()
        month_prod_cap = all_shifts_cap[all_shifts_cap["Date"].str.startswith(selected_month_cap)]
        month_by_line_cap = month_prod_cap.groupby("Line")["الفعلي (KG)"].sum()

    prod_per_worker_rows = []
    for dept, prod_lines in WORKERS_TO_PRODUCTION_LINE.items():
        month_kg = float(sum(month_by_line_cap.get(pl, 0) for pl in prod_lines))
        worker_count = int(dept_summary_cap.loc[dept_summary_cap["الخط"] == dept, "العمالة الحالية"].sum())
        per_worker = (month_kg / worker_count) if worker_count > 0 else 0
        lines_label = " + ".join(pl.replace("الاكلير ص١", "اكلير ص1").replace("الاكلير ص٣", "اكلير ص3") for pl in prod_lines)
        prod_per_worker_rows.append({
            "القسم / الخط": f"{dept} ({lines_label})" if len(prod_lines) > 1 else dept,
            "إنتاج الشهر (KG)": round(month_kg, 0),
            "عدد العمال الحالي": worker_count,
            "متوسط الإنتاج لكل عامل (KG)": round(per_worker, 1),
        })

    prod_per_worker_df = pd.DataFrame(prod_per_worker_rows)

    if prod_per_worker_df.empty or prod_per_worker_df["إنتاج الشهر (KG)"].sum() == 0:
        st.info("لسه مفيش بيانات إنتاج مسجلة لشهر ده في صفحة Production عشان نحسب متوسط الإنتاج لكل عامل")
    else:
        st.dataframe(prod_per_worker_df, use_container_width=True, hide_index=True)
        st.bar_chart(prod_per_worker_df.set_index("القسم / الخط")["متوسط الإنتاج لكل عامل (KG)"])

    # ---------- تقدير تقريبي: إنتاجية التعبئة الكلية (اللفافات + الترانسلاب) ----------
    st.markdown("##### 📦 تقدير تقريبي: إنتاجية التعبئة الكلية (اللفافات + الترانسلاب)")
    st.caption(
        "تقدير تقريبي فقط: إجمالي كراتين الباكينج المسجلة هذا الشهر ÷ إجمالي عدد عمال اللفافات والترانسلاب مع "
        "بعض (مفيش تقسيم دقيق للإنتاج بين القسمين حاليًا، فالرقم ده متوسط تقريبي مش دقيق لكل قسم لوحده)."
    )
    ...
    if gsheet_exists(pack_sheet_cap):
        ...
        total_cartons_cap = pack_df_cap[day_cols_cap].sum().sum() if day_cols_cap else 0

    lafafat_translab_workers = int(
        dept_summary_cap.loc[dept_summary_cap["الخط"].isin(["اللفافات", "الترانسلاب"]), "العمالة الحالية"].sum()
    )
    cartons_per_worker_cap = (total_cartons_cap / lafafat_translab_workers) if lafafat_translab_workers > 0 else 0

    cc1, cc2, cc3 = st.columns(3)
    cc1.metric("📦 إجمالي كراتين الشهر", f"{total_cartons_cap:,.0f}")
    cc2.metric("👥 عدد عمال اللفافات + الترانسلاب", f"{lafafat_translab_workers}")
    cc3.metric("📊 كرتونة لكل عامل", f"{cartons_per_worker_cap:,.1f}")
