...
if page == "الرئيسية":
    ...
    def _energy_kpi_cards_month(calc, monthly_targets_map):
        """كروت KPI تراكمية للشهر لحد دلوقتي — بتقارن بالتارجت الشهري المحسوب (يومي × أيام تشغيل)"""
        cards = []
        for u in ENERGY_UTILITIES:
            total_cons = calc[f"استهلاك {u['key']}"].sum()
            tgt = monthly_targets_map.get(u["key"], 0)
            pct = (total_cons / tgt * 100) if tgt > 0 else 0
            total_cartons = calc["الإنتاج (كراتين)"].sum()
            total_tons = calc["الإنتاج (طن)"].sum()
            per_carton = (total_cons / total_cartons) if total_cartons > 0 else 0
            per_ton = (total_cons / total_tons) if total_tons > 0 else 0
            cards.append({"icon": u["icon"], "label": f"إجمالي {u['key']} الشهر ({u['unit']})", "value": f"{total_cons:,.0f}", "color": u["color"]})
            cards.append({
                "icon": "🎯", "label": f"{u['key']} % من التارجت الشهري", "value": f"{pct:,.1f}%",
                "color": "#16a34a" if (tgt == 0 or pct <= 100) else "#dc2626"
            })
            cards.append({"icon": "📦", "label": f"{u['key']} / كرتونة", "value": f"{per_carton:,.3f}", "color": u["color"]})
            cards.append({"icon": "🏋️", "label": f"{u['key']} / طن", "value": f"{per_ton:,.2f}", "color": u["color"]})
        return cards

    def _energy_kpi_cards_day(calc, day, daily_targets_map, label_suffix=""):
        """كروت KPI ليوم واحد بالظبط — بتقارن بالتارجت اليومي مباشرة"""
        cards = []
        row_df = calc[calc["اليوم"] == day]
        row = row_df.iloc[0] if not row_df.empty else None
        for u in ENERGY_UTILITIES:
            actual = float(row[f"استهلاك {u['key']}"]) if row is not None else 0.0
            tgt = daily_targets_map.get(u["key"], 0)
            pct = (actual / tgt * 100) if tgt > 0 else 0
            cartons = float(row["الإنتاج (كراتين)"]) if row is not None else 0.0
            tons = float(row["الإنتاج (طن)"]) if row is not None else 0.0
            per_carton = (actual / cartons) if cartons > 0 else 0
            per_ton = (actual / tons) if tons > 0 else 0
            cards.append({"icon": u["icon"], "label": f"استهلاك {u['key']} اليوم{label_suffix} ({u['unit']})", "value": f"{actual:,.1f}", "color": u["color"]})
            cards.append({
                "icon": "🎯", "label": f"{u['key']} % من التارجت اليومي{label_suffix}", "value": f"{pct:,.1f}%",
                "color": "#16a34a" if (tgt == 0 or pct <= 100) else "#dc2626"
            })
            cards.append({"icon": "📦", "label": f"{u['key']} / كرتونة{label_suffix}", "value": f"{per_carton:,.3f}", "color": u["color"]})
            cards.append({"icon": "🏋️", "label": f"{u['key']} / طن{label_suffix}", "value": f"{per_ton:,.2f}", "color": u["color"]})
        return cards

    def _combined_calc_and_totals(factory_calcs, day, n_days):
        """بيجمع بيانات كذا مصنع مع بعض كأنهم مصنع واحد: سلسلة الشهر كله (للرسم) + قيم اليوم المحدد (للكروت)"""
    ...
    energy_col_config = {"اليوم": st.column_config.NumberColumn("اليوم", disabled=True)}
    for u in ENERGY_UTILITIES:
        energy_col_config[f"{u['key']} - أول اليوم"] = st.column_config.NumberColumn(
            f"{u['icon']} {u['key']} - أول اليوم ({u['unit']})", min_value=0.0, format="%.1f"
        )
        energy_col_config[f"{u['key']} - آخر اليوم"] = st.column_config.NumberColumn(
            f"{u['icon']} {u['key']} - آخر اليوم ({u['unit']})", min_value=0.0, format="%.1f"
        )
    ...
    edited_energy = st.data_editor(
        ...
        hide_index=True,
        num_rows="fixed",
        column_config=energy_col_config,
        key=f"energy_editor_{energy_factory_pick}_{selected_month_e}"
    )
    ...
    kpi_row(_energy_kpi_cards_month(energy_calc, monthly_targets_map_e))
    ...
    # ==========================
    # تقرير الطاقة اليومي — الثلاث مصانع كأنهم مصنع واحد فوق، وتفاصيل كل مصنع تحت (زي تقرير الأعطال بالظبط)
    # ==========================
    st.divider()
    st.markdown("#### 🖨️ تقرير الطاقة اليومي")
    st.caption(
        "تقرير ليوم واحد: فوق كروت KPI ورسم بياني للثلاث مصانع مجمّعين كأنهم مصنع واحد ومربوطين بإجمالي إنتاج المصانع الثلاثة، "
    ...
    if st.button("📊 تجهيز تقرير الطاقة اليومي", key="energy_full_report_btn"):
        ...
        # ---------- الجزء العلوي: الثلاث مصانع مجمّعين كأنهم مصنع واحد ----------
        ...
        combined_cards = []
        for u in ENERGY_UTILITIES:
            actual = day_values.get(u["key"], 0.0)
            tgt = combined_daily_targets.get(u["key"], 0.0)
            pct = (actual / tgt * 100) if tgt > 0 else 0
            per_carton = (actual / cartons_day) if cartons_day > 0 else 0
            per_ton = (actual / tons_day) if tons_day > 0 else 0
            combined_cards.append({"icon": u["icon"], "label": f"إجمالي {u['key']} اليوم — 3 مصانع ({u['unit']})", "value": f"{actual:,.1f}", "color": u["color"]})
            combined_cards.append({
                "icon": "🎯", "label": f"{u['key']} % من التارجت اليومي المجمّع", "value": f"{pct:,.1f}%",
                "color": "#16a34a" if (tgt == 0 or pct <= 100) else "#dc2626"
            })
            combined_cards.append({"icon": "📦", "label": f"{u['key']} / كرتونة (3 مصانع)", "value": f"{per_carton:,.3f}", "color": u["color"]})
            combined_cards.append({"icon": "🏋️", "label": f"{u['key']} / طن (3 مصانع)", "value": f"{per_ton:,.2f}", "color": u["color"]})

        combined_cards_html = "".join(
            f"""<div style="flex:1; min-width:150px; border:1px solid #e5e7eb; border-radius:8px; padding:8px 10px; margin:3px;">
        ...
        combined_figs = _energy_trend_fig(
            combined_trend, "الثلاث مصانع (مجمّع)", combined_daily_targets, days_in_month_e,
        ...
        <div style="margin-bottom:16px; page-break-inside:avoid; border:2px solid {BRAND_GREEN}; border-radius:8px; padding:8px;">
            <h2 style="background:{BRAND_GREEN}; color:#fff; padding:6px 10px; border-radius:6px; margin-bottom:8px;">
                🏭 الثلاث مصانع مجمّعين (كأنهم مصنع واحد) — يوم {report_day_e}
            </h2>
            <div style="display:flex; flex-wrap:wrap;">{combined_cards_html}</div>
            <div style="display:flex; flex-wrap:wrap; gap:6px; margin-top:8px;">{combined_charts_html}</div>
        </div>
        """

        # ---------- الجزء السفلي: تفاصيل كل مصنع لوحده (زي تقرير الأعطال بالظبط) ----------
        energy_report_blocks_html = ""
        for f_name, f_conf in ENERGY_FACTORY_CONFIG.items():
            f_calc = factory_calcs[f_name]
            f_daily_targets = factory_daily_targets[f_name]
            f_cards = _energy_kpi_cards_day(f_calc, report_day_e, f_daily_targets)
            cards_html = "".join(
                f"""<div style="flex:1; min-width:150px; border:1px solid #e5e7eb; border-radius:8px; padding:8px 10px; margin:3px;">
                    <div style="font-size:11px; color:#6b7280;">{c['icon']} {c['label']}</div>
                    <div style="font-size:16px; font-weight:800; color:{c['color']};">{c['value']}</div>
            ...
            f_figs = _energy_trend_fig(f_calc, f_name, f_daily_targets, days_in_month_e,
                                        figsize=(6.6, 2.6), title_size=11)
            charts_html = "".join(fig_to_base64_img(f_figs[u["key"]]) for u in ENERGY_UTILITIES)
            energy_report_blocks_html += f"""
            <div style="margin-top:16px; page-break-inside:avoid; border-top:3px solid {BRAND_GREEN}; padding-top:8px;">
        ...
        energy_full_html = build_printable_html(
            f"تقرير الطاقة اليومي — يوم {report_day_e} — شهر {selected_month_e}",
            f"تاريخ الطباعة: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            pd.DataFrame(),
            landscape=True,
            extra_html=combined_block_html + "<h2 style='margin-top:10px;'>📋 تفاصيل كل مصنع لوحده</h2>" + energy_report_blocks_html,
        )
        st.download_button(
            "🖨️ تحميل تقرير الطاقة اليومي (PDF)",
            data=energy_full_html.encode("utf-8"),
            file_name=f"energy_report_{selected_month_e}_day{report_day_e}.html",
            mime="text/html",
            help="افتح الملف بعد التحميل، ودوس Ctrl+P واختار 'Save as PDF' عشان تحوله PDF أو تطبعه",
            key="energy_full_report_download_btn"
        )
